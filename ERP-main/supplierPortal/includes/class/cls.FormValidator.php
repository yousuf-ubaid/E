...
define("E_VAL_DECI_CHECK_FAILED", "Please provide decimal input for %s");
...
define("E_VAL_DATE_CHECK_FAILED", "Please provide valid date input for %s");
...
class FormValidator
{
    ...
    function validate_for_decimal_input($input_value, &$validation_success)
    {
    ...
    isProfane($word)
    -------------------------------------------------------------------------
    Find if a single word is considered profanity by Google 
    -------------------------------------------------------------------------*/
    function validate_profane($word)
    {
        $true = "{\"response\": \"true\"}";
    ...
    */
    ...
    function ValidateCommand($command, $command_value, $input_value, &$default_error_message, $variable_name, $formvariables)
    {
        ...
        switch ($command) {
            ...
            case 'deci':
            case 'decimal': {
                $bret = $this->test_datatype($input_value, "[^\d+(\.\d+)?$]");
                if (false == $bret) {
                    $default_error_message = sprintf(E_VAL_DECI_CHECK_FAILED, $variable_name);
                }
                break;
            }
            ...
            case "dd": /*Multiple select drop down */
            case "dropDown": {
            ...
            case "profane": {
                ...
                $stringarray = explode(" ", $input_value);

                // check each word using Profanity API. Use addStars() function each time
                // a word is considered profane
                foreach ($stringarray as $word) {
                    if ($this->validate_profane($word)) {
                        $bret = false;
                        break;
                    } else {
                        $bret = true;
