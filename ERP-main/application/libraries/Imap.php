...
class Imap
{
    ...
    /**
     * @param array $config Options: host, encrypto, user, pass, port, folders, accountType
     *
    ...
    public function connect(array $config = [])
    {
        ...
        if (isset($config['folder']) && $config['folder'] === false) {
            $folder .= $config['folder'];
        }

        $this->mailbox = '{' . $config['host'] . $enc . '}' . $folder;
     ...
     * @return boolean
     */
    public function set_timeout($timeout = 60, $type = 'open')
    {
        $types = [
     ...
     * @return integer
     */
    public function get_timeout($type = 'open')
    {
        $types = [
            'open' => IMAP_OPENTIMEOUT,
            'read' => IMAP_READTIMEOUT,
     ...
     * @return Imap
     */
    public function set_expunge_on_disconnect($active = true)
    {
     ...
     */
    public function get_attachment($uid, $index = 0)
    {
     ...
     * @return array
     */
    protected function _get_attachments($uid, $structure, $part_number = '', $index = null)
    {
        $id = imap_msgno($this->stream, $uid);
        $attachments = [];
     ...
     * @return string
     */
    protected function struc_decoding($text, $encoding = 5)
    {
        switch ($encoding) {
            case ENC7BIT: // 0 7bit
                return $text;
    ...
    public function get_message($uid)
    {
        ...
        $email = [
            ...
            'date' => isset($header->date) ? $header->date : '',//date('c', strtotime(substr($header->date, 0, 30))),
            ...
            'downloaded_attachments' => $this->getEmailAttachment($id),
    ...
    public function get_message_read($uid)
    {
        $cache_id = $this->folder . ':message_' . $uid;
    ...
    public function get_messages($uids)
    {
        ...
        foreach ($uids as $key => $uid) {
            $message = $this->get_message((int)$uid);
            $items[$key]['uid'] = $uid;
            $items[$key]['from'] = $message['from'];
            $items[$key]['isRead'] = $message['read'];
            $items[$key]['deleted'] = $message['deleted'];
            $items[$key]['draft'] = $message['draft'];
            $items[$key]['msg'] = $message['subject'];
            $unixTimestamp = strtotime($message['date']);
            $unixTimestamp = date("m/d/Y", $unixTimestamp);
            $items[$key]['date'] = $unixTimestamp;
            $items[$key]['udate'] = $message['udate'];
        }
    ...
    public function get_eml($uid)
    {
        ...
        $data['header'] = $headers;
        $data['body'] = imap_qprint($body);

        return $data;
    }

    public function fun($function, ...$params)
    {
        ...
        foreach ($thread as $key => $uid) {
            ...
            switch ($item[1]) {
                case 'num':
                    ...
                    $items[$node]['from'] = $message['from'];
                    $items[$node]['isRead'] = $message['read'];
                    $items[$node]['deleted'] = $message['deleted'];
                    $items[$node]['draft'] = $message['draft'];
                    $items[$node]['msg'] = $message['subject'];
                    $unixTimestamp = strtotime($message['date']);
                    $unixTimestamp = date("m/d/Y", $unixTimestamp);
                    $items[$node]['date'] = $unixTimestamp;
                    $items[$node]['udate'] = $message['udate'];
                    break;
                case 'next':
     ...
     * @return array
     */
    public function paginate(array $uids, $page = null)
    {
        $count = (count($uids) - (10 * $page - 10));
        if ($count < 10 ) {
        ...
        return $this->getHeaders(array_slice($uids, $per_page * $page - $per_page, $per_page));
    }
     ...
     */
    public function search($search_criteria = 'ALL', $sort_by = 'date', $descending = true)
    {
    ...
    public function getHeaders($uids)
    {
        $record = [];
        if ($uids) {
            foreach ($uids as $uid) {
                ...
                $email = [
                    ...
                    'uid' => (int)$uid,
                    'from' => isset($header->from[0]) ? (array)$this->to_address($header->from[0]) : [],
                    'date' => isset($header->date) ? $unixTimestamp : '',//date('c', strtotime(substr($header->date, 0, 30))),
                    'udate' => isset($header->udate) ? $header->udate : '',
                    'subject' => $this->convert_to_utf8($subject),
                    'recent' => strlen(trim($header->Recent)) > 0,
                    'read' => strlen(trim($header->Unseen)) < 1,
    ...
    /*public function listMessages($page = 1, $per_page = 10, $sort = null)
    {
        $limit = ($per_page * $page);
    ...
    public function getEmailAttachment($uid)
    {
        ...
        if (isset($structure->parts) && count($structure->parts)) {
            for ($i = 0; $i < count($structure->parts); $i++) {
                ...
                if ($attachments[$i]['is_attachment']) {
                    $attachments[$i]['attachment'] = imap_fetchbody($this->stream, $uid, $i + 1);
        ...
        foreach ($attachments as $key => $attachment) {
            if ($attachment['is_attachment'] == 1) {
                 ...
                 */
                $new_name = $uid . "-" . $filename;
                $attachment_names[$key]['new_filename'] = $new_name;
                $fp = fopen(APPPATH.'../attachments/email_received/'.$uid . "-" . $filename, "w+");
                fwrite($fp, $attachment['attachment']);
                fclose($fp);
                $attachment_names[$key]['file_size'] = $this->formatBytes(filesize(APPPATH.'../attachments/email_received/'.$uid . "-" . $filename));
            }
    ...
    protected function formatBytes($bytes, $precision = 4) {
        $base = log($bytes, 1024);
