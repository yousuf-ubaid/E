...
class MaterialReceiptNote extends ERP_Controller
{
    ...
    function fetch_material_receipt()
    {
        ...
        if ($fshowallsegmentYN==1 || $fshowallsegmentYN=='On') {
            $where2 = "srp_erp_materialreceiptmaster.segmentID in (select segmentID from srp_erp_segment_usergroups where srp_erp_segment_usergroups.userGroupID='".$userGroupID."') ";
            $this->datatables->where($where2);
        }

        $this->datatables->select("srp_erp_materialreceiptmaster.mrnAutoID as mrnAutoID,companyCode,mrnCode,comment,employeeName,DATE_FORMAT(confirmedDate,'%Y-%m-%d') AS confirmedDate,confirmedYN,approvedYN,createdUserID,wareHouseDescription,DATE_FORMAT(receivedDate,'.$convertFormat.') AS receivedDate,receiptType,det.totalValue as tot_value,isDeleted,companyLocalCurrency,companyLocalCurrencyDecimalPlaces,confirmedByEmpID, srp_erp_materialreceiptmaster.RefNo AS RefNo");
        ...
        $this->datatables->add_column('MRN_detail', '<b>Received By : </b> $2 <br> <b>Warehouse : </b> $1 <br> <b>Received Date : </b> $3 <b>&nbsp;&nbsp; Type : </b> $4 <br> <b>Ref No :</b>$5', 'wareHouseDescription,employeeName,receivedDate,receiptType,RefNo');
        ...
        $this->datatables->add_column('edit', '$1', 'load_material_receipt_action(mrnAutoID,confirmedYN,approvedYN,createdUserID,isDeleted,confirmedByEmpID)');
    ...
    function fetch_material_issue_approval()
    {
        ...
        }else
        {
            ...
            $this->datatables->join('(SELECT SUM(qtyReceived) as qtyReceived,mrnAutoID FROM srp_erp_materialreceiptdetails GROUP BY mrnAutoID) det', '(det.mrnAutoID = srp_erp_materialreceiptmaster.mrnAutoID)', 'left');
            $this->datatables->from('srp_erp_materialreceiptmaster');
            $this->datatables->join('srp_erp_documentapproved', 'srp_erp_documentapproved.documentSystemCode = srp_erp_materialreceiptmaster.mrnAutoID AND `srp_erp_documentapproved`.`approvedEmpID` = `srp_erp_materialreceiptmaster`.`approvedbyEmpID` ');
        ...
        if ($fshowallsegmentYN==1 || $fshowallsegmentYN=='On') {
            ...
            $this->datatables->group_by('srp_erp_materialreceiptmaster.mrnAutoID');
    ...
    function fetch_stock_transfer_approval()
    {
        ...
        }else
        {
            $this->datatables->select('stockTransferAutoID,stockTransferCode,form_wareHouseCode ,form_wareHouseLocation , form_wareHouseDescription ,  to_wareHouseCode , to_wareHouseLocation,to_wareHouseDescription,confirmedYN ,srp_erp_documentapproved.approvedYN as approvedYN, approvalLevelID,documentApprovedID,confirmedByName,DATE_FORMAT(tranferDate,\'' . $convertFormat . '\') AS tranferDate', false);
            $this->datatables->from('srp_erp_stocktransfermaster');
            $this->datatables->join('srp_erp_documentapproved', 'srp_erp_documentapproved.documentSystemCode = srp_erp_stocktransfermaster.stockTransferAutoID AND `srp_erp_documentapproved`.`approvedEmpID` = `srp_erp_stocktransfermaster`.`approvedbyEmpID`');
    ...
    {
        echo json_encode($this->Material_receipt_note_modal->chk_balance_qty());
    }
    ...
    function fetch_existing_batch_details(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo json_encode($this->Material_receipt_note_modal->fetch_existing_batch_details());
        }
    }
