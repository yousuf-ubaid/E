...
class Pos_dashboard extends ERP_Controller
{
    ...
    function loadGeneralItemizedSalesReport()
    {
        ...
        $itemizedSalesReport = $this->Pos_dashboard_model->get_itemizedSalesReportByFilters($dateFrom, $dateTo, $cashier);
        $refund_details = $this->Pos_dashboard_model->get_itemizedRefundReport($dateFrom, $dateTo, $cashier);

        $dateTo = date('Y-m-d 23:59:59', strtotime($dateTo));
        $data['paymentMethod'] = $this->Pos_general_report_model->get_report_paymentMethod_admin($dateFrom, $dateTo, $cashier, $Outlets);
        $cash_collection = 0;
    ...
    function loadGeneralItemizedSalesReportDetail()
    {
        $tmpCashierSource = $this->input->post('cashier');
        if (isset($tmpCashierSource) && !empty($tmpCashierSource)) {
            $tmpCashier = join(",", $tmpCashierSource);
        ...
        $companyInfo = $this->Pos_dashboard_model->get_currentCompanyDetail();
        //$itemizedSalesReport = $this->Pos_dashboard_model->get_itemizedSalesReport($dateFrom, $dateTo);
        $itemizedSalesReport = $this->Pos_dashboard_model->get_itemizedSalesReportByFiltersDetail($dateFrom, $dateTo, $cashier);
        $refund_det = $this->Pos_dashboard_model->get_itemWise_refundDetails($dateFrom, $dateTo, $cashier);
        $dateTo = date('Y-m-d 23:59:59', strtotime($dateTo));
        $data['paymentMethod'] = $this->Pos_general_report_model->get_report_paymentMethod_admin($dateFrom, $dateTo, $cashier, $Outlets);
        $cash_collection = 0;
        ...
        $data['cash_collection'] = $cash_collection;
        $data['companyInfo'] = $companyInfo;
        $data['refund'] = $refund_det;
        $data['itemizedSalesReport'] = $itemizedSalesReport;
        $data['cashier'] = $tmpCashierSource;
        $data['cashierTmp'] = get_cashiers();


        $this->load->view('system/pos/generalReports/pos-general-itemized-sales-report-detail', $data);
    }



    function loadGeneralItemizedSalesReportDetailPdf()
    {
        $tmpCashierSource = $this->input->post('cashier');
        if (isset($tmpCashierSource) && !empty($tmpCashierSource)) {
            $tmpCashier = join(",", $tmpCashierSource);
        ...
        $data['itemizedSalesReport'] = $itemizedSalesReport;
        $data['refund'] = $refund_det;
        $html = $this->load->view('system/pos/generalReports/pos-general-itemized-sales-report-detail-pdf', $data, true);
        //echo $html;
        $this->load->library('pdf');
        $this->pdf->printed($html, 'A4');
    }
