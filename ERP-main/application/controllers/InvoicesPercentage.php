...
class InvoicesPercentage extends ERP_Controller
{
    ...
    function fetch_invoices_buyback()
    {
        ...
        $this->datatables->add_column('status', '$1', 'buyback_deliveredstatus(invoiceAutoID,deliveryStatus, approvedYN, confirmedYN)');
        $this->datatables->add_column('edit', '$1', 'load_invoice_action_buyback(invoiceAutoID,confirmedYN,approvedYN,createdUserID,confirmedYN,isDeleted,printDN)');
        $this->datatables->edit_column('DT_RowClass', '$1', 'set_deleted_class(isDeleted)');
        echo $this->datatables->generate();
    }
    ...
    function fetch_invoices_buyback_dayClose()
    {
        ...
        $this->datatables->select('srp_erp_customerinvoicemaster_temp.invoiceAutoID as invoiceAutoID,invoiceCode,invoiceNarration,srp_erp_customermaster.customerName as customermastername,transactionCurrencyDecimalPlaces,transactionCurrency, confirmedYN,approvedYN,srp_erp_customerinvoicemaster_temp.createdUserID,DATE_FORMAT(invoiceDate,\'' . $convertFormat . '\') AS invoiceDate,DATE_FORMAT(invoiceDueDate,\'' . $convertFormat . '\') AS invoiceDueDate,invoiceType,(((IFNULL(addondet.taxPercentage,0)/100)*((IFNULL(det.transactionAmount,0)-(IFNULL(det.detailtaxamount,0)))))+IFNULL(det.transactionAmount,0)) as total_value,ROUND((((IFNULL(addondet.taxPercentage,0)/100)*((IFNULL(det.transactionAmount,0)-(IFNULL(det.detailtaxamount,0)))))+IFNULL(det.transactionAmount,0)), 2) as total_value_search,isDeleted,srp_erp_customerinvoicemaster_temp.referenceNo as referenceNo');
        $this->datatables->join('(SELECT SUM(transactionAmount) as transactionAmount,sum(totalafterTax) as detailtaxamount,invoiceAutoID FROM srp_erp_customerinvoicedetails_temp GROUP BY invoiceAutoID) det', '(det.invoiceAutoID = srp_erp_customerinvoicemaster_temp.invoiceAutoID)', 'left');
        $this->datatables->join('(SELECT SUM(taxPercentage) as taxPercentage ,InvoiceAutoID FROM srp_erp_customerinvoicetaxdetails_temp  GROUP BY InvoiceAutoID) addondet', '(addondet.InvoiceAutoID = srp_erp_customerinvoicemaster_temp.InvoiceAutoID)', 'left');
        $this->datatables->where($where);
        $this->datatables->join('srp_erp_customermaster', 'srp_erp_customermaster.customerAutoID = srp_erp_customerinvoicemaster_temp.customerID', 'left');
        $this->datatables->from('srp_erp_customerinvoicemaster_temp');
        $this->datatables->add_column('invoice_detail', '<b>Customer Name : </b> $2 <br> <b>Document Date : </b> $3 <b style="text-indent: 1%;">&nbsp | &nbsp Due Date : </b> $4 <br> <b>Type : </b> $5 <br> <b>Comments : </b> $1 <br> <b>Ref No : </b> $6', 'trim_desc(invoiceNarration),customermastername,invoiceDate,invoiceDueDate,invoiceType,referenceNo');
        $this->datatables->edit_column('total_value', '<div class="pull-right"><b>$2 : </b> $1 </div>', 'format_number(total_value,transactionCurrencyDecimalPlaces),transactionCurrency');
        $this->datatables->add_column('confirmed', '$1', 'confirm_user_approval_drilldown(confirmedYN,"HCINV",invoiceAutoID)');
        $this->datatables->add_column('approved', '$1', 'confirm_ap_user(approvedYN,confirmedYN,"HCINV",invoiceAutoID)');
    ...
    function save_direct_invoice_detail_buyback()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo json_encode($this->InvoicesPercentage_model->save_direct_invoice_detail_buyback());
        }
    }
    ...
    function save_con_base_items()
    {
        ...
        foreach ($ids as $key => $id) {
         ...
         //   $this->form_validation->set_rules("wareHouseAutoID[{$key}]", "Line {$num} WareHouse", 'trim|required');
            ...
            $this->form_validation->set_rules("noofitems[{$key}]", "Line {$num} No Item", 'trim|required');
            $this->form_validation->set_rules("grossqty[{$key}]", "Line {$num} Gross Qty", 'trim|required');
            $this->form_validation->set_rules("buckets[{$key}]", "Line {$num} 	Buckets", 'trim|required');
            $this->form_validation->set_rules("bucketweightID[{$key}]", "Line {$num} B weigh", 'trim|required');
        }
    ...
    {
        $invoiceAutoID = ($this->uri->segment(3)) ? $this->uri->segment(3) : trim($this->input->post('invoiceAutoID') ?? '');
        $printtype = $this->uri->segment(4);
        $data['extra'] = $this->InvoicesPercentage_model->fetch_invoice_template_data($invoiceAutoID);
        $typetoprint =  $this->uri->segment(5);
       
        if($typetoprint == 1)
        {
           
            //print invoice only- full page or print invoice only - half page
            if(($printtype == 0) || ($printtype == 3) && ($printtype != ''))
            {
                $data['printtype'] =0;
            } else if ($printtype == 1 || ($printtype == 4)){//print invoice and delivery note-full page or print invoice and delivery note-half page
                $data['printtype'] = 1;
            }else if($printtype == 2 || ($printtype == 5))//print delivery note only- full page or print delivery note only - half page
            {
                $data['printtype'] = 2;
            }else
            {
        ...
        {
            if(($data['extra']['master']['isPrintDN'] == 0) && ($data['extra']['master']['isPrintDN'] != ''))
            {
                $data['printtype'] =0;
            } else if ($data['extra']['master']['isPrintDN'] == 1){

                $data['printtype'] = 1;
            }else if($data['extra']['master']['isPrintDN'] == 2)
            {
                $data['printtype'] = 2;
            }else
            {
        ...
        if ($this->input->post('html')) {
            echo $this->load->view('system/invoices/erp_invoice_print_buyback', $data, true);
        } else {
            //$printSize = $data['extra']['printSize'];
            $printSize = $this->uri->segment(4);
            if($printSize == 3 || $printSize == 4 || $printSize == 5){
                $printSizeText='A5-L';
            }else{
                $printSizeText='A4';
            }
            //var_dump($printSize);
            $html = $this->load->view('system/invoices/erp_invoice_printView_buyback', $data, true);
            $this->load->library('pdf');
            $pdf = $this->pdf->printed($html, $printSizeText, $data['extra']['master']['approvedYN'],0);
        }
    }

    function dayClosed_invoices()
    {
        $invoiceAutoID = ($this->uri->segment(3)) ? $this->uri->segment(3) : trim($this->input->post('invoiceAutoID') ?? '');
        $data['extra'] = $this->InvoicesPercentage_model->fetch_dayClosed_invoice_data($invoiceAutoID);
        $data['html'] = $this->input->post('html');
        $data['approval'] = $this->input->post('approval');
    ...
    function check_dayClose_buybackInvoice(){
        $companyID = $this->common_data['company_data']['company_id'];
        $invoiceAutoID = $this->input->post('invoiceAutoID');
...
AND srp_erp_taxcalculationformuladetails.companyID = $companyID AND sortOrder < $sortOrder  ")->result_array();
    ...
    function save_invoice_item_detail_buyback()
    {
        ...
        foreach ($searches as $key => $search) {
            ...
            $this->form_validation->set_rules("deduction[{$key}]", 'Bucket Size', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo json_encode($this->InvoicesPercentage_model->save_invoice_item_detail_buyback());
        }
    }
    ...
    function update_invoice_item_detail()
    {
        ...
        $this->form_validation->set_rules("deduction", 'Bucket Size', 'trim|required');
    ...
    function get_tax_drop_buyback(){
        echo json_encode($this->InvoicesPercentage_model->get_tax_drop_buyback());
    }

    function update_isEliminated(){
        echo json_encode($this->InvoicesPercentage_model->update_isEliminated());
    }
    function add_print_taglineYN()
    {
        echo json_encode($this->InvoicesPercentage_model->add_print_tagline());
    }
    function print_tageline_buyback()
    {
        $invoiceAutoID = ($this->uri->segment(3)) ? $this->uri->segment(3) : trim($this->input->post('invoiceDetailsAutoID') ?? '');
        $data['extra'] = $this->InvoicesPercentage_model->fetch_invoice_template_data_print_tagline($invoiceAutoID);
        $data['html'] = $this->input->post('html');
        ...
        $this->session->set_flashdata($invoiceAutoID);
        $html = $this->load->view('system/invoices/print_taglinebuyback', $data, true);
        if ($this->input->post('html')) {
            echo $html;
        } else {
            $this->load->library('pdf');
    ...
    function generateprint_tageline_buyback()
    {
        $invoiceAutoID = trim($this->input->post('invoiceID') ?? '');
        $data['extra'] = $this->InvoicesPercentage_model->fetch_invoice_template_data_print_tagline_ONSAVE($invoiceAutoID);
        $data['signature'] = '';
        $data["type"] = "pdf";
        $html = $this->load->view('system/invoices/print_taglinebuyback', $data, true);
        $this->load->library('pdf');
    ...
    {
        echo json_encode($this->InvoicesPercentage_model->fetch_sales_price());
    }

    function update_deliveryorder_status(){
        $this->form_validation->set_rules('invoiceAutoID', 'Delivery Order', 'trim|required');
        $status = trim($this->input->post('statusdo') ?? '');

        if ($status == 1 || $status == 2) {
            $this->form_validation->set_rules('delivereddate', 'Delivery  Date', 'trim|required');
        }

        if ($this->form_validation->run() == FALSE) {
            echo json_encode(array('e', validation_errors()));
        } else {
            echo json_encode($this->InvoicesPercentage_model->update_deliveryorder_status());
        }
    }

    function buyback_deliveryorder_collectionheader()
    {
        echo json_encode($this->InvoicesPercentage_model->buyback_deliveryorder_collectionheader());
    }
    function update_deliveryorder_collectiondetails()
    {
        echo json_encode($this->InvoicesPercentage_model->update_deliveryorder_collectiondetails());
    }

    function fetch_deliveryorderstaus()
    {
        $invoiceautoId = $this->input->post('invoiceAutoID');
        $companyid = current_companyID();
        $convertFormat = convert_date_format_sql();
        $data = $this->db->query('SELECT deliveryStatus,DeliveryComment,DATE_FORMAT(deliveredDate,\'' . $convertFormat . '\') AS deliveredDate
                                       FROM `srp_erp_customerinvoicemaster_temp` where  companyID = '.$companyid.' AND invoiceAutoID = '.$invoiceautoId.' ')->row_array();
        echo json_encode($data);
    }

    function fetch_dn_details(){
        $data= $this->InvoicesPercentage_model->fetch_dn_details();
        echo json_encode($data);
    }
}
