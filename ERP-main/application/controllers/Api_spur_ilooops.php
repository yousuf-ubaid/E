 ...
 *
 * ==========================================
 * Technical Documentation : Created By Divya lagoon Nagercoil
 * ==========================================
 *
 *
 *
 ...
 * Class Api_spur
 */
class Api_spur_ilooops extends REST_Controller
{
    ...
    /**
     * This will Insert  checkout details 
...
//        $companyType = $output['token']->companyType;
        ...
        $attDateTime = $request->attDateTime;
       

        $Mobilecheck = $this->db->query("SELECT srp_employeesdetails.isMobileCheckIn AS mobilecheckin FROM srp_employeesdetails  WHERE   EIdNo = {$userID}")->row_array();
         $attType='I';
         $attTime=$request->attTime;
         //$attDateTime=$request->attendanceDate;
         $latitude=$request->latitude;
         $longitude=$request->longitude;
         $uploadType=3;
       
         $Mobilecheckin=$Mobilecheck['mobilecheckin'];
        if(!empty($request)){

        if($Mobilecheckin==1)
        {

        /*$attData = $this->Auth_mobileUsers_Model->getMobileAttendanceLocations($userID, $company_id,0, $latitude, $longitude);
        //print_R($attData);
        $latitudeatt=$attData[0]['latitude'];
        $longitudeatt=$attData[0]['longitude'];
        $device_id=$attData[0]['deviceID'];
        $EmployeeMacId=$attData[0]['empMachineID'];*/

        $device_id=$request->device_id;
        $EmployeeMacId=$request->empMachineID;
        ...
        $this->db->insert('srp_erp_pay_empattendancetemptable', $int_data);
        $last_id = $this->db->insert_id();

        $data = array('id'=>$last_id,'status'=>'sucess','message'=>'Attendance Check In synchronize successfully done.');
    ...
    else
    {
        ...
        $data = array('status'=>'failure','message'=>'Enable Mobile CheckIn In Webapplication.');
...
//        $companyType = $output['token']->companyType;
        ...
        $data = array('id'=>$last_id,'status'=>'sucess','message'=>'Attendance Check Out synchronize  successfully done.');
        return $this->set_response($data, REST_Controller::HTTP_OK);
    ...
    else
    {
        ...
        return $this->response($data, REST_Controller::HTTP_OK);
            ...
            $final_output['message'] = 'Parameter Missing.Pass All parameter';
     ...
     function attendancelist_post()
    {
        ...
        $from_date = $request->from_date;//$this->input->post('from_date');
        $to_date = $request->to_date;//$this->input->post('to_date');
        ...
        $att_type_arr =$request->att_type;//$this->input->post('att_type');
        $att_type_list = implode(',', $att_type_arr);

        if ($request_type == 'myemployee') {
            $emp_id_arr =$request->empID;
            ...
            $att_rec = $this->db->query("SELECT isWeekEndDay,approvedComment,approvedYN,empID,ECode, Ename2,empMachineID, floorDescription, ID, machineID, att_rev.floorID, attendanceDate, 
            presentTypeID, attendanceDate checkIn,attendanceDate checkOut, lateHours, earlyHours, OTHours,normalDay,mustCheck,normalTime, weekend, holiday, NDaysOT, weekendOTHours, 
            holidayOTHours,realTime,att_rev.empComment
            FROM srp_erp_pay_empattendancereview AS att_rev
            JOIN srp_employeesdetails ON att_rev.empID = srp_employeesdetails.EIdNo
            LEFT JOIN srp_erp_pay_floormaster ON srp_erp_pay_floormaster.floorID = att_rev.floorID 
    ...
    public function attendancestatus_post()
    {
       ...
       $empMachineID=$request->empMachineID;
       $device_id = $request->device_id;
       ...
       if(!empty($request))
       {
        ...
        $attstatus = $this->db->query("SELECT autoID,attType FROM srp_erp_pay_empattendancetemptable  WHERE   empMachineID = {$empMachineID}
         and device_id={$device_id} and companyID={$company_id} and attDate={$attendanceDate}
		 AND autoID = (SELECT MAX(autoID)
              FROM srp_erp_pay_empattendancetemptable
              WHERE empMachineID = {$empMachineID}
              AND device_id like'$device_id%'
              AND companyID = {$company_id}
              AND attDate = {$attendanceDate})
		   ")->row_array();

            //print_r($this->db->last_query()); 
            $status=$attstatus['attType'];
            //print_r( $status);
       ...
       if($status=='I')
       {
        ...
        $final_output['message'] = 'Already Check IN';
       }
       else if($status=='O')
       {
        $final_output['success'] =true;
        $final_output['status'] = $status;
        $final_output['message'] = 'Already Check Out';

       }
       else
       {
        //26-12-2023  got confirmation from hisham modified by divya
        $final_output['success'] =true;
        $final_output['status'] = 'O';
        $final_output['message'] = 'clocked out';
       }
     $this->response($final_output, 200);
    ...
    public function login_post()
    {
        ...
        $deviceid = isset($request_1->deviceid) ? $request_1->deviceid : '';
        ...
        $fireBase_token=$deviceid.'123';
		$session_data['fireBase_token']=$fireBase_token;
			 
            $this->load->model('Auth_mobileUsers_Model');
            $this->Auth_mobileUsers_Model->save_firebase_token_mobileapp($fireBase_token,$deviceid,$empid['EIdNo'], $empid['Erp_companyID']);
        //echo $token;
		//exit();
		...
		else
		{
			if(empty($deviceid))
			{
		$data = array('status'=>'failure','message'=>'Kindly Provide and Pass Device Id value');
        return $this->response($data, REST_Controller::HTTP_OK);
			}
...
/** ---------------------------  PRIVATE FUNCTIONS --------------------------- */
    public function reportmanageremployeelist_get()
    {

        $tokenKey = $_SERVER['HTTP_SME_API_KEY'];
        $output['token'] = $this->jwt->decode($tokenKey, "token");
        $userID = $output['token']->id;
        $reportingmanager = $this->db->query("SELECT srp_erp_employeemanagers.managerID AS reportingManager FROM srp_erp_employeemanagers LEFT JOIN srp_employeesdetails ON srp_employeesdetails.EIdNo = srp_erp_employeemanagers.managerID WHERE active = 1 AND  empID = {$userID}")->row_array();
        $reportingmanagerid=$reportingmanager['reportingManager'];
        $emplist["emp"] = $this->Auth_mobileUsers_Model->get_emplist($reportingmanagerid);
        

        if ($emplist) {

            $final_output['success'] = true;
            $final_output['message'] = 'Reporting Manager against Employee ID retrieved successfully';
            $final_output['data'] = $emplist;
    ...
    /**
     * This will return  Attendance details Based On Date 
     * @param SME-API-KEY token
     * @param attdate 
     * @created Divya
     * @created at 2023-11-13
     */
    function get_employee_Attendancedetail_get()
    {
        $tokenKey = $_SERVER['HTTP_SME_API_KEY'];
        $output['token'] = $this->jwt->decode($tokenKey, "token");
        ...
        Select Erp_companyID,empMachineID
    ...
    if($get_employee_record){
        ...
        $ex_record[] = $this->db->where('attDate', $attdate)->where('empMachineID',$emp_id)->where('companyID',$get_employee_record['Erp_companyID'])
            ->get('srp_erp_pay_empattendancetemptable')->result_array();
           // print_R($ex_record);

            if ($ex_record) {

                $final_output['success'] = true;
                $final_output['message'] = 'Record retrieved successfully';
                $final_output['data'] = $ex_record;
                $this->response($final_output, REST_Controller::HTTP_OK);
            } else {
                $this->response([
                    'success' => FALSE,
...
//        $companyType = $output['token']->companyType;
    ...
    public function logout_post($return = true)
    {
		...
		$fireBasetoken=$requestdata->fireBasetoken;
		//$fireBasetoken=$requestdata->fireBasetoken;
            ...
            if ($tokenExist) {
                ...
                if ($result) {
                    if(!empty($empid)) {
                        ...
                        $this->db->where('player_id',$fireBasetoken);
    ...
    /** --------------------------- COPIED FROM MOBILE BACK NED (handed over to Semira from this point ) ---------------------------*/
    ...
    public function saveemployeesLeave_post()
    {
       ...
       // $coveringAvailabilityValidated = $this->post('coveringAvailabilityValidated');//$request->;
         $available =0;//model this value not take
        //
...
//        --------------------------------Covering Emp Validations-------------------------------------------------------
            ...
            $data['leaveDet'] = $this->Mobile_leaveApp_Model->getLeavedetails($userID, $companyID);
            $data['message'] ='New Leave Record Inserted Successfully';
...
//--------------------------------Covering Emp Validations-------------------------------------------------------
    ...
    public function leaveConfirmation_post()
    {
        $tokenKey = $_SERVER['HTTP_SME_API_KEY'];
        $output['token'] = $this->jwt->decode($tokenKey, "token");
        ...
        $data['result'] = $this->Mobile_leaveApp_Model->leaveConfirm($user, $companyID, $userID, $companycode,$leaveMasterID,$empID);
        $data['leaveDet'] = $this->Mobile_leaveApp_Model->getLeavedetails($userID, $companyID);
        $this->set_response($data, REST_Controller::HTTP_OK);
    }
    ...
    public function payroll_months_post()
    {
        //$companyID = $this->company_id;
        //$emp_id = $this->user_id;
    ...
    public function payslip_post()
    {
        ...
        $request = json_decode($request_body);
        $id = $request->id;
        $isNonPayroll = $request->isNonPayroll;
       

        $records = $this->Auth_mobileUsers_Model->get_paySlipData_profile($id, $isNonPayroll);
...
//                }
    ...
    public function update_employeesLeave_post()
    {
        ...
        //following two values are retrieved from leave calculation method.
        $appliedLeave = $leave_data['appliedLeave'];
        $appliedLeave = ($appliedLeave == '' ? 0 : $appliedLeave);
        $workingDays = $leave_data['workingDays'];
        $workingDays = ($workingDays == '' ? 0 : $workingDays);
...
AND empmanagers.managerID = '$current_userID'
)
)
)
...
AND `pvType` <> 'SC'
	...
	*,CASE WHEN( currentLevelNo = 1 ) THEN IF( repManager = $current_userID, 1, 0 ) WHEN( currentLevelNo = 2 ) THEN IF( ( '$current_userID' =
	...
	JOIN srp_erp_employeemanagers empmanagers ON empdetail.EIdNo = empmanagers.empID
	AND empmanagers.active = 1 WHERE empmanagers.companyID = '$companyID' AND empmanagers.managerID = '$current_userID'
	)
	)
	)
	AND srp_erp_documentapproved.approvedYN = ''
