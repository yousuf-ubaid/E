...
//        $this->datatables->add_column('item_inventryCode', '$1 - $2 <b></b>', 'itemSystemCode,itemDescription');
        ...
        $this->datatables->add_column('edit', '$1', 'edit_item(itemAutoID,isActive,isSubitemExist, deletedYN,masterConfirmedYN,masterApprovedYN)');
    ...
    function load_item_wise_prfitability_report()
    {
        ...
        $this->form_validation->set_rules('filterFrom', 'Date From', 'trim|required');
        $this->form_validation->set_rules('filterTo', 'Date To', 'trim|required');
//echo "count ".count( $this->input->post('items') ); exit;
        if ($this->form_validation->run() == FALSE) {
            $errors = validation_errors();
            echo '<div class="alert alert-danger">' . $errors . '</div>';
        } else {
    ...
    function export_excel_sales_detail_report()
    {
        ...
        $this->excel->getActiveSheet()->getStyle('A4:R4')->applyFromArray($styleArray);
        $this->excel->getActiveSheet()->fromArray($header, null, 'A4');
        $this->excel->getActiveSheet()->fromArray($details, null, 'A6');
        $this->excel->getActiveSheet()->getStyle('F6:M'.$rowCount)->applyFromArray($styleArrayNumber);
...
//        $this->datatables->add_column('item_inventryCode', '$1 - $2 <b></b>', 'itemSystemCode,itemDescription');
    ...
    function export_excel_item_master(){
        ...
        $this->excel->getActiveSheet()->getStyle('A2')->applyFromArray($styleArray);
        $this->excel->getActiveSheet()->getStyle('A4:U4')->applyFromArray($styleArray);
        $this->excel->getActiveSheet()->fromArray($header, null, 'A4');
