...
class Approvel_user extends ERP_Controller{
    ...
    function load_approvel_user(){
        ...
        $this->datatables->select('approvalUserID,levelNo,companyCode,srp_erp_approvalusers.documentID as documentID,srp_erp_approvalusers.document as document,employeeID,employeeName,fromAmount,toAmount')
            ->where($where)
      ...
      /*  ->edit_column('documentID', '$1 - $2', 'documentID,document')*/
        ...
        ->add_column('levelNo', '$1', 'showLevelno(documentID,fromAmount,toAmount,levelNo,approvalUserID)')
    ...
    function save_approveluser(){
        ...
        if($this->input->post('levelno')>0){
            ...
            if(trim($this->input->post('documentid') ?? '') == 'TRQ'){
                $this->form_validation->set_rules('type', 'Travel Request Type', 'trim|required');
            }

            /**added : almansoori chnges for personal application */
            if(trim($this->input->post('documentid') ?? '') == 'PAA'){
                $this->form_validation->set_rules('type', 'Personal Application Type', 'trim|required');
                //$this->form_validation->set_rules('criteria', 'Criteria', 'trim|required');
            }
            if(trim($this->input->post('expenseCategorycheck') ?? '') == 'EC'){
                $this->form_validation->set_rules('expenseClaimCategory', 'Category', 'trim|required');
            }
        }
    ...
    function fetch_document_closed_users_modal(){
        echo json_encode($this->Approvel_user_model->fetch_document_closed_users_modal());
    }

    function getApprovalTypeID(){
        echo json_encode($this->Approvel_user_model->getApprovalTypeID());
    }

    function getApprovalDocumentDetails(){
        echo json_encode($this->Approvel_user_model->getApprovalDocumentDetails());
    }

    /** added : almansoori chnges for personal application */
    function all_types_drop(){
        echo json_encode($this->Approvel_user_model->all_types_drop());
    }

    function get_company_approval_type()
    {
        $this->db->select("srp_erp_documentcodemaster.approvalType, srp_erp_documentcodes.specificUserYN");
        $this->db->from('srp_erp_documentcodemaster');
        $this->db->join('srp_erp_documentcodes', 'srp_erp_documentcodemaster.documentID = srp_erp_documentcodes.documentID');
        $this->db->where('srp_erp_documentcodemaster.companyID', current_companyID());
        $this->db->where('srp_erp_documentcodemaster.documentID', 'EC');
    ...
    function getSpecialUser(){
        ...
        $appravalID=$this->input->post('approvalID');
        $empID=$this->input->post('empID');
        $this->db->select("srp_erp_appoval_specific_users.*,emp.Ename2");
        $this->db->from('srp_erp_appoval_specific_users');
        $this->db->join('srp_employeesdetails emp',' emp.EIdNo=srp_erp_appoval_specific_users.empID');
        $this->db->where('approvalUserID', $appravalID);
        $this->db->where('createduserID', $empID);
        $data = $this->db->get()->result_array();
        echo json_encode($data);
    }
    ...
    function save_special_user(){
        ...
        $this->form_validation->set_rules('specialUseremp','Employee name','trim|required');
        ...
        if($this->form_validation->run()==FALSE){
            ...
            echo json_encode($this->Approvel_user_model->save_special_user());
        }

    }

    function delete_specialUser(){
        echo json_encode($this->Approvel_user_model->delete_specialUser());
    }
}