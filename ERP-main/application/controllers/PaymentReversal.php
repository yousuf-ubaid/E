...
class PaymentReversal extends ERP_Controller
{
    ...
    function fetch_payment_reversal()
    {
        ...
        $this->datatables->select("srp_erp_paymentvouchermaster.payVoucherAutoId as payVoucherAutoId,srp_erp_paymentvouchermaster.companyCode,PVcode,PVchequeNo,DATE_FORMAT(PVchequeDate,'$convertFormat') AS PVchequeDate,approvedYN ,PVdate,srp_erp_paymentvouchermaster.transactionCurrency,det.transactionAmount as amount,det.currency as detailCurrency,dets.taxPercentage as taxPercentage,det.currencydecimal as decimal,(((IFNULL(dets.taxPercentage,0)/100)*IFNULL(tyepdet.transactionAmount,0))+IFNULL(det.transactionAmount,0)-IFNULL(debitnote.transactionAmount,0)-IFNULL(SR.transactionAmount,0)) as totamount, srp_erp_paymentvouchermaster.referenceNo AS referenceNo, srp_erp_paymentvouchermaster.referenceNo As referenceNo");
        $this->datatables->join('(SELECT SUM(transactionAmount) as transactionAmount,payVoucherAutoId,srp_erp_paymentvoucherdetail.transactionCurrency as currency,srp_erp_paymentvoucherdetail.transactionCurrencyDecimalPlaces as currencydecimal FROM srp_erp_paymentvoucherdetail WHERE srp_erp_paymentvoucherdetail.type!="debitnote" AND srp_erp_paymentvoucherdetail.type!="SR" GROUP BY payVoucherAutoId) det', '(det.payVoucherAutoId = srp_erp_paymentvouchermaster.payVoucherAutoId)', 'left');
        ...
        $this->datatables->edit_column('totamount', '<div class="pull-right"><b>$2 : </b> $1 </div>', 'format_number(totamount,decimal),detailCurrency');
        $this->datatables->add_column('edit', '$1', 'load_prvr_action(payVoucherAutoId,0)');
        $this->datatables->edit_column('PVdate', '<span >$1 </span>', 'convert_date_format(PVdate)');
        //$this->datatables->edit_column('PVchequeDate', '<span >$1 </span>', 'convert_date_format(PVchequeDate)');
    ...
    function fetch_reversed_payment(){
        ...
        $convertFormat = convert_date_format_sql();
        $this->datatables->select("srp_erp_paymentvouchermaster.payVoucherAutoId as payVoucherAutoId,srp_erp_paymentvouchermaster.companyCode,srp_erp_paymentvouchermaster.PVcode as PVcode,PVchequeNo,DATE_FORMAT(PVchequeDate,'$convertFormat') AS PVchequeDate,srp_erp_paymentvouchermaster.approvedYN as approvedYN ,PVdate,srp_erp_paymentvouchermaster.transactionCurrency,det.transactionAmount as amount,det.currency as detailCurrency,dets.taxPercentage as taxPercentage,det.currencydecimal as decimal,(((IFNULL(dets.taxPercentage,0)/100)*IFNULL(tyepdet.transactionAmount,0))+IFNULL(det.transactionAmount,0)-IFNULL(debitnote.transactionAmount,0)-IFNULL(SR.transactionAmount,0)) as totamount,CONCAT(srp_erp_paymentreversalmaster.documentSystemCode ,' :- ',srp_erp_paymentreversalmaster.narration) as documentSystemCode,srp_erp_paymentreversalmaster.documentDate as documentDate, srp_erp_paymentvouchermaster.referenceNo AS referenceNo");
        $this->datatables->join('(SELECT SUM(transactionAmount) as transactionAmount,payVoucherAutoId,srp_erp_paymentvoucherdetail.transactionCurrency as currency,srp_erp_paymentvoucherdetail.transactionCurrencyDecimalPlaces as currencydecimal FROM srp_erp_paymentvoucherdetail WHERE srp_erp_paymentvoucherdetail.type!="debitnote" AND srp_erp_paymentvoucherdetail.type!="SR" GROUP BY payVoucherAutoId) det', '(det.payVoucherAutoId = srp_erp_paymentvouchermaster.payVoucherAutoId)', 'left');
        $this->datatables->join('(SELECT SUM(transactionAmount) as transactionAmount,payVoucherAutoId FROM srp_erp_paymentvoucherdetail WHERE srp_erp_paymentvoucherdetail.type="GL" OR srp_erp_paymentvoucherdetail.type="Item"  GROUP BY payVoucherAutoId) tyepdet', '(tyepdet.payVoucherAutoId = srp_erp_paymentvouchermaster.payVoucherAutoId)', 'left');
        $this->datatables->join('(SELECT IFNULL(SUM(taxPercentage),0) AS taxPercentage,payVoucherAutoId FROM srp_erp_paymentvouchertaxdetails GROUP BY payVoucherAutoId) dets', '(dets.payVoucherAutoId = srp_erp_paymentvouchermaster.payVoucherAutoId)', 'left');
        ...
        $this->datatables->join('(SELECT SUM(transactionAmount) as transactionAmount,payVoucherAutoId FROM srp_erp_paymentvoucherdetail WHERE srp_erp_paymentvoucherdetail.type="debitnote" GROUP BY payVoucherAutoId) debitnote', '(debitnote.payVoucherAutoId = srp_erp_paymentvouchermaster.payVoucherAutoId)', 'left');
        $this->datatables->join('(SELECT SUM(transactionAmount) as transactionAmount,payVoucherAutoId FROM srp_erp_paymentvoucherdetail WHERE srp_erp_paymentvoucherdetail.type="SR" GROUP BY payVoucherAutoId) SR', '(SR.payVoucherAutoId = srp_erp_paymentvouchermaster.payVoucherAutoId)', 'left');
        $this->datatables->join('srp_erp_paymentreversalmaster', '(srp_erp_paymentreversalmaster.payVoucherAutoId = srp_erp_paymentvouchermaster.payVoucherAutoId)', 'left');
        $this->datatables->from('srp_erp_paymentvouchermaster');
        //$this->datatables->edit_column('total_value', '<div class="pull-right"><b>$1 : </b>  </div>', 'transactionCurrency');
        //$this->datatables->where($where);
        //$this->datatables->where($where);
