...
if (!function_exists('load_rrvr_action')) { /*get po action list*/
    function load_rrvr_action($receiptVoucherAutoId,$reversed)
    {
        $CI =& get_instance();
        ...
        $status = '<span class="pull-right">';
            ...
            $status .= '<a onclick=\'fetch_receipt_reversal_view("' . $receiptVoucherAutoId . '",' . $reversed . '); \'><span title="View" rel="tooltip" class="glyphicon glyphicon-ok"></span></a>';
        $status .= '</span>';
        return $status;
    }
}
