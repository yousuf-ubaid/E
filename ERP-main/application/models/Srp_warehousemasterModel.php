...
class Srp_warehousemasterModel extends ERP_Model
{
    ...
    */
        ...
        if (empty($this->input->post('warehouseredit'))) {
            ...
            if ($Warehouse) {
               ...
               // $this->db->set('warehouseType', (trim(($this->input->post('ismanufacturingHN') != ""))) ? trim($this->input->post('ismanufacturingHN') ?? '') : NULL);
                $this->db->set('warehouseType', (($this->input->post('wareHouseType') != "")) ? $this->input->post('wareHouseType') : NULL);
                //   $this->db->set('isPosLocation', (($this->input->post('isPosLocation') != "")) ? $this->input->post('isPosLocation') : NULL);
                $this->db->set('isActive', (trim(($this->input->post('isActive') != ""))) ? trim($this->input->post('isActive') ?? '') : 1);
            ...
            if ($Warehouse) {
                ...
                if($this->input->post('is_mfqWarehouse_active') == 1 && trim($this->input->post('isActive') ?? '') == 0){
                    $mfq_data['isActive'] = trim($this->input->post('isActive') ?? '');
                    $this->db->where('warehouseAutoID', $this->input->post('warehouseredit'));
                    $this->db->update('srp_erp_mfq_warehousemaster', $mfq_data);
                }
                ...
                $data['warehouseType'] = ((($this->input->post('wareHouseType') != "")) ? $this->input->post('wareHouseType') : NULL);

                $this->db->set('WIPGLAutoID', (($this->input->post('glcodeid') != "")) ? $this->input->post('glcodeid') : NULL);
                $this->db->set('warehouseType', (trim(($this->input->post('wareHouseType') != ""))) ? trim($this->input->post('wareHouseType') ?? '') : NULL);
                // $this->db->set('warehouseType', (trim(($this->input->post('ismanufacturingHN') != ""))) ? trim($this->input->post('ismanufacturingHN') ?? '') : NULL);
    ...
    function get_warehouse()
    {
        ...
        $tmpArray['printTemplateMasterID'] = get_bill_print_template_id();

        $s3_status = $this->input->post('s3');

        if(isset($s3_status) && $s3_status == 'true'){
            $result1['warehouseImage'] = $this->s3->createPresignedRequest($result1['warehouseImage'], '1 hour'); // s3 attachment link
        }

        $this->db->select('*');
        $this->db->from('srp_erp_pos_shiftdetails');
        $this->db->where(['wareHouseID' => $result1['wareHouseAutoID'], 'isClosed' => 0]);
        $is_pos_shifted = $this->db->get()->row_array();

        $this->db->select('*');
        $this->db->from('srp_erp_mfq_warehousemaster');
        $this->db->where(['warehouseAutoID' => $result1['wareHouseAutoID'], 'isActive' => 1]);
        $is_mfqWarehouse_active = $this->db->get()->row_array();

        $tmpArray['is_mfqWarehouse_active'] = (!empty($is_mfqWarehouse_active) ? 1 : 0);
        $tmpArray['is_pos_shifted'] = (!empty($is_pos_shifted) ? 1 : 0);
    ...
    function saveAssignedItems(){
        ...
        $result = $this->db->query('INSERT INTO srp_erp_warehouseitems (
                                    wareHouseAutoID, wareHouseLocation, wareHouseDescription, itemAutoID,barCodeNo,salesPrice,ActiveYN,
                                    itemSystemCode, itemDescription, unitOfMeasureID,unitOfMeasure,currentStock, companyID,
                                    companyCode
                                ) SELECT
                                 '.$wareHouseAutoID.',"'.$wareHouseLocation.'", "'.$wareHouseDescription.'", itemAutoID,barcode,companyLocalSellingPrice,isActive, itemSystemCode,itemDescription,
