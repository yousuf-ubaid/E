...
")->row_array();
    ...
    function save_general_ot_template_frm()
    {
        ...
        foreach ($empID as $key => $emp) {
            ...
            $salaCatID = isset($salaCatIDs['salaryCategoryID']) ? $salaCatIDs['salaryCategoryID'] : null;
            if ($detail) {
	...
	srp_erp_pay_overtimegroupdetails.overTimeID = $caegoryID ")->row_array();
                       ...
                       /* echo '<pre>';print_r($detail_arr);echo '</pre>';
                        ...
                        if (!empty($detail_arr['formulaString'])) {
                            ...
                            if (is_array($as) && isset($as['transactionAmount'])) {
                            ...
                            if (is_array($detail_allowance) && isset($detail_allowance['nopaySystemID'])) {
                            ...
                            //print_r($as_allow);
                            if (is_array($as_allow) && isset($as_allow['transactionAmount']) && isset($hourorDays[$key])) {
                                $transactionAmount_allow = $as_allow['transactionAmount'] * $hourorDays[$key];
                            } else {
                                $transactionAmount_allow = 0; 
                            }                            
                            ...
                            $data['companyReportingCurrencyDecimalPlaces'] = $as_allow['companyReportingCurrencyDecimalPlaces'] ?? 0; 
                            ...
                            $data['nonPayrollSalaryCategoryID'] = $detail_allowance['salaryCategoryID'] ?? 0;
                        }
        ...
        if ($results) {
            return array('s', 'Details successfully saved',$data);
        }
    }
...
WHERE
    ...
    function comfirm_general_ot_template()
    {
        $MasterID = $this->input->post('MasterID');
        $this->load->library('approvals');
        $otcode = $this->db->query("SELECT otCode,documentDate FROM srp_erp_generalotmaster WHERE generalOTMasterID={$MasterID} ")->row_array();

        $confirm = $this->db->query("SELECT confirmedYN FROM srp_erp_generalotmaster WHERE generalOTMasterID={$MasterID} AND confirmedYN=1")->row_array();
        if (!empty($confirm)) {
            return array('w', 'Document already confirmed');
        } else {
            $validate_code = validate_code_duplication($otcode['otCode'], 'otCode', $MasterID,'generalOTMasterID', 'srp_erp_generalotmaster');
            if(!empty($validate_code)) {
                return ['e', 'The document Code Already Exist.(' . $validate_code . ')'];
            }

            $isAutoApproval = get_document_auto_approval('ATS');
            if($isAutoApproval == 0){ // If auto approval
                $this->db->trans_start();
                $this->approvals->auto_approve($MasterID, 'srp_erp_generalotmaster', 'generalOTMasterID', 'ATS', $otcode['otCode'], $otcode['documentDate']);
                $this->insert_record_to_attendance_review($MasterID);
                $this->db->trans_complete();
            ...
            $approvals_status = $this->approvals->CreateApproval('ATS', $MasterID, $otcode['otCode'], 'General Over Time', 'srp_erp_generalotmaster', 'generalOTMasterID', 0, $otcode['documentDate']);
            if ($approvals_status == 1) {
                return ['s', 'Approvals created successfully'];
            }
            if ($approvals_status == 3) {
                return ['w', 'There are no users exist to perform \'Attendance Summary\' approval for this company.'];
            } else {
    ...
    function save_general_ot_approval()
    {
        ...
        if ($approvals_status) {
            if ($approvals_status == 1) {
                $this->insert_record_to_attendance_review($system_code);
            }
    ...
    function insert_record_to_attendance_review($system_code){
        $companyID = current_companyID();
    ...
    function delete_general_ot_template()
    {
        ...
        if (!empty($confirmed)) {
            ...
            if ($status) {
                $this->db->delete('srp_erp_generalotdetail', array('generalOTMasterID' => trim($generalOTMasterID)));
