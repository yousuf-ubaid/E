...
--REVISION HISTORY
--Date: 21 - NOV 2016 : comment started

============================================================
--Date: 21 - NOV 2016 : comment started
--Date: 14 - AUG 2018 : Bug fixes in the report - loaded source from menu sales payment, dashboard figures mismatched result resolved.
...
class Pos_dashboard_model extends ERP_Model
{
    ...
    function get_sales_profit_generalPOS($interval = 'yesterday')
    {
        ...
        $this->db->select("sum( IFNULL( id.transactionAmount, 0 ) ) AS totalRev, (sum( IFNULL( id.transactionAmount, 0 ) )) -  sum(( IFNULL( id.wacAmount, 0 ) ) * ( IFNULL( id.qty, 0 ) )) AS profit");
        $this->db->from("srp_erp_pos_invoicedetail id");
        $this->db->join("srp_erp_pos_invoice invoice",'invoice.invoiceID = id.invoiceID','left');
        ...
        if (!empty($result)) {
            ...
            return array('sales' => format_number($sales, 0, '.', ''), 'profit' => format_number($profit, 0, '.', ''));
        } else {
            return array('sales' => 0, 'profit' => 0);
        }
    }
    ...
    function getLastSevenDaySalesGeneral()
    {
        ...
        $q = "SELECT invoiceDate, sum(netTotal) as totalSales , count(invoiceDate) AS countSalesDay from srp_erp_pos_invoice WHERE invoiceDate BETWEEN '" . $startDate . "' AND '" . $endDate . "' AND companyID = " . current_companyID() . "  And isVoid = 0  GROUP BY invoiceDate";
        $result = $this->db->query($q)->result_array();
    ...
    function get_itemizedSalesReportByFilters($dateFrom, $dateTo, $cashier)
    {
        ...
        if ($outletID > 0) {
            $where_tmp[] = " (srp_erp_pos_invoice.wareHouseAutoID = '" . $outletID . "') ";
        }
        ...
        if ($Item != null) {
            $where_tmp[] = " (srp_erp_itemmaster.itemAutoID IN(" . $Item . ")) ";
        }

        if ($cashier != null) {
            $where_tmp[] = " (srp_erp_pos_invoice.createdUserID IN(" . $cashier . ")) ";
        }
        ...
        if ($mainCategoryID != null) {
            $mainCategoryID=join(',',$mainCategoryID);
            $where_tmp[] = " (srp_erp_itemmaster.mainCategoryID IN(" . $mainCategoryID . ")) ";
        }
        if ($subcategoryID != null) {
            $subcategoryID=join(',',$subcategoryID);
            $where_tmp[] = " (srp_erp_itemmaster.subcategoryID IN(" . $subcategoryID . ")) ";
        }
        if ($subSubCategoryID != null) {
            $subSubCategoryID=join(',',$subSubCategoryID);
            $where_tmp[] = " (srp_erp_itemmaster.subSubCategoryID IN(" . $subSubCategoryID . ")) ";
        }

        $where_tmp[] = " ( srp_erp_pos_invoice.createdDateTime  BETWEEN '" . $dateFrom . " 00:00:00' AND '" . $dateTo . " 23:59:59' ) AND srp_erp_pos_invoice.isVoid = 0 AND srp_erp_pos_invoice.companyID = " . current_companyID();

        $where = join('AND', $where_tmp);

        $this->db->select("srp_erp_pos_invoice.invoiceID, srp_erp_pos_invoice.invoiceDate, srp_erp_pos_invoicedetail.invoiceDetailsID, srp_erp_pos_invoicedetail.itemDescription, srp_erp_pos_invoicedetail.itemSystemCode AS itemSystemCode, SUM(srp_erp_pos_invoicedetail.qty) as qty, SUM(srp_erp_pos_invoicedetail.transactionAmount) as price,srp_erp_itemmaster.seconeryItemCode as seconeryItemCode,srp_erp_itemmaster.barcode as barcode,srp_erp_itemmaster.mainCategory as mainCategory,srp_erp_itemcategory.description as subCategory,subct.description as subsubCategory");
        ...
        $this->db->join("srp_erp_itemmaster", "srp_erp_pos_invoicedetail.itemAutoID = srp_erp_itemmaster.itemAutoID", "LEFT");
        $this->db->join("srp_erp_itemcategory", "srp_erp_itemmaster.subcategoryID = srp_erp_itemcategory.itemCategoryID", "LEFT");
        $this->db->join("srp_erp_itemcategory subct", "srp_erp_itemmaster.subSubCategoryID = subct.itemCategoryID", "LEFT");
        // $this->db->join("srp_erp_pos_salesreturndetails", "srp_erp_pos_invoicedetail.invoiceID = srp_erp_pos_salesreturndetails.invoiceID and srp_erp_pos_invoicedetail.itemAutoID = srp_erp_pos_salesreturndetails.itemAutoID", "LEFT");
        $this->db->where($where);
    ...
    function get_itemizedSalesReport($dateFrom, $dateTo)
    {
        ...
        $this->db->select("srp_erp_pos_invoice.invoiceID, srp_erp_pos_invoice.invoiceDate, srp_erp_pos_invoicedetail.invoiceDetailsID, srp_erp_pos_invoicedetail.itemDescription, srp_erp_pos_invoicedetail.itemSystemCode AS itemSystemCode, SUM(srp_erp_pos_invoicedetail.qty) as qty, SUM(srp_erp_pos_invoicedetail.transactionAmount) as price,SUM(IFNULL(srp_erp_pos_salesreturndetails.qty,0)) AS returnqty,SUM(IFNULL(srp_erp_pos_salesreturndetails.transactionAmount,0))AS returnprice,srp_erp_itemmaster.seconeryItemCode as seconeryItemCode,srp_erp_itemmaster.barcode as barcode,srp_erp_itemmaster.mainCategory as mainCategory,srp_erp_itemcategory.description as subCategory");
        $this->db->from('srp_erp_pos_invoice');
        $this->db->join("srp_erp_pos_invoicedetail", "srp_erp_pos_invoice.invoiceID = srp_erp_pos_invoicedetail.invoiceID", "LEFT");
        $this->db->join("srp_erp_itemmaster", "srp_erp_pos_invoicedetail.itemAutoID = srp_erp_itemmaster.itemAutoID", "LEFT");
        $this->db->join("srp_erp_itemcategory", "srp_erp_itemmaster.subcategoryID = srp_erp_itemcategory.itemCategoryID", "LEFT");
    ...
    {
        $q = "SELECT id.itemDescription, sum( IFNULL( id.qty, 0 ) ) qtySum,sum(IFNULL(id.transactionAmount, 0)) transactionAmount
              FROM
    ...
    function get_itemizedSalesReportByFiltersDetail($dateFrom, $dateTo, $cashier)
    {
        ...
        if(!empty($InvoiceCode)){
            foreach ($InvoiceCode as $index=>$key){
            if(!is_numeric($key)){
                unset($InvoiceCode[$index]);
            }
//                $invoiceID = $this->db->query("select invoiceID from srp_erp_pos_invoice where invoiceCode='$key'")->row('invoiceID');
//                array_push($InvoiceIdArray,$invoiceID);
            }
        }
        $companyID=current_companyID();
        if (isset($outlet) && !empty($outlet)) {
        ...
        if (isset($InvoiceCode) && !empty($InvoiceCode)) {
            $tmpinv = join(",", $InvoiceCode);
            $inv = $tmpinv;
        } else {
            $inv = null;
        }
        ...
        if ($inv != null) {
            $where_tmp[] = " (srp_erp_pos_invoice.invoiceID IN(" . $inv . ")) ";
        }
        ...
        $this->db->select("srp_erp_pos_invoice.invoiceCode,srp_erp_pos_invoice.invoiceID,srp_erp_pos_invoice.documentSystemCode, srp_erp_pos_invoice.invoiceDate, srp_erp_pos_invoicedetail.invoiceDetailsID, srp_erp_pos_invoicedetail.itemDescription, srp_erp_pos_invoicedetail.itemSystemCode AS itemSystemCode, SUM(srp_erp_pos_invoicedetail.qty) as qty, SUM(srp_erp_pos_invoicedetail.transactionAmount) as price,SUM(IFNULL(srp_erp_pos_salesreturndetails.qty,0)) AS returnqty,SUM(IFNULL(srp_erp_pos_salesreturndetails.transactionAmount,0))AS returnprice,srp_erp_itemmaster.seconeryItemCode as seconeryItemCode,srp_erp_itemmaster.barcode as barcode,srp_erp_itemmaster.mainCategory as mainCategory,srp_erp_itemcategory.description as subCategory,subct.description as subsubCategory,IFNULL(rtn.	totalreturn,0) as totalreturn");
        $this->db->from('srp_erp_pos_invoice');
        $this->db->join("srp_erp_pos_invoicedetail", "srp_erp_pos_invoice.invoiceID = srp_erp_pos_invoicedetail.invoiceID", "LEFT");
        $this->db->join("srp_erp_itemmaster", "srp_erp_pos_invoicedetail.itemAutoID = srp_erp_itemmaster.itemAutoID", "LEFT");
        $this->db->join("srp_erp_itemcategory", "srp_erp_itemmaster.subcategoryID = srp_erp_itemcategory.itemCategoryID", "LEFT");
        ...
        $this->db->join('(SELECT SUM( netTotal ) AS totalreturn, invoiceID FROM srp_erp_pos_salesreturn WHERE companyID = '.$companyID.'  GROUP BY invoiceID) rtn', '(rtn.invoiceID = srp_erp_pos_invoice.invoiceID)', 'left');

        $this->db->join("srp_erp_pos_salesreturndetails", "srp_erp_pos_invoicedetail.invoiceID = srp_erp_pos_salesreturndetails.invoiceID and srp_erp_pos_invoicedetail.itemAutoID = srp_erp_pos_salesreturndetails.itemAutoID", "LEFT");
        $this->db->where($where);
        $this->db->group_by("srp_erp_pos_invoicedetail.itemAutoID");
        $this->db->group_by("srp_erp_pos_invoicedetail.invoiceID");
    ...
    function get_itemizedRefundReport($dateFrom, $dateTo, $cashier){
        ...
        if ($outletID > 0) {
            $where_tmp[] = " (rtn.wareHouseAutoID = '" . $outletID . "') ";
        }

        if ($Outlets != null) {
            $where_tmp[] = " (rtn.wareHouseAutoID IN(" . $Outlets . ")) ";
        }
        ...
        if ($cashier != null) {
            $where_tmp[] = " (rtn.createdUserID IN(" . $cashier . ")) ";
        }
        ...
        $q = " SELECT
                invoice.invoiceCode,
                invoice.invoiceID,
                invoice.documentSystemCode,
                rtn.createdDateTime,
                rtn_det.invoiceDetailID,
                rtn_det.itemDescription,
                rtn.returnMode,
                rtn_det.itemSystemCode AS itemSystemCode,
                rtn.salesReturnID,
                srp_erp_itemmaster.seconeryItemCode AS seconeryItemCode,
                srp_erp_itemmaster.barcode AS barcode,
                srp_erp_itemmaster.mainCategory AS mainCategory,
                srp_erp_itemcategory.description AS subCategory,
                subct.description AS subsubCategory,
                SUM(IFNULL(rtn_det.qty,0)) AS returnqty,
                -- SUM(IFNULL( rtn_det.transactionAmount, 0 ) + IFNULL( rtn_det.taxAmount, 0 )) AS returnprice
                SUM( (IFNULL( rtn_det.transactionAmount, 0 ) + IFNULL( rtn_det.taxAmount, 0 ))  - (rtn_det.generalDiscountPercentage/100 * (IFNULL( rtn_det.transactionAmount, 0 ) + IFNULL( rtn_det.taxAmount, 0 ))) ) AS returnprice 
               FROM
               srp_erp_pos_salesreturndetails AS rtn_det
               LEFT JOIN srp_erp_pos_invoice AS invoice ON invoice.invoiceID = rtn_det.invoiceID
               LEFT JOIN srp_erp_pos_salesreturn AS rtn ON rtn.salesReturnID = rtn_det.salesReturnID
               LEFT JOIN srp_erp_itemmaster ON rtn_det.itemAutoID = srp_erp_itemmaster.itemAutoID
               LEFT JOIN srp_erp_itemcategory ON srp_erp_itemmaster.subcategoryID = srp_erp_itemcategory.itemCategoryID
               LEFT JOIN srp_erp_itemcategory AS subct ON srp_erp_itemmaster.subSubCategoryID = subct.itemCategoryID 
               WHERE
                isVoid = 0
                AND rtn_det.companyID = '$companyID' 
                AND rtn.createdDateTime BETWEEN '" . $dateFrom . " 00:00:00' AND '" . $dateTo . " 23:59:59'
                AND ". $where . "
               GROUP BY rtn_det.invoiceID,rtn_det.itemAutoID
...
//var_dump($this->db->last_query());exit;
    ...
    function get_itemWise_refundDetails($dateFrom, $dateTo, $cashier){
        $outletID = $this->input->post('outletID');
        $outlet = $this->input->post('outletID_f');
        $items = $this->input->post('items');
        ...
        if ($inv != null) {
            $where_tmp[] = " (rtn.invoiceID IN(" . $inv . ")) ";
        }
        ...
        if (isset($filter['wareHouseAutoID']) && $filter['wareHouseAutoID']) {
            $this->db->where('wareHouseAutoID', $filter['wareHouseAutoID']);
            $warehouse = "AND wareHouseAutoID = ".$filter['wareHouseAutoID'];
        }
        $where = join('AND', $where_tmp);
