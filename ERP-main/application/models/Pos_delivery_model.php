...
class Pos_delivery_model extends ERP_Model
{
    ...
    function confirm_delivery_order()
    {
        ...
        try {
            ...
            if (!$deliveryOrderID) {
                ...
                $this->db->update('srp_erp_pos_menusalesmaster', array('customerID' => $posCustomerAutoID), array('menuSalesID' => $invoiceID));
    ...
    function get_deliveryOrder($invoiceID, $outletID = 0)
    {
        if ($outletID == 0) {
            $outletID = get_outletID();
        }
        ...
        $this->db->where('orders.wareHouseAutoID', $outletID);
        $result = $this->db->get()->row_array();
        return $result;
    }

    function get_deliveryOrder_by_deliveryOrderID($deliveryOrderID, $outletID = 0)
    {
        if ($outletID == 0) {
            $outletID = get_outletID();
        }
        ...
        //$this->db->where('orders.id_store', $outletID);
        $this->db->where('orders.wareHouseAutoID', $outletID);
        $result = $this->db->get()->row_array();
        return $result;
    }
    ...
    function get_totalAdvance($invoiceID, $outletID = 0)
    {
        if ($outletID == 0) {
            $outletID = get_outletID();
        }
    ...
    function send_pos_email_advancePayment_deilvery($msg)
    {
        ...
        if (!$mail->Send()) {
            echo json_encode(array("error" => 1, "message" => "Mailer Error: ". $mail->ErrorInfo));
        } else {
