...
(((IFNULL(addondet.taxPercentage, 0) / 100) * IFNULL(tyepdet.transactionAmount,0)) + IFNULL(det.transactionAmount, 0) - IFNULL(Creditnots.transactionAmount,0))/companyLocalExchangeRate AS companyLocalAmount,
(((IFNULL(addondet.taxPercentage, 0) / 100) * IFNULL(tyepdet.transactionAmount,0)) + IFNULL(det.transactionAmount, 0) - IFNULL(Creditnots.transactionAmount,0))/companyReportingExchangeRate AS companyReportingAmount
...
WHERE
	`srp_erp_customerreceiptmaster`.`receiptVoucherAutoId` = '.$rvautoid.'

')->row_array();


        $creditnoteAmount = $this->db->query('SELECT
 IFNULL(Creditnots.transactionAmount,0) AS transactionAmount,
IFNULL(Creditnots.transactionAmount,0)/companyLocalExchangeRate AS companyLocalAmount,
IFNULL(Creditnots.transactionAmount,0)/companyReportingExchangeRate AS companyReportingAmount
...
AND (
	...
	(IFNULL(Advance.transactionAmount,0) + IFNULL(Invoice.transactionAmount, 0))-IFNULL(Creditnots.transactionAmount, 0) as Amount,((IFNULL(Advance.transactionAmount,0) + IFNULL(Invoice.transactionAmount, 0))-IFNULL(Creditnots.transactionAmount, 0))/srp_erp_customerreceiptmaster.companyLocalExchangeRate AS compLocalAmount,((IFNULL(Advance.transactionAmount,0) + IFNULL(Invoice.transactionAmount, 0))-IFNULL(Creditnots.transactionAmount, 0))/srp_erp_customerreceiptmaster.companyReportingExchangeRate AS compReportingAmount,srp_erp_customermaster.receivableAutoID,srp_erp_customermaster.receivableSystemGLCode,srp_erp_customermaster.receivableGLAccount,srp_erp_customermaster.receivableDescription,srp_erp_customermaster.receivableType,srp_erp_chartofaccounts.GLAutoID,srp_erp_chartofaccounts.GLSecondaryCode,srp_erp_chartofaccounts.systemAccountCode,srp_erp_chartofaccounts.GLDescription,srp_erp_chartofaccounts.subCategory
...
LEFT JOIN (
	...
	`Advance`.`receiptVoucherAutoId` = srp_erp_customerreceiptmaster.receiptVoucherAutoId
)
...
LEFT JOIN (
	...
	`Invoice`.`receiptVoucherAutoId` = srp_erp_customerreceiptmaster.receiptVoucherAutoId
)

WHERE srp_erp_customerreceiptmaster.receiptVoucherAutoId = '.$rvautoid.' GROUP BY srp_erp_customerreceiptmaster.receiptVoucherAutoId')->row_array();
...
WHERE srp_erp_customerreceiptmaster.receiptVoucherAutoId = '.$rvautoid.' GROUP BY srp_erp_customerreceiptmaster.receiptVoucherAutoId')->row_array();
                ...
                if ($payReversalMaster) {
                    if ($rvmaster['RVType'] == 'Invoices' || $rvmaster['RVType'] == 'CustomerInvoices') {
                        ...
                        if ($payPaymentVoucherM) {
                            ...
                            if ($payVouvherD) {
                                if(!empty($rvdetailSupp) && $rvdetailSupp['transactionAmount']>0){
                                ...
                                if(!empty($creditnoteAmount) && $creditnoteAmount['transactionAmount']>0){

                                    $CNdata['documentID'] = "CN";
                                    $CNdata['companyFinanceYearID'] = $companyFinanceYearID['companyFinanceYearID'];
                                    $CNdata['FYPeriodDateFrom'] = $companyFinanceYearID['dateFrom'];
                                    $CNdata['FYPeriodDateTo'] = $companyFinanceYearID['dateTo'];
                                    $CNdata['companyFinancePeriodID'] = $companyFinanceYearID['companyFinancePeriodID'];
                                    $CNdata['creditNoteCode'] = $this->sequence->sequence_generator('CN');
                                    $CNdata['creditNoteDate'] = $reversalDate;
                                    $CNdata['comments'] = 'Receipt reversal of receipt voucher ' . $rvmaster['RVcode'];
                                    $CNdata['creditNoteDate'] = $reversalDate;
                                    $CNdata['customerID'] = $rvmaster['customerID'];
                                    $CNdata['customerCode'] = $rvmaster['customerSystemCode'];
                                    $CNdata['customerName'] = $rvmaster['customerName'];
                                    $CNdata['customerAddress'] = $rvmaster['customerAddress'];
                                    $CNdata['customerTelephone'] = $rvmaster['customerTelephone'];
                                    $CNdata['customerFax'] = $rvmaster['customerFax'];
                                    $CNdata['customerReceivableAutoID'] = $rvmaster['customerreceivableAutoID'];
                                    $CNdata['customerReceivableSystemGLCode'] = $rvmaster['customerreceivableSystemGLCode'];
                                    $CNdata['customerReceivableGLAccount'] = $rvmaster['customerreceivableGLAccount'];
                                    $CNdata['customerReceivableDescription'] = $rvmaster['customerreceivableDescription'];
                                    $CNdata['customerReceivableType'] = $rvmaster['customerreceivableType'];
                                    $CNdata['transactionCurrencyID'] = $rvmaster['transactionCurrencyID'];
                                    $CNdata['transactionCurrency'] = $rvmaster['transactionCurrency'];
                                    $CNdata['transactionExchangeRate'] = $rvmaster['transactionExchangeRate'];
                                    $CNdata['transactionAmount'] = $creditnoteAmount['transactionAmount'];
                                    $CNdata['transactionCurrencyDecimalPlaces'] = $rvmaster['transactionCurrencyDecimalPlaces'];
                                    $CNdata['companyLocalCurrencyID'] = $rvmaster['companyLocalCurrencyID'];
                                    $CNdata['companyLocalCurrency'] = $rvmaster['companyLocalCurrency'];
                                    $CNdata['companyLocalExchangeRate'] = $rvmaster['companyLocalExchangeRate'];
                                    $CNdata['companyLocalAmount'] = $creditnoteAmount['companyLocalAmount'];
                                    $CNdata['companyLocalCurrencyDecimalPlaces'] = $rvmaster['companyLocalCurrencyDecimalPlaces'];
                                    $CNdata['companyReportingCurrencyID'] = $rvmaster['companyReportingCurrencyID'];
                                    $CNdata['companyReportingCurrency'] = $rvmaster['companyReportingCurrency'];
                                    $CNdata['companyReportingExchangeRate'] = $rvmaster['companyReportingExchangeRate'];
                                    $CNdata['companyReportingAmount'] = $creditnoteAmount['companyReportingAmount'];
                                    $CNdata['companyReportingCurrencyDecimalPlaces'] = $rvmaster['companyReportingCurrencyDecimalPlaces'];
                                    $CNdata['customerCurrencyID'] = $rvmaster['customerCurrencyID'];
                                    $CNdata['customerCurrency'] = $rvmaster['customerCurrency'];
                                    $CNdata['customerCurrencyExchangeRate'] = $rvmaster['customerExchangeRate'];
                                    $CNdata['customerCurrencyAmount'] = $creditnoteAmount['transactionAmount']/$rvmaster['customerExchangeRate'];
                                    $CNdata['customerCurrencyDecimalPlaces'] = $rvmaster['customerCurrencyDecimalPlaces'];
                                    $CNdata['confirmedYN'] = 1;
                                    $CNdata['confirmedByEmpID'] = $this->common_data['current_userID'];
                                    $CNdata['confirmedByName'] = $this->common_data['current_user'];
                                    $CNdata['confirmedDate'] = current_date();
                                    $CNdata['approvedYN'] = 1;
                                    $CNdata['approvedbyEmpID'] = $this->common_data['current_userID'];
                                    $CNdata['approvedbyEmpName'] = $this->common_data['current_user'];
                                    $CNdata['approvedDate'] = current_date();
                                    $CNdata['companyCode'] = $this->common_data['company_data']['company_code'];
                                    $CNdata['companyID'] = $this->common_data['company_data']['company_id'];
                                    $CNdata['createdUserGroup'] = $this->common_data['user_group'];
                                    $CNdata['createdPCID'] = $this->common_data['current_pc'];
                                    $CNdata['createdUserID'] = $this->common_data['current_userID'];
                                    $CNdata['createdUserName'] = $this->common_data['current_user'];
                                    $CNdata['createdDateTime'] = $this->common_data['current_date'];
                                    $CNM = $this->db->insert('srp_erp_creditnotemaster', $CNdata);
                                    $creditNoteMasterAutoID= $this->db->insert_id();

                                    if($CNM){
                                        $CNDdata['creditNoteMasterAutoID'] = $creditNoteMasterAutoID;
                                        $CNDdata['description'] = 'Receipt reversal of receipt voucher ' . $rvmaster['RVcode'];
                                        $CNDdata['transactionAmount'] = $creditnoteAmount['transactionAmount'];
                                        $CNDdata['companyLocalAmount'] = $creditnoteAmount['companyLocalAmount'];
                                        $CNDdata['companyLocalExchangeRate'] = $rvmaster['companyLocalExchangeRate'];
                                        $CNDdata['companyReportingAmount'] = $creditnoteAmount['companyReportingAmount'];
                                        $CNDdata['companyReportingExchangeRate'] = $rvmaster['companyReportingExchangeRate'];
                                        $CNDdata['customerAmount'] = $creditnoteAmount['transactionAmount']/$rvmaster['customerExchangeRate'];
                                        $CNDdata['customerCurrencyExchangeRate'] = $rvmaster['customerExchangeRate'];
                                        $CNDdata['companyCode'] = $this->common_data['company_data']['company_code'];
                                        $CNDdata['companyID'] = $this->common_data['company_data']['company_id'];
                                        $CNDdata['createdUserGroup'] = $this->common_data['user_group'];
                                        $CNDdata['createdPCID'] = $this->common_data['current_pc'];
                                        $CNDdata['createdUserID'] = $this->common_data['current_userID'];
                                        $CNDdata['createdUserName'] = $this->common_data['current_user'];
                                        $CNDdata['createdDateTime'] = $this->common_data['current_date'];
                                        $CND = $this->db->insert('srp_erp_creditnotedetail', $CNDdata);
                                    }
                                }
                                ...
                                if($bankGlBankLeager['GLAutoID']){
                                    ...
                                    $invGLbnk['transactionAmount'] = ($rvdetail['transactionAmount'])*-1;
                                    ...
                                    $invGLbnk['companyLocalAmount'] = ($rvdetail['companyLocalAmount']+$bankTaxamount)*-1;
                                    ...
                                    $invGLbnk['companyReportingAmount'] = ($rvdetail['companyReportingAmount']+$bankTaxamount)*-1;
    ...
    function fetch_receipt_reversal_master($receiptReversalAutoID){
        
        $this->db->select('rrvr.*,pm.payVoucherAutoId');
        $this->db->from('srp_erp_receiptreversalmaster as rrvr');
        $this->db->join('srp_erp_paymentvouchermaster as pm','rrvr.receiptReversalAutoID = pm.rrvrID','left');
        $this->db->where('rrvr.receiptReversalAutoID', $receiptReversalAutoID);
