...
class Suppliermastergroup_model extends ERP_Model
{
    ...
    function save_supplier_master()
    {
       ...
       // $liability = fetch_gl_account_desc(trim($this->input->post('liabilityAccount') ?? ''));
        $liability = fetch_gl_account_desc_cus_group(trim($this->input->post('liabilityAccount') ?? ''));
        ...
        if (trim($this->input->post('groupSupplierAutoID') ?? '')) {
            ...
            $data['groupSupplierSystemCode'] = $this->sequence->sequence_generator_group(
                'SUP',
                0,
                $grpid,
    ...
    function save_supplier_link()
    {
        ...
        $results=$this->db->delete('srp_erp_groupsupplierdetails', array('companyGroupID' => $grpid, 'groupSupplierMasterID' => $this->input->post('groupSupplierMasterID')));
    ...
    function save_supplier_duplicate(){
        ...
        $policySupplier = getPolicyValues('SM', 'All');
        $this->load->library('sequence');

        $supplierSystemCode = null;
        ...
        foreach($companyid as $val)
        {
            ...
            $this->db->where('groupChartofAccountMasterID', $CurrentCus['liabilityAutoID']);
            $this->db->where('companyID', $val);
            $this->db->where('companyGroupID', $grpid);
            ...
            if (!empty($CurrentCOAexsist))
            {
                ...
                array_push($comparr, array("companyname" => $companyName['company_name'], "message" => "Supplier name already exist" . " (" . $CurrentCus['groupSupplierName'] . ")"));
            }
            ...
            if ($i==0)
            {
                if (empty($linkexsist))
                {
                    ...
                    $data['supplierName'] = $CurrentCus['groupSupplierName'];
                    ...
                    $data['liabilityAutoID'] = $categoryCOAexsist['chartofAccountID'];
                    $lieglDet=fetch_gl_account_desc_cus_group_company($categoryCOAexsist['chartofAccountID'],$val);
                    $data['liabilitySystemGLCode'] = $lieglDet['systemAccountCode'];
                    $data['liabilityGLAccount'] = $lieglDet['GLSecondaryCode'];
                    $data['liabilityDescription'] = $lieglDet['GLDescription'];
                    $data['liabilityType'] = $lieglDet['subCategory'];
                    ...
                    if (1 == $policySupplier && false === $sequenceGenerated)
                    {
                        $supplierSystemCode = $this->sequence->sequence_generator_group('SUP', 0, $grpid, $this->common_data['company_data']['groupCode'], $grpid);
                        $sequenceGenerated = true;
                    }
                    
                    if (0 == $policySupplier)
                    {
                        $supplierSystemCode = $this->sequence->sequence_generator_group('SUP', 0, $val, $companyCode['company_code']);
                    }

                    $data['supplierSystemCode'] = $supplierSystemCode;
    ...
    function updategroppolicy()
    {
        ...
        if ($results) {
            return array('s', 'Supplier Master policy updated successfully');
        } else {
            return array('e', 'Supplier Master policy updated failed');
        }
