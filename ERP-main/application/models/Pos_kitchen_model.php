...
class Pos_kitchen_model extends ERP_Model
{
    ...
    function get_invoiceIDs_pendingOrders_API($kitchenLocationID = null,$warehouseID,$companyID)
    {

        $isHoldBillShow = $this->pos_policy->AutoPrintKOTonHoldAPI($warehouseID,$companyID);

        if($isHoldBillShow){
            $holdBillFilter = "(salesMaster.isOrderPending = 1 || salesMaster.isHold = 1)";
        }else{
            $holdBillFilter = "salesMaster.isOrderPending = 1";
        }
...
//var_dump($this->db->last_query());exit;
    ...
    function get_invoiceIDs_pendingOrders_autoPrint($kitchenLocationID = null)
    {
        ...
        $isHoldBillShow = $this->pos_policy->AutoPrintKOTonHold();
...
//var_dump($this->db->last_query());exit;
    ...
    function updateSendToKitchen()
    {
        ...
        if ($result) {
            ...
            $q = "UPDATE `srp_erp_pos_menusalesitems` SET `KOTAlarm` = 0, `isOrderPending` = 1, isSamplePrinted = 1   WHERE `menuSalesID` = '" . $invoiceID . "' AND ( `KOTAlarm` = -1 OR `isOrderPending` = -1) AND id_store='" . $outletID . "'";
            ...
            if ($result) {
                ...
                if(empty($KOTStartexist['KOTStartDateTime'])){
                    $tmpData['KOTStartDateTime'] = "$KOTStartDateTime";
                }
                $tmpData['preparationTime'] = 20;
    ...
    function get_srp_erp_pos_menusalesmaster_kitchenStatus_API($id,$warehouseID)
    {
        $this->db->select("menuSales.*, cType.customerDescription, delivery.deliveryOrderID, delivery.deliveryDate, delivery.deliveryTime, table.diningTableDescription, crew.crewLastName as crewLastName,cusm.CustomerName as cusname,cusm.CustomerAddress1 as cusaddress,cusm.customerTelephone as custel");
        ...
        $this->db->where('menuSales.wareHouseAutoID', $warehouseID);
        $result = $this->db->get()->row_array();
        return $result;
    }
    ...
    function get_srp_erp_pos_menusalesitems_invoiceID_kotAlarm_API($invoiceID, $kotID = null,$outletID)
    {
...
//                salesMaster.preparationTime,
//                salesMaster.KOTStartDateTime,
//                kitchen.kitchenLocationID,
//                salesItem.kotID AS kotID,
//								salesItem.menuSalesID,
//								salesItem.menuSalesItemID,
//								salesItem.isOrderPending,
//								salesItem.isOrderInProgress,
//								salesItem.isOrderCompleted,
//								salesItem.kotID,
//								salesItem.qty,
//								salesItem.discountPer,
//								salesItem.discountAmount,
//								salesItem.kitchenNote,
//								category.autoID,
//								warehouse.warehouseMenuID,
...
//								menuMaster.menuMasterID,
//								concat( '$path', menuMaster.menuImage ) AS menuImage,
//								kitchen.description as KOT_description,
//								kitchen.description as kitchenName
//                FROM
//                    srp_erp_pos_menusalesmaster salesMaster
//                LEFT JOIN srp_erp_pos_menusalesitems salesItem ON salesItem.menuSalesID = salesMaster.menuSalesID
//                LEFT JOIN srp_erp_pos_warehousemenumaster warehouse ON warehouse.warehouseMenuID = salesItem.warehouseMenuID
...
//                LEFT JOIN srp_erp_pos_kitchenlocation kitchen ON kitchen.kitchenLocationID =  warehouse.kotID
//                LEFT JOIN srp_erp_pos_menusize ms ON ms.menuSizeID = menuMaster.menuSizeID
//                LEFT JOIN srp_erp_pos_warehousemenucategory category ON warehouse.warehouseMenuCategoryID = category.autoID
//                WHERE
//                salesItem.isOrderPending = 1
//                AND salesItem.isOrderInProgress = 0
//                AND salesMaster.companyID='$companyID' AND salesMaster.wareHouseAutoID='$outletID'
//                AND salesMaster.createdDateTime >= DATE_SUB(NOW(),INTERVAL 3 HOUR)
//                GROUP BY
//	                salesMaster.menuSalesID
//                ORDER BY
//                    salesMaster.menuSalesID");
    ...
    function progressOrder_Countdown_master($id)
    {
        $outletID = get_outletID();
        $data['isOrderPending'] = 1;
        $data['isOrderInProgress'] = 1;
        $data['isOrderCompleted'] = 1;
        $data['KOTEndDateTime'] = $this->input->post('currdtTime');
        $data['KOTDoneByEmpID'] = current_userID();
        $this->db->where('menuSalesID', $id);
        $this->db->where('id_store', $outletID);
        $result = $this->db->update('srp_erp_pos_menusalesmaster', $data);
        return $result;
    ...
    function get_pendingOrders_countdown_kitchen2($kitchenLocationID = null)
    {
        $counterInfo = get_counterData();
        //$shiftID = $counterInfo['shiftID'];
        $warehouseID = get_outletID(); //$counterInfo['wareHouseID'];
    ...
    function getWaiterNameByMenuSalesID($menuSalesID){
