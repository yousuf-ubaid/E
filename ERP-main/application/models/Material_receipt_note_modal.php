...
class Material_receipt_note_modal extends ERP_Model
{
    function save_material_receipt_header()
    {
        $this->db->trans_start();
        $date_format_policy = date_format_policy();
        $isuDate = $this->input->post('receivedDate');
        ...
        $data['receivedDate'] = trim($issueDate);
        $data['RefNo'] = trim($this->input->post('issueRefNo') ?? '');
        ...
        if (trim($this->input->post('mrnAutoID') ?? '')) {
            $this->db->where('mrnAutoID', trim($this->input->post('mrnAutoID') ?? ''));
            $this->db->update('srp_erp_materialreceiptmaster', $data);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                $this->session->set_flashdata('e', 'Material Receipt Note Update Failed ' . $this->db->_error_message());
                ...
                //update material receipt note
                $this->update_pull_purchase_order_items($this->input->post('mrnAutoID'),$purchaseOrderID);

                $this->session->set_flashdata('s', 'Material Receipt Note Updated Successfully.');
                $this->db->trans_commit();
                return array('status' => true, 'last_id' => $this->input->post('mrnAutoID'), 'receiptType' => $this->input->post('receiptType'));
            }
        } else {
            ...
            //$data['mrnCode'] = $this->sequence->sequence_generator($data['documentID']);
            $this->db->insert('srp_erp_materialreceiptmaster', $data);
            ...
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                $this->session->set_flashdata('e', 'Material Receipt Note Saved Failed ' . $this->db->_error_message());
                $this->db->trans_rollback();
                return array('status' => false);
            } else {
                update_warehouse_items();
                ...
                //update material receipt note
                $this->update_pull_purchase_order_items($last_id,$purchaseOrderID);


                $this->session->set_flashdata('s', 'Material Receipt Note Saved Successfully.');
                $this->db->trans_commit();
                return array('status' => true, 'last_id' => $last_id, 'receiptType' => $this->input->post('receiptType'));
            }
        }
    }

    function update_pull_purchase_order_items($mrnAutoID,$purchaseOrderID){

        //get purchase order items
        $this->db->where('purchaseOrderID',$purchaseOrderID);
        $items = $this->db->from('srp_erp_purchaseorderdetails')->get()->result_array();

        //master record
        $this->db->where('mrnAutoID',$mrnAutoID);
        $masterMrn = $this->db->from('srp_erp_materialreceiptmaster')->get()->row_array();
        ...
        foreach($items as $item){
            ...
            $ex_item = $this->db->where('mrnAutoID',$mrnAutoID)->where('itemAutoID',$item['itemAutoID'])->where('companyID',$this->common_data['company_data']['company_id'])->from('srp_erp_materialreceiptdetails')->get()->row_array();

            if(empty($ex_item)){

                $itemAutoID[] = $item['itemAutoID'];
                $UnitOfMeasureID[] = $item['unitOfMeasureID'];
                $a_segment[] = $masterMrn['segmentID'].'|'.$masterMrn['segmentCode'];
                $quantityRequested[] = $item['requestedQty'];
                $unitCost[] = $item['unitAmount'];
                $comment[] = $item['comment'];
                $uom[] = $item['defaultUOM'].'|'.$item['defaultUOM'];
                ...
                $_POST['uom'] = $uom;
                $_POST['mrnAutoID'] = $mrnAutoID;

            }

        }

        if(count($itemAutoID) > 0){
            $detail_res = $this->save_material_detail_multiple();
        }
    ...
    function load_material_receipt_header()
    {
        ...
        $this->db->select('srp_erp_materialreceiptmaster.*,DATE_FORMAT(receivedDate,\'' . $convertFormat . '\') AS receivedDate, srp_erp_mfq_warehousemaster.mfqWarehouseAutoID AS mfqWarehouseAutoID');
        $this->db->where('mrnAutoID', $this->input->post('mrnAutoID'));
        $this->db->join('srp_erp_mfq_warehousemaster', 'srp_erp_mfq_warehousemaster.warehouseAutoID = srp_erp_materialreceiptmaster.wareHouseAutoID', 'LEFT');
        return $this->db->get('srp_erp_materialreceiptmaster')->row_array();
    }
    ...
    function fetch_template_data($mrnAutoID)
    {
        ...
        $this->db->select('*,DATE_FORMAT(createdDateTime,\'' . $convertFormat . '\') AS createdDateTime,DATE_FORMAT(receivedDate,\'' . $convertFormat . '\') AS receivedDate,(DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\')) AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
        ...
        $this->db->select('srp_erp_materialreceiptdetails.*,IFNULL(itemIssueCode, stockTransferCode) AS itemIssueCode,'.$item_code_alias.'');
        $this->db->where('mrnAutoID', $mrnAutoID);
        $this->db->from('srp_erp_materialreceiptdetails');
        $this->db->join('srp_erp_itemissuemaster', 'srp_erp_itemissuemaster.itemIssueAutoID = srp_erp_materialreceiptdetails.itemIssueAutoID', 'left');
        $this->db->join('srp_erp_itemmaster', 'srp_erp_itemmaster.itemAutoID = srp_erp_materialreceiptdetails.itemAutoID', 'left');
        $this->db->join('srp_erp_stocktransfermaster_bulk', 'srp_erp_stocktransfermaster_bulk.stockTransferAutoID = srp_erp_materialreceiptdetails.stockTransferAutoID', 'left');
    ...
    function save_material_detail()
    {
        ...
        $mrnAutoID = $this->input->post('mrnAutoID');
        $batch_arr = $this->input->post('batchNumber');
        $batch_expire_arr = $this->input->post('expireDate');
        $batchNumber = '';
        $batchExpireDate = '';
        ...
        $masterDetails = $this->db->query("SELECT WIPGLAutoID, systemAccountCode, GLSecondaryCode, GLDescription, subCategory
                    ...
                    WHERE srp_erp_materialreceiptmaster.companyID = $companyID AND mrnAutoID = {$mrnAutoID}")->row_array();
        ...
        if (!empty($this->input->post('mrnDetailID'))) {
            $this->db->select('mrnAutoID,itemDescription,itemSystemCode');
            $this->db->from('srp_erp_materialreceiptdetails');
            $this->db->where('mrMasterID', trim($this->input->post('mrMasterID') ?? ''));
            $this->db->where('mrnAutoID', trim($this->input->post('mrnAutoID') ?? ''));
            $this->db->where('itemAutoID', trim($this->input->post('itemAutoID') ?? ''));
            $this->db->where('mrnDetailID !=', trim($this->input->post('mrnDetailID') ?? ''));
        ...
        if ($masterRecord['receiptType'] == 'Material Request') {
            if (!empty($this->input->post('mrnDetailID'))) {
                $this->db->select('itemIssueAutoID,,qtyRequested');
                $this->db->from('srp_erp_materialreceiptdetails');
                $this->db->where('mrnAutoID', trim($this->input->post('mrnAutoID') ?? ''));
                $this->db->where('mrnDetailID', trim($this->input->post('mrnDetailID') ?? ''));
                $req_QTY = $this->db->get()->row_array();
                if (!empty($req_QTY)) {
                    $mrid = $req_QTY['itemIssueAutoID'];
                    $itmid = $this->input->post('itemAutoID');
                    $itmDid = $this->input->post('mrnDetailID');
	                ...
	                SUM(qtyReceived) as qtyReceived
                        ...
                        WHERE
                            `srp_erp_materialreceiptdetails`.`itemIssueAutoID` = $mrid
                        AND `srp_erp_materialreceiptdetails`.`itemAutoID` = $itmid
                        AND `srp_erp_materialreceiptdetails`.`companyID` = $companyID
                        AND `srp_erp_materialreceiptdetails`.`mrnDetailID`!= $itmDid
                    ")->row_array();

                    $qtyrequested = $req_QTY['qtyRequested'] - $issuedQTY['qtyReceived'];
                    if (!empty($req_QTY['itemIssueAutoID']) && $req_QTY['itemIssueAutoID'] > 0 && $qtyrequested < $this->input->post('quantityRequested')) {
        ...
        $data['mrnAutoID'] = trim($this->input->post('mrnAutoID') ?? '');
        ...
        if($itemBatchPolicy==1){
            ...
            $batchNumber = isset($batch_arr[0]) ? $batch_arr[0] : '';
            $batchExpireDate = isset($batch_expire_arr[0]) ? $batch_expire_arr[0] : '';

            $data['batchNumber'] = $batchNumber;
            $data['batchExpireDate'] = $batchExpireDate;
        ...
        $data['qtyReceived'] = trim($this->input->post('quantityRequested') ?? '');
        ...
        if ($masterRecord['receiptType'] == 'Material Request') {
            ...
            if ($data['financeCategory'] == 1 or $data['financeCategory'] == 3) {
                ...
                if(!empty($masterDetails['WIPGLAutoID'])) {
                    $data['BLGLAutoID'] = $masterDetails['WIPGLAutoID'];
                    $data['BLSystemGLCode'] = $masterDetails['systemAccountCode'];
                    $data['BLGLCode'] = $masterDetails['GLSecondaryCode'];
                    $data['BLDescription'] = $masterDetails['GLDescription'];
                    $data['BLType'] = $masterDetails['subCategory'];
        ...
        $data['totalValue'] = ($data['unitCost'] *$data['qtyReceived']);
        $data['modifiedPCID'] = $this->common_data['current_pc'];
        $data['modifiedUserID'] = $this->common_data['current_userID'];
        $data['modifiedUserName'] = $this->common_data['current_user'];
        $data['modifiedDateTime'] = $this->common_data['current_date'];

        if (trim($this->input->post('mrnDetailID') ?? '')) {
            $this->db->where('mrnDetailID', trim($this->input->post('mrnDetailID') ?? ''));
            $this->db->update('srp_erp_materialreceiptdetails', $data);

            $this->db->select('srp_erp_materialreceiptdetails.*,srp_erp_materialreceiptmaster.wareHouseAutoID');
            $this->db->from('srp_erp_materialreceiptdetails');
            $this->db->join('srp_erp_materialreceiptmaster', 'srp_erp_materialreceiptmaster.mrnAutoID = srp_erp_materialreceiptdetails.mrnAutoID', 'left');
            $this->db->where('srp_erp_materialreceiptdetails.itemAutoID', trim($this->input->post('itemAutoID') ?? ''));
            $detail = $this->db->get()->row_array();
            $grvDetailsID = $this->input->post('mrnDetailID');
            $subData['uom'] = $data['unitOfMeasure'];
            $subData['uomID'] = $data['unitOfMeasureID'];
            $subData['grvDetailsID'] = $this->input->post('mrnDetailID');
            $subData['wareHouseAutoID'] = $detail['wareHouseAutoID'];
            $this->edit_sub_itemMaster_tmpTbl($this->input->post('quantityRequested'), $item_data['itemAutoID'], $data['mrnAutoID'], $grvDetailsID, 'MRN', $data['itemSystemCode'], $subData);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
            ...
            $data['createdUserName'] = $this->common_data['current_user'];
            $data['createdDateTime'] = $this->common_data['current_date'];
            $this->db->insert('srp_erp_materialreceiptdetails', $data);
            $last_id = $this->db->insert_id();
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
    ...
    function save_material_detail_multiple()
    {
        $companyLocalCurrencyID = $this->common_data['company_data']['company_default_currencyID'];
        $projectExist = project_is_exist();
        $mrnDetailID = $this->input->post('mrnDetailID');
        $mrnAutoID = $this->input->post('mrnAutoID');
        ...
        $masterDetails = $this->db->query("SELECT WIPGLAutoID, systemAccountCode, GLSecondaryCode, GLDescription, subCategory
                    ...
                    INNER JOIN srp_erp_warehousemaster ON srp_erp_warehousemaster.wareHouseAutoID = srp_erp_materialreceiptmaster.wareHouseAutoID
                    LEFT JOIN srp_erp_chartofaccounts ON srp_erp_chartofaccounts.GLAutoID = srp_erp_warehousemaster.WIPGLAutoID
                    WHERE srp_erp_materialreceiptmaster.companyID = $compID AND mrnAutoID = $mrnAutoID")->row_array();
        ...
        foreach ($itemAutoIDs as $key => $itemAutoID) {
            ...
            $this->db->from('srp_erp_materialreceiptmaster');
            $this->db->where('mrnAutoID', trim($mrnAutoID));
            $master = $this->db->get()->row_array();

            $companyID = current_companyID();
            $mfqWarehouseAutoID = $this->db->query("SELECT mfqWarehouseAutoID FROM srp_erp_mfq_warehousemaster WHERE warehouseAutoID = {$master['wareHouseAutoID']} AND companyID = {$companyID}")->row('mfqWarehouseAutoID');
            ...
            if($itemBatchPolicy == 1){
                ...
                $batchNumber = isset($batch_arr[$key]) ? $batch_arr[$key] : '';
                $batchExpireDate = isset($batch_expire_arr[$key]) ? $batch_expire_arr[$key] : '';
            ...
            $data['defaultUOMID'] = $item_data['defaultUnitOfMeasureID'];
            $data['conversionRateUOM'] = conversionRateUOM_id($data['unitOfMeasureID'], $data['defaultUOMID']);
            $data['qtyReceived'] = $quantityRequested[$key];
            $data['unitCost'] = $unitCost[$key];
            ...
            if ($data['financeCategory'] == 1 or $data['financeCategory'] == 3) {
                ...
                if(!empty($masterDetails['WIPGLAutoID'])) {
                    $data['PLGLAutoID'] = $masterDetails['WIPGLAutoID'];
                    $data['PLSystemGLCode'] = $masterDetails['systemAccountCode'];
                    $data['PLGLCode'] = $masterDetails['GLSecondaryCode'];
                    $data['PLDescription'] = $masterDetails['GLDescription'];
                    $data['PLType'] = $masterDetails['subCategory'];
                } else if($mfqWarehouseAutoID) {
                    $wipGLDesc=fetch_gl_account_desc($this->common_data['controlaccounts']['WIP']);
                    $data['PLGLAutoID'] = $this->common_data['controlaccounts']['WIP'];
                    $data['PLSystemGLCode'] = $wipGLDesc['systemAccountCode'];
                    $data['PLGLCode'] = $wipGLDesc['GLSecondaryCode'];
                    $data['PLDescription'] = $wipGLDesc['GLDescription'];
                    $data['PLType'] = $wipGLDesc['subCategory'];
                } else {
                    $data['PLGLAutoID'] = $item_data['costGLAutoID'];
                    $data['PLSystemGLCode'] = $item_data['costSystemGLCode'];
                    $data['PLGLCode'] = $item_data['costGLCode'];
            ...
            if ($item_data['isSubitemExist'] == 1) {
                ...
                $this->add_sub_itemMaster_tmpTbl($qty, $itemAutoID, $mrnAutoID, $last_id, 'MRN', $item_data['itemSystemCode'], $subData);
        ...
        foreach ($itemAutoIDs as $key => $itemAutoID) {
            ...
            if ($item_data['isSubitemExist'] == 1) {
                ...
                if ($data['previousStock'] < $data['currentStock']) {
                    ...
                    $subData['grv_detailID'] = $last_id;
                    $subData['warehouseAutoID'] = $stockadjustmentMaster['wareHouseAutoID'];
                    $this->add_sub_itemMaster_tmpTbl($qty, $itemAutoID, $stockAdjustmentAutoID, $last_id, 'MRN', $item_data['itemSystemCode'], $subData);
                }
    ...
    function fetch_material_receipt_detail()
    {
        ...
        $this->db->select('srp_erp_materialreceiptdetails.*,srp_erp_itemmaster.isSubitemExist,srp_erp_materialreceiptmaster.wareHouseAutoID,IFNULL(itemIssueCode, stockTransferCode) AS itemIssueCode,'.$item_code_alias.'');
        $this->db->from('srp_erp_materialreceiptdetails');
        $this->db->join('srp_erp_itemmaster', 'srp_erp_itemmaster.itemAutoID = srp_erp_materialreceiptdetails.itemAutoID', 'left');
        $this->db->join('srp_erp_materialreceiptmaster', 'srp_erp_materialreceiptmaster.mrnAutoID = srp_erp_materialreceiptdetails.mrnAutoID', 'left');
        $this->db->join('srp_erp_itemissuemaster', 'srp_erp_itemissuemaster.itemIssueAutoID = srp_erp_materialreceiptdetails.itemIssueAutoID', 'left');
        $this->db->join('srp_erp_stocktransfermaster_bulk', 'srp_erp_stocktransfermaster_bulk.stockTransferAutoID = srp_erp_materialreceiptdetails.stockTransferAutoID', 'left');
        $this->db->where('srp_erp_materialreceiptdetails.mrnAutoID', trim($this->input->post('mrnAutoID') ?? ''));
        $data['detail'] = $this->db->get()->result_array();
        return $data;
    }
    ...
    {
        $this->db->select('srp_erp_materialreceiptdetails.*,srp_erp_itemmaster.seconeryItemCode');
        $this->db->from('srp_erp_materialreceiptdetails');
        $this->db->join('srp_erp_itemmaster','srp_erp_itemmaster.itemAutoID = srp_erp_materialreceiptdetails.itemAutoID','Left');
        $this->db->where('mrnDetailID', trim($this->input->post('mrnDetailID') ?? ''));
        return $this->db->get()->row_array();
    ...
    {
        $id = $this->input->post('mrnDetailID');
        $this->db->delete('srp_erp_itemmaster_subtemp', array('receivedDocumentDetailID' => $id, 'receivedDocumentID' => 'MRN'));
        /** end update sub item master */
        $this->db->where('mrnDetailID', $id);
        $result = $this->db->delete('srp_erp_materialreceiptdetails');
        if ($result) {
    ...
    {
        $this->db->select('sa.stockAdjustmentDetailsAutoID,sa.itemDescription,sa.itemSystemCode,w.currentStock as wareHouseStock,im.companyLocalWacAmount as LocalWacAmount,sa.defaultUOMID,sa.segmentID,sa.segmentCode,sa.currentStock,sa.currentWac,sa.itemAutoID,sa.projectID');
    ...
    function delete_material_receipt_header()
    {
        ...
        /* $this->db->select('*');
         ...
         if ($datas) {
             $this->session->set_flashdata('w', 'please delete all detail records before deleting this document.');
             return true;
         } else {
             $data = array(
                 'isDeleted' => 1,
        ...
        $masterID = trim($this->input->post('mrnAutoID') ?? '');
        $this->db->select('*');
        $this->db->from('srp_erp_materialreceiptdetails');
        $this->db->where('mrnAutoID', $masterID);
        $datas = $this->db->get()->row_array();
        ...
        if (!empty($datas)) {
            ...
            $documentCode = $this->db->get_where('srp_erp_materialreceiptmaster', ['mrnAutoID'=> $masterID])->row('mrnCode');
            ...
            if($length > 1){
                ...
                $this->db->where('mrnAutoID', $masterID)->delete('srp_erp_materialreceiptdetails');
                $this->db->where('mrnAutoID', $masterID)->delete('srp_erp_materialreceiptmaster');
            }
    ...
    function material_item_confirmation()
    {
        ...
        if (empty($results)) {
            ...
            if (!empty($Confirmed)) {
                ...
                /*         $materialReceiptDetail = $this->db->query("select
                                                                GROUP_CONCAT(itemAutoID) as itemAutoID
                                                                from 
                                                                srp_erp_materialreceiptdetails
                                                                where 
                                                                companyID = $companyID 
                                                                AND mrnAutoID = $system_code")->row("itemAutoID");


                        if(!empty($materialReceiptDetail)){ 
                        $wacTransactionAmountValidation  = fetch_itemledger_transactionAmount_validation("$materialReceiptDetail");
                ...
                $isProductReference_completed = isProductReference_completed_document_mrn($system_code);
                if ($isProductReference_completed == 0) {
                    $this->db->select('mrnCode,documentID,DATE_FORMAT(receivedDate, "%Y") as invYear,DATE_FORMAT(receivedDate, "%m") as invMonth,companyFinanceYearID');
                    $this->db->where('mrnAutoID', trim($this->input->post('mrnAutoID') ?? ''));
                    $this->db->from('srp_erp_materialreceiptmaster');
                    $master_dt = $this->db->get()->row_array();

                    $this->load->library('sequence');
                    if($master_dt['mrnCode'] == "0" || empty($master_dt['mrnCode'])) {
                        if($locationwisecodegenerate == 1)
                        {
                                ...
                                }else
                                {
                                    if($locationemployee!='')
                                    {
                                        $mrncode = $this->sequence->sequence_generator_location($master_dt['documentID'],$master_dt['companyFinanceYearID'],$locationemployee,$master_dt['invYear'],$master_dt['invMonth']);
                                    }else
                                    {
                                        return array('error' => 2, 'message' => 'Location is not assigned for current employee');
                                    }
                        ...
                        {
                            $mrncode = $this->sequence->sequence_generator_fin($master_dt['documentID'], $master_dt['companyFinanceYearID'], $master_dt['invYear'], $master_dt['invMonth']);
                        }

                        $validate_code = validate_code_duplication($mrncode, 'mrnCode', trim($this->input->post('mrnAutoID') ?? ''),'mrnAutoID', 'srp_erp_materialreceiptmaster');
                        ...
                        $this->db->where('mrnAutoID', trim($this->input->post('mrnAutoID') ?? ''));
                        $this->db->update('srp_erp_materialreceiptmaster', $pvCd);
                    } else {
                        $validate_code = validate_code_duplication($master_dt['mrnCode'], 'mrnCode', trim($this->input->post('mrnAutoID') ?? ''),'mrnAutoID', 'srp_erp_materialreceiptmaster');
                        if(!empty($validate_code)) {
                            return array('error' => 1, 'message' => 'The document Code Already Exist.(' . $validate_code . ')');
                        }
                    }
                    ...
                    $this->db->select('mrnAutoID, mrnCode,receivedDate');
                    $this->db->where('mrnAutoID', trim($this->input->post('mrnAutoID') ?? ''));
                    $this->db->from('srp_erp_materialreceiptmaster');
                    $app_data = $this->db->get()->row_array();

                    $autoApproval= get_document_auto_approval('MRN');

                    if($autoApproval==0){
                        $approvals_status = $this->approvals->auto_approve($app_data['mrnAutoID'], 'srp_erp_materialreceiptmaster','mrnAutoID', 'MRN',$app_data['mrnCode'],$app_data['receivedDate']);
                    }elseif($autoApproval==1){
                        $approvals_status = $this->approvals->CreateApproval('MRN', $app_data['mrnAutoID'], $app_data['mrnCode'], 'Material Receipt Note', 'srp_erp_materialreceiptmaster', 'mrnAutoID',0,$app_data['receivedDate']);
                    }else{
                    ...
                    if ($approvals_status == 1) {
                        ...
                        if($batchNumberPolicy==1){
                            ...
                            $this->db->select('srp_erp_materialreceiptdetails.qtyReceived AS requestedQty,srp_erp_materialreceiptdetails.conversionRateUOM,srp_erp_materialreceiptdetails.batchNumber,srp_erp_materialreceiptdetails.batchExpireDate,srp_erp_materialreceiptdetails.itemAutoID,srp_erp_materialreceiptmaster.wareHouseAutoID');
                            $this->db->from('srp_erp_materialreceiptdetails');
                            $this->db->where('srp_erp_materialreceiptdetails.mrnAutoID', trim($this->input->post('mrnAutoID') ?? ''));
                            ...
                            foreach($material_results as $key => $value){
                                $res = set_item_batch($value['requestedQty'],$value['itemAutoID'],$value['batchNumber'],$value['batchExpireDate'],$value['wareHouseAutoID'],$value['conversionRateUOM']);
                            }
                        ...
                        if($autoApproval==0) {
                            $result = $this->save_material_receipt_approval(0,$app_data['mrnAutoID'], 1, 'Auto Approved',$updatedBatchNumberArray);
                            if($result){
                        ...
                        return array('error' => 1, 'message' => 'Error Message: Approval Error!');
                    }
                } else {
                    return array('error' => 1, 'message' => 'Please complete you sub item configuration, fill all the product reference numbers!.');
                }
            }
        }
    }
    ...
    function stock_transfer_confirmation()
    {
        ...
        if (!empty($Confirmed)) {
            ...
            if ($validate) {
                ...
                return array('error' => 0, 'message' => 'Document Successfully Approved.');
                //return array('status' => true);
            } else {
    ...
    function save_material_receipt_approval($autoappLevel=1,$system_idAP=0,$statusAP=0,$commentsAP=0,$updatedBatchNumberArray=[])
    {
        $this->db->trans_start();
        $this->load->library('Approvals');
        $batchNumberPolicy = getPolicyValues('IB', 'All');
        ...
        if($autoappLevel==0){
            ...
            $approvals_status = $this->approvals->approve_document($system_code, $level_id, $status, $comments, 'MRN');
        }

        if ($approvals_status == 1) {

            $this->db->select('srp_erp_materialreceiptmaster.*,wm.warehouseType');
            $this->db->where('srp_erp_materialreceiptmaster.mrnAutoID', $system_code);
            $this->db->from('srp_erp_materialreceiptmaster');
            $this->db->join('srp_erp_warehousemaster as wm','srp_erp_materialreceiptmaster.wareHouseAutoID = wm.wareHouseAutoID','left');
            $master = $this->db->get()->row_array();

            // $this->db->select('*,COALESCE((srp_erp_materialreceiptdetails.qtyReceived),0) AS qtyUpdatedReceived,COALESCE((srp_erp_materialreceiptdetails.totalValue),0) AS UpdatedTotalValue,srp_erp_materialreceiptdetails.batchNumber,srp_erp_materialreceiptdetails.batchExpireDate');
            $this->db->select('*,srp_erp_materialreceiptdetails.qtyReceived AS qtyUpdatedReceived,srp_erp_materialreceiptdetails.totalValue AS UpdatedTotalValue,srp_erp_materialreceiptdetails.batchNumber,srp_erp_materialreceiptdetails.batchExpireDate');
            $this->db->from('srp_erp_materialreceiptdetails');
            ...
            for ($i = 0; $i < count($details_arr); $i++) {
                if ($details_arr[$i]['itemCategory'] == 'Inventory' || $details_arr[$i]['itemCategory'] == 'Non Inventory' || $details_arr[$i]['itemCategory'] =='Service') {
                   ...
                   // $item_arr[$i]['currentStock'] = ($item['currentStock'] + ($details_arr[$i]['qtyUpdatedReceived'] / $details_arr[$i]['conversionRateUOM']));
                    ...
                    $itemledger_arr[$i]['narration'] = $details_arr[$i]['comments'];
            ...
            if (!empty($itemledger_arr)) {
                ...
                $res = $this->db->insert_batch('srp_erp_itemledger', $itemledger_arr);
            ...
            if (!empty($generalledger_arr) && $master['warehouseType'] != 4) {
            ...
            if($master['jobID']) {
                ...
                $this->Inventory_modal->updateJobQty($master['jobID'], $system_code, 'MRN');
            }
            /**End Of update Manufacturing Job Qty Based on Job Number */

            $this->session->set_flashdata('s', 'Material Receipt Approval Successful.');
    ...
    function save_stock_adjustment_approval()
    {
        ...
        if ($approvals_status == 1) {
            ...
            for ($i = 0; $i < count($details_arr); $i++) {
                ...
                $location = $details_arr[$i]['wareHouseLocation'];
                $wareHouseAutoID = $details_arr[$i]['wareHouseAutoID'];
                $this->db->query("UPDATE srp_erp_warehouseitems SET currentStock = {$details_arr[$i]['currentStock']}  WHEREwareHouseAutoID='{$wareHouseAutoID}' and itemAutoID='{$itemSystemCode}'");
    ...
    function fetch_item_for_grv()
    {
        ...
        $this->db->join('srp_erp_stockreturndetails', 'srp_erp_grvmaster.grvAutoID = srp_erp_stockreturndetails.grvAutoID', "LEFT");
    ...
    function save_grv_base_items()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $this->session->set_flashdata('s', 'Good Received note : ' . count($grv_data) . ' Item Details Saved Successfully.');
            $this->db->trans_commit();
            return array('status' => true);
        }
    }
    ...
    function fetch_inv_item_stock_adjustment()
    {
        ...
        $mrnAutoID = $_GET['mrnAutoID'];
        $itemfilter = '';

        //check po details
        $mrnMaster = get_material_receipt_note_detail($mrnAutoID);

        if($mrnMaster && isset($mrnMaster['purchaseOrderID']) && $mrnMaster['purchaseOrderID'] > 0) {

            $purchaseOrderID = $mrnMaster['purchaseOrderID'];
            
            $items = $this->db->where('purchaseOrderID',$purchaseOrderID)->where('companyID',$companyID)->select('GROUP_CONCAT(itemAutoID) as items')->from('srp_erp_purchaseorderdetails')->get()->row('items');

            $itemfilter = " AND itemAutoID IN ($items)";
        }
 
        $data = $this->db->query('SELECT 
                                    mainCategoryID,subcategoryID,seconeryItemCode,subSubCategoryID,revanueGLCode,itemSystemCode,costGLCode,assteGLCode,defaultUnitOfMeasure,
                                ...
                                WHERE (
                                    itemSystemCode LIKE "' . $search_string . '" 
                                    OR partNo LIKE "' . $search_string . '"
                                    OR itemDescription LIKE "' . $search_string . '" 
                                    OR seconeryItemCode LIKE "' . $search_string . '" 
    ...
    function fetch_item_for_sales_return()
    {
        ...
        $this->db->join('srp_erp_salesreturndetails', 'mainTable.invoiceAutoID = srp_erp_salesreturndetails.invoiceAutoID', "LEFT");
    ...
    function save_sales_return_approval()
    {
        ...
        if ($approvals_status == 1) {
            ...
            $this->db->where('salesReturnAutoID', $system_id);
            $this->db->from('srp_erp_salesreturndetails');
            $detailTbl = $this->db->get()->result_array();
    ...
    function edit_sub_itemMaster_tmpTbl($qty, $itemAutoID, $masterID, $detailID, $code = 'MRN', $itemCode = null, $data = array())
    {
        ...
        if ($count_subItemMaster != $qty) {
            ...
            if ($qty > 0) {
                ...
                for ($i = 1; $i <= $qty; $i++) {
                    $data_subItemMaster[$x]['itemAutoID'] = $itemAutoID;
                    $data_subItemMaster[$x]['subItemSerialNo'] = $i;
                    $data_subItemMaster[$x]['subItemCode'] = $itemCode . '/MRN/' . $grvDetailsID . '/' . $i;
                    $data_subItemMaster[$x]['uom'] = $uom;
                    $data_subItemMaster[$x]['uomID'] = $uomID;
                    $data_subItemMaster[$x]['wareHouseAutoID'] = $wareHouseAutoID;
                    $data_subItemMaster[$x]['receivedDocumentID'] = $code;
    ...
    function fetch_material_issue_code()
    {
        ...
        $this->db->select('receivedDate,itemType,wareHouseAutoID,segmentID');
        $this->db->from('srp_erp_materialreceiptmaster');
        $this->db->where('mrnAutoID', trim($mrnAutoID));
        ...
        $wareHouseAutoID=$result['wareHouseAutoID'];
        $companyID=current_companyID();
        ...
        $data = $this->db->query("SELECT
                            ...
                            documentID
            FROM `srp_erp_itemissuemaster`
            INNER JOIN (
                SELECT
                    TRIM(TRAILING '.' FROM (TRIM(TRAILING 0 FROM ((ROUND((SUM(IFNULL(`srp_erp_itemissuedetails`.qtyIssued,0)) - SUM(IFNULL(`det`.`qtyReceived`, 0)) ), 2 )))))) AS qty,
                    srp_erp_itemissuedetails.itemIssueAutoID
                FROM `srp_erp_itemissuedetails`
                LEFT JOIN (
                    SELECT SUM(qtyReceived) AS qtyReceived, itemIssueDetailID, itemIssueAutoID
                    FROM srp_erp_materialreceiptdetails
                    GROUP BY itemIssueDetailID
                ) AS det ON det.`itemIssueDetailID` = `srp_erp_itemissuedetails`.`itemIssueDetailID` AND det.`itemIssueAutoID` = `srp_erp_itemissuedetails`.`itemIssueAutoID`
                WHERE `srp_erp_itemissuedetails`.`companyID` = {$companyID}
                GROUP BY srp_erp_itemissuedetails.itemIssueAutoID
            ) rsvdqty ON rsvdqty.itemIssueAutoID = srp_erp_itemissuemaster.itemIssueAutoID
            WHERE `issueDate` <= '{$receivedDate}' AND `itemType` = '{$itemType}' AND `issueType` = 'Material Request' AND `requestedWareHouseAutoID` = '{$wareHouseAutoID}' AND `companyID` = '{$companyID}' AND `approvedYN` = 1 AND rsvdqty.qty > 0
            
            UNION
            
                    SELECT 
                                det.towarehouseAutoID AS wareHouseAutoID,
                                qty AS qty,
                                tranferDate AS documentDate,
                                '' AS name,
                                srp_erp_stocktransfermaster_bulk.stockTransferAutoID AS documentAutoID,
                                stockTransferCode AS documentCode,
                                documentID
                            FROM srp_erp_stocktransfermaster_bulk
                            INNER JOIN (
                                SELECT TRIM( TRAILING '.' FROM (TRIM(TRAILING 0 FROM ((ROUND(( SUM( IFNULL( srp_erp_stocktransferdetails_bulk.transfer_QTY, 0 )) - SUM( IFNULL( `mrndet`.`qtyReceived`, 0 ))), 2 )))))) AS qty,
                                    srp_erp_stocktransferdetails_bulk.stockTransferAutoID, srp_erp_stocktransferdetails_bulk.towarehouseAutoID
                                FROM srp_erp_stocktransferdetails_bulk
                                    LEFT JOIN (
                                        SELECT SUM( qtyReceived ) AS qtyReceived, stockTransferDetailsID, stockTransferAutoID FROM srp_erp_materialreceiptdetails GROUP BY stockTransferDetailsID 
                                    ) AS mrndet ON mrndet.`stockTransferDetailsID` = `srp_erp_stocktransferdetails_bulk`.`stockTransferDetailsID` AND mrndet.`stockTransferAutoID` = `srp_erp_stocktransferdetails_bulk`.stockTransferAutoID 
                                    WHERE towarehouseAutoID = {$wareHouseAutoID}
                                GROUP BY srp_erp_stocktransferdetails_bulk.stockTransferAutoID, srp_erp_stocktransferdetails_bulk.towarehouseAutoID  
                            )det ON det.stockTransferAutoID = srp_erp_stocktransfermaster_bulk.stockTransferAutoID
                            WHERE tranferDate <= '{$receivedDate}' AND `itemType` = '{$itemType}' AND `companyID` = '{$companyID}' AND `approvedYN` = 1  AND receiptType = 1 AND det.qty > 0")->result_array();
        return $data;

        /*$this->db->from('srp_erp_itemissuemaster');
        $this->db->where('issueDate <=', trim($result['receivedDate'] ?? ''));
        $this->db->where('itemType', trim($result['itemType'] ?? ''));
        $this->db->where('issueType', 'Material Request');
        $this->db->where('requestedWareHouseAutoID', trim($result['wareHouseAutoID'] ?? ''));
        //$this->db->where('segmentID', trim($result['segmentID'] ?? ''));
    ...
    function fetch_material_issue_detail_table()
    {
        ...
        $companyID = current_companyID();
        if($documentID == 'STB') {
            $stockTransferAutoID = trim($this->input->post('itemIssueAutoID') ?? '');
            $warehouseID = trim($this->input->post('wareHouseAutoID') ?? '');

            $data['detail'] = $this->db->query("SELECT 'STB' AS documentID,
	`srp_erp_stocktransferdetails_bulk`.*, TRIM(TRAILING '.' FROM(TRIM(TRAILING 0 FROM((ROUND( `det`.`qtyReceived`, 2 )))))) AS `qtyReceived`,TRIM(TRAILING '.' FROM (TRIM(TRAILING 0 FROM((ROUND( transfer_QTY - IFNULL( `det`.`qtyReceived`, 0 ), 2 )))))) AS `balQTY`,
	ware_house.currentStock AS `stock`,$item_code_alias
FROM
	`srp_erp_stocktransferdetails_bulk`
    LEFT JOIN srp_erp_itemmaster ON srp_erp_itemmaster.itemAutoID = srp_erp_stocktransferdetails_bulk.itemAutoID
    LEFT JOIN ( SELECT SUM( qtyReceived ) AS qtyReceived, stockTransferDetailsID, stockTransferAutoID FROM srp_erp_materialreceiptdetails GROUP BY stockTransferDetailsID ) AS det ON det.`stockTransferDetailsID` = `srp_erp_stocktransferdetails_bulk`.`stockTransferDetailsID` 
	AND det.`stockTransferAutoID` = `srp_erp_stocktransferdetails_bulk`.`stockTransferAutoID`
	LEFT JOIN(
    ...
    ) AS ware_house ON ware_house.itemAutoID = srp_erp_stocktransferdetails_bulk.itemAutoID AND ware_house.wareHouseAutoID = {$warehouseID}
WHERE
	`srp_erp_stocktransferdetails_bulk`.`stockTransferAutoID` = {$stockTransferAutoID} AND towarehouseAutoID = {$warehouseID} AND (transfer_QTY - IFNULL(det.qtyReceived, 0)) > 0 
GROUP BY
	srp_erp_stocktransferdetails_bulk.stockTransferDetailsID")->result_array();
            ...
            $companyID = current_companyID();
            $data['detail'] = $this->db->query("SELECT 'MI' AS documentID,
	`srp_erp_itemissuedetails`.*, 
	/*`det`.`qtyReceived` AS `qtyReceived`,*/
	TRIM(TRAILING '.' FROM (TRIM(TRAILING 0 FROM ((ROUND(`det`.`qtyReceived` , 2 ))))))  AS `qtyReceived`,
	TRIM(TRAILING '.' FROM (TRIM(TRAILING 0 FROM ((ROUND(qtyIssued - IFNULL(`det`.`qtyReceived`, 0 ), 2 ))))))  AS `balQTY`,
	ware_house.currentStock AS `stock`, $item_code_alias
        FROM
            `srp_erp_itemissuedetails`
        ...
        LEFT JOIN (
            SELECT
            SUM(qtyReceived) as qtyReceived,itemIssueDetailID,itemIssueAutoID
            FROM
                srp_erp_materialreceiptdetails
            GROUP BY
                itemIssueDetailID
        ) AS det ON det.`itemIssueDetailID` = `srp_erp_itemissuedetails`.`itemIssueDetailID`
        AND det.`itemIssueAutoID` = `srp_erp_itemissuedetails`.`itemIssueAutoID`
        LEFT JOIN(
                SELECT SUM(transactionQTY/convertionRate) AS currentStock, wareHouseAutoID, itemAutoID 
                FROM srp_erp_itemledger WHERE companyID = {$companyID} GROUP BY wareHouseAutoID, itemAutoID
            ) AS ware_house ON ware_house.itemAutoID = srp_erp_itemissuedetails.itemAutoID AND ware_house.wareHouseAutoID = {$warehouseID}
        WHERE
            `srp_erp_itemissuedetails`.`itemIssueAutoID` = $itemIssueAutoID
        AND `srp_erp_itemissuedetails`.`companyID` = $companyID
        GROUP BY srp_erp_itemissuedetails.itemIssueDetailID")->result_array();
                }
        //echo $this->db->last_query();
        return $data;
    }
    ...
    function save_material_issue_note_base_items()
    {
        ...
        $mrnAutoID =  trim($this->input->post('mrnAutoID') ?? '');
        ...
        $companyID = current_companyID();
        foreach ($itemIssueDetailID as $key => $itemIssueDetailID) {

            if ($qty[$key] != 0) {
                if($docID == 'MI') {
                    $this->db->select('srp_erp_itemissuedetails.*');
                    $this->db->from('srp_erp_itemissuedetails');
                    $this->db->where('itemIssueDetailID', $itemIssueDetailID);
                    $itemDetail = $this->db->get()->row_array();

                    $issueMaster = $this->db->query("SELECT srp_erp_itemissuemaster.wareHouseAutoID FROM srp_erp_itemissuemaster WHERE itemIssueAutoID = {$itemDetail['itemIssueAutoID']}")->row_array();
                    ...
                    $data['mrnAutoID'] = trim($this->input->post('mrnAutoID') ?? '');
                    $data['itemIssueAutoID'] = trim($itemDetail['itemIssueAutoID'] ?? '');
                    $data['itemIssueDetailID'] = trim($itemIssueDetailID);
                    ...
                    $data['financeCategory'] = $item_data['financeCategory'];
                    $data['itemCategory'] = $item_data['mainCategory'];
                    $data['currentlWacAmount'] = $itemDetail['currentlWacAmount'];
                    $data['unitCost'] = $itemDetail['currentlWacAmount'];
                    $data['currentStock'] = $item_data['currentStock'];
                    $data['mrMasterID'] = $itemDetail['mrAutoID'];
                    $data['mrDetailID'] = $itemDetail['mrDetailID'];

                } else {
                    $this->db->select('*');
                    $this->db->from('srp_erp_stocktransferdetails_bulk');
                    $this->db->where('stockTransferDetailsID', $itemIssueDetailID);
                    $itemDetail = $this->db->get()->row_array();

                    $mfqWarehouseAutoID = $this->db->query("SELECT mfqWarehouseAutoID FROM srp_erp_mfq_warehousemaster WHERE warehouseAutoID = {$itemDetail['towarehouseAutoID']} AND companyID = {$companyID}")->row('mfqWarehouseAutoID');

                    $item_data = fetch_item_data($itemDetail['itemAutoID']);
                    $data['mrnAutoID'] = trim($this->input->post('mrnAutoID') ?? '');
                    $data['stockTransferAutoID'] = trim($itemDetail['stockTransferAutoID'] ?? '');
                    $data['stockTransferDetailsID'] = trim($itemIssueDetailID);
                    $data['itemAutoID'] = $itemDetail['itemAutoID'];
                    $data['itemSystemCode'] = $item_data['itemSystemCode'];
                    $data['itemDescription'] = $item_data['itemDescription'];
                    $data['unitOfMeasure'] = trim($itemDetail['unitOfMeasure'] ?? '');
                    ...
                    $data['defaultUOMID'] = $item_data['defaultUnitOfMeasureID'];
                    $data['conversionRateUOM'] = conversionRateUOM_id($data['unitOfMeasureID'], $data['defaultUOMID']);
                    $data['qtyRequested'] = $itemDetail['transfer_QTY'];
                    $data['qtyReceived'] = $qty[$key];
                    $data['comments'] = $itemDetail['comments'];;
                    $data['remarks'] = '';
                    $data['currentWareHouseStock'] = $itemDetail['currentWareHouseStock'];
                ...
                $data['totalValue'] = ($data['currentlWacAmount'] *$data['qtyReceived']);
                $data['modifiedPCID'] = $this->common_data['current_pc'];
                $data['modifiedUserID'] = $this->common_data['current_userID'];
                $data['modifiedUserName'] = $this->common_data['current_user'];
                $data['modifiedDateTime'] = $this->common_data['current_date'];
        ...
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
            $this->session->set_flashdata('e', 'Item Issue Detail : Details Save Failed ' . $this->db->_error_message());
            $this->db->trans_rollback();
            return array('status' => false);
        } else {
            $this->session->set_flashdata('s', 'Item Issue Detail : Item Details Saved Successfully.');
            $this->db->trans_commit();
            return array('status' => true);
        }
    ...
    function fetch_signaturelevel()
    {
        ...
        $this->db->where('documentID', 'MRN');
        $this->db->from('srp_erp_documentcodemaster ');
        return $this->db->get()->row_array();

    }

    function chk_balance_qty(){
        $mrnDetailID=$this->input->post('mrnDetailID');
        $issuedqty=$this->input->post('issuedqty');
        $issuedetl=$this->db->query("SELECT
                itemIssueDetailID
            FROM
                `srp_erp_materialreceiptdetails`
            WHERE
                mrnDetailID = '$mrnDetailID'")->row_array();
                    $itemIssueDetailID=$issuedetl['itemIssueDetailID'];

                    $qtyIssued=$this->db->query("SELECT
                qtyIssued
            FROM
                `srp_erp_itemissuedetails`
            WHERE
                itemIssueDetailID = '$itemIssueDetailID'")->row_array();
                    ...
                    $qtyReceived=$this->db->query("SELECT
                SUM(qtyReceived) as qtyReceived
            FROM
                `srp_erp_materialreceiptdetails`
            WHERE
                itemIssueDetailID = '$itemIssueDetailID'
            AND mrnDetailID != '$mrnDetailID'
            GROUP BY itemIssueDetailID")->row_array();

        if(!empty($qtyReceived)){
            $balanceQty=$qtyIssued['qtyIssued']-$qtyReceived['qtyReceived'];
        }else{
            $balanceQty=$qtyIssued['qtyIssued'];
        }
        //echo $itemIssueDetailID;exit;
        if($issuedqty>$balanceQty){
            return array('w','Received Qty canot be greater than balance Qty. Balance QTY = '.$balanceQty);
        }else{
    ...
    function fetch_existing_batch_details(){
        ...
        if($master_dt){
            $wareHouseID = $master_dt['wareHouseAutoID'];

            $ex_batch_details = get_item_batch_details_on_warehouse($itemAutoID,$batchNumber,$wareHouseID);
    ...
    function check_item_exists_in_warehouse($items,$warehouseID){
        ...
        foreach($items as $item){
            ...
            // print_r($item['secondaryItemCode']); exit;

            $this->db->select('witem.warehouseItemsAutoID');
            $this->db->from('srp_erp_warehouseitems as witem');
            $this->db->join('srp_erp_itemmaster as item','witem.itemAutoID = item.itemAutoID','left');
            $this->db->where('item.seconeryItemCode', $item['secondaryItemCode']);
            $this->db->where('witem.wareHouseAutoID',$warehouseID);
            $master_dt = $this->db->get()->row_array();
            
            if(empty($master_dt)){
                $status = 'error';
                $message .= $item['secondaryItemCode']. ' Not present in the requested WareHouse ';
            }
