...
class Authoritymaster_model extends ERP_Model
{
    ...
    function save_supplier_master()
    {
        ...
        if (trim($this->input->post('taxAuthourityMasterID') ?? '')) {
            ...
            $validate_code = $this->db->query("SELECT authoritySystemCode AS Code FROM srp_erp_taxauthorithymaster WHERE companyID = {$companyID} AND authoritySystemCode LIKE '%{$data['authoritySystemCode']}%'")->row('Code');
