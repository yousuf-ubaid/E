...
class Recurring_je_model extends ERP_Model
{
    ...
    function save_recurring_journal_entry_header()
    {
        ...
        $RJVNarration = ($this->input->post('RJVNarration'));
        $data['RJVNarration'] = str_replace('<br />', PHP_EOL, $RJVNarration);
    ...
    function fetch_Journal_entry_template_data($RJVMasterAutoId)
    {
        ...
        $this->db->select('*, DATE_FORMAT(createdDateTime,\''.$convertFormat.'\') AS createdDateTime, DATE_FORMAT(RJVStartDate,\''.$convertFormat.'\') AS RJVStartDate,DATE_FORMAT(RJVEndDate,\''.$convertFormat.'\') AS RJVEndDate,DATE_FORMAT(approvedDate,\'' . $convertFormat . ' %h:%i:%s\') AS approvedDate,CASE WHEN confirmedYN = 2 || confirmedYN = 3   THEN " - " WHEN confirmedYN = 1 THEN 
CONCAT_WS(\' on \',IF(LENGTH(confirmedbyName),confirmedbyName,\'-\'),IF(LENGTH(DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' )),DATE_FORMAT( confirmedDate, \'' . $convertFormat . ' %h:%i:%s\' ),NULL)) ELSE "-" END confirmedYNn');
    ...
    function recurring_journal_entry_confirmation()
    {
        ...
        $this->db->select('RJVMasterAutoId, RJVcode,RJVStartDate');
        $this->db->where('RJVMasterAutoId', trim($this->input->post('RJVMasterAutoId') ?? ''));
        $this->db->from('srp_erp_recurringjvmaster');
        $app_data = $this->db->get()->row_array();

        $validate_code = validate_code_duplication($app_data['RJVcode'], 'RJVcode', trim($this->input->post('RJVMasterAutoId') ?? ''),'RJVMasterAutoId', 'srp_erp_recurringjvmaster');
        if(!empty($validate_code)) {
            $this->session->set_flashdata('e', 'The document Code Already Exist.(' . $validate_code . ')');
            return false;
        }

        $approvals_status = $this->approvals->CreateApproval('RJV', $app_data['RJVMasterAutoId'], $app_data['RJVcode'], 'Recurring Journal Voucher', 'srp_erp_recurringjvmaster', 'RJVMasterAutoId',1,$app_data['RJVStartDate']);
