...
$voucherType = array('' => $this->lang->line('common_select_type')/*'Select Type'*/,
    /*'Direct' => $this->lang->line('accounts_payable_tr_pv_direct_payment')*/ /*'Direct Payment'*//*,'Supplier' =>$this->lang->line('accounts_payable_tr_pv_supplier_payment') *//*'Supplier Payment'*//*, 'Employee' =>$this->lang->line('accounts_payable_tr_pv_employee_payment')*/ /*'Employee Payment'*//*,*/
...
<div class="m-b-md" id="wizardControl">
    <a class="btn btn-primary" href="#step1" data-toggle="tab"><?php echo $this->lang->line('accounts_payable_step_one');?><!--Step 1--> - <?php echo $this->lang->line('accounts_payable_tr_pv_header');?><!--Payment Voucher Header--></a>
    <a class="btn btn-default btn-wizard" href="#step2" onclick="fetch_details(4);" data-toggle="tab"><?php echo $this->lang->line('accounts_payable_step_two');?><!--Step 2 --> - <?php echo $this->lang->line('accounts_payable_tr_pv_detail');?><!--Payment Voucher Detail--></a>
    <a class="btn btn-default btn-wizard" href="#step3" onclick="load_conformation();" data-toggle="tab"><?php echo $this->lang->line('accounts_payable_step_three');?><!--Step 3--> - <?php echo $this->lang->line('accounts_payable_tr_pv_confirmation');?><!--Payment Voucher Confirmation--></a>
</div>
<hr>
            ...
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        ...
                        <div class="col-sm-7">
                            <?php
                            echo form_dropdown('prqpartyID', all_supplier_drop(), '', 'class="form-control select2" id="prqpartyID" required onchange="fetch_supplier_currency_by_id(this.value)"'); ?>
                        </div>
                    </div>
                </form>
            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('.headerclose').click(function () {
            fetchPage('system/payment_voucher/payment_voucher_management_suom', PayVoucherAutoId, 'Payment Voucher');
        });
            ...
            $.ajax({
                ...
                success: function (data_arr) {
                    ...
                    if (data_arr['status']) {
                        ...
                        $("#a_link").attr("href", "<?php echo site_url('Payment_voucher/load_pv_conformation_suom'); ?>/" + PayVoucherAutoId);
                        $("#de_link").attr("href", "<?php echo site_url('Double_entry/fetch_double_entry_payment_voucher'); ?>/" + PayVoucherAutoId + '/PV');
    ...
    function load_conformation() {
        if (PayVoucherAutoId) {
            $.ajax({
                ...
                url: "<?php echo site_url('Payment_voucher/load_pv_conformation_suom'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
    ...
    function fetch_detail(type, partyID, currencyID, tab) {
        $.ajax({
            ...
            url: "<?php echo site_url('Payment_voucher/fetch_detail_suom'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (!jQuery.isEmptyObject(data)) {
                        ...
                        if (data['modeOfPayment'] == 0 && data['pvType'] == 'Supplier') {
                            $('.paymentType').removeClass('hide');
                            //$(".paymentmoad").show();
                            ...
                            $(".paymentmoad").show();
                        }else if(data['modeOfPayment'] == 0 && (data['pvType'] == 'Direct'||data['pvType'] == 'Employee')){
                                $(".paymentmoad").show();
                        }
                        ...
                        if(data['paymentType']==1){
                            ...
                            if (data['pvType'] == 'Direct' || data['pvType'] == 'Employee') {
                                $('#bankTransferDetails').wysihtml5({
                                    toolbar: {
                                        "font-styles": false,
                                        "emphasis": false,
    ...
    function attachment_modal_paymentVoucher(documentSystemCode, document_name, documentID) {
        if (documentSystemCode) {
            $.ajax({
                ...
                success: function (data) {
                    $('#paymentVoucher_attachment_label').html('<span aria-hidden="true" class="glyphicon glyphicon-hand-right color"></span> &nbsp;' + document_name + "<?php echo $this->lang->line('common_attachment');?>");<!--Attachments-->
