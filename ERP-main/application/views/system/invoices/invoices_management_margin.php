...
<div id="filter-panel" class="collapse filter-panel">
    <?php echo form_open('', 'role="form" id="invoicemaster_margin_filter_form"'); ?>
        <div class="row">
            ...
            <div class="form-group col-sm-4">
                ...
                <div style="width: 60%;">
                    <?php echo form_dropdown('status', $status_arr, '', 'class="form-control" id="status" onchange="Otable.draw()"'); ?></div>
...
<div class="row">
    ...
    <div class="col-md-3 text-right">
        <button type="button" class="btn btn-primary" onclick="fetchPage('system/invoices/erp_invoices_margin',null,'Add New Customer Invoice','CINV');"><i class="fa fa-plus"></i> <?php echo $this->lang->line('sales_markating_transaction_create_invoice');?></button><!--Create Invoice-->
...
$(document).ready(function() {
    $('.headerclose').click(function(){
        fetchPage('system/invoices/invoices_management_margin','','Customer Invoices');
    });
    invoiceAutoID = null;
    number_validation();
    invoice_table();
...
function invoice_table(selectedID=null){
     Otable = $('#invoice_table').DataTable({
        ...
        "bDestroy": true,
        "bStateSave": true,
        "sAjaxSource": "<?php echo site_url('Invoices/fetch_invoices_margirn'); ?>",
        "aaSorting": [[0, 'desc']],
...
function delete_item(id,value){
    ...
    function referback_customer_invoice(id,issysgen){
    if(issysgen!=1)
    {
        swal({
                title: "<?php echo $this->lang->line('common_are_you_sure');?>",/*/!*Are you sure?*!/*/
...
function excel_export() {
    var form = document.getElementById('invoicemaster_margin_filter_form');
    form.target = '_blank';
    form.method = 'post';
    form.post = $('#invoicemaster_margin_filter_form').serializeArray();
