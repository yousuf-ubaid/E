            ...
            <tr>
                ...
                <td style="width:50%;">
                    <table>
                        ...
                        <?php if(!empty($extra['master']['referenceNo'])){?>
                        <tr>
                        ...
                        if(!empty($invoice_referenceno_so_qut)){?>
                            <tr>
                                <td><strong><?php echo $this->lang->line('common_reference_number');?></strong></td><!--Reference Number-->
                                <td><strong>:</strong></td>
                                <td><?php echo $invoice_referenceno_so_qut; ?></td>
                            </tr>
                        <?php }?>


                     <?php if (!empty($invoice_referenceno) && (empty($extra['master']['referenceNo']))){?>
                        <tr>
                            <td style="vertical-align: top;"><strong>Reference Number</strong></td><!--Reference Number-->
                            <td style="vertical-align: top;"><strong>:</strong></td>
                            <td>
                                    <?php if (!empty($invoice_referenceno)){
                                    $a =1;
...
<div class="table-responsive">
    <table class="table table-bordered table-striped">
        ...
        <tfoot>
            <tr>
                <?php if($group_based_tax == 1) { ?>
                    <td class="text-right sub_total" colspan="6"> <?php echo $this->lang->line('common_total');?> </td><!--Total-->
                <?php } else { ?>
            ...
            <table class="table table-bordered table-striped">
                ...
                foreach ($extra['delivery_order_itemwise'] as $val) {
                    ...
                    <tr>
                        ...
                        <td style="text-align:right; font-size: 12px;"><?php echo format_number($val['due_amount'], $extra['master']['transactionCurrencyDecimalPlaces']); ?></td>
                        <td style="text-align:right; font-size: 12px;"><?php echo format_number($val['transactionAmount'], $extra['master']['transactionCurrencyDecimalPlaces']); ?></td>
                        <td style="text-align:right; font-size: 12px;"><?php echo format_number($val['balance_amount'], $extra['master']['transactionCurrencyDecimalPlaces']); ?></td>
                    </tr>
                    <?php
                    $num ++;
                    $gran_total += $val['transactionAmount'];
...
<div class="table-responsive">
    <table style="100%">
        ...
        if($extra['master']['retensionTransactionAmount']>0){
            $retensionRebate += $extra['master']['retensionTransactionAmount'];?>
            <tr>
        ...
        if($extra['master']['rebateAmount']>0){
            $retensionRebate += $extra['master']['rebateAmount'];?>
            <tr>
        ...
        <?php }
        if($retensionRebate > 0){?>
            <tr>
                <td><h5 class="text-right">Net Total  (<?php echo $extra['master']['transactionCurrency']; ?> )<!--Total-->
                        : </h5></td>
                <td><h5 style="text-align: right"> <?php echo format_number($gran_total-$retensionRebate, $extra['master']['transactionCurrencyDecimalPlaces']); ?></h5></td>
            </tr>
        <?php }
        ?>
    </table>
...
<script>
    ...
    a_link=  "<?php echo site_url('invoices/load_invoices_conformation_ds'); ?>/<?php echo $extra['master']['invoiceAutoID'] ?>";
