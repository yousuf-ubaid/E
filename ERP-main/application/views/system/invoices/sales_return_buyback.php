...
/*echo head_page('Sales Return', true);*/ ?>
    ...
    <div class="row">
        ...
        <div class="col-md-3 text-right">
            <button type="button" class="btn btn-primary pull-right"
                    onclick="fetchPage('system/inventory/erp_sales_return_buyback',null,'Add New Sales Return','SLR');"><i
    ...
    <div class="table-responsive">
        <table id="sales_return_table" class="<?php echo table_class(); ?>">
            ...
            <tr>
                ...
                <th style="width:120px;"><?php echo $this->lang->line('common_action');?> </th><!--Action-->
            </tr>
            </thead>
        </table>
    </div>
...
<div id="salesReturnCreateNew_div"></div>
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">Sales Return Template</h4>
            </div>
            <div class="modal-body">
                <input type="hidden" id="salesReturnAutoID">
                <div class="row">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary btn-sm" onclick="print_sales_return_temp()">Print</button>
...
<script type="text/javascript">
    ...
    function sales_return_table(selectedID=null) {
        ...
        Otable = $('#sales_return_table').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Inventory/fetch_sales_return_table_buyback'); ?>",
            "aaSorting": [[0, 'desc']],
    ...
    function load_printtemp(salesReturnAutoID)
    {
        $('#printSize').val(1);
        $('#salesReturnAutoID').val(salesReturnAutoID);
        $('#print_temp_modal').modal('show');
    }
    function print_sales_return_temp(){
        var printSize =  $('#printSize').val();
        var salesReturnAutoID =   $('#salesReturnAutoID').val();

        if(salesReturnAutoID==''){
            myAlert('e', 'Select Print Type');
        }else{
            window.open("<?php echo site_url('Inventory/load_sales_return_conformation_buyback') ?>" +'/'+ salesReturnAutoID +'/'+ printSize +'/'+1);
        }
    }
</script>