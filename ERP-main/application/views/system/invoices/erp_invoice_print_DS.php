            ...
            <tr>
                ...
                <td style="width:50%;">
                    <table>
                        <tr>
                            <td colspan="3">
                                <h3 style="'.$print_style_heading.'">' . $this->common_data['company_data']['company_name'] . '.</h3>
                                <p style="'.$print_style.'">' . $this->common_data['company_data']['company_address1'] . ', ' . $this->common_data['company_data']['company_address2'] . ', ' . $this->common_data['company_data']['company_city'] . ', ' . $this->common_data['company_data']['company_country'] . '</p>
                                <br>
                                <h4 style="'.$print_style_heading_sub.'">'.$invoiceheaderName.'</h4>
                            </td>
                        </tr>
                         <tr>
                            <td style="'.$print_style.'">Invoice Number</td><!--Invoice Number-->
                            <td style="'.$print_style.'">:</td>
                            <td style="'.$print_style.'">'.$extra['master']['invoiceCode'].'</td>
                        </tr>
                          <tr>
                            <td style="'.$print_style.'">Document Date</td>
                            <td style="'.$print_style.'">:</td>
...
<div class="table-responsive">
   ';
    ...
    $html .= '<tr>';
    $html .= '
            <td style="'.$print_style.'"> Customer Telephone</td>
            <td style="'.$print_style.'">:</td>
    ...
    if($group_based_tax == 1){
        $html .= '<tr>
                    <td style="'.$print_style.'">Customer VATIN</td>
                    <td style="'.$print_style.'">:</td>
    ...
    if (!empty($extra['item_detail'])) {
        ...
        if($istaxEnable == 1 && $group_based_tax != 1) {
            $col_name.='<th class="theadtr" style="min-width: 10%;'.$print_style_table.'">' . $this->lang->line('common_total') . '</th><!--Total-->
                        <th class="theadtr" style="min-width: 10%;'.$print_style_table.'">' . $this->lang->line('sales_markating_view_invoice_tax') . '</th><!--Tax-->';
        } else if ($group_based_tax == 1) {
        ...
        $html .= '
    <div class="table-responsive">
        <table class="table table-bordered table-striped" style="width: 97%;margin-left: 18.89px;">
            <thead>
                <tr>
                    <th class="theadtr" colspan="6" style="'.$print_style_table.'">' . $this->lang->line('sales_markating_view_invoice_item_details') . '</th><!--Item Details-->
                    <th class="theadtr" colspan="'.$colspan.'" style="'.$print_style_table.'">' . $this->lang->line('common_price') . ' (' . $extra['master']['transactionCurrency'] . ') </th><!--Price-->
                </tr>
                <tr>
                    <th class="theadtr" style="min-width: 5%">#</th>
                    <th class="theadtr" style="min-width: 15%;'.$print_style_table.'">' . $this->lang->line('sales_markating_view_invoice_item_code') . '</th><!--Item Code-->
                    <th class="theadtr" style="min-width: 35%;'.$print_style_table.'">' . $this->lang->line('sales_markating_view_invoice_item_description') . '</th><!--Item Description-->
                     <th class="theadtr" style="min-width: 10%;'.$print_style_table.'">WareHouse</th><!--UOM-->
                    <th class="theadtr" style="min-width: 10%;'.$print_style_table.'">' . $this->lang->line('common_uom') . '</th><!--UOM-->
                   
                    <th class="theadtr" style="min-width: 5%;'.$print_style_table.'">' . $this->lang->line('common_qty') . '</th><!--Qty-->
                    <th class="theadtr" style="min-width: 10%;'.$print_style_table.'">' . $this->lang->line('sales_markating_view_invoice_unit') . '</th><!--Unit-->
                    <th class="theadtr" style="min-width: 10%;'.$print_style_table.'">' . $this->lang->line('sales_markating_view_invoice_discount') . '</th><!--Discount-->';
                    if($group_based_tax != 1){
                        $html .= '<th class="theadtr" style="min-width: 10%;'.$print_style_table.'">' . $this->lang->line('sales_markating_sales_net_unit_price') . '</th><!--Net Unit Cost-->';
                    }
                     $html .= $col_name.'
                    <th class="theadtr" style="min-width: 10%;'.$print_style_table.'">' . $this->lang->line('sales_markating_view_invoice_net') . '</th><!--Net-->
                </tr>
            </thead>
        <tbody> <br>';
        $num = 1;
        ...
        foreach ($extra['item_detail'] as $val) {
            $group_by_item[$val["unitOfMeasureID"]][] = $val;
        }
        if(!empty($group_by_item))
        {
            $arrsizre = 0;
            foreach ($group_by_item as $key => $item) {
                $totalqty = 0;
                $total_rowwise = 0;
                $totalcountrow = 0;
                ...
                {
                    $contractcd = '';
                    if (!empty($val2['contractCode'])) {
                        $contractcd = '(' . $val2['contractCode'] . ')';
                    }

                    $arrsizre=count($val2);
                    $arrsizre=$arrsizre-1;
                    $html .= '<tr> <td style="text-align:right;font-size: 12px; '.$print_style.'">' . $num . '.&nbsp;</td>
                    <td style="text-align:center;font-size: 12px; '.$print_style.'">' . $val2['itemSystemCode'] . '</td>
                    <td style="font-size: 12px; '.$print_style.'">' . $contractcd . ' ' . $val2['itemDescription'] . ' -  ' . $val2['remarks'] . '</td>
                    <td style="text-align:center;font-size: 12px; '.$print_style.'">' . $val2['warehouse'] . '</td>
                    <td style="text-align:center;font-size: 12px; '.$print_style.'">' . $val2['unitOfMeasure'] . '</td>
                    <td style="text-align:right;font-size: 12px; '.$print_style.'">' .format_number( $val2['requestedQty'],2). '</td>
                    <td style="text-align:right;font-size: 12px; '.$print_style.'">' . format_number($val2['unittransactionAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                    <td style="text-align:right;font-size: 12px; '.$print_style.'">' . format_number($val2['discountAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                    ';
                    if ($group_based_tax != 1) {
                        $html .= '<td style="text-align:right;font-size: 12px; '.$print_style.'">' . format_number($val2['unittransactionAmount'] - $val2['discountAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                                    <td style="text-align:right;font-size: 12px; ' . $print_style . '">' . format_number((($val2['unittransactionAmount'] - $val2['discountAmount']) * $val2['requestedQty']), $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                    }
                    if($istaxEnable == 1 && $group_based_tax != 1)
                    {
                        $html .= '<td style="text-align:right;font-size: 12px; '.$print_style.'">' . format_number($val2['totalAfterTax'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                                    <td style="text-align:right;font-size: 12px; '.$print_style.'">' . format_number($val2['transactionAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                    } else if ($group_based_tax == 1) {
                        $html .= '<td style="text-align:right;font-size: 12px;">' . format_number(($val2['unittransactionAmount'] - $val2['discountAmount']) * $val2['requestedQty'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                                    <td style="text-align:right;font-size: 12px;">' . format_number(($val2['taxpercentageLedger']), 2) . '</td>';
                        $html .= '<td style="text-align:right;font-size: 12px;">' . format_number($val2['amount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                                    <td style="text-align:right;font-size: 12px;">' . format_number(($val2['taxAmount'] - $val2['amount']), $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                        $html .= ' <td style="text-align:right;font-size: 12px;">' . format_number($val2['transactionAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';

                        $amount_tal += $val2['amount'];
                        $amount_applicable_tal += ($val2['unittransactionAmount'] - $val2['discountAmount']) * $val2['requestedQty'];
                        $amount_other_tal += $val2['taxAmount'] - $val2['amount'];
                    }
                    $html .='</tr>';
                    $num++;
                    $totalqty +=  $val2['requestedQty'];
                ...
                $html .= '<tr>
                            <td colspan="2" style="text-align: right;font-weight: bold;"><div style="margin-left: 30px"><b>Total </b> &nbsp;</div></td>
                            <td colspan="4" style="text-align: right; font-weight: bold;">'. format_number(($totalqty), 2) . '</td>';
                if ($group_based_tax == 1) {
                    $html .= '<td class="text-right" colspan="2" style="font-size: 12px;text-align:right;">&nbsp;</td>';
                    $html .= '<td class="text-right" style="text-align:right; font-weight: bold;">' . format_number($amount_applicable_tal, $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                    $html .= '<td class="text-right" style="text-align:right; font-weight: bold;">&nbsp;</td>';
                    $html .= '<td class="text-right" style="text-align:right; font-weight: bold;">' . format_number($amount_tal, $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                    $html .= '<td class="text-right" style="text-align:right; font-weight: bold;">' . format_number($amount_other_tal, $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                }
            }
            if($extra['item_detail_count'] == $num -1)
            {
                $html .= '<td colspan="6"  style="text-align: right; font-weight: bold;">'. format_number(($gran_total), $dPlace) . '</td>
                                </tr>';
            }
        }
        $html .= '</tbody>
    ...
    $amount_applicable_tal = 0;
    if($group_based_tax==1) {
        $grop_based_col_tax_html.='<th class="theadtr" style="width: 12%;'.$print_style_table.'">Tax<br>Applicable<br>Amount</th>
    ...
    <div class="table-responsive">
         <table class="table table-bordered table-striped" style="width: 97%;margin-left: 18.89px;">
            ...
            <tr>
                ...
                <th class="theadtr" style="width: 28%;text-align: left;'.$print_style_table.'">' . $this->lang->line('common_description') . '</th><!--Description-->
                <th class="theadtr" style="width: 15%;'.$print_style_table.'">' . $this->lang->line('common_segment') . '</th><!--Segment-->';
                if($group_based_tax != 1) {
                    $html .= '<th class="theadtr" style="width: 15%;'.$print_style_table.'">' . $this->lang->line('common_amount') . '(' . $extra['master']['transactionCurrency'] . ') </th><!--Amount-->';
                }

                $html .= '<th class="theadtr" style="width: 12%;'.$print_style_table.'">Discount</th>
                 '.$grop_based_col_tax_html.'
                <th class="theadtr" style="width: 15%;'.$print_style_table.'">Net Amount (' . $extra['master']['transactionCurrency'] . ')</th>
            </tr>
            </thead>
            <tbody>';
        ...
        foreach ($extra['gl_detail'] as $val) {
            ...
            if($group_based_tax==1){
                $grop_based_col_tax_html_val.='<td style="text-align:right;font-size: 12px;">' . format_number($val['transactionAmount'] - $val['taxAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
            ...
            $html .= '<tr>
                    <td style="text-align:right;font-size: 12px;'.$print_style.'">' . $num . '.&nbsp;</td>
                    <td style="font-size: 12px;'.$print_style.'">' . $val['description'] . '</td>
                    <td style="text-align:center;font-size: 12px;'.$print_style.'">' . $val['segmentCode'] . '</td>';
            if($group_based_tax != 1) {
                $html .= '<td style="text-align:right;'.$print_style.'">' . format_number($val['transactionAmount'] + $val['discountAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
            }
                     $html .= '<td style="text-align:right;'.$print_style.'">(' . format_number($val['discountPercentage'], 2) . ' %) ' . format_number($val['discountAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                    '.$grop_based_col_tax_html_val.'
                    <td style="text-align:right;'.$print_style.'">' . format_number($val['transactionAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                </tr>';
        ...
        $html .= '</tbody>
            ...
            <tr>
                ...
                if($group_based_tax==1) {
                    ...
                    $html .= '<td class="text-right sub_total" style="text-align:right;">' . format_number($amount_total, $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                    $html .= '<td class="text-right sub_total" style="text-align:right;">' . format_number($amount_total_other, $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                }
    ...
    if (!empty($extra['tax'])) {
        ...
        foreach ($extra['tax'] as $value) {
            $html .= '<tr>
                    <td style="font-size: 12px;'.$print_style.'">' . $x . '.</td>
                    <td style="font-size: 12px;'.$print_style.'">' . $value['taxShortCode'] . '</td>
                    <td style="font-size: 12px;'.$print_style.'">' . $value['taxDescription'] . '</td>
                    <td class="text-right" style="font-size: 12px; text-align:right;'.$print_style.'">' . $value['taxPercentage'] . ' % </td>
                    <td class="text-right" style="font-size: 12px; text-align:right;'.$print_style.'">' . format_number((($value['taxPercentage'] / 100) * ($tax_transaction_total - $disc_nettot + $t_extraCharge)), $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                    </tr>';
            $x++;
    ...
    if ($extra['master']['retensionTransactionAmount'] > 0 || $extra['master']['rebateAmount'] > 0) {
        $html .= '<div class="table-responsive">
        <h5 class="text-right" style="text-align:right;margin-right: 4%"> ' . $this->lang->line('common_total') . ' (' . $extra['master']['transactionCurrency'] . ' )<!--Total-->
    ...
    if($extra['master']['retensionTransactionAmount'] > 0) {
        ...
        $html .= '<div class="table-responsive">
    <h5 class="text-right" style="text-align:right;margin-right: 4%">Retention Amount (' . $extra['master']['retensionTransactionAmount'] . ' )<!--Total-->
: ' . format_number($extra['master']['rebateAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
    ...
    if($extra['master']['rebateAmount'] > 0) {
        $rebateRetension += $extra['master']['rebateAmount'];
        $html .= '<div class="table-responsive">
    <h5 class="text-right" style="text-align:right;margin-right: 4%">Rebate Amount (' . $extra['master']['transactionCurrency'] . ' )<!--Total-->
: ' . format_number($extra['master']['rebateAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
</div>';
    }
    $html .= '<div class="table-responsive">
    <h5 class="text-right" style="text-align:right;margin-right: 4%">Net Total (' . $extra['master']['transactionCurrency'] . ' )<!--Total-->
