                ...
                <tr>
                    ...
                    <td class="mailbox-star" width="10%">
                        <?php if ($val['isSubmitted'] == 1) { ?>
                            <div class="input-group issubmited">
                                ...
                                <input type="text" name="submiteddate"
                                       ...
                                       value="<?php echo $val['submittedDate']; ?>">
                            </div>
                        <?php } ?>
                        ...
                        <?php if ($val['isSubmitted'] == 1) { ?>
                            <div class="skin-section extraColumns" style="text-align: right">
                                <input id="isApproved_<?php echo $val['donorID'] ?>"
                                       ...
                                       name="isapproved" value="<?php echo $val['donorID'] ?>"
                                >
                            </div>
                        <?php } ?>
                    </td>
                    <td class="mailbox-star" width="10%">
                        <?php if ($val['isSubmitted'] == 1) { ?>
                            <?php if ($val['isApproved'] == 1) { ?>
                                <div class="input-group approveddatecls">
                                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                                    <input type="text" name="approveddate"
                                           data-inputmask="'alias': '<?php echo $date_format_policy ?>'"
                                           id="approveddateID_<?php echo $val['proposalDonourID']; ?>"
                                           data-id="<?php echo $val['donorID']; ?>"
                    ...
                    <!-- ****************-->
                    <td class="mailbox-star" width="10%"><input type="text" class="form-control number"
                                                                name="addcommitments"
                                                                id="add_commitment_<?php echo $val['donorID'] ?>"
                                                                onchange="update_commitment_amt(<?php echo $val['donorID'] ?>,this.value)"
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('.issubmited').datetimepicker({
                ...
                var thisID = $(this).parent().find('.issubmitteddate').attr('id');
                setTimeout(function () {
                    var date = $('#' + thisID).val()
                    var donorid = $('#' + thisID).attr('data-id')
                    //alert(donor)
                    update_date_donors(date, donorid);
                }, 100);
            });
            ...
            $('.approveddatecls').datetimepicker({
                ...
                var thisID = $(this).parent().find('.approveddate').attr('id');
                setTimeout(function () {
                    var approveddate = $('#' + thisID).val()
                    var donorid = $('#' + thisID).attr('data-id')
                    update_date_donors_appproved(approveddate, donorid);
                }, 100);
            });
            $("[rel=tooltip]").tooltip();
        ...
        $('input').on('ifChecked', function (event) {
            if ($(this).hasClass('issubmitted')) {
                update_pp_donors_submited(this.value, 1);
            } else {
                update_pp_donors_approved(this.value, 1);
            }

        });
        $('input').on('ifUnchecked', function (event) {
            if ($(this).hasClass('issubmitted')) {
                update_pp_donors_submited(this.value, 0);

            } else {
                update_pp_donors_approved(this.value, 0);
            }
        });

        function update_pp_donors_submited(donorID, status) {
            ...
            $.ajax({
                ...
                type: 'post',
                dataType: 'json',
                data: {proposalID: proposalID, donorID: donorID, status: status},
        ...
        function update_pp_donors_approved(donorID, statusapproved) {
            var proposalID = $('#proposaldonor').val();
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'json',
                data: {proposalID: proposalID, donorID: donorID, statusapproved: statusapproved},
                url: "<?php echo site_url('OperationNgo/update_donors_isapproved_status'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
        ...
        function update_commitment_amt(donorID, amount) {
            var proposalID = $('#proposaldonor').val();
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'json',
                data: {proposalID: proposalID, donorID: donorID, amount: amount},
                url: "<?php echo site_url('OperationNgo/update_donors_commited_amt'); ?>",
        ...
        function update_date_donors(date, donorid) {
            var proposalID = $('#proposaldonor').val();
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'json',
                data: {proposalID: proposalID, date: date, donorid: donorid},
        ...
        function update_date_donors_appproved(approveddate, donorid) {
            var proposalID = $('#proposaldonor').val();
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'json',
                data: {proposalID: proposalID, approveddate: approveddate, donorid: donorid},
