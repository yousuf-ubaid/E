...
$customer = all_mfq_customer_drop(false,1);
?>
    ...
    <form role="form" id="mfq_job_filter" class="" autocomplete="off">
        ...
        <div class="row" style="padding-left: 2%">
            ...
            <div class="form-group col-sm-2">
                ...
                <?php echo form_dropdown('status_filter', array('1'=>'Active','2'=>'Not Active','3'=>'All'), '', '  class="form-control" id="status_filter" onchange="load_statusbased_mfq_customer()" '); ?>
            </div>
            <div class="form-group col-sm-3">
                <label for="filter_customerID">Customer :</label>
                <?php //echo form_dropdown('filter_customerID[]', all_mfq_customer_drop(false,1), '', 'class="form-control" multiple="multiple" id="filter_customerID" onchange="oTable.draw()"'); ?>
                <div id="div_load_mfq_customers">
                        <select name="filter_customerID[]" class="form-control" id="filter_customerID" onchange="oTable.draw()" multiple="multiple">
                            <?php
...
<style>
    ...
    td.details-control {
        background: url("<?php echo base_url() ?>images/action-icon-02.png") no-repeat center center;
        cursor: pointer;
    }

    tr.shown td.details-control {
        background: url('<?php echo base_url() ?>images/action-icon-06.png') no-repeat center center;
    }
        ...
        <div class="table-responsive">
            <table id="tbl_mfq_job" class="table table-condensed" width="100%">
                ...
                <tr>
                    ...
                    <th style="min-width: 20%"><?php echo $this->lang->line('manufacturing_customer');?></th>
                    <th style="min-width: 20%"><?php echo $this->lang->line('manufacturing_item');?></th>
...
<div class="modal fade" id="usage_qty_modal" role="dialog" aria-labelledby="myModalLabel"
    ...
    <div class="modal-dialog modal-lg" style="width: 70%">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" onclick="saveUsageQty_job()">Save</button>
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="job_commInvoice_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     data-width="95%" data-keyboard="false" data-backdrop="static">
...
<div class="modal fade" id="job_packingListInvoice_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     data-width="95%" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg" style="width: 60%">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12" style="margin-top: 10px">
                        <div id="job_packingListInvoice_print">
...
<div class="modal fade" id="pulled_documents_job_modal" role="dialog" aria-labelledby="myModalLabel">
...
<script type="text/javascript">
    ...
    function saveUsageQty_job() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: data,
            url: "<?php echo site_url('MFQ_Job/save_usage_qty_job'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
    ...
    function validate_usage_init(element,usageQty,colname,type,estimate=null,usage=null){
    ...
    function load_commInvoice_view(estimateMasterID) {
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'html',
            ...
            url: "<?php echo site_url('MFQ_Estimate/fetch_job_comm_invoice_view'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                stopLoad();
                $("#job_commInvoice_print").html(data);
                $("#job_commInvoice_modal").modal();
            },
            error: function (jqXHR, textStatus, errorThrown) {
                stopLoad();
                myAlert('e', '<br>Message: ' + errorThrown);
    ...
    function load_packingListInvoice_view(estimateMasterID) {
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'html',
            ...
            url: "<?php echo site_url('MFQ_Estimate/fetch_job_packing_list_invoice_view'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                stopLoad();
                $("#job_packingListInvoice_print").html(data);
                $("#job_packingListInvoice_modal").modal();
            },
            error: function (jqXHR, textStatus, errorThrown) {
                stopLoad();
                myAlert('e', '<br>Message: ' + errorThrown);
    ...
    function pulled_documents(workProcessID, documentCode) {
        $.ajax({
            async: true,
            type: 'POST',
            dataType: 'html',
            ...
            success: function (data) {
                ...
                $("#pulled_documents_job_modal").modal('show');
            }, error: function () {
                alert('An Error Occurred! Please Try Again.');
                stopLoad();
            }
    ...
    function variance_documents(workProcessID, documentCode) {

        swal({
            title: "Are you sure?",
            text: "You want to create a variance record..!",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
        ...
        function () {
            $.ajax({
                ...
                dataType: 'html',
                data: {workProcessID: workProcessID},
                url: '<?php echo site_url('MFQ_Job/create_variance_documents'); ?>',
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    stopLoad();
                    oTable.draw();
                    myAlert('s','Variance Document Created Successfully');
                    // $("#pulled_documents").html(data);
                    // $('.drilldown-title').html("Job - " + documentCode);
                    // $("#pulled_documents_job_modal").modal('show');
                }, error: function () {
    ...
    function trace_mfq_document(workProcessID, DocumentID) {
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'json',
            ...
            url: "<?php echo site_url('Tracing/trace_job_document'); ?>",
            beforeSend: function() {
                startLoad();
            },
            success: function(data) {
                ...
                //myAlert(data[0], data[1]);
                $(window).scrollTop(0);
                load_document_tracing(workProcessID, DocumentID);
            },
            error: function() {
                stopLoad();
                swal("Cancelled", "Your file is safe :)", "error");
    ...
    function load_statusbased_mfq_customer() {
        ...
        $.ajax({
            ...
            url: "<?php echo site_url('MFQ_Job/load_statusbased_mfq_customer'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                $('#div_load_mfq_customers').html(data);
