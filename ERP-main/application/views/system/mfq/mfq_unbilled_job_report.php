...
echo head_page($this->lang->line('manufacturing_unbilled_jobs'), false);
...
<div>
    <fieldset class="scheduler-border">
        ...
        <?php echo form_open('login/loginSubmit', ' name="frm_rpt_unbilled_jobs" id="frm_rpt_unbilled_jobs" class="form-group" role="form"'); ?>
            <div class="col-md-12">
                ...
                <div class="form-group col-sm-2" style="margin-left: 25px;">
                    ...
                    <?php echo form_dropdown('filter_subJobStatus[]', array(1=>'Open', 3=>'Delivered', 4=>'Overdue', 5=>'Closed'), [3,4,5], 'class="form-control" id="filter_subJobStatus" multiple="multiple"'); ?>
                </div>
            ...
            <div class="col-md-12">
                ...
                <div class="form-group col-sm-1">
                    <label for=""></label>
                    <button style="margin-top: 30%" type="button" onclick="get_unbilled_jobs_report()" class="btn btn-primary btn-xs">
...
<script>
    ...
    $(document).ready(function (e) {
        $('.headerclose').click(function () {
            fetchPage('system/mfq/mfq_unbilled_job_report', '', '<?php echo $this->lang->line('manufacturing_unbilled_jobs'); ?>')
        });
        $('.select2').select2();
        ...
        $('#filter_subJobStatus').multiselect2({
            enableCaseInsensitiveFiltering: true,
            includeSelectAllOption: true,
            numberDisplayed: 1,
            buttonWidth: '180px',
        ...
        url = '<?php echo site_url('MFQ_Report/get_unbilled_job_report'); ?>';
        urlPdf = '<?php echo site_url('MFQ_Report/get_unbilled_job_report_pdf'); ?>';
        get_unbilled_jobs_report();
    });

    function get_unbilled_jobs_report() {
        $.ajax({
            type: "POST",
            url: url,
            data: $("#frm_rpt_unbilled_jobs").serialize(),
            dataType: "html",
            cache: false,
            beforeSend: function () {
                startLoad();
    ...
    function generateReportPdf() {
        var form = document.getElementById('frm_rpt_unbilled_jobs');
        form.target = '_blank';
        form.action = urlPdf;
        form.submit();
    }
