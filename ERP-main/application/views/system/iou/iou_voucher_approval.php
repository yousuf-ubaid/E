...
echo head_page($this->lang->line('iou_voucher_approval'), false);
?>
...
<div class="row">
    <div class="col-md-5">
        <table class="<?php echo table_class() ?>">
            <tr>
                <td><span class="label label-success">&nbsp;</span> <?php echo $this->lang->line('common_approved'); ?></td>
                <td><span class="label label-danger">&nbsp;</span><?php echo $this->lang->line('common_not_approved'); ?></td>
            </tr>
        </table>
    </div>
...
<div class="table-responsive">
    <table id="Return_table" class="<?php echo table_class() ?>">
        ...
        <tr>
            ...
            <th style="min-width: 10%"><?php echo $this->lang->line('iou_code'); ?> </th>
            <th style="min-width: 20%"><?php echo $this->lang->line('common_employee_name'); ?></th>
            <th style="min-width: 15%"><?php echo $this->lang->line('common_narration'); ?> </th>
            <th style="min-width: 20%"><?php echo $this->lang->line('iou_voucherdate'); ?> </th>
            <th style="min-width: 20%"><?php echo $this->lang->line('common_value'); ?> </th>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"><?php echo $this->lang->line('iou_voucher_approval'); ?></h4>
            </div>
                            ...
                            <div class="zx-tab-pane active" id="Tab-home-v">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        <input type="hidden" name="iouvoucherid" id="iouvoucherid">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/iou/iou_voucher_approval', '', '<?php echo $this->lang->line('iou_voucher_approval'); ?>')
        });
        Return_table();
        $('#pv_approval_form').bootstrapValidator({
            ...
            fields: {
                ...
                Level: {validators: {notEmpty: {message: "<?php echo $this->lang->line('iou_level_order_status_is_required'); ?>"}}},
                //comments: {validators: {notEmpty: {message: 'Comments are required.'}}},
                iouvoucherid: {validators: {notEmpty: {message: "<?php echo $this->lang->line('iou_voucher_auto_id_is_required'); ?>"}}},
                documentApprovedID: {validators: {notEmpty: {message: "<?php echo $this->lang->line('common_document_approved_id_is_required'); ?>"}}}
            },
            ...
            if(status == 1)
            {
             swal({
                     title: '',
                     text: "<?php echo $this->lang->line('iou_this_transaction_will_generate_a_payment_voucher'); ?>," +
                         "<?php echo $this->lang->line('iou_are_you_sure_you_want_to_continue'); ?>",
                     type: 'warning',
    ...
    function Return_table() {
        Otable = $('#Return_table').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Iou/iou_voucher_approval'); ?>",
            "aaSorting": [[1, 'desc']],
            ...
            "columnDefs": [{"visible":false,"searchable": true,"targets": [9,10], "orderable": false}, {"targets": [0,5], "searchable": false}],
    ...
    function fetch_approval(voucherAutoID, documentApprovedID, Level) {
        if (voucherAutoID) {
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'html',
                data: {'voucherAutoID': voucherAutoID, 'html': true, 'approval': 1},
                url: "<?php echo site_url('Iou/load_iou_voucher_confirmation'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    $('#iouvoucherid').val(voucherAutoID);
                    $('#documentApprovedID').val(documentApprovedID);
                    $('#Level').val(Level);
                    $("#return_approval_modal").modal({backdrop: "static"});
                    $('#conform_body').html(data);
                    $('#comments').val('');
                    customerInvoice_attachment_View_modal('IOU', voucherAutoID);
                    stopLoad();
                    refreshNotifications(true);
                }, error: function () {
                    stopLoad();
