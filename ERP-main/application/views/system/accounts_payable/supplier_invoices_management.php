...
<div class="row">
    ...
    <div class="col-md-3 text-right">
        <button type="button" class="btn btn-danger-new size-sm pull-right" data-toggle="modal" data-target="#bulkPrint">
            <span title="" rel="tooltip" class="glyphicon glyphicon-print" data-original-title="Print"></span> Bulk Print
        </button>
        <button type="button" class="btn btn-primary-new size-sm pull-right mr-1"
                onclick="fetchPage('system/accounts_payable/erp_supplier_invoices',null,'<?php echo $this->lang->line('accounts_payable_trans_add_new_supplier_invoice');?>','SI');"><!--Add New Supplier Invoices-->
...
<div class="table-responsive">
    <table id="supplier_invoices_table" class="<?php echo table_class(); ?>" width="100%">
        <thead>
        <tr>
...
<!-- Modal bulk print-->
<div class="modal fade" id="bulkPrint" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Bulk Print Option</h5>
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12">
                    <div class="form-group col-sm-6">
                        <div class="custom_padding">
                            <label for="supplierPrimaryCode">Code</label>
                            <label for="supplierPrimaryCode"><?php echo $this->lang->line('common_from');?><!--From--></label><br>
                            <input type="text" name="invoiceCodeFrom" id="invoiceCodeFrom"
                                class="input-small w-100" required>                            
                        </div>
                    ...
                    <div class="form-group col-sm-6">
                        <div class="custom_padding">       
                            <label for="supplierPrimaryCode">Code</label>                     
                            <label for="supplierPrimaryCode">&nbsp&nbsp<?php echo $this->lang->line('common_to');?><!--To--></label><br>
                            <input type="text" name="invoiceCodeTo" id="invoiceCodeTo" class="input-small w-100" required>
                        </div>

                    </div>
                    <div class="form-group set-f-100 col-sm-6" style="visibility:hidden">
                        <label for="supplierPrimaryCode"><?php echo $this->lang->line('common_supplier_name');?> </label><br>
                        <?php  echo form_dropdown('supplierPrimaryCode[]', $supplier_arr, '', 'class="form-control" id="supplierPrimaryCodeID" '); ?>
                    </div>
                    <!--<div class="form-group col-sm-6">
                        <label for="supplierPrimaryCode"><?php //echo $this->lang->line('common_status');?></label><br>
                        <?php //echo form_dropdown('status', array('all' =>$this->lang->line('common_all') /*'All'*/, '1' =>$this->lang->line('common_draft')/* 'Draft'*/, '2' =>$this->lang->line('common_confirmed') /*'Confirmed'*/, '3' =>$this->lang->line('common_approved') /*'Approved'*/,'4'=>'Refer-back'), '', 'class="form-control" id="status"'); ?>
                        
                    </div>-->
                    <div class="form-group col-sm-12 text-right">   
                        <button type="button" class="btn btn-info-new size-xs pull-right"
                                onclick="clear_all_filters()"><?php echo $this->lang->line('common_clear');?> <!--Clear-->
                        </button>                        
                        <button type="button" class="btn btn-primary-new size-xs pull-right mr-1"
                                onclick="rangePrint()"><?php echo $this->lang->line('common_submit');?> <!--Submit-->
                        </button>               
                    </div>
                </div>
            </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary-new size-sm" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/accounts_payable/supplier_invoices_management', '', 'Supplier Invoices');
        });
        InvoiceAutoID = null;
        number_validation();
        ...
        $('#supplierPrimaryCodeModal').multiselect2({
            enableCaseInsensitiveFiltering: true,
            includeSelectAllOption: true,
            numberDisplayed: 1,
            buttonWidth: '180px',
    ...
    function supplier_invoices_table(selectedID=null) {
         Otable = $('#supplier_invoices_table').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Payable/fetch_supplier_invoices'); ?>",
            "aaSorting": [[0, 'desc']],
    ...
    function rangePrint(){
        var invoiceCodeFrom = $("#invoiceCodeFrom").val();
        var invoiceCodeTo = $("#invoiceCodeTo").val();
        var supplierID = $('#supplierPrimaryCodeID option:selected').val();
        var link_open= '';
        var fromVal = 0;
        var toVal = 0;
       
        if (invoiceCodeFrom.length != 0 && invoiceCodeTo.length != 0) {
            
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'json',
                data: {'invoiceCodeFrom': invoiceCodeFrom, 'invoiceCodeTo': invoiceCodeTo, 'supplierID': supplierID},
                url: "<?php echo site_url('Payable/getInvoiceAutoID'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    stopLoad();
                    fromVal= data['InvoiceAutoIDFrom']['InvoiceAutoID'];
                    toVal= data['InvoiceAutoIDTo']['InvoiceAutoID'];

                    if(toVal <= fromVal){
                        myAlert('w', 'Please enter correct range');
                    } else if(toVal == ''){
                        myAlert('w', 'Please enter correct To input');
                    } else if(fromVal == ''){
                        myAlert('w', 'Please enter correct From input');
                    } else {
                        link_open = "<?php echo site_url('Payable/load_supplier_invoice_range_conformation/') ?>"+fromVal+'/'+toVal;
    ...
    function Retension_model(id){
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {
                'invoiceAutoID': id,'documentID' : 'SUP'
            },
    ...
    function create_retension_invoice(){
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {
                'invoiceAutoID': retensionInviceautoid,'documentID' : 'SUP','amount' : retensionInvoicedValue
            },
            url: "<?php echo site_url('Payable/create_retension_invoice'); ?>",
            beforeSend: function() {
                startLoad();
            },
            success: function(data) {
