...
$title = $this->lang->line('inventory_stock_counting_approval');
...
<div class="table-responsive">
    <table id="stock_counting_table" class="<?php echo table_class() ?>">
        ...
        <tr>
            ...
            <th style="min-width: 10%"> SCNT <?php echo $this->lang->line('common_code');?><!--Code--> </th>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"> Stock Counting Approval </h4><!--Stock Adjustment Approval-->
            </div>
                        ...
                        <div class="zx-tab-content">
                            <div class="zx-tab-pane active" id="Tab-home-v">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        <input type="hidden" name="stockCountingAutoID" id="stockCountingAutoID">
                            ...
                            <div class="tab-pane hide" id="Tab-profile-v">
                                <div class="table-responsive">
                                    ...
                                    &nbsp <strong>Stock Counting Attachments </strong><!--Stock Adjustment Attachments-->
                                    <br><br>
...
<div aria-hidden="true" role="dialog" id="stock_insufficient_modal" class="modal fade" style="display: none;">
    <div class="modal-dialog" style="width: 60%;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php echo export_buttons('stock_insufficient_table_SCNT', 'Stock Insufficient', true, false); ?>
                <form role="form" id="stock_insufficient_form" class="form-horizontal">
                    <table class="table table-bordered table-condensed no-color" id="stock_insufficient_table_SCNT">
                        <thead>
                        <tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/inventory/stock_counting_approval', 'Test', 'Stock Counting Approval');
        });
        ...
        $('#ST_approval_form').bootstrapValidator({
            ...
            fields: {
                ...
                stockCountingAutoID: {validators: {notEmpty: {message: '<?php echo $this->lang->line('transaction_stock_adjustment_id_is_required');?>.'}}},/*Stock Adjustment ID is required*/
    ...
    function stock_counting_table() {
         Otable = $('#stock_counting_table').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('StockCounting/fetch_stock_counting_approval'); ?>",
            "aaSorting": [[1, 'desc']],
    ...
    function fetch_approval(stockCountingAutoID, documentApprovedID, Level) {
        if (stockCountingAutoID) {
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'html',
                data: {'stockCountingAutoID': stockCountingAutoID, 'html': true,'approval': 1},
                url: "<?php echo site_url('StockCounting/load_stock_counting_approval_conformation'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    $('#stockCountingAutoID').val(stockCountingAutoID);
                    $('#documentApprovedID').val(documentApprovedID);
                    $('#Level').val(Level);
                    $("#stock_adjustment_modal").modal({backdrop: "static"});
                    $('#conform_body').html(data);
                    $('#comments').val('');
                    stockAdjustment_attachment_View_modal('SCNT', stockCountingAutoID);
                    stopLoad();
                    refreshNotifications(true);
                }, error: function () {
                    stopLoad();
