...
foreach($companyID as $val){
    ...
    $itemval= get_group_item_details($groupItemMasterID,$val);
    ?>
    <div class="row">
        ...
        <div class="form-group col-sm-5" >
            <div class="">
                <?php
                if(!empty($itemval)){
                    echo form_dropdown('ItemAutoID[]', dropdown_companyitems($val,$itemval['ItemAutoID'],true), $itemval['ItemAutoID'], 'class="form-control select2" id="ItemAutoID_'.$val.'" required"');
                }else{
                    echo form_dropdown('ItemAutoID[]', dropdown_companyitems($val,null,true), '', 'class="form-control select2" id="ItemAutoID_'.$val.'" required"');
                }
                ?>
            </div>
        </div>
