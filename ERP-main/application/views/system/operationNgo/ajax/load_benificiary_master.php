    ...
    <div class="table-responsive mailbox-messages">
        <table class="table table-hover table-striped">
            ...
            foreach ($header as $val) {
                $printHelpAndNestArray = fetch_ngo_project_shortCode($val['benificiaryID']);
                    ...
                    <td class="mailbox-name">
                        <div class="contact-box">
                            <?php if ($val['benificiaryImage'] != '') {
                                $benificiaryImg = get_all_operationngo_images($val['benificiaryImage'],'uploads/ngo/beneficiaryImage/','bnfamdetail');?>
                            ...
                            <div class="link-box"><strong class="contacttitle"><a class="link-person noselect" href="#"
                                                                                  onclick="fetchPage('system/operationNgo/beneficiary_edit_view','<?php echo $val['benificiaryID'] ?>','View Beneficiary','NGO')"><?php echo $val['fullName']; ?>
                    ...
                    <td class="mailbox-attachment">
                        <span class="pull-right">
                            ...
                            if ($val['confirmedYN'] == 0) { ?>
                                <a href="#"
                                   onclick="fetchPage('system/operationNgo/create_beneficiary','<?php echo $val['benificiaryID'] ?>','Edit Beneficiary','NGO')"><span
                                        title="Edit" rel="tooltip" class="glyphicon glyphicon-pencil"></span></a>&nbsp;&nbsp;<a
                                    onclick="delete_beneficiary(<?php echo $val['benificiaryID'] ?>);"><span title="Delete" rel="tooltip"
                                                                                                             class="glyphicon glyphicon-trash"
                                                                                                             style="color:rgb(209, 91, 71);"></span></a>
                                <?php
                            } else {
                                if (in_array('HN',$testArray)) {
                                    $status .= '<a target="_blank" href="' . site_url('OperationNgo/load_beneficiary_print_view/') . '/' . $val['benificiaryID'] . '" ><span title="Print" rel="tooltip" class="glyphicon glyphicon-print"></span></a>&nbsp;&nbsp;|&nbsp;&nbsp;';
                                }
                                if (in_array('DA',$testArray)) {
                                    $status .= '<a target="_blank" href="' . site_url('OperationNgo/load_damageAssesment_print_view/') . '/' . $val['benificiaryID'] . '" ><span title="Print" rel="tooltip" class="glyphicon glyphicon-print"></span></a>&nbsp;&nbsp;|&nbsp;&nbsp;';
                                }
