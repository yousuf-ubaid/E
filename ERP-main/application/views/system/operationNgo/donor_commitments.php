...
$title = $this->lang->line('operationngo_donor_commitments');
...
<div class="row">
    ...
    <div class="col-md-3 text-right">
        <button type="button" class="btn btn-primary pull-right"
                onclick="fetchPage('system/operationNgo/create_commitments',null,'<?php echo $this->lang->line('operationngo_add_new_commitments');?>'/*Add New Commitments*/,'NGO');"><i
                class="fa fa-plus"></i> <?php echo $this->lang->line('operationngo_create_commitment');?><!--Create Commitment-->
        </button>
    </div>
</div>
...
<div class="row">
    <div class="col-md-12">
        <div class="table-responsive mailbox-messages" id="DonorMaster_view">
<!-- /.table -->
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        load_donor_commitments('#', 1);
        //getDonorCommitmentsTable();

    });

    function referback_donor_commitment(commitmentAutoId){
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'json',
            data: {'commitmentAutoId': commitmentAutoId},
            url: "<?php echo site_url('OperationNgo/referback_donor_commitment'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                myAlert(data[0],data[1]);
                load_donor_commitments();
                stopLoad();
            },
    ...
    function getDonorCommitmentsTable(filtervalue) {
        ...
        $.ajax({
            ...
            url: "<?php echo site_url('OperationNgo/load_donorCommitmentsView'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
        ...
        swal({
            ...
            function () {
                $.ajax({
                    ...
                    url: "<?php echo site_url('OperationNgo/delete_commitment_project'); ?>",
                    beforeSend: function () {
                        startLoad();
                    },
                    success: function (data) {
    ...
    function load_donor_commitments(value, id){
        $('.donorsorting').removeClass('selected');
        $('#sorting_'+ id).addClass('selected');
        if(value != '#'){
            $('#search_cancel').removeClass('hide');
        }
        getDonorCommitmentsTable(value)
    }
