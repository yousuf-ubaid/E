...
if (!empty($header)) {
    if ($header['isClosed'] != 1) {
     ...
     <div class="row">
            ...
            <div class="col-md-3 text-right projecteditbtn">
                <button type="button" class="btn btn-primary pull-right"
                        onclick="fetchPage('system/task_managment/create_new_task_employee',<?php echo $header['taskID'] ?>,'View Task','CRM','view');">
                    View
                </button>
            </div>
        </div>
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane" id="subtask">
            <br>
            <?php if ($header['completedDate'])  {  ?>
            <div class="row">
                    <!-- <div class="col-sm-12">
                        <button type="button" class="btn btn-primary pull-right"
                                onclick="create_sub_task()" id="beneficiaryassign" disabled>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#task_view_comment_form').bootstrapValidator({
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: data,
                url: "<?php echo site_url('Task_management/update_task_edit_view_comment'); ?>",
                beforeSend: function () {
                    startLoad();
                },
    ...
    function sub_task_view_assignee() {
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: {taskID: taskID},
            url: "<?php echo site_url('Task_management/load_subtask_detail_edit'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
    ...
    function chat_box_subtask_edit(subTaskID,taskID)
    {
        $.ajax({
            ...
            dataType: 'html',
            data: {'subTaskID':subTaskID,'taskID':taskID},
            url: "<?php echo site_url('Task_management/load_subtask_chats_edit'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
    ...
    function check_edit_approval(){
      ...
      <?php if($masterID == 'dashboardtask'){?>
        if(<?php echo $this->common_data['current_userID']?>==<?php if(!empty($superadmn)){ echo $superadmn['isadmin'];}else{echo 000;}  ?>){
            fetchPage('system/task_managment/create_new_task_employee_employee',<?php echo $header['taskID'] ?>,'Edit Task','dashboardtask','dashboardtask');
        }else if(<?php echo $this->common_data['current_userID']?>==<?php echo $header['crtduser'] ?>){
            fetchPage('system/task_managment/create_new_task_employee',<?php echo $header['taskID'] ?>,'Edit Task','dashboardtask','dashboardtask');
        ...
        <?php } else {?>
        if(<?php echo $this->common_data['current_userID']?>==<?php if(!empty($superadmn)){ echo $superadmn['isadmin'];}else{echo 000;}  ?>){
            fetchPage('system/task_managment/create_new_task_employee',<?php echo $header['taskID'] ?>,'Edit Task','CRM','CRM');
        }else if(<?php echo $this->common_data['current_userID']?>==<?php echo $header['crtduser'] ?>){
            fetchPage('system/task_managment/create_new_task_employee',<?php echo $header['taskID'] ?>,'Edit Task','CRM','CRM');
        }else if(<?php echo $tskass ?>==1){
            fetchPage('system/task_managment/create_new_task_employee',<?php echo $header['taskID'] ?>,'Edit Task','CRM','CRM');
