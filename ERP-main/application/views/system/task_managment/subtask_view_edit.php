    ...
    <div class="table-responsive mailbox-messages">
        <table class="table table-hover table-striped">
            ...
            foreach ($detail as $row) {
                ...
                $timespentsubtask = $this->db->query("select SUM(timeSpent) as subtasktimespent from srp_erp_task_subtasksessions where companyID = $companyid And taskID = '{$row['taskID']}' And subTaskID = '{$row['subTaskID']}'")->row_array();
                ...
                <tr>
                    ...
                    <td class="mailbox-name"><a href="#"><?php
                            ...
                            $assignees = $this->db->query("select empID, employees.Ename2 as Assigneename from srp_erp_task_assignees crmassignees LEFT join srp_employeesdetails employees on crmassignees.empID = employees.EIdNo where
crmassignees.companyID = '{$companyID}' And crmassignees.documentID = 10  And crmassignees.MasterAutoID = '{$row['subTaskID']}'")->result_array();
                    ...
                    <td class="mailbox-name">
                        ...
                        <?php if($row['status'] == 0){?>
                            <a style="cursor: pointer"
                                                onclick="load_sub_task_status_edit('<?php echo $row['subTaskID'] ?>','<?php echo $row['taskID'] ?>')"> <span
                                class="label"
                    ...
                    <td class="mailbox-name">
                        <?php
                        $companyid = current_companyID();
                        $totalnoofcahts = $this->db->query("SELECT chat.* FROM srp_erp_task_chat chat LEFT JOIN srp_employeesdetails empdetails ON empdetails.EIdNo = chat.empID WHERE companyID = $companyid AND taskID = '{$row['taskID']}' AND subTaskID =  '{$row['subTaskID']}' ORDER BY chatID asc")->result_array();
                        ?>
                            <a href="#"
                               onclick="chat_box_subtask_edit('<?php echo $row['subTaskID'] ?>','<?php echo $row['taskID'] ?>')"><span
                    ...
                    <td class="mailbox-name"><a href="#">
                      ...
                      <span class="pull-right">
                          <?php
                          $companyid = current_companyID();
                          $subtasksession = $this->db->query("select *,DATE_FORMAT(createdDateTime, '%h:%i:%s') as timesessionstarted from srp_erp_task_subtasksessions where companyID = $companyid And taskID  = '{$row['taskID']}' And subTaskID = '{$row['subTaskID']}' ORDER BY sessionID DESC LIMIT 1")->row_array();





                          if($subtasksession['status']== 0){ ?>
                          <a href="#"
                                   onclick="assign_validation_start_edit('<?php echo $row['subTaskID'] ?>','<?php echo $row['taskID']?>','<?php echo $row['createdUserID']?>')"><span
                                        title="Start" rel="tooltip"
                                        class="fa fa-play-circle playbtn"></span></a>&nbsp;&nbsp;|&nbsp;&nbsp;
                                        <?php } else if ($subtasksession['status']== 1) {?>
                              <a href="#"
                                 onclick="assign_validation_stop_edit('<?php echo $row['subTaskID'] ?>','<?php echo $row['taskID']?>','<?php echo $subtasksession['sessionID'] ?>','<?php echo $row['createdUserID']?>')"><span
                              ...
                              <a href="#"
                                 onclick="assign_validation_start_edit('<?php echo $row['subTaskID'] ?>','<?php echo $row['taskID']?>','<?php echo $row['createdUserID']?>')"><span
                                      ...
                                      class="fa fa-play-circle playbtn"></span></a>&nbsp;&nbsp;|&nbsp;&nbsp;
                                        <?php }?>
                            <a href="#"
                               onclick="sub_task_attachment_model_edit('<?php echo $row['subTaskID'] ?>','<?php echo $row['taskID']?>')"><span
                                    title="Start" rel="tooltip"
                                    class="glyphicon glyphicon-paperclip attachment"></span></a> |&nbsp; <a href="#"
                               onclick="edit_subtask_edit('<?php echo $row['subTaskID'] ?>','<?php echo $row['taskID']?>')"><span
                                    title="Edit" rel="tooltip"
                                    class="glyphicon glyphicon-pencil"></span></a>



                          <?php if ($subtasksession['status'] == 1 && $subtasksession['status'] != 2 && $subtasksession['status'] != 0){ ?>
