...
?>
<div class="m-b-md mb-1" id="wizardControl">
    <a class="btn btn-primary" href="#step1" data-toggle="tab">
        Bills [Shift <?php echo !empty($shiftID)?$shiftID :'' ;?>]</a>
    ...
    <?php
    if(show_all_closed_bills_in_teminal())
    {
    ?>
        <a class="btn btn-default" href="#step3" data-toggle="tab">Bill History</a>
    <?php
    }
    ?>
    ...
    <div id="step3" class="tab-pane">
        <div class="row" style="margin-top: 4%;">
            <div class="form-group col-sm-3">
                ...
                <div class="input-group">
                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                    <input type="text" name="from_bill" value="<?php echo date('Y-m-d') ?>"
                           id="from_bill"
                           class="form-control" required>
                </div>
            </div>
            <div class="form-group col-sm-3">
                ...
                <div class="input-group">
                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                    <input type="text" name="to_bill" value="<?php echo date('Y-m-d') ?>"
                           id="to_bill"
                           class="form-control" required>
                </div>
            </div>
        ...
        <div style="background-color: #ffffff; padding:10px;" class="table-responsive">
            <table class="<?php echo table_class_pos() ?> table-row-select" id="bill-history">
                <thead>
                <tr>
                    <th>#</th>
                    <th> Shift ID</th>
...
<script>
    ...
    $(document).ready(function (e) {
        ...
        $("#step3").on('change','#customer',function (e) {
            loadBillHistory();
        });
        /*End of Bill history filtering...*/
    ...
    function loadPrintTemplateVoid(id, outletID) {
        $.ajax({
            type: 'POST',
            dataType: 'html',
            url: "<?php echo site_url('Pos_restaurant/loadPrintTemplateVoid'); ?>",
            data: {menuSalesID: id, outletID: outletID},
            cache: false,
            beforeSend: function () {
                startLoad();
            },
    ...
    function loadPrintTemplateVoidHistory(id, outletID) {
        $.ajax({
            type: 'POST',
            dataType: 'html',
    ...
    function loadPrintTemplateBillHistory(id, outletID) {
        $.ajax({
            type: 'POST',
            dataType: 'html',
            url: "<?php echo site_url('Pos_restaurant/loadPrintTemplateBillHistory'); ?>",
            data: {menuSalesID: id, outletID: outletID},
            cache: false,
            beforeSend: function () {
                startLoad();
    ...
    function loadBillHistory() {
        $('#bill-history').DataTable({
            "bProcessing": true,
            "bServerSide": true,
            "bDestroy": true,
            "sAjaxSource": "<?php echo site_url('Pos_restaurant/billHistoryForClosedBills'); ?>",
            "aaSorting": [[0, 'desc']],
            "fnDrawCallback": function (oSettings) {
                if (oSettings.bSorted || oSettings.bFiltered) {
                    for (var i = 0, iLen = oSettings.aiDisplay.length; i < iLen; i++) {
            ...
            "fnServerData": function (sSource, aoData, fnCallback) {
                aoData.push({"name": "datefrom", "value": $("#from_bill").val()});
                aoData.push({"name": "dateto", "value": $("#to_bill").val()});
