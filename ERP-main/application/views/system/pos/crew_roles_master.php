                ...
                <div class="modal-body" style="min-height: 120px; ">
                    <div class="form-horizontal">
                        <fieldset>
                            ...
                            <div class="form-group">
                                <label class="col-md-4 control-label"> Is Delivery Person <i class="fa fa-cutlery"
                                                                                    aria-hidden="true"></i></label>
                                <div class="col-md-4">
                                    <div class="checkbox" style="margin-top: 7px;">
                                        <label for="checkboxes-0">
                                            <input type="checkbox" class="form-control" id="isDeliveryPerson" value="1"
...
<script type="text/javascript">
    ...
    // $("#isWaiter").change(function(){
    // });

    $('#isWaiter').on('ifChanged', function(event){
        let status = event.target.checked;
        if(status==true){
            $('#isDeliveryPerson').prop('checked', false).iCheck('update');
        }
    });

    $('#isDeliveryPerson').on('ifChanged', function(event){
        let status = event.target.checked;
        if(status==true){
    ...
    function edit_pos_crew_roles_config(id) {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data['isDeliveryPerson'] == 1) {
                    $('#isDeliveryPerson').prop('checked', true).iCheck('update');
                } else {
                    $('#isDeliveryPerson').prop('checked', false).iCheck('update');
                }
