                            ...
                            <tr>
                                ...
                                <?php
                                  if($isOutletTaxEnabled==true){
                                      echo '<th>'.$this->lang->line('pos_config_outlet_tax').'</th>';
                                  }
                                ?>
                                <th>&nbsp;</th>
                            </tr>
...
<!-- Add Tax -->
<div class="modal fade pddLess" data-backdrop="static" id="outlet_tax_modal" tabindex="-2" role="dialog">
    <div class="modal-dialog" style="min-width: 50%; margin-top: 6px;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="from_outlet_tax" name="from_outlet_tax" class="form-horizontal" method="post">
                    ...
                    <div class="form-group">
                        <label for="taxDescription" class="col-md-3 control-label">
                            Tax Description</label>
                        <div class="col-md-5">
                            <input type="text" id="taxDescription" name="taxDescription" class="form-control"/>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-3">
                            <buton class="btn btn-xs btn-primary" type="button" onclick="save_outlet_tax()">
                ...
                <table class="<?php echo table_class_pos(4) ?>" id="outlet_tax_table"
                       style="font-size:12px width:100%">
                    <thead>
                    <tr>
                        <th>#</th>
...
<script>
    ...
    function delete_outlet_tax(outlet_tax_id){
        bootbox.confirm('Are you sure you want to delete this outlet tax?', function (result) {
            if (result) {
                $.ajax({
                    type: 'post',
                    dataType: 'json',
                    data: {outlet_tax_id:outlet_tax_id},
                    url: "<?php echo site_url('Pos_config/delete_outlet_tax'); ?>",
                    beforeSend: function () {
                        startLoad();
                    },
                    success: function (data) {
                        stopLoad();
                        load_outlet_tax_table(current.outlet_id);
                    }, error: function () {
                        stopLoad();
                        myAlert('e', 'An Error Occurred! Please Try Again.');
                    }
    ...
    function load_outlet_tax_table(outlet_id) {
        $('#outlet_tax_table').DataTable({
            "bProcessing": true,
            "bServerSide": true,
            "bDestroy": true,
            "StateSave": true,
            "sAjaxSource": "<?php echo site_url('Pos_config/load_outlet_tax_table'); ?>",
            "aaSorting": [[0, 'desc']],
            ...
            "fnServerData": function (sSource, aoData, fnCallback) {
                aoData.push({'name': 'outlet_id', 'value': outlet_id});
                $.ajax({
                    'dataType': 'json',
                    'type': 'POST',
                    'url': sSource,
    ...
    function outlet_tax_modal(outlet_id) {
        current.outlet_id = outlet_id;
        $("#outlet_tax_modal").modal('show');
        load_outlet_tax_table(outlet_id);
    }

    function save_outlet_tax() {

        var outlet_id = current.outlet_id;
        var tax_taxPercentage = $("#tax_taxPercentage").val();
        var tax_taxmasterID = $("#tax_taxmasterID").val();
        var taxDescription = $("#taxDescription").val();

        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Pos_config/save_outlet_tax') ?>",
            data: {
                outlet_id: outlet_id,
                tax_taxPercentage: tax_taxPercentage,
                tax_taxmasterID: tax_taxmasterID,
    ...
    function toggleMenuAndItems() {
        $("#menu_edit_body").toggle();
        toggleItemsAndMenu();
    }

    function toggleItemsAndMenu() {
        $("#menu_item_list").toggle();
    }

    function backFromMenuItem() {
        $("#levelNo").val($("#levelNo_parent").val());
        $("#masterLevelID").val($("#masterLevelID_parent").val());
    ...
    function checkSubExist(menuCategoryID, masterLevelID, levelNo, wareHouseAutoID) {

        $("#masterLevelID_parent").val($("#masterLevelID").val());
        $("#levelNo_parent").val($("#levelNo").val());
        $.ajax({
            type: "GET",
            url: "<?php echo site_url('Pos_config/check_sub_exist_warehouse_menu_category') ?>",
            data: {
                menuCategoryID: menuCategoryID,
                masterLevelID: masterLevelID,
                levelNo: levelNo,
            ...
            success: function (data) {
                ...
                if (data['masterExist'] == true) {
                    posConfig_menu_inTab(data['menuCategoryID'], data['levelNo'], data['warehouse_id']);
                } else if (data['masterExist'] == false) {
                    //debugger;
                    LoadMenus(data['menuCategoryID'], data['warehouse_id'], data['autoID']);
                }
    ...
    function posConfig_menu_inTab(masterLevelID, levelNo, wareHouseID) {
        var id = $("#segment-congif-id").val();
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Pos_config/pos_config_menu_warehouse') ?>",
            data: {masterLevelID: masterLevelID, levelNo: levelNo, wareHouseID: wareHouseID, id: id},
            dataType: "html",
            cache: false,
            beforeSend: function () {
                $("#menu_edit_body").html('<?php echo loader_div() ?>');
    ...
    function refreshCategory(wareHouseID) {
        posConfig_menu_inTab(0, 0, wareHouseID);
        $("#masterLevelID").val(0);
    ...
    function prev_gotoCategory(parentId, parentLevel, wareHouseID) {
        var masterLevelID_parent = parentId;
        var levelNo_parent = parentLevel;
        posConfig_menu_inTab(masterLevelID_parent, levelNo_parent, wareHouseID);
        ...
        //loadCategoryDetail(wareHouseAutoID);
    }
    ...
    function add_menuCategory_setupModal(wareHouseAutoID) {
        $("#menuCategoryID").val('0');
        loadCategoryDetail(wareHouseAutoID);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data['error'] == 0) {
                    ...
                    posConfig_menu_inTab($("#masterLevelID").val(), $("#levelNo").val(), data['warehouseID']);
    ...
    function LoadMenus(categoryID, warehouseID, autoID = 0) {
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Pos_config/load_menu_items_warehouse') ?>",
            data: {categoryID: categoryID, warehouseID: warehouseID, autoID: autoID},
            dataType: "html",
            cache: false,
            beforeSend: function () {
                toggleMenuAndItems();
                $("#menu_item_list").show();
                $("#menu_item_list").html('<div style="text-align: center; padding:20px 0px;"><i class="fa fa-refresh fa-2x fa-spin" aria-hidden="true"></i> Loading... </div>');
            },
            success: function (data) {
                $("#menu_item_list").html(data);
            },
            error: function (jqXHR, textStatus, errorThrown) {
                myAlert('e', '<br>Message: ' + errorThrown);
            }
    ...
    function loadMenuItemsSetup(autoID) {
        $.ajax({
            ...
            beforeSend: function () {
                ...
                $("#menu_edit_body").html('<div style="text-align: center; padding:20px 0px;"><i class="fa fa-refresh fa-2x fa-spin" aria-hidden="true"></i> Loading... </div>');
            },
            success: function (data) {
                $("#menu_edit_body").html(data);
...
<div class="modal fade pddLess" data-backdrop="static" id="modal_Menu" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" style="min-width: 95%; margin-top: 6px;">
        <div class="modal-content"> <!-- <div class="color-line"></div>-->
            ...
            <input type="hidden" id="segment-congif-id" value="0">
            <div class="modal-body" id="menu_edit_body"
                 style="min-height: 200px; background-color: rgba(0, 0, 0, 0.04);">
            </div>
            <div id="menu_item_list"
                 style="min-height: 200px; background-color: rgba(0, 0, 0, 0.04);display: none;"></div>
            <div class="modal-footer" style="padding: 5px 10px;">
            ...
            <div class="modal-body" id="" style="min-height: 150px; background-color: rgba(0, 0, 0, 0.04);">
                <form id="fromAddMenuCategory_addMenuSetup" name="fromAddMenuCategory" class="form-horizontal"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            <div id="menu-category-select"></div>
...
<script>
    ...
    function loadCategoryDetail(wareHouseAutoID) {
        $("#menu-category-select").empty();
        var levelNo = $("#levelNo").val();
        var masterLevelID = $("#masterLevelID").val();

        data = {wareHouseID: wareHouseAutoID, levelNo: levelNo, masterLevelID: masterLevelID};

        $.ajax({
            type: 'POST',
            url: '<?php echo site_url("Pos_config/load_warehouse_category"); ?>',
            dataType: 'json',
            data: data,
            async: false,
            success: function (data) {
                $("#menu-category-select").append(data);
            }
        });
        $('#menuCategoryID_MenuSetup').select2();
    }
