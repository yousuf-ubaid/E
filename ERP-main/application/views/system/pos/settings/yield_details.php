                ...
                <div class="tab-pane active" id="tab_1">
                    <div class="posContainer">
                        ...
                        <h4 class="" style="font-weight: bold">
                            <?php
                            echo $yield['Description'];
                            echo '<small class="">&nbsp;&nbsp;&nbsp;' . $yield['itemSystemCode'] . '</small>'; ?>
                            <span class="btn btn-primary btn-xs pull-right" onclick="openAddYieldItemModal()"><i
                                        class="fa fa-plus"></i>
                                <?php echo $this->lang->line('pos_config_add_item'); ?><!--Add Item--></span>
                            <span class="btn btn-default btn-xs pull-right" style="margin-right: 2px;"
                                  onclick="fetchPage('system/pos/settings/yield_setup','','Yield Setup')"><i
                        ...
                        <table class="<?php echo table_class_pos(1) ?>" id="tbl_yieldDetail"
                            ...
                            <th>
                                <div style="font-size:16px; font-weight: bold" id="grand_total" class="ar">0.00</div>
                            </th>
                            <th>&nbsp;</th>
                            </tfoot>
...
<script>
    var defaultUOM = '';
    var unitcost = '';
    var companyDecimal = <?php echo get_company_currency_decimal(); ?>;
    ...
    function fetchYieldDetail() {
        $('#tbl_yieldDetail').DataTable({
            ...
            "fnInitComplete": function (tmp) {
                calculateGrandTotal($("#yieldIDhn").val())
            },
    ...
    function calculateGrandTotal(id) {
        $d = <?php echo get_company_currency_decimal(); ?>;
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'json',
            data: {'yieldID': id},
            url: "<?php echo site_url('Pos_config/sum_total_yield'); ?>",
            success: function (data) {
                if (data['cost'] > 0) {
                    $("#grand_total").html(parseFloat(data['cost']).toFixed($d));
                }
    ...
    function openAddYieldItemModal() {
        ...
        var itemCostBasedOnWacPolicy = <?php echo cost_based_on_itemmaster_wac(); ?>; //checking if COSTWAC policy enabled.
        if(itemCostBasedOnWacPolicy){
            $('#cost').attr('readonly', true);
            $('#unitCost').attr('readonly', true);
        }else{
            $('#cost').attr('readonly', false);
            $('#unitCost').attr('readonly', false);
        }
    }
    ...
    function edit_yieldDetail(id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var itemCostBasedOnWacPolicy = <?php echo cost_based_on_itemmaster_wac(); ?>; //checking if COSTWAC policy enabled.
                if(itemCostBasedOnWacPolicy){
                    var companyLocalWacAmount = (parseFloat(data['companyLocalWacAmount'])).toFixed(companyDecimal);
                    $('#cost').val(companyLocalWacAmount);
                    $('#unitCost').val(companyLocalWacAmount);
                    $('#cost').attr('readonly', true);
                    $('#unitCost').attr('readonly', true);
                }else{
                    var cost = (parseFloat(data['cost'])).toFixed(companyDecimal);
                    var unitCost = (parseFloat(data['unitCost'])).toFixed(companyDecimal);
                    $('#cost').val(cost);
                ...
                setTimeout(function () {
                    load_default_uom_and_cost(data['uom']);
                }, 300);
        ...
        <div class="modal-content"> <!-- <div class="color-line"></div>-->
            ...
            <form role="form" id="fromyielddetail" class="form-group">
                <div class="modal-body" style="min-height: 100px; ">
                    <div class="row" style="">
                        <input type="hidden" id="yieldIDhn" name="yieldIDhn"
                               value="<?php echo $this->input->post('page_id'); ?>">
                    ...
                    <div class="row" style="">
                        <div class="form-group col-sm-4">
                            <label class="lbl">
                                <?php echo $this->lang->line('pos_config_unit_cost'); ?><!--Unit Cost--> <?php required_mark(); ?></label>
                            <input type="number" step="any" class="form-control" id="unitCost"
                                   name="unitCost" onchange="changeTotalCost()">
                        </div>
                        <div class="form-group col-sm-4">
                            <label class="lbl">
                                <?php echo $this->lang->line('pos_config_total_cost'); ?><!--Total Cost--> <?php required_mark(); ?></label>
                ...
                <div class="modal-footer" style="padding: 5px 10px;">
                    <button type="button" onclick="addyieldDetail()" class="btn btn-primary btn-xs">
