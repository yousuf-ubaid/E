...
if (!isset($pdf)) {
    ...
    <button type="button" id="btn_print_void" class="btn btn-default btn-xs"><i
        ...
        <a href="" class="btn btn-excel btn-xs" id="btn-excel" download="Void_Report.xls"
           onclick="var file = tableToExcel('container_void_report3', 'Void Report'); $(this).attr('href', file);">
            <i class="fa fa-file-excel-o" aria-hidden="true"></i> Excel
        </a>
</span>
<?php } ?>
<div id="container_void_report3">
    ...
    <h3 class="text-center">Void Bills</h3>
    ...
    <div style="margin:4px 0px;">
        <?php
        $cash = $this->lang->line('posr_waiter');
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                ...
                <table class="<?php echo table_class_pos(5) ?>">
                    ...
                    <tr>
                        ...
                        <th rowspan="3"><?php echo $this->lang->line('posr_waiter'); ?></th>
                    ...
                    <tr>
                        ...
                        if (!empty($paymentglConfigMaster)) {
                            foreach ($paymentglConfigMaster as $config) {
                                if ($config['description'] == 'Credit Sales') {
                                    echo "<th colspan='3'>" . $config['description'] . "</th>";
                                } else {
                    ...
                    if (!empty($recordDetail)) {
                        foreach ($recordDetail as $detail) {
                            ...
                            <tr>
                                ...
                                <td><a href="#"
                                       onclick="viewDrillDown_report(<?php echo $detail['salesMasterMenuSalesID']; ?>,<?php echo $detail['wareHouseAutoID']; ?>)"><?php echo $detail['invoiceCode'] ?></a>
                                </td>
                                ...
                                <td>
                                    <?php
                                    $commissionAmount = 0;
                                    if (!empty($detail['DeliveryCommissionType'])) {
                                        $commissionAmount = $detail['billNetTotal'] * ($detail['deliveryCommission'] / 100);
                                ...
                                <td class="amountAlign">
                                    <?php
                                    echo number_format($commissionAmount, $detail['companyLocalDecimal']);
                                    //$detail['deliveryCommissionAmount']
                                    ?>
                                </td>
                                <td class="amountAlign">
                                    <?php
                                    /** ------------------------------------------------- Net Total ------------- */
                                    if((isset($detail['isDelivery']) && $detail['isDelivery']==1) &&(isset($detail['isOnTimeCommision']) && $detail['isOnTimeCommision']!=1)){
                                        $netRowTotal = $detail['billNetTotal'];
                                    }else{
                                        $netRowTotal = $detail['billNetTotal'] - $commissionAmount;
                                    }
                                ...
                                if (!empty($paymentglConfigMaster)) {
                                    foreach ($paymentglConfigMaster as $config) {
                                        ...
                                        if ($newName == 'CreditSales') { ?>
                                            <td style="width: 10px;"><?php echo $detail['customerName']; ?></td>
                                            <td style="width: 10px;"><?php echo $detail['customerTelephone']; ?></td>
                                            <td class="amountAlign">
                                                <?php
                            ...
                            $deliveryCommissionTotal += $commissionAmount;
...
<script>
    ...
    $(document).ready(function (e) {
        $("#btn_print_void").click(function (e) {
            $.print("#container_void_report3");
        });
        var date = new Date,
            hour = date.getHours(),
