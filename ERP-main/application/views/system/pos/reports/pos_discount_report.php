<?php echo head_page('<i class="fa fa-bar-chart"></i> Discount Report', false);
...
?>
<form id="frm_discountReport" method="post" class="form-inline" role="form">
    ...
    &nbsp;&nbsp;Discounts:
    <?php //echo form_dropdown('customerID[]', $get_discountType, '', ''); ?>
    <select name="customerID[]" class="form-control select2" multiple id="customerID">
        <?php
        if (!empty($get_discountType)) {
            echo '<option value="-1">General Discount</option>';

            foreach ($get_discountType as $key => $discount) {
                ?>
                <option value="<?php echo $key ?>"><?php echo $discount ?></option>';
                <?php
            }
        }
        ?>
    ...
    <button type="button" onclick="load_discount_Report()" class="btn btn-primary btn-sm">
...
<script>
    ...
    function load_discount_Report() {
        $("#promoOrder_outletID").val($("#wareHouseAutoID").val());
        $.ajax({
            type: 'POST',
            dataType: 'html',
            url: "<?php echo site_url('Pos_restaurant/LoadDiscountReport'); ?>",
            data: $("#frm_discountReport").serialize(),
            cache: false,
            beforeSend: function () {
