...
$html .= '
    <table class="table" style="width: 100%;">
        ...
        <tr>
            ...
            if($extra['master']['isGroupBasedTax'] == 1){
                $html .= '<th style="min-width: 50%" class="theadtr" colspan="6">'. $this->lang->line('common_price').' ('.  $extra['master']['transactionCurrency']  .')</th>';
            } else {
                $html .= '<th style="min-width: 50%" class="theadtr" colspan="4">'. $this->lang->line('common_price').' ('.  $extra['master']['transactionCurrency']  .')</th>';
            }
            $html .= '</tr>
        <tr>
            <th style="width: 4%" class="theadtr">#</th>
            '.$imagede.'
            <th style="width: 10%" class="theadtr">Part No</th>
    ...
    if (!empty($extra['detail'])) {
        foreach ($extra['detail'] as $val) {
            ...
            '.$imagetd.'
            <td class="text-center" style="font-size: 12px;width:15%" >'. $val['partNo'].'</td>
            ...
            <td style="text-align:right; font-size: 12px;">'. number_format(($unit_price), $extra['master']['transactionCurrencyDecimalPlaces']).'</td>
            <td style="text-align:right; font-size: 12px;">'. number_format($unit_discount, $extra['master']['transactionCurrencyDecimalPlaces']).' ( '.$val['discountPercentage'].'%)</td>
            <td style="text-align:right; font-size: 12px;">'. number_format($net_unit_discount_amt, $extra['master']['transactionCurrencyDecimalPlaces']).'</td>';
            if($extra['master']['isGroupBasedTax'] == 1 ){
 ...
 if ($extra['master']['Note']) {
    ...
    <table style="width: 100%">
        ...
        <tr>
            ...
            <td style="width: 80%">' . $extra['bankDetails']['bankName'] . '</td>
        </tr>
        <tr>
            <td><strong>' . $this->lang->line('common_branch') . '</strong></td><!--Branch-->
            <td><strong>:</strong></td>
            <td>' . $extra['bankDetails']['bankBranch'] . '</td>
        </tr>
        ...
        <tr>
            ...
            <td><strong>:</strong></td>
            <td>' . $extra['bankDetails']['GLDescription'] . '</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><strong>:</strong></td>
            <td>' . $extra['bankDetails']['bankCurrencyCode'] . '</td>
        </tr>
        </tbody>
    </table>
</div>';
