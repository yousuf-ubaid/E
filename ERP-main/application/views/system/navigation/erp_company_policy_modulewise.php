...
$pageName = $page[0];
$pageID = $page[1];
echo head_page($pageName.' Policy', false);
...
$policyMasters = $this->db->query("SELECT companyPolicyDescription, companypolicymasterID,fieldType FROM srp_erp_companypolicymaster Where moduleID = $pageID")->result_array();
...
<div aria-hidden="true" role="dialog" tabindex="-1" id="add_usergrouptopolicy_modal"
     class=" modal fade bs-example-modal-lg">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                <h5 class="modal-title">Add User Group to Policy</h5>
            </div>

            <div class="modal-body">
                <div id="addusergrouptopolicy">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-primary " onclick="saveusergrouptopolicy()"><i
                            class="fa fa-floppy-o"></i> <?php echo $this->lang->line('common_save');?><!--Save-->
                </button>
                <button data-dismiss="modal" class="btn btn-default" type="button"><?php echo $this->lang->line('common_Close');?><!--Close--></button>
            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/navigation/erp_company_policy_modulewise', '0', '<?php echo $this->input->post('page_name')?>')
        })
    });
    ...
    function xfetch_company_policy_table() {
        var Otable = $('#company_policy_table').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('CompanyPolicy/fetch_company_policy_modulewise'); ?>",
            "aaSorting": [[1, 'desc']],
            ...
            "fnServerData": function (sSource, aoData, fnCallback) {
                aoData.push({"name": "moduleID", "value": <?php echo $pageID?>});
                $.ajax({
                    'dataType': 'json',
                    'type': 'POST',
                    'url': sSource,
    ...
    function addUserGrptopolicy(polcyId) {
        var data=0;
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'html',
            data: {polcyId: polcyId},
            url: "<?php echo site_url('CompanyPolicy/get_usergroupfor_policy'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                stopLoad();
                $('#add_usergrouptopolicy_modal').modal('show');
                $('#addusergrouptopolicy').html(data);
            }, error: function () {
                alert('<?php echo $this->lang->line('common_an_error_occurred_Please_try_again');?>.');/*An Error Occurred! Please Try Again*/
                stopLoad();
                myAlert(data[0], data[1]);
    ...
    function saveusergrouptopolicy(){
        var data = $("#usergrouptopolicy_form").serializeArray();
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'json',
            data: data,
            url: "<?php echo site_url('CompanyPolicy/saveusergrouptopolicy'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                ...
                myAlert(data[0], data[1]);
                if (data[0] == 's') {
                    $('#add_usergrouptopolicy_modal').modal('hide');
                }
            }, error: function () {
                alert('<?php echo $this->lang->line('common_an_error_occurred_Please_try_again');?>.');/*An Error Occurred! Please Try Again*/
                stopLoad();
