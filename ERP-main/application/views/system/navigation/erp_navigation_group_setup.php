...
<script type="text/javascript">
    ...
    function isCheckedViewNav(x,navigationMenuID){

        if (x.checked == true) {

            $('#addedit' + navigationMenuID).prop('disabled', false);
            $('#printexcel' + navigationMenuID).prop('disabled', false);
        }
        else{

            $('#addedit' + navigationMenuID).prop('checked', false);
            $('#addedit' + navigationMenuID).prop('disabled', true);

            $('#printexcel' + navigationMenuID).prop('checked', false);
            $('#printexcel' + navigationMenuID).prop('disabled', true);
    ...
    function saveNavigationgroupSetup() {
        ...
        var viewID = [];
       $('.viewVal:checked').each(function (i, e) {
           viewID.push(e.value);
        });
        viewID = viewID.join(',');

        var addEditID = [];
        $('.addEditVal:checked').each(function (i, e) {
            addEditID.push(e.value);
        });
        addEditID = addEditID.join(',');

        var printExcelID = [];
        $('.printExcelVal:checked').each(function (i, e) {
            printExcelID.push(e.value);
        });
        printExcelID = printExcelID.join(',');
       // alert(addEditID);
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'json',
            data: {navigationID: navigationID,viewID:viewID,addEditID:addEditID,printExcelID:printExcelID,userGroupID: $('#userGroupID').val()},
