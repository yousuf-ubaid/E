...
<div class="row">
    ...
    <div class="col-md-3 text-right">
        <button type="button" class="btn btn-primary-new size-sm pull-right"
                onclick="fetchPage('system/procurement/erp_purchase_order_new',null,'<?php echo $this->lang->line('procurement_approval_add_purchase_order'); ?>','PO');">
...
<div class="modal fade" id="po_document_version_View" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document" style="width:90%">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="document_version_ViewTitle">Document Version</h4><!--Modal title-->
                <input type="hidden" name="masterPoID" id="masterPoID">
            </div>
            <div class="modal-body" id="po_document_version_View_body">
                
            </div>
            <div class="modal-footer">
                <a class="btn btn-primary" onclick="add_quotation_version_po()" role="button">Add Version </a><!--Quotation Version-->
...
<div class="modal fade bs-example-modal-sm" role="dialog" aria-labelledby="myLargeModalLabel"
     id="max_portal_po_model">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="usergroup-title">Submision Details </h4>
            </div>
            <?php echo form_open('', 'role="form" id="max_portal_Status"'); ?>
            <div class="modal-body">
                ...
                <div class="row" style="margin-top: 10px;" id="statusvoucher">
                    ...
                    <div class="form-group col-sm-6">
                        <span class="input-req" title="Required Field">
                            <?php echo form_dropdown('statuspo', array(''=>'Select Status','2'=>'Submitted'), '', 'class="form-control select2" id="statuspo" required'); ?>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-sm btn-primary" onclick="updatepostatuson_maxportal()"><span class="glyphicon glyphicon-floppy-disk"
...
<script type="text/javascript">
    ...
    function purchase_order_table(selectedID=null) {
        Otable = $('#purchase_order_table').DataTable({
            ...
            "bDestroy": true,
            "StateSave": true,
            "sAjaxSource": "<?php echo site_url('Procurement/fetch_purchase_order'); ?>",
            "aaSorting": [[0, 'desc']],
          /*  "columnDefs": [
                {
                    "targets": [7],
    ...
    function po_version_View_modal(documentID, para1){
        title = "Quotation";
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'html',
            data: {'purchaseOrderID': para1,'html': true},
            url: "<?php echo site_url('Procurement/load_purchase_order_conformation'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
               // $('#document_version_ViewTitle').html(title);
                $('#po_document_version_View_body').html(data);
                $('#po_document_version_View').modal('show');
                $("#masterPoID").val(para1);
                stopLoad();
            }, error: function () {
    ...
    function add_quotation_version_po(){
        $('#po_document_version_View').modal('hide');
        var id = $("#masterPoID").val();

        if(id){

            swal({
                title: "<?php echo $this->lang->line('common_are_you_sure');?>",/*Are you sure*/
                text: "You want to reverse this version",/*You want to reverse this version*/
            ...
            function () {
                $.ajax({
                    ...
                    dataType : 'json',
                    data : {'masterID':id},
                    url :"<?php echo site_url('Procurement/add_quotation_version_po'); ?>",
                    beforeSend: function () {
                        startLoad();
                    },
                    success : function(data){
    ...
    function open_po_sending_model_max_portal(code,autoID,collectedStatus) {
        $('#poID').val(autoID);
        $('#max_portal_po_model').modal("show");
    }

    function updatepostatuson_maxportal()
    {
        var data = $('#max_portal_Status').serializeArray();
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'json',
            data: data,
            url: "<?php echo site_url('Procurement/open_po_sending_model_max_portal'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                ...
                if (data[0] == 's') {
                    myAlert(data[0], data[1]);
                    $('#max_portal_po_model').modal("hide");
                    Otable.draw();
