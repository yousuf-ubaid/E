...
<div class="row">
    ...
    <div class="col-md-3 text-right">
        ...
        <button type="button" class="btn btn-primary pull-right"
                onclick="fetchPage('system/hrm/personal_application_mse_new', null, 'Add Personal Action', 'PAA');">
...
<!--model for referback view -->
<div class="modal fade" id="reject_drill_user_modal_mse" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="ap_user_label_referback">Approval Rejected Model</h4>
            </div>
            <div class="modal-body">
                <dl class="dl-horizontal">
                    <dt><?php echo $this->lang->line('common_document_code'); ?><!--Document code--></dt>
                    <dd id="c_document_code_reject_mse">...</dd>
                </dl>
                <table class="table table-striped table-condensed table-hover">
                    <thead>
                    <tr>
                        ...
                        <th><?php echo $this->lang->line('common_name'); ?><!--Name--></th>
                        <th><?php echo $this->lang->line('common_level'); ?><!--Level--></th>
                        <th>Referred-back Date<!--Referred-back Date--></th>
                        <th>Status</th>
                        <th><?php echo $this->lang->line('common_comments'); ?><!--Comments--></th>
                    </tr>
                    </thead>
                    <tbody id="reject_ap_user_body_mse" class="no-padding">
                    <tr class="danger">
                        <td colspan="5" class="text-center">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/hrm/personal_application_mse', 'Test', 'Personal Action');
        });
    ...
    function personal_action_table(selectedID=null) {
        Otable = $('#personal_action_table').DataTable({
            ...
            "bDestroy": true,
            "StateSave": true,
            "sAjaxSource": "<?php echo site_url('Employee/fetch_personal_action_mse_table'); ?>",
            "aaSorting": [[1, 'desc']],
        ...
        swal({
            ...
            function () {
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {'id': id},
                    url: "<?php echo site_url('Employee/referback_persional_action_mse'); ?>",
                    beforeSend: function () {
                        startLoad();
                    },
                    success: function (data) {
    ...
    function fetch_approval_user_modal_pa_mse(documentID, documentSystemCode, typeId) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            data: {'documentID': documentID, 'documentSystemCode': documentSystemCode, 'typeId': typeId},
            url: '<?php echo site_url('Employee/fetch_approval_user_modal_pa_mse'); ?>',
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
    ...
    function fetch_approval_reject_user_modal_mse(documentSystemCode) {
    $.ajax({
        type: 'post',
        dataType: 'json',
        data: {'documentID': 'PAA', 'documentSystemCode': documentSystemCode},
        url: '<?php echo site_url('Approvel_user/fetch_reject_user_modal'); ?>',
        beforeSend: function () {
            startLoad();
        },
        success: function (data) {
            $('#reject_ap_user_body_mse').empty();

            if (data.rejected && data.rejected.length > 0) {
                var x = 1;
                $.each(data.rejected, function (key, value) {
                    var comment = value.comment ? value.comment : ' - ';
                    var bePlanVar = '<span class="label label-warning">&nbsp;</span>';

                    $('#reject_ap_user_body_mse').append('<tr><td>' + x + '</td><td>' + value.ECode + ' - ' + value.Ename2 + '</td><td class="text-center"> Level ' + value.rejectedLevel + '</td><td style="text-align: center">' + value.referbackDate + '</td><td style="text-align: center">' + bePlanVar + '</td><td>' + comment + '</td></tr>');
                    x++;
                });

                $("#c_document_code_reject_mse").html(data.document_code);

            } else {
                $('#reject_ap_user_body_mse').append('<tr class="danger"><td colspan="6" class="text-center"><b>No Records Found</b></td></tr>');
            }

            $("#reject_drill_user_modal_mse").modal({ backdrop: "static", keyboard: true });
