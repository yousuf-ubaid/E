...
<div class="row">
    ...
    <div class="col-md-3 text-right">
        <?php
        if(($header['clsd']!=1)&&($header['clsd']!=3)){
            ?>
        ...
        <?php
            ...
            ?>
            <?php if($header['clsd']==1){?>
            <button type="button"  class="btn btn-primary pull-right projecteditbtn"
                    onclick="closed_project_warnig(1)">
                <span title="" rel="tooltip" class="glyphicon glyphicon-pencil" data-original-title="Edit"></span>
                Edit
            </button>
                <?php } else { ?>
                <button type="button"  class="btn btn-primary pull-right projecteditbtn"
                        onclick="closed_project_warnig(3)">
                    <span title="" rel="tooltip" class="glyphicon glyphicon-pencil" data-original-title="Edit"></span>
                    Edit
                </button>
                <?php }?>
...
<ul class="nav nav-tabs" id="main-tabs">
    ...
    <?php if($page == 'projectTask'){?>
    <li class="active"><a href="#tasks" onclick="project_tasks()" data-toggle="tab"><i class="fa fa-television"></i>Tasks </a></li>
    <?php } else {?>
                ...
                <table class="property-table">
                    ...
                    <tr>
                        ...
                        <td><span class="tddata">
                                <?php echo $header['documentSystemCodeproject'] ?>
                            </span>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="label" style="background-color: <?php echo $header['statusBackgroundColor'] ?>; color:<?php echo $header['statusTextColor'] ?>; font-size: 11px;"><?php echo $header['statusDescription'] ?></span>&nbsp;
                            <?php if($header['clsd']==3 || $header['clsd']==1)
                            { ?>
                            <span class="tddata">
                                On <?php echo $header['clsdDate'] ?></span>
                            <?php }?>
                    ...
                    <tr>
                        ...
                        <td><span class="tddata"><?php echo "Yes"?></span>
                        </td>
                    </tr>
                    <?php }?>
                    <?php if(($header['clsd']==1) || ($header['clsd']==3)){?>
                        <tr>
                            <?php if($header['clsd']==1){?>
                            <td class="ralign"><span class="title">Closed Reason</span></td>
                             <?php } else {?>
                                <td class="ralign"><span class="title">Cancelled Reason</span></td>
                            <?php }?>
                            <td><span class="tddata"><?php echo $header['reason']?></span></td>
                        </tr>
                    <?php }?>
    ...
    <div class="modal-dialog" style="width: 59%;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="hidden" id="projectid" name="projectid" value="<?php echo $header['projectID']; ?>">
                <div class="row">
    ...
    <script type="text/javascript">
        ...
        $('#emailcomposemasterform').bootstrapValidator({
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: data,
                url: "<?php echo site_url('CrmMailBox/compose_email_project'); ?>",
                contentType: false,
                cache: false,
                processData: false,
                beforeSend: function () {
        ...
        function closed_project_warnig(closetype){
            if(closetype == 1)
            {
                myAlert('w','You canot Edit this project. This Project has been closed')
            }else
            {
                myAlert('w','You canot Edit this project. This Project has been cancelled')
            }
        ...
        function emailssentview() {
            ...
            $.ajax({
                ...
                dataType: 'html',
                data: {'projectid': projectid},
                url: "<?php echo site_url('CrmMailbox/projectview'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
