function item_wise_discount(tmp_this, typeOfSource, itemId) {
    var inputData = {'typeOfSource': typeOfSource, 'tmp_this': tmp_this};

    localStorage.setItem('discountSource',typeOfSource);
    if (typeOfSource == 'P') {
        var oldDiscountPercentageId = '#discountPercentage_' + itemId;
        localStorage.setItem('discountPercentageId',oldDiscountPercentageId);
    } else if (typeOfSource == 'A') {
        var oldDiscountAmountId = '#discountAmount_' + itemId;
        localStorage.setItem('discountAmountId',oldDiscountAmountId);
    }
    checkPosAuthentication(12, inputData);
    /*
     if (typeOfSource == 'P') {
     calculateFooter('P');
     } else if (typeOfSource == 'A') {
...
function calculateDelivery() {
    var total = parseFloat($("#final_payableNet_amt").text()) + parseFloat($("#delivery_advancePaymentAmount").val());
    ...
    if ($("#deliveryPersonID option:selected").data('otp') == 0) {
        return_amount = paidAmount - total;

        $("#returned_change_toDelivery").text(return_amount.toFixed(2));
        /**/
        $("#returned_change_toDelivery").val(return_amount.toFixed(2));
    }
}
...
function calculateReturn() {
   ...
   // console.log($("#return_change").text());
...
function calculatePromo() {
    ...
    if (!isNaN(promotion) && promotion > 0) {
        ...
        $("#promotional_discount").val((promotionAmount).toFixed(data.decimalplaces));
    } else {
...
function calculatePromoUpdate() {
    ...
    if (!isNaN(promotion) && promotion > 0) {
        var promotionAmount = (promotion / 100) * payableAmount;
        $("#promotional_discountUpdate").val((promotionAmount).toFixed(data.decimalplaces));
    } else {
