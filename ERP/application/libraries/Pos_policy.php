...
class pos_policy
{
    ...
    function get_policy($pos_type=1)
    {
        ...
        $q = "SELECT
                ...
                WHERE
                    ...
                    WHERE pm.posType=$pos_type";
        return $this->ci->db->query($q)->result_array();
    }

    function isPartialPaymentEnabled(){
        $this->outletID = get_outletID();
        $this->companyID = current_companyID();

        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 21);
        $this->ci->db->where('outletID', $this->outletID);
        $this->ci->db->where('companyID', $this->companyID);
        $this->ci->db->limit(1);
        $policyID = $this->ci->db->get()->row('posPolicyID');
    ...
    function isSplitBillEnabled(){
        $this->outletID = get_outletID();
        $this->companyID = current_companyID();

        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 22);
        $this->ci->db->where('outletID', $this->outletID);
        $this->ci->db->where('companyID', $this->companyID);
        $this->ci->db->limit(1);
        $policyID = $this->ci->db->get()->row('posPolicyID');
    ...
    function isODWEnabled(){
        $this->outletID = get_outletID();
        $this->companyID = current_companyID();

        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 23);
        $this->ci->db->where('outletID', $this->outletID);
        $this->ci->db->where('companyID', $this->companyID);
        $this->ci->db->limit(1);
        $policyID = $this->ci->db->get()->row('posPolicyID');
    ...
    function isHideZeroDiscountFields(){
        $this->outletID = get_outletID();
        $this->companyID = current_companyID();

        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 24);
        $this->ci->db->where('outletID', $this->outletID);
        $this->ci->db->where('companyID', $this->companyID);
        $this->ci->db->limit(1);
        $policyID = $this->ci->db->get()->row('posPolicyID');
    ...
    function AutoPrintKOTonHold(){
        $this->outletID = get_outletID();
        $this->companyID = current_companyID();

        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 29);
        $this->ci->db->where('outletID', $this->outletID);
        $this->ci->db->where('companyID', $this->companyID);
        $this->ci->db->limit(1);
        $policyID = $this->ci->db->get()->row('posPolicyID');
    ...
    function AutoPrintKOTonHoldAPI($outletID,$companyID){

        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 29);
        $this->ci->db->where('outletID', $outletID);
    ...
    function posSessionClosePayment(){
        $this->outletID = get_outletID();
        $this->companyID = current_companyID();

        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 30);
        $this->ci->db->where('outletID', $this->outletID);
        $this->ci->db->where('companyID', $this->companyID);
        $this->ci->db->limit(1);
        $policyID = $this->ci->db->get()->row('posPolicyID');
    ...
    function pinBasedAccess(){
        $this->outletID = get_outletID();
        $this->companyID = current_companyID();

        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 31);
        $this->ci->db->where('outletID', $this->outletID);
        $this->ci->db->where('companyID', $this->companyID);
        $this->ci->db->limit(1);
        $policyID = $this->ci->db->get()->row('posPolicyID');
    ...
    function waiterSelectionMandatory(){
        $this->outletID = get_outletID();
        $this->companyID = current_companyID();

        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 32);
        $this->ci->db->where('outletID', $this->outletID);
        $this->ci->db->where('companyID', $this->companyID);
        $this->ci->db->limit(1);
        $policyID = $this->ci->db->get()->row('posPolicyID');
    ...
    function showStaffButton(){
        ...
        $this->ci->db->select('*');
        $this->ci->db->from('srp_erp_pos_policydetail');
        $this->ci->db->where('posPolicyMasterID', 33);
        $this->ci->db->where('outletID', $this->outletID);
        $this->ci->db->where('companyID', $this->companyID);
        $this->ci->db->limit(1);
        $policyID = $this->ci->db->get()->row('posPolicyID');
