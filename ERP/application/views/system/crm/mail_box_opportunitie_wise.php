...
<?php if ((!empty($mailbox)&&($mailbox['successYN']==1))) { ?>
    <button type="button" onclick="compose_email_opportunitie()" class="btn btn-primary pull-right"><i
...
<script type="text/javascript">
    ...
    function getInboxMails(page, uriSegment = 0) {
        var opportunityID = <?php echo $opportunityID ?>;
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'json',
            data: {pageID: page, folder: 'INBOX','opportunityID':opportunityID},
            url: "<?php echo site_url('CrmMailbox/getInboxMailsopportunitie'); ?>/" + uriSegment,
            beforeSend: function () {
                startLoad();
            },
