...
foreach($companyID as $val){
    ...
    $categoryval= get_group_customer_category_details($groupSupplierCategoryID,$val);
    ?>
    <div class="row">
        ...
        <div class="form-group col-sm-5" >
            <div class="">
                ...
                if(!empty($categoryval)){
                    echo form_dropdown('partyCategoryID[]', dropdown_company_group_supplier_categories($val,$categoryval['partyCategoryID']), $categoryval['partyCategoryID'], 'class="form-control select2" id="partyCategoryID_'.$val.'" required"');
                }else{
                    echo form_dropdown('partyCategoryID[]', dropdown_company_group_supplier_categories($val), '', 'class="form-control select2" id="partyCategoryID_'.$val.'" required"');
                }
                ?>
            </div>
        </div>
