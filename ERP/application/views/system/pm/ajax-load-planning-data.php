...
<table id="cTable" class="table " style="width: 100%">
    ...
    <?php if ($header) {
        ...
        foreach ($header as $value) {
            ...
            if($value['type'] == 1)
            {
                //$action= '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i>';
                $action= '<a class="text-yellow" onclick="open_recovery_plan('.$value['headerID'].','.$value['timelineID'].');"><span title="Recovery Plan" rel="tooltip" class="fa fa-exclamation-triangle"></span></a>';
                $category[$value["phaseDescription"]. ' (Planned Completion Date : '.$value["plannedcompletionDate"].') | '.$action.''][] = $value;
            }else{
                $category[$value["phaseDescription"]. ' (Planned Completion Date : '.$value["plannedcompletionDate"].' )'][] = $value;
            }

            }
             foreach ($category as $key => $phase) {

                 echo '<tr><td colspan="9"><div><strong>' . $key .'</strong></div></td></tr>';
                 foreach ($phase as $key2=>$val) { ?>
                     <?php if ($val['masterID'] == 0) {?>
                     <tr class="header">
                         <?php if ($master['confirmedYN'] != 1) { ?>
                             <td colspan=""><a href="#"><i class="fa fa-minus-square"
                                 ...
                                 <a href="#" data-type="text"
                                    ...
                                    data-pk="<?php echo $val['projectPlannningID'] ?>"
                                    data-name="description"
                                    data-title="Task" class="xeditable "
                                    data-value="<?php echo $val['description'] ?>">
                         ...
                         <?php } ?>
                         <td style="text-align: center"><?php echo $val['startDate'] ?></td>
                         ...
                         <?php if ($master['confirmedYN'] != 1) { ?>
                             <td style="text-align: center"><a href="#" data-type="select"
                                                               ...
                                                               data-value="<?php echo $val['bgColor'] ?>">
                                     <?php //echo $subvalue['bgColor'] ?>
                                 </a></td>
                         <?php } else { ?>
                             <td style="text-align: center"><?php echo $val['bgColor'] ?></td>
                         <?php } ?>
                         ...
                         <?php if ($master['confirmedYN'] != 1) { ?>
                             <td style="text-align: center">
                                 <a href="#" data-type="number"
                                    ...
                                    data-value="<?php echo $val['percentage']  ?>">
                                     <?php echo $val['percentage']  ?>
                                 </a>
                             </td>
                         <?php } else { ?>
                         ...
                         <td style="text-align: center">
                             <?php if ($master['confirmedYN'] != 1) { ?>
                                 <select name="sortOrder" id="inlinesortOrder"
                                         onchange="changeSortOrder('m',this.value,<?php echo $val['projectPlannningID'] ?>)">
                                     <?php if (!empty($sortOrder)) {
                                         foreach ($sortOrder as $s) {
                                             $select = '';
                                             if ($val['sortOrder'] == $s['sortOrder']) {
                                                 $select = 'selected';
                                             }
                                             ?>
                         ...
                         <?php if ($master['confirmedYN'] != 1) { ?>
                             <td class="pull-right" colspan=""><a
                                         onclick='attachment_modal(<?php echo $val['projectPlannningID'] ?>,"<?php echo $val['description'] ?>","PLC", 0);'><i
                                             class="glyphicon glyphicon-paperclip" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;<a
                                         onclick="addplanningSub(<?php echo $val['projectPlannningID'] ?>,'<?php echo $val['description'] ?>',<?php echo $val['timelineID']?>)"><i
                                             class="fa fa-plus" aria-hidden="true"></i></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a
                                         onclick="deleteplanningTask(<?php echo $val['projectPlannningID'] ?>)">
                                     <span style="color:#ff3f3a" class="glyphicon glyphicon-trash "></span></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a
                                         onclick='relationship_pm(<?php echo $val['projectPlannningID'] ?>,"<?php echo $val['description'] ?>",<?php echo $val['headerID'] ?>)'>
                     ...
                     <?php }?>
<?php foreach ($header as $subvalue) {
    if ($val['projectPlannningID'] == $subvalue['masterID']) {?>
    <tr class="subheader">
        ...
        <td style="text-align: center"><?php echo $subvalue['ename2'] ?></td>
        <td style="text-align: center">
            <?php if ($master['confirmedYN'] != 1) { ?>
                <select name="sortOrder" id="inlinesortOrder"
        ...
        <?php if ($master['confirmedYN'] != 1) { ?>
        <td>
            <a onclick='relationship_pm(<?php echo $val['projectPlannningID'] ?>,"<?php echo $val['description'] ?>",<?php echo $val['headerID'] ?>)'>
                <span class="glyphicon glyphicon-random"></span></a> |
