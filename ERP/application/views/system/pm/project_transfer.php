...
<div class="row">
    <div class="col-md-7 pull-right">
        <button type="button" class="btn btn-primary btn-sm pull-right" onclick="openprojecttransfer_modal()" ><i class="fa fa-plus-square"></i>&nbsp; <?php echo $this->lang->line('common_add');?><!--Add--> </button>
    </div>
</div><hr>
<div class="table-responsive">
    <table id="load_projecttransfer" class="<?php echo table_class(); ?>">
        <thead>
        <tr>
            <th style="min-width: 5%">#</th>
            <th style="width: auto">Transfered Date</th>
            <th style="width: auto">Transfered By</th>
            <th style="width: auto">Narration</th>
...
<div class="modal fade bs-example-modal-sm" role="dialog" aria-labelledby="myLargeModalLabel" id="new_projecttransfer">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="usergroup-title">Project Transfer</h4>
            </div>
            <?php echo form_open('', 'role="form" id="project_transfer_form"'); ?>
            <div class="modal-body">
                ...
                <div class="modal-footer">
                    <button type="button" onclick="save_project_transfer_master()" class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-floppy-disk"
...
<script type="text/javascript">
    ...
    function load_religions(selectedRowID=null){
        var Otable = $('#load_projecttransfer').DataTable({
            "language": {
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Boq/fetch_projecttransfer_master'); ?>",
            "aaSorting": [[0, 'desc']],
    ...
    function openprojecttransfer_modal(){
        $('#narration').val('');
        $('#new_projecttransfer').modal({backdrop: "static"});
    }

    function save_project_transfer_master() {
        var postData = $('#project_transfer_form').serializeArray();
        $.ajax({
            type: 'post',
            url: '<?php echo site_url('Boq/save_project_transfer_master'); ?>',
            data: postData,
            dataType: 'json',
            beforeSend: function () {
                startLoad();
            ...
            success: function (data) {
                ...
                if (data[0] == 's') {
                    $('#new_projecttransfer').modal('hide');
                }
    ...
    function load_details(id, template){
        let page = 'system/pm/project_transfer_detail';
        fetchPage(page, id,'Project Transfer', ' ');
    }
