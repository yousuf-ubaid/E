    ...
    if (!empty($attachment)) {
        foreach ($attachment as $row) {
         /*    $file = base_url() . 'attachments/CRM/SubTask/'.$row['myFileName'];
            ...
            $link = $this->s3->createPresignedRequest($row['myFileName'], '1 hour');
            ?>
            <div class="row">
                <div class="col-sm-12">
                    <div class="past-info">
    ...
    <script>
        ...
        function document_uplode() {
            ...
            $.ajax({
                type: 'POST',
                dataType: 'JSON',
                url: "<?php echo site_url('Task_management/attachement_upload_subtask'); ?>",
                data: formData,
                contentType: false,
                cache: false,
                processData: false,
            ...
            swal({
                ...
                function () {
                    $.ajax({
                        ...
                        url: "<?php echo site_url('Task_management/delete_subtask_attachments'); ?>",
                        beforeSend: function () {
                            startLoad();
                        },
                        success: function (data) {
