    ...
    <div class="table-responsive mailbox-messages">
        <table class="table table-hover table-striped">
            ...
            foreach ($detail as $row) {
                $timespentsubtask = $this->db->query("select SUM(timeSpent) as subtasktimespent from srp_erp_task_subtasksessions where  taskID = '{$row['taskID']}' And subTaskID = '{$row['subTaskID']}'")->row_array();
                ?>
                <tr>
                    ...
                    <td class="mailbox-name"><a href="#"><?php
                            $assignees = $this->db->query("select empID, employees.Ename2 as Assigneename from srp_erp_task_assignees crmassignees LEFT join srp_employeesdetails employees on crmassignees.empID = employees.EIdNo where
                                                             crmassignees.documentID = 10  And crmassignees.MasterAutoID = '{$row['subTaskID']}'")->result_array();
                            if (!empty($assignees)) {
                    ...
                    <td class="mailbox-name">
                        ...
                        $totalnoofcahts = $this->db->query("SELECT chat.* FROM srp_erp_task_chat chat LEFT JOIN srp_employeesdetails empdetails ON empdetails.EIdNo = chat.empID WHERE taskID = '{$row['taskID']}' AND subTaskID =  '{$row['subTaskID']}' ORDER BY chatID asc")->result_array();
                        ?>
                            <a href="#"
                    ...
                    <td class="mailbox-name"><a href="#">
                      <span class="pull-right">
                          <?php
                          $subtasksession = $this->db->query("select *,DATE_FORMAT(createdDateTime, '%h:%i:%s') as timesessionstarted from srp_erp_task_subtasksessions where taskID  = '{$row['taskID']}' And subTaskID = '{$row['subTaskID']}' ORDER BY sessionID DESC LIMIT 1")->row_array();

                          if($subtasksession && $subtasksession['status']== 0){ ?>
                          <a href="#"
                          ...
                          <?php } else if ($subtasksession && $subtasksession['status']== 1) {?>
                              <a href="#"
                          ...
                          <?php } else  {?>
                            ...
                            <a href="#"
                               onclick="sub_task_attachment_model('<?php echo $row['subTaskID'] ?>','<?php echo $row['taskID']?>')"><span
                                    ...
                                    class="glyphicon glyphicon-paperclip attachment"></span></a> |&nbsp; 
                                    <?php
                            $subtaskedit = $this->db->query("select status from srp_erp_task_subtasks where  taskID  = '{$row['taskID']}' And subTaskID = '{$row['subTaskID']}' ORDER BY subTaskID DESC LIMIT 1")->row_array();

                            if($subtaskedit['status']== 2){ ?>
                                  <a href="#" onclick="" >
                                        <span title="Task completed" rel="tooltip" class="glyphicon glyphicon-pencil disable"></span>
                                    </a>
                          ...
                          <?php if ($subtasksession && $subtasksession['status'] == 1 && $subtasksession['status'] != 2 && $subtasksession['status'] != 0){ ?>
                             ...
                             <?php
                                      ...
                                      $minutes = ((int)$totaltimespentminutes % 60);
                                     ?></span>
                         </div>
