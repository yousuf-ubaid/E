...
$status_arr_filter = all_task_status_filter(false);
...
<div class="row">
    ...
    <div class="col-md-3 text-right">
        <button type="button" class="btn btn-primary-new size-sm pull-right"
                onclick="fetchPage('system/task_managment/create_new_task_employee',null,'<?php echo $this->lang->line('crm_add_new_task');?>','CRM','CRM');"><i
        ...
        <div class="box-body no-padding">
            <div class="row" style="margin-top: 2%;">
                <div class="col-sm-8" style="margin-left: 34%;">
                    <div class="btn-group" role="group" aria-label="Basic">
                        ...
                        <button type="button" class="btn-sm btn-secondary" style="background-color:#b8959d ;" onclick="setButtonValue(1)">
                            Created By Me
                        </button>
                        <!-- Button for "Assigned To Me" -->
                        <button type="button" class="btn-sm btn-secondary" style="background-color:#b8959d ;" onclick="setButtonValue(2)">
                            Assigned To Me
                        </button>
                        <!-- Button for "All Tasks" -->
                        <button type="button" class="btn-sm btn-secondary" style="background-color:#b8959d ;" onclick="setButtonValue(3)">
                            All Tasks
                        </button>
            ...
            <div class="row" style="margin-top: 2%;">
                ...
                <div class="col-sm-7">
                    ...
                    <!-- <div class="col-sm-2">
                        <*/?php echo form_dropdown('assigneesID',array('' =>  "Task Type", '1' => "Created By Me", '2' => "Assigne To Me", '3' => "All Task") , '', 'class="form-control" id="filter_tasktypeID"  onchange="startMasterSearch()"'); ?>
                    </div> -->
                    <div class="col-sm-3">
                        <?php echo form_dropdown('Responsible Person', $employees, '', 'class="form-control select2" id="responsibleperson"  onchange="startMasterSearch()"'); ?>
                    </div>
   ...
   var per_page = 10;
    $(document).ready(function () {
        ...
        $('.headerclose').click(function () {
            fetchPage('system/task_management/task_managemnt_employee', '', 'Tasks');
        });
    ...
    function getTaskManagement_tableView(pageID,uriSegment = 0) {
        ...
        var responsible = $('#responsibleperson').val();
        var priority = $('#filter_Priority').val();
        var assignees = $('#filter_assigneesID').val();
        var createdby = $('#createdby').val();
        var tasktype = $('#tasktype').val();
        // var task = $('#buttonValue').val();
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'json',
            data: {'searchTask': searchTask, status: status, priority: priority, assignees: assignees, category:category,'pageID':pageID,createdby:createdby,tasktype:tasktype,responsible:responsible},
            url: "<?php echo site_url('Task_management/load_taskManagement_employee_view'); ?>/" + uriSegment,
            beforeSend: function () {
                startLoad();
            },
    ...
    function edit_task(taskid, createdUserIDtask, assignuser, permission) {
    if ((createdUserIDtask == '<?php echo $cuurentuser ?>') || ('<?php echo $admin['isSuperAdmin']?>' == 1) || ('<?php echo $isgroupadmin['adminYN']?>' == 1) || (assignuser == 1) || (permission == 4) ) {
        fetchPage('system/task_managment/create_new_task_employee', taskid, 'Edit Task', 'CRM', 'CRM');
    } else {
