...
$title = $this->lang->line('assetmanagement_request_approval');
...
<div class="table-responsive">
    <table id="asset_request_table" class="<?php echo table_class() ?>">
        <thead>
        <tr>
            <th style="min-width: 5%">#</th>
            <th style="min-width: 10%"><?php echo $this->lang->line('assetmanagement_asset_request_arn');?><!--ARN Number--></th>
            <th style="min-width: 10%"><?php echo $this->lang->line('assetmanagement_asset_request_approval_exp_delivery_date');?><!--Exp Delivery Date--></th>
            <th style="min-width: 30%"><?php echo $this->lang->line('assetmanagement_asset_request_narration');?><!--Narration--></th>
...
<div class="modal fade" id="asset_request_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel"><?php echo $this->lang->line('assetmanagement_request_approval');?><!--Asset Request Approval--></h4>
            </div>
            <form class="form-horizontal" id="arn_approval_form">
                <div class="modal-body">
                    <div class="col-sm-1">
                        <!-- Nav tabs -->
                        <ul class="zx-nav zx-nav-tabs zx-tabs-left zx-vertical-text">
                            <li id="ar_attachement_approval_Tabview_v" class="active"><a href="#Tab-home-v" data-toggle="tab" onclick="tabView()"><?php echo $this->lang->line('common_view');?><!--View--></a></li>
                            <li id="ar_attachement_approval_Tabview_a"><a href="#Tab-profile-v" data-toggle="tab" onclick="tabAttachement()"><?php echo $this->lang->line('common_attachment');?><!--Attachment--></a>
                            </li>
                        </ul>
                    </div>
                    <div class="col-sm-11">
                        <div class="zx-tab-content">
                            ...
                            <div class="tab-pane hide" id="Tab-profile-v">
                                <div class="table-responsive">
                                    ...
                                    &nbsp <strong><?php echo $this->lang->line('assetmanagement_approval_asset_request_attachments');?><!--Asset Request Attachments--></strong>
                                    <br><br>
                                    <table class="table table-striped table-condensed table-hover">
                                        ...
                                        <tbody id="ar_attachment_body" class="no-padding">
                                        <tr class="danger">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/asset_management/asset_request_approval', 'Test', 'Asset Request Approval')
        });

        asset_request_table();
        $('#arn_approval_form').bootstrapValidator({
            live: 'enabled',
            ...
            fields: {
                ...
                id: {validators: {notEmpty: {message: '<?php echo $this->lang->line('procurement_approval_purchase_request_id_is_required');?>.'}}},/*Purchase Request ID is required*/
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: data,
                url: "<?php echo site_url('AssetManagement/save_asset_request_approval'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    ...
                    stopLoad();
                    if(data == true){
                        $("#request_request_modal").modal('hide');
                        Otable.draw();
    ...
    function asset_request_table() {
        Otable = $('#asset_request_table').DataTable({
            "language": {
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('AssetManagement/fetch_asset_request_approval'); ?>",
            "aaSorting": [[1, 'desc']],
    ...
    function fetch_approval(reqid, documentApprovedreq, Level) {
        if (reqid) {
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'html',
                data: {'req': reqid, 'html': true,'approval':1},
                url: "<?php echo site_url('AssetManagement/load_asset_request_conformation'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    ...
                    $('#Level').val(Level);
                    $("#asset_request_modal").modal({backdrop: "static"});
                    $('#conform_body').html(data);
                    $('#comments').val('');
                    assetRequest_attachment_View_modal('ARN',id);
                    stopLoad();
                    refreshNotifications(true);
                }, error: function () {
                    stopLoad();
    ...
    function assetRequest_attachment_View_modal(documentID, documentSystemCode) {
        $("#Tab-profile-v").removeClass("active");
        $("#Tab-home-v").addClass("active");
        $("#ar_attachement_approval_Tabview_a").removeClass("active");
        $("#ar_attachement_approval_Tabview_v").addClass("active");
        if (documentSystemCode) {
            $.ajax({
                ...
                success: function (data) {
                    $('#ar_attachment_body').empty();
                    $('#ar_attachment_body').append('' +data+ '');
