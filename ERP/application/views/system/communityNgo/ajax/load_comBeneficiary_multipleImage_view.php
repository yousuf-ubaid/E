                    ...
                    if (!empty($docDet)) {
                        foreach ($docDet as $doc) {
                            ...
                            if ($doc['beneficiaryImage'] != '') {
                                ...
                                $linkStart = '<i class="fa fa-times-circle pull-right" aria-hidden="true" style="color: red;" onclick="delete_comBeneficiary_doc(' . $doc['beneficiaryImageID'] . ', \'' . $doc['beneficiaryImage'] . '\')"></i>
                            ...
                            ' . $linkStart . '
                                <img class="" src="' . base_url() . 'uploads/ngo/beneficiaryImage/' . $doc['beneficiaryImage'] . '" style="width:100px; height:85px; ">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#empDoc_form').bootstrapValidator({
            ...
            $.ajax({
                ...
                dataType: 'JSON',
                data: formData,
                url: '<?php echo site_url('CommunityNgo/upload_comBeneficiary_multipleImage'); ?>',
                contentType: false,
                cache: false,
                processData: false,
                beforeSend: function () {
                ...
                success: function (data) {
                    ...
                    if (data[0] == 's') {
                        setTimeout(function () {
                            fetch_comBeneficiary_imageView();
                        }, 400);
                    }else {
    ...
    function delete_comBeneficiary_doc(beneficiaryImageID, description) {
        swal({
                title: "Are you sure?",
            ...
            function () {
                $.ajax({
                    async: true,
                    url: "<?php echo site_url('CommunityNgo/delete_comBeneficiary_multipleImage'); ?>",
                    type: 'post',
                    dataType: 'json',
    ...
    function update_pp_imageStatus(beneficiaryImageID, status) {
        ...
        $.ajax({
            ...
            url: "<?php echo site_url('CommunityNgo/update_comBeneficiary_multipleImage'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
