<?php if($isFromPrint == 'Y' && $isForReverse != 'Y') { ?>
    <?php
...
$totalTDCount = count($header_det) + 1;
...
foreach($header_det as $key=>$det){
    ...
    /**** adda new column for save the total amount except header type  ****/
    if( $det['detailType'] == 'H'){
...
<h5 class="selected-employee-det well well-sm" style="display: none; margin-bottom: -7px;"></h5>
    ...
    <div class="fixHeader_Div" style="margin-top: 1%">
        ...
        <tr class="theadtr" style="font-size:12px">
            <th style="width: auto" colspan="<?php echo $totalTDCount ?>"> <strong><?php echo $this->lang->line('common_currency');?><!--Currency--> : <?php echo $group['currency']; ?> </strong> </th>
        </tr>
                    ...
                    foreach ($header_det as $key => $row) {
                        if ($row['detailType'] == 'H') {
                            ...
                            if( $row['isCalculate'] == 1 ){
                                $header_det[$key]['sum'] += round($thisEmpDet[ $row['fieldName'] ], $dPlace);
                                $empTr .= '<td align="right">'. format_number($thisEmpDet[ $row['fieldName'] ], $dPlace) . '</td>';
                            }else{
                                switch($row['fieldName']){
                                    case 'Ename2':
                                    case 'bankDetails':
                                        $data = '<div style="min-width:150px">'.$thisEmpDet[ $row['fieldName'] ].'</div>';
                                        break;
                                    case 'comments':
                                        if($isFromPrint == 'Y' || $masterData['confirmedYN']==1){
                                        ...
                                        break;
                                    default :
                                        $data = isset($thisEmpDet[ $row['fieldName'] ]) ? $thisEmpDet[ $row['fieldName'] ] : '';
                                }
                            ...
                            $header_det[$key]['sum'] += $det['netSalary'];
                        }
                        else if ($row['detailType'] == 'T') {
        ...
        <?php echo $isClosingTag_tFootOpen; ?>
            <tr>
                <td>&nbsp;</td>
                <?php
                array_walk($header_det, function(&$value, $i) use ($isFromPrint,$dPlace){
