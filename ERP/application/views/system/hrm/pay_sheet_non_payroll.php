...
$title = $this->lang->line('hrms_payroll_monthly_allowance');
echo head_page($title, false);

$paySheetTemplate_arr = paySheetTemplate_drop('Y');
$postData = $this->input->post('data_arr');
$default_template = getDefault_template('Y');
...
if ($postData != null) {
    $payrollCalender_arr = payrollCalender(date('Y'), 1, 'Y');

} else {
    $payrollCalender_arr = payrollCalender(date('Y'), 0, 'Y');
}
...
?>
    <style type="text/css">
        ...
        #date-container { padding-right: 0px }
        ...
        <?php echo form_open('', ' id="paySheetForm" role="form" autocomplete="off"'); ?>
            <div class="row" style="/*margin: 2%; margin-bottom: 1%*/ margin-top: 1%;">
                <div class="col-sm-12" style="padding:0px">
                    <div class="col-sm-2" id="date-container">
                        <div class="form-group">
                            <label for="payMonth" class=""><?php echo $this->lang->line('hrms_payroll_paroll_month');?><!--Payroll&nbsp;Month--></label>
                            <div class="input-group" style="">
                            <span class="input-group-addon" style="padding: 0px">
                                <input type="number" name="payYear" class="input-group-addon  form-control payDate formInput"
                                       id="payYear" value="<?php echo date('Y'); ?>" onchange="getVisibleDate()"/> <!--onchange="getNotPayrollProcessedMonths(this)"-->
                            </span>
                                <?php echo form_dropdown('payMonth', $payrollCalender_arr, '', 'class="form-control payDate formInput" id="payMonth" required onchange="getVisibleDate()"'); ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            <div class="input-group filterDate" style=";">
                                ...
                                <input type="text" name="processingDate" data-inputmask="'alias': '<?php echo $date_format_policy ?>'" value="<?php echo $current_date; ?>"
                                       id="processingDate" class="form-control formInput" required>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group filterDate" style=";">
                                ...
                                <input type="text" name="visibleDate" data-inputmask="'alias': '<?php echo $date_format_policy ?>'"
                                       value="<?php echo $current_date; ?>" id="visibleDate" class="form-control formInput" required>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row" id="btnDiv" style="">
                <div class="col-md-12">
                    <div class="row">
                        <div class="col-sm-2">
                            <div class="form-group" id="segment-container" style="/*display: none;*/">
                        ...
                        <div class="col-sm-2">
                            ...
                            <div class="input-group" id="hide-zero-column">
                                ...
                                <input type="text" class="form-control" disabled="" value="Hide zero column">
                            </div>
                        </div>
                        <div class="col-sm-8">
...
<div class="modal fade" id="expense_model" role="dialog" data-keyboard="false" data-backdrop="static"  style="z-index: 999999"  >
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button class="btn btn-primary btn-sm" type="button" style="font-size:12px;" onclick="load_employeeToPayroll('withNoPay')">
                    <?php echo $this->lang->line('common_load');?><!--Load-->
                </button>
...
<script type="text/javascript">
    ...
    if (payrollDet != null) {
        ...
        $('.hideBtn, #loadBtn, #hide-zero-column').hide();
        segmentDrop.attr('disable', true);
    ...
    $('.headerclose').click(function () {
        fetchPage('system/hrm/non_payroll_master', autoID, 'HRMS');
    });
    ...
    function print_btn() {
        ...
        if (payrollID == '') {
            myAlert('e', '<?php echo $this->lang->line('hrms_payroll_please_reload_the_page_and_try_again');?>.');/*Please reload the page and try again*/
        }
        ...
        if (templateId != '' && payrollID != '') {
            ...
            form.target='_blank';
            form.method='post';
            form.action='<?php echo site_url('Template_paysheet/paySheetPrint_segmentWise/'); ?>'+payrollID + "/" + templateId + "/Y/" + payrollHeaderDet;
            form.submit();
        }
    ...
    function getPayrollDet(payrollDet) {
        ...
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {'payrollID': payrollID, 'isNonPayroll':'Y'},
            ...
            success: function (data) {
                ...
                if (data[0] == 'e') {
                    ...
                    payrollAccountReview.attr('href', printUrl+'/' + payrollID + '/Y/' + data['documentCode'] + '-' + data['payrollYear']);
                    ...
                    if( data['approvedYN'] == 1 ){ //if (data['confirmedYN'] == 1) {
    ...
    function load_template() {
        ...
        $.ajax({
            ...
            type: 'post',
            dataType: 'html',
            data: {'hidden_payrollID': hidden_payrollID, 'templateId': templateId, 'isNonPayroll' : 'Y', 'segmentID':segmentID, 'hideZeroColumn':hideZeroColumn},
    ...
    function create_payroll(withNoPay) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: postData,
            url: "<?php echo site_url('Template_paysheet/loadPaySheetData'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
    ...
    function confirm_payroll(isConfirm) {
        ...
        if( isConfirm == 1){
            swal({
                    ...
                    text: "<?php echo $this->lang->line('hrms_payroll_you_want_to_confirm_this_payroll');?>",
                    type: "warning",
        ...
        swal({
            ...
            function () {
                ...
                $.ajax({
                    ...
                    type: 'post',
                    dataType: 'json',
                    data: {'visibleDate':visibleDate, 'payrollID': payrollID, 'isNonPayroll': 'Y'},
        ...
        $.ajax({
            ...
            beforeSend: function () {
                ...
                if (data[0] == 's') {
                    ...
                    fetchPage('system/hrm/pay_sheet_non_payroll', 0, 'Load', '', payrollDetail_arr);
                }
    ...
    function load_btn(withNoPay=null) {
        var templateId = $('#templateId').val();
        var payrollID = $('#hidden_payrollID').val();
        if (templateId != '') {
            var payrollDet = null;
            ...
            if (payrollID != '') {
                ...
                create_payroll(withNoPay);
            }
        }
        else {
            myAlert('e', '<?php echo $this->lang->line('hrms_payroll_please_select_a_pay_sheet_Template');?>');/*Please select a Pay sheet Template*/
        }
    }
    ...
    function bankTransfer() {
        ...
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {'payrollID': payID, 'isNonPayroll':'Y'},
    ...
    function getNotPayrollProcessedMonths(obj) {
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {'year': obj.value, 'payrollType' : 'Y'},
        ...
        swal({
            ...
            function () {
                $.ajax({
                    ...
                    type: 'post',
                    dataType: 'json',
                    data: {'payrollID' : payrollID, 'empID': empID, 'isNonPayroll':'Y'},
    ...
    function openEmployeeModal(){
        ...
        if( load_payYear == '' || load_payMonth == ''){
            msg += '<?php echo $this->lang->line('hrms_payroll_paroll_month');?> <br/>';<!--Payroll Month-->
        }
        if( load_processingDate == ''){
            msg += '<?php echo $this->lang->line('hrms_payroll_processing_date');?>  <br/>';<!--Processing Date-->
        }
        if( load_payNarration == ''){
            msg += '<?php echo $this->lang->line('hrms_payroll_narration');?>  <br/>';<!--Narration-->
        }
        if( load_templateId == ''){
            msg += '<?php echo $this->lang->line('hrms_payroll_paysheet_template');?>  <br/>';<!--Paysheet Template-->
        }
        ...
        if(msg == '' ){
            ...
            myAlert('e', '<?php echo $this->lang->line('hrms_payroll_fill_the_following_details_and_try_again');?> <br/>'+msg); <!--Fill the following details and try again-->
        }
    ...
    function load_employeeForModal(){
        $('#emp_modalTB').DataTable({
            ...
            "bDestroy": true,
            "StateSave": true,
            "sAjaxSource": "<?php echo site_url('Template_paysheet/getEmployeesDataTable'); ?>",
            "aaSorting": [[1, 'asc']],
            ...
            "fnServerData": function (sSource, aoData, fnCallback) {
                ...
                aoData.push({'name':'payYear', 'value':$('#payYear').val()});
                aoData.push({'name':'payMonth', 'value':$('#payMonth').val()});
                aoData.push({'name':'isNonPayroll', 'value':'Y'});
    ...
    function load_employeeToPayroll(withNoPay=null){
        if( empTempory_arr.length > 0 ){
            load_btn(withNoPay);
        }
        else{
            myAlert('e', '<?php echo $this->lang->line('hrms_payroll_please_select_at_least_one_employee');?>');/*Please select at least one employee*/
        }
    }
