...
$selected_arr = array();
$shift_arr = fetch_shifts(true);

$is_pulled_op = getPolicyValues('OPA', 'All'); //Disable attendance Clock in / Clock out Time
...
if (!empty($tempAttData)) {
    foreach ($tempAttData as $key => $row) {
        ...
        $shiftDropDown = form_dropdown('shiftID', $shift_arr, $row['shiftID'], ' id="shiftID" onchange="change_shift_date_time(this,\''.$row['attendanceDate'].'\','.$row['empID'].')" ');
        ...
        if(isset($manual_attendance)){
            $clockinFloor = form_dropdown('floorID', $floors_arr, $row['floorID'], ' class="col-md-12 form-control select2 floorID" id="floorID" onchange="change_location(this)" ');
            $clockoutFloor = form_dropdown('clockoutFloorID', $floors_arr, $row['clockoutFloorID'], ' class="col-md-12 form-control select2 clockoutFloorID" id="clockoutFloorID" onchange="change_location(this)" ');;
            
            $clockinFloor .= '<a class="btn btn-sm" name="floorID" style="margin-right: 100px" onclick="apply_for_all(this)"><i class="fa fa-arrow-down"></i></a>&nbsp';
            $clockoutFloor .= '<a class="btn btn-sm" name="clockoutFloorID" onclick="apply_for_all(this)"><i class="fa fa-arrow-down"></i></a>';

        }else{
            $clockinFloor = $row['clockinFloorDescription'];
            $clockoutFloor = $row['clockoutFloorDescription'];
        }
        ...
        <tr data-code="<?php echo $datacode; ?>" style="white-space: nowrap;<?php echo $bg ?>"
            ...
            class="<?php echo $class; ?> <?php echo $tr_data2; ?> linerow">
            ...
            <td style="text-align: center">
                ...
                if($is_pulled_op == 1){
                    echo '&nbsp &nbsp  <a class="" onclick="load_job_occurence('.$row['empID'],',\''.$row['attendanceDate'].'\')"><i class="fa fa-gear"></i></a>';
                }
            ...
            <td><?php echo $shiftDropDown ?></td>
            <td><?php echo $job_select ?></td>

            <td style="text-align: center"><?= $clockinFloor; ?></td>
            <td style="text-align: center"><?= $clockoutFloor; ?></td>
            <td style="text-align: center">
                <span class="onDutySpan"><?php echo $onDuty ?></span>
                <input type="hidden" class="onDuty" name="onDuty[]" value="<?php echo $row['onDuty']; ?>"/>
            </td>
            <td style="text-align: center">
            ...
            <td style="text-align: center">
                <span class="offDutySpan"><?php echo $offDuty ?> </span>
                <input type="hidden" class="offDuty" name="offDuty[]" value="<?php echo $row['offDuty']; ?>"/>
            </td>
            <td>
                <input type="date" class="trInputs checkInDate" onchange="update_date(this)" id="checkInDate-<?php echo $i; ?>"  <?php echo $disabled2 ?> name="checkInDate" value="<?php echo ($row['checkInDate']) ? $row['checkInDate'] : $row['attendanceDate']; ?>"/>
            </td>
            <td>
                <div class="input-group  bootstrap-timepicker timepicker timeDiv">
                    ...
                    <input <?php echo $disabled2 ?> type="text" name="checkIn" class="form-control checkIn timeTxt trInputs "
            ...
            <td class="hide" align="right"><span class="shiftHours"><?php echo $shiftHour; ?></span></td>
            ...
            <td align="center"><?php makeTimeTextBox('specialNDaysOT', $specialNDaysOTsarr); ?></td>
...
?>
    ...
    <script type="text/javascript">
        ...
        function load_job_occurence(empID, date){
          
            $('#date_job_model').modal('show');

            $.ajax({
                url: "<?php echo site_url('Employee/load_relevant_job_details'); ?>",
                data: {'empID': empID,'date': date},
                type: 'post',
                beforeSend: function () {
                    $('#date_job_content').html('<i class="fa fa-spinner fa-spin"></i> loading..</div>');
                },
                success: function(data){
                    $('#date_job_content').html(data);
                }
            });
        ...
        function change_location(ev){
            ...
            trID = $(ev).closest('tr').attr('data-id');
            masterID = $(ev).closest('tr').attr('data-masterid');
            name = $(ev).attr('name');
            value = ev.value;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data['error'] == 0) {
                        ...
                        $("tr[data-id='" + trID + "']").find(".h_specialNDaysOT").val(data['data']['h_specialNDaysOT']);
                        ...
                        $("tr[data-id='" + trID + "']").find(".m_specialNDaysOT").val(data['data']['m_specialNDaysOT']);
                        ...
                        $("tr[data-id='" + trID + "']").find(".offDuty").html(data['data']['offDuty']);
                        $("tr[data-id='" + trID + "']").find(".offDutySpan").html(data['data']['offDuty']);
                        $("tr[data-id='" + trID + "']").find(".onDuty").html(data['data']['onDuty']);
                        $("tr[data-id='" + trID + "']").find(".onDutySpan").html(data['data']['onDuty']);
                    }
        ...
        function apply_for_all(ev){
            
            var locationname = $(ev).attr('name');

            if(locationname == 'floorID'){
                var floorID = $(ev).closest('tr').find('.floorID').val();

                $('.linerow').each(function(i,row) {
                    $(row).closest('tr').find('.floorID').val(floorID).change();
                });
          
            }else{
                //clockoutFloorID
                var clockoutFloorID = $(ev).closest('tr').find('.clockoutFloorID').val();

                $('.linerow').each(function(i,row) {
                    $(row).closest('tr').find('.clockoutFloorID').val(clockoutFloorID).change();
                });
        ...
        function change_shift_date_time(ev,attendanceDate,empID){
            ...
            $.ajax({
                ...
                type: 'post',
                dataType: 'json',
                data: {shiftID: shiftID,attendanceDate:attendanceDate,empID:empID},
                url: "<?php echo site_url('Employee/update_shift_details_emp'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    stopLoad();
                    $(ev).closest('tr').find('.checkInDate').change();
