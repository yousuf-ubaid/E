    ...
    <table class="<?php echo table_class(); ?>" id="staff-attendance-table">
        <thead>
            <tr>
                ...
                <th style="width: auto"><?php echo $this->lang->line('hrms_attendance_floor');?><!--Name--></th>
                <th style="max-width: 62px;">
                    Attendance Date
                    <!-- <input type="checkbox" id="selectAllChk" style="margin:2px" onchange="selectAll(this)" <?php echo $disabled; ?>/> -->
                </th>
                <th style="width: 80px; z-index: 10"> <?php echo $this->lang->line('hrms_attendance_clock_in');?><!--clock in--></th>
                <th style="width: 80px; z-index: 10"> <?php echo $this->lang->line('hrms_attendance_clock_out');?><!--clock out--></th>
                <th style="width: 100px !important; z-index: 10"> <?php echo $this->lang->line('hrms_attendance_present_type');?><!--Present Type--></th>
                <th style="width: 80px; z-index: 10"> <?php echo $this->lang->line('common_status');?><!--status--></th>
        ...
        foreach($emp_arr as $key=>$row){
            ...
            $clockin = ( !empty($row->checkIn) )? $row->checkIn : $clockin;
            $clockout = ( !empty($row->checkOut) )? $row->checkOut : $clockout;
            $AttPresentTypeID = ( !empty($row->presentTypeID) )? $row->presentTypeID : 0;
            $status = ( !empty($row->approvedYN) )? $row->approvedYN : 1;
            $status = ( !empty($row->approvedYN) )? $row->approvedYN : null;
        // $remarks = ( !empty($row->AttPresentRemarks) )? $row->AttPresentRemarks : '';
        //  $isChecked = ( $row->isAttended == 1 )? 'checked' : '';
        // var_dump( $status);
            $statusLabel = '';
            if ($status == 1) {
                $statusLabel = '<span class="label label-success">&nbsp;</span>';
            } elseif ($status == null) {
                $statusLabel = '<span class="label" style="background-color: #F08000">&nbsp;</span>';
            }
            ...
            echo '<tr data-value="'.$tr_data.'">
                    ...
                    <td>
                        <div class="input-group  bootstrap-timepicker timepicker">
                            ...
                            <input type="text" name="att-time[]" class="form-control timeTxt trInputs" value="'.$clockin.'" style="width:80px" '.$disabled.' />
                        </div>
                    </td>
                    <td>
                        <div class="input-group  bootstrap-timepicker timepicker">
                            ...
                            <input type="text" name="att-time[]" class="form-control timeTxt trInputs" value="'.$clockout.'" style="width:80px" '.$disabled.' />
                        </div>
                    </td>
