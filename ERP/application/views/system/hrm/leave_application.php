...
<div id="filter-panel" class="collapse filter-panel">
    <div class="row">
        ...
        <div class="form-group col-sm-2">
            ...
            <select name="empFilter[]" class="form-control" id="empFilter" multiple="multiple">
                <?php
                foreach ($employeeDrop as $empD){
                    echo '<option value="'.$empD['EIdNo'].'">'.$empD['ECode'].' - '.$empD['employee'].'</option>';
                }
                ?>
            </select>
        </div>
        ...
        <div class="form-group col-sm-3">
            ...
            <div style="width: 60%;">
                ...
                <button type="button" class="btn btn-primary" onclick="search_leave_app()" style="margin-top: -9%; margin-left: 250px; position: absolute;">
...
<div class="table-responsive">
    <table id="leaveDetailTB" class="<?php echo table_class(); ?>">
        ...
        <tr>
            ...
            <th style="min-width: 12%">Application&nbsp;Type</th>
                ...
                <div class="panel-body">
                    ...
                    <div class="row" style="margin-bottom: 3px">
                        <div class="col-xs-4 col-sm-2">
                            <label>
                              <?php echo $this->lang->line('hrms_leave_management_employee_name'); ?><!--Employee Name--> </label>
                        </div>
                    ...
                    <div class="row" style="margin-bottom: 3px">
                        ...
                        <div class="col-xs-7 col-sm-4" id="leaveTypeDropDown">
                            <select id="leaveTypeID" name="leaveTypeID" class="form-control leaveType frm_input " required>
                                <option></option>
                            </select>
                        </div>
                    ...
                    <div class="row" style="margin-top: 8px">
                        ...
                        <div class="col-xs-7 col-sm-4 " id="coveringEmpID_div">
                            <select name="coveringEmpID[]" id="coveringEmpID"  class="form-control coveringEmp frm_input"  required>
                                <option></option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-6 col-md-offset-3">
                            <div class="testimonials">
                                <div class="active item">
                                    <blockquote><p>
                                        <?php echo '_' . $this->lang->line('hrms_leave_management_please_select_an_employee_and_leave_type_continue'); ?><!--Please select an employee to continue--></p>
                                    </blockquote>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#empFilter").multiselect2('selectAll', false);
        $("#empFilter").multiselect2('updateButtonText');
                ...
                $.ajax({
                    ...
                    success : function(data){
                        ...
                        if( data[0] == 's') {
                            ...
                            setTimeout(function () {
                                fetchPage('system/hrm/leave_application', cancelID, 'HRMS')
                            }, 300);
                        }
    ...
    $('.headerclose').click(function () {
        fetchPage('system/hrm/leave_application', 'Test', 'HRMS');
    });
    window.masterTable = $('#leaveDetailTB').DataTable({
        ...
        "fnServerData": function (sSource, aoData, fnCallback) {
            ...
            aoData.push({name:'empFilter[]', value: $('#empFilter').val()});
    ...
    function loadleaveEmployees() {
        $.ajax({
            async: true,
            url: "<?php echo site_url('Employee/leaveApplicationEmployee'); ?>",
            type: 'post',
            dataType: 'json',
            data: {},
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                var output = [];
                var select = $('#empName');
                var option = $('<option></option>').attr('selected', true).attr('data-manager', '').attr('data-DateAssumed', '').attr('data-department', '').attr('data-leaveGroupID', '').attr('data-designation', '').attr('data-policy', '').attr('data-ecode', '').text('Please select').val('');
    ...
    function get_covering_employee_list(coveringEmp = 0, confirmedYN = 0){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#coveringEmpID').select2({ multiple: true }); // set multiple option to true
            }, error: function () {
    ...
    function loadLeaveTypeDropDown(empID, confirmedYN=null) {
        $.ajax({
            ...
            type: 'post',
            dataType: 'html',
            data: {'empID': empID, confirmedYN: confirmedYN,'leaveMasterID':$('#leaveMasterID').val()},
            beforeSend: function () {
               /* startLoad();*/
            },
            success: function (data) {
    ...
    function openLeaveDetails(id, code) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                load_leave_page(empID, leaveDet['policyMasterID'], leaveDet['leaveGroupID'], leaveTypeID, 0);
                ...
                setTimeout(function () {
                    ...
                    if (policyMasterID == 2) {
                        ...
                        if (leaveDet['isCalenderDays'] == 1) {
                            ...
                            $('#workingDaysSpan').text(leaveDet['days']);
                    ...
                    if(leaveDet['confirmedYN'] ==3 ){
                        $('#confirmedBy').html('<b><?php echo $this->lang->line('common_referred_back_by');?> : </b>'+leaveDet['approvedbyEmpName']);<!--Referred back by-->
                        $('#approvedBy').html('<b> <?php echo $this->lang->line('common_comment');?>: </b>'+leaveDet['approvalComments']);<!--Comment-->
                    }

                    else if(leaveDet['confirmedYN'] ==1){
                        var tempData = '<b><?php echo $this->lang->line('common_confirmed_by');?>: </b>'+leaveDet['confirmedByName']; <!--Confirmed By-->
                        tempData += ' &nbsp; <b><?php echo $this->lang->line('common_confirmed_on');?>: </b>'+leaveDet['confirmedDate'];
                        $('#confirmedBy').html(tempData);
                    }
                    if(leaveDet['approvedYN'] ==1){
                        var tempData = '<b><?php echo $this->lang->line('common_approved_by');?>: </b>'+leaveDet['approvedbyEmpName']; <!--Approved By -->
                        tempData += ' &nbsp; <b><?php echo $this->lang->line('common_confirmed_on');?>: </b>'+leaveDet['approvedDate'];
                        $('#approvedBy').html(tempData);
                    }
                    $('#leaveCode').text(' [ ' + code + ' ] ');
                    $('#leaveMasterID').val(id);
                    submitBtn.hide();
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data[0] == 's') {
                            ...
                            setTimeout(function () {
                                fetchPage('system/hrm/leave_application', refID, 'HRMS')
                            }, 300);
                        }
                    }, error: function () {
                        stopLoad();
    ...
    function load_leave_page(empID, policyMasterID, leaveGroupID, leaveTypeID, showYN=0, isDateSet=0) {
        
     //   return false;
        var applicationType = $('#applicationType').val();
        var startDate = $('#startDate').val();
        var endDate = $('#endDate').val();
        var leaveMaster = $('#leaveMasterID').val();
    ...
    function new_leaveForm() {
        ...
        $('#dateofJoin').html('');
        $('#coveringEmpID').empty().append(
            $("<option></option>").attr("value", '').text('Select a employee')
        );
