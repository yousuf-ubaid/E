...
$hideVatIn = getPolicyValues('HVATIN', 'All');
$policyFocusCompany = getPolicyValues('FSSP', 'All');
...
if (!empty($extra)) {
    if ($printHeaderFooterYN == 1) {
        $htmlHeader .= '<div class="table-responsive">
                    ...
                    <table>
                        <tr>
                            <td colspan="3">
                                ...
                                <p> Tel: '.$this->common_data['company_data']['company_phone'].'</p>
                                <p> Email: '.$this->common_data['company_data']['company_email'].'</p>
                            </td>
                        </tr>
                    </table>
                </td>
    ...
    $htmlHeader .= '<table class="table-header-info table-border" style="width: 100%">
                <tbody>
                <tr>
                    <td style="width:25%"><strong> ' . $this->lang->line('common_customer_name') . '</strong></td><!--Customer Name-->
                    
                    <td style="width:25%"> ' . $custnam . '</td>
        
                    <td style="width:25%"><strong>' . $this->lang->line('common_invoice_number') . '</strong></td><!--Invoice Number-->
                   
                    <td style="width:25%">' . $extra['master']['invoiceCode'] . '</td>
                </tr>';
    $cussyscodee = '';
    if (!empty($extra['customer']['customerSystemCode'])) {
        $htmlHeader .= '<tr>
    ...
    $htmlHeader .= '<tr>
                    ...
                    <td><strong>Client PO No.</strong></td><!--Client PO No.-->
    ...
    if (!empty($extra['master']['salesPersonID'])) {
        $htmlHeader .= '  <td><strong> ' . $this->lang->line('sales_markating_view_invoice_sales_person') . '</strong></td><!--Sales Person -->
                    
                    <td> ' . $extra['master']['SalesPersonName'] . ' (' . $extra['master']['SalesPersonCode'] . ')</td>';
    } else {
        $htmlHeader .= '  <td><strong> ' . $this->lang->line('sales_markating_narration') . ' </strong></td><!--Narration-->
                    
                    <td> ' . $extra['master']['invoiceNarration'] . '</td>';
    }

    $htmlHeader .= '      <td><strong>' . $this->lang->line('sales_markating_view_invoice_invoice_due_date') . '</strong></td><!--Invoice Due Date-->
                    
                    <td> ' . $extra['master']['invoiceDueDate'] . '</td>
                </tr>
                <tr>
                <td><strong>Customer VAT No</strong></td>
    ...
    if($policyFocusCompany != 1){
        ...
        $htmlHeader .= '<tr>
        <td><strong> Project </strong></td><!--Customer Telephone-->
    ...
    {
        $htmlHeader .='<td><strong> PO Number </strong></td>
                
                <td>'. $POView .'</td>' ;
    }

    $htmlHeader .= $container_bl;
    $htmlHeader .= '</tr><tr>';

    if (!empty($extra['master']['salesPersonID'])) {

        $htmlHeader .= '<td style="vertical-align: top"><strong> ' . $this->lang->line('sales_markating_narration') . ' </strong></td><!--Narration-->
    ...
    $htmlHeader .= '</tr>';
        if($group_based_tax == 1 ){
            ...
            if($hideVatIn != 1){
                $htmlHeader .= '  <td><strong>VATIN</strong></td><!--Narration-->
...
if($invoiceType == 'Project') {
    ...
    if ($extra['master']['bankGLAutoID']) {
        ...
        $html .= '<div class="table-responsive">
                    <h5>' . $this->lang->line('sales_markating_view_invoice_remittance_details') . '</h5><!--Remittance Details-->
                    <table style="width: 100%">
                        <tbody>
                        <tr>
    ...
    if (!empty($extra['item_detail'])) {
        ...
        if(($istaxEnable == 1) && ($group_based_tax!=1))
        {
            ...
            if($HideOtherTaxDetails != 1){
                $col_name .= '<th class="theadtr" style="min-width: 10%">Other<br>Tax</th>';
            }
        }
        else {
        ...
        <table class="table table-bordered table-striped" style="width: 100%">
            ...
            <tr>
                ...
                <th class="theadtr" style="min-width: 50%">' . $this->lang->line('sales_markating_view_invoice_item_description') . '</th><!--Item Description-->
                ...
                '.$col_name.'
                <th class="theadtr" colspan="'.$NetColspan.'" style="width: 9%">' . $this->lang->line('sales_markating_view_invoice_net') . '</th><!--Net-->
            </tr>
            </thead>
            <tbody>';
        ...
        foreach ($extra['item_detail'] as $val) {
            ...
            if($itemBarcodeView == 1) {
                ...
                $item_description_view = '<td style="font-size: 11px;;min-width:50%;">' . $contractcd . ' '.$val['itemSystemCode'] .' - '. $val['itemDescription']. ' -  ' . $val['remarks'] . '</td>';
            }
            //<td style="text-align:center;font-size: 12px;">' . $val['itemSystemCode']. '</td>'
            
            
            $html .= '<tr style="line-height:1.5em">
            ...
            $html .= ' <td colspan="'.$NetColspan.'" style="text-align:right;font-size: 12px;">' . format_number($val['transactionAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
            $html .='</tr>';
        ...
        $html .= '</tbody>
         ...
         $html .= '<td class="text-right sub_total" style="width:10%;font-size: 12px;text-align:right;">' . format_number($item_total, $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
            </tr>
            </tfoot>
        </table>
    </div>';
    ...
    if($group_based_tax==1) {
        $grop_based_col_tax_html.='<th class="theadtr" style="width: 10%">Tax<br>Applicable<br>Amount</th>
                                   <th class="theadtr" style="width: 5%">VAT %</th>
                                   <th class="theadtr" style="width: 10%">VAT<br> Amount</th>
    ...
    $item_count_remain = $item_count % 14;

    if($item_count_remain > 7){
        $html .= "<pagebreak />";
    }
    ...
    if (!empty($extra['gl_detail'])) {
        ...
        <div class="table-responsive">
            <table class="table table-border">
                ...
                '.$grop_based_col_tax_html.'
                    <th class="theadtr" style="width: 10%">Net Amount (' . $extra['master']['transactionCurrency'] . ')</th>
                </tr>
                </thead>
                <tbody>';
    ...
    if($isDOItemWisePolicy  == 1) { //Delivery Order itemwise (SME-3138)
        ...
        if ((($taxEnabled == 1) || ($taxEnabled == null) || ($extra['item_detail_tax'] > 0))&&  ($group_based_tax!=1)) {
            $colspan = 6;
            $istaxEnable = 1;
            $footercolspan = 8 + (($RetensionEnabled == 1) ? 2 : 0);
        } else if($group_based_tax==1){
            $colspan = 4;
            $istaxEnable = 0;
            $footercolspan = 5 + (($RetensionEnabled == 1) ? 2 : 0);
        }
        else {
            $colspan = 4;
            $istaxEnable = 0;
        ...
        $colspan = $colspan + $RetensionEnabled;
    ...
    if (!empty($extra['delivery_order_itemwise'])) {
            ...
            if($RetensionEnabled == 1){
                $col_name.='
                    <th class="theadtr" style="min-width: 10%">ICV <br> Retention <br> '.$retensionPercentage.'%</th>';
                $col_name.='
                    <th class="theadtr" style="min-width: 10%">Taxable <br> Value</th>';
            }
            ...
            <table class="table table-border table-striped" style="width: 100%">
                ...
                <tr>
                    ...
                    <th class="theadtr" style="min-width: 10%">' . $this->lang->line('sales_markating_view_invoice_unit') . '</th><!--Unit-->
                    <th class="theadtr" style="min-width: 10%">' . $this->lang->line('common_sub_total') . '</th>
                    '.$col_name.'
            ...
            foreach ($extra['delivery_order_itemwise'] as $val) {
                ...
                // <td style="text-align:center;font-size: 12px;">' . $val['partNo'] . '</td>
                $html .= '<tr>
                        <td style="text-align:right;font-size: 12px;">' . $num . '.&nbsp;</td>
                        <td style="font-size: 12px;">'.$val['partNo'] .' - '. $val['itemDescription'] . ' -  ' . $val['remarks'] . '</td>
                        <td style="text-align:center;font-size: 12px;">' . $val['unitOfMeasure'] . '</td>
                        <td style="text-align:right;font-size: 12px;">' . $val['requestedQty'] . '</td>
                        <td style="text-align:right;font-size: 12px;">' . format_number($val['unittransactionAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                        <td style="text-align:right;font-size: 12px;">' . format_number(($val['unittransactionAmount'] * $val['requestedQty']),$extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                        ';
                
                if($RetensionEnabled == 1){
                    $html .= '<td style="text-align:right;font-size: 12px;">' . format_number($val['retensionValue'],$extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                    $html .= '<td style="text-align:right;font-size: 12px;">' . format_number((($val['requestedQty'] * $val['unittransactionAmount']) - $val['retensionValue']),$extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                }
                ...
                $html .= '
                        ...
                        if($istaxEnable == 1 &&  $group_based_tax!=1) {
                            ...
                            $amount_tal += $val['amount'];
                            $amount_applicable_tal += ($val['unittransactionAmount'] - $val['discountAmount']) * $val['requestedQty'] - $val['retensionValue'];
                            $amount_other_tal += $val['taxAmount'] - $val['amount'];
                        }
                $html .= ' <td style="text-align:right;font-size: 12px;">' . format_number(($val['requestedQty'] * $val['unittransactionAmount']) + $val['taxAmount'] - $val['retensionValue'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
               ...
               // $html .= ' <td style="text-align:right;font-size: 12px;">' . format_number($val['transactionAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                ...
                $num++;
                $gran_total += $val['transactionAmount'] + $val['taxAmount'] - $val['retensionValue'];
                $item_total += $val['transactionAmount'] + $val['taxAmount'] - $val['retensionValue'];
                $p_total += $val['transactionAmount'] +  $val['taxAmount'] - $val['retensionValue'];

                $tax_transaction_total += ($val['transactionAmount'] + $val['taxAmount'] - $val['retensionValue']);

            }
            $html .= '</tbody>
                <tfoot>
                <tr>
                <td class="text-left sub_total" colspan="2" style="text-align:left;">DN: ' . $docd . '</td>
                    <td class="text-right sub_total" colspan="'.$footercolspan.'" style="text-align:right;">' . $this->lang->line('sales_markating_view_invoice_total') . '<!--Item Total -->(' . $extra['master']['transactionCurrency'] . ') </td>';
            if ($group_based_tax == 1) {
                // $html .= '<td class="text-right sub_total" style="font-size: 12px;text-align:right;">' . format_number($amount_applicable_tal, $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
                // $html .= '<td class="text-right sub_total" style="font-size: 12px;text-align:right;">&nbsp;</td>';
                // $html .= '<td class="text-right sub_total" style="font-size: 12px;text-align:right;">' . format_number($amount_tal, $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>';
    ...
    if($group_based_tax == 1) {
        ...
        if (!empty($extra['delivery_order_itemwise'])) {
            $taxAmount_do_item = $vatAmount_do_item =  array_sum(array_column($extra['delivery_order_itemwise'], 'taxAmount'));
            //$vatAmount_do_item = array_sum(array_column($extra['delivery_order_itemwise'], 'amount'));
        }
        
        if (!empty($extra['delivery_order'])) {
            $taxAmount_DO = $vatAmount_DO = array_sum(array_column($extra['delivery_order'], 'taxAmount'));
        ...
        //$extra['master']['companyLocalExchangeRate'];
        ...
        if($group_based_tax == 1 && $extra['master']['transactionCurrencyID'] != $extra['master']['companyLocalCurrencyID']) {
                                ...
                                <tr>
                                    <td style="width:40%;text-align:left;">
                                        <table class="table-border">
                                            <tr>
                                                <td>
                                                    <h5 class="text-left" style="text-align:left;font-size: 10px;margin:5px 0px 5px 0px">
                                                    Sub Total (' . $extra['master']['companyLocalCurrency'] . ' )<!--Total--></h5>
                                                </td>
                                                
                                                <td>
                                                    <h5 style="font-size: 10px">' . format_number(($gran_total - $grandtotalVAT_amount - $grandtotal_otherTax_amount)/$extra['master']['companyLocalExchangeRate'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>
                                                    <h5 class="text-left" style="text-align:left;font-size: 10px;margin:5px 0px 5px 0px">
                                                    VAT (' . $extra['master']['companyLocalCurrency'] . ' )<!--Total--></h5>
                                                </td>
                                                
                                                <td aligh="right" style="text-align:right">
                                                    <h5 style="font-size: 10px">' . format_number($grandtotalVAT_amount/$extra['master']['companyLocalExchangeRate'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                                </td>
                                            </tr>';
                                            if($HideOtherTaxDetails != 1){ 
                                            $html .='<tr>
                                                        <td>
                                                            <h5 class="text-left" style="text-align:left;font-size: 10px;margin:5px 0px 5px 0px">
                                                            Other Tax (' . $extra['master']['companyLocalCurrency'] . ' )<!--Total--></h5>
                                                        </td>
                                                        
                                                        <td aligh="right" style="text-align:right">
                                                            <h5 style="font-size: 10px;text-align:right">' . format_number($grandtotal_otherTax_amount/$extra['master']['companyLocalExchangeRate'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                                        </td>
                                                    </tr>';
                                            }
                                            $html .='<tr>
                                                <td>
                                                    <h5 class="text-left" style="text-align:left;font-size: 10px;margin:5px 0px 5px 0px">
                                                    ' . $this->lang->line('common_total') . ' (' . $extra['master']['companyLocalCurrency'] . ' )<!--Total--></h5>
                                                </td>
                                                
                                                <td aligh="right" style="text-align:right">
                                                    <h5 style="font-size: 10px;text-align:right">' . format_number($gran_total/$extra['master']['companyLocalExchangeRate'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                                </td>
                                            </tr>
                                        </table>                           
                                    </td>
                                        ...
                                        <table class="table-border">
                                            <tr>
                                                <td style="text-align:left">
                                                    <h5 class="text-left" style="text-align:left;font-size: 10px;margin:5px 0px 5px 0px">
                                                    Sub Total (' . $extra['master']['transactionCurrency'] . ' )<!--Total--></h5>
                                                </td>
                                                
                                                <td>
                                                    <h5 style="font-size: 10px">' . format_number(($gran_total - $grandtotalVAT_amount - $grandtotal_otherTax_amount), $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="text-align:left">
                                                    <h5 class="text-left" style="text-align:left;font-size: 10px;margin:5px 0px 5px 0px">
                                                    VAT (' . $extra['master']['transactionCurrency'] . ' )<!--Total--></h5>
                                                </td>
                                                
                                                <td>
                                                    <h5 style="font-size: 10px">' . format_number($grandtotalVAT_amount, $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                                </td>
                                            </tr>';
                                            if($HideOtherTaxDetails != 1){ 
                                                $html .='<tr>
                                                            <td style="text-align:left">
                                                                <h5 class="text-left" style="text-align:left;font-size: 10px;margin:5px 0px 5px 0px">
                                                                Other Tax (' . $extra['master']['transactionCurrency'] . ' )<!--Total--></h5>
                                                            </td>
                                                            
                                                            <td>
                                                                <h5 style="font-size: 10px">' . format_number($grandtotal_otherTax_amount, $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                                            </td>
                                                        </tr>';
                                                }
                                                $html .='<tr>
                                                    <td style="text-align:left">
                                                        <h5 class="text-left" style="text-align:left;font-size: 10px;margin:5px 0px 5px 0px">
                                                        ' . $this->lang->line('common_total') . ' (' . $extra['master']['transactionCurrency'] . ' )<!--Total--></h5>
                                                    </td>
                                                    
                                                    <td>
                                                        <h5 style="font-size: 10px">' . format_number($gran_total, $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                                    </td>
                                                </tr>
                                        </table>                           
                                    </td>
            ...
            //             &nbsp;&nbsp;&nbsp;&nbsp; :  ' . format_number(($gran_total - $gran_total_tax) / $extra['master']['companyLocalExchangeRate'], $extra['master']['companyLocalCurrencyDecimalPlaces']) . '</h5>
            //     ';
            // $html .= '
            ...
            $html .= '<div class="table-responsive">
                        <h5 class="text-right" style="text-align:right;margin:5px 7px 3px 5px;font-size: 12px;">
            ...
            $html .= '<div class="table-responsive">
                            <h5 class="text-right" style="text-align:right;margin:5px 7px 5px 0px;font-size: 12px;">
                                ...
                                <td style="width:40%;text-align:right;">
                                    <table>
                                        <tr>
                                            ...
                                            <td>
                                                <h5>' . format_number(($gran_total - $gran_total_tax), $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                            ...
                                            <td style="width:10px"><h5>:</h5></td>
                                            <td>
                                                <h5>' . format_number($grandtotalVAT_amount, $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                            </td>
                                        </tr>';
                                        if($HideOtherTaxDetails != 1){ 
                                $html .= '<tr>
                                            ...
                                            <td style="width:10px"><h5>:</h5></td>
                                            <td>
                                                <h5>' . format_number($grandtotal_otherTax_amount, $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                            </td>
                                        </tr>';
                                        }
                                $html .= '<tr>
                                            ...
                                            <td style="width:10px"><h5>:</h5></td>
                                            <td>
                                                <h5>' . format_number($gran_total, $extra['master']['transactionCurrencyDecimalPlaces']) . '</h5>
                                            </td>
                                        </tr>
                                    </table>                           
                                </td>                                
    ...
    $gl_count_remain = $item_count % 14;

    if($gl_count_remain > 8 && $gl_count > 10){
        $html .= "<pagebreak />";
    }
    ...
    if ($extra['master']['bankGLAutoID']) {
        ...
        $html .='<div class="table-responsive table-bottom-info">
        <table class="table-border" style="width: 100%; ">
        <tbody>
        <tr>
            <td style="width: 20%;"><strong>Exchange rate</strong></td>
            
            <td style="width: 80%;">'.format_number(1/$extra['master']['companyLocalExchangeRate'],$extra['master']['transactionCurrencyDecimalPlaces']).'</td>
        </tr>
        <tr>
            <td style="width: 20%;"><strong>Amount in words</strong></td><!--Account-->
           
            <td style="width: 80%">' . $numberinword . $str1 . ' ('. $extra['master']['transactionCurrency'] .')'. '</td>
        </tr>
        </tbody>
        </table>
        </div>';
        ...
        $html .= '<div class="table-responsive table-bottom-info">
        <h5 class="tbl">' . $this->lang->line('sales_markating_view_invoice_remittance_details') . '</h5><!--Remittance Details-->
        <table style="width: 100%">
            <tbody>
            <tr>
                <td style="width: 18%"><strong>' . $this->lang->line('common_bank') . '</strong></td><!--Bank-->
    ...
    <h5 class="tbl">' . $this->lang->line('sales_markating_view_invoice_notes') . '</h5><!--Notes-->
    <table style="width: 100%">
        <tbody>
        <tr>
