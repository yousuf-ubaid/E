...
if($this->common_data['company_data']['legalName'] != '') {
    $companyName = $this->common_data['company_data']['legalName'];
} else {
...
if (!empty($extra)) {
    ...
    $invoiceheaderName = $this->lang->line('sales_marketing_simplified_tax_invoice');
...
<div class="table-responsive">
    ...
    $html .= '<tr>
        ...
        <tr>
            <td><strong>' . $this->lang->line('common_customer_weburl') . ' </strong></td><!--Customer Telephone-->
            <td><strong>:</strong></td>
    ...
    $html .= '<tr>
            ...
            <td><strong> ' . $this->lang->line('sales_markating_view_invoice_invoice_date') . ' / <br>Date of Payment</strong></td><!--Invoice Date-->
            <td><strong>:</strong></td>
    ...
    if($invoiceType == 'Project') {
                ...
                $html .= '
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td style="text-align:left;font-size: 11px;" >' . $extra['master']['transactionCurrency'] . '</td>
                            ...
                            <tr>
                                <td style="font-size: 11px;width: 15%; text-align: left;" >Conversion Rate(' . $extra['master']['companyLocalCurrency'] . ') : ' . format_number((1/$extra['master']['companyLocalExchangeRate']), 4) . '</td>
                                <td style="text-align:left;font-size: 11px;" >' . $extra['master']['companyLocalCurrency'] . '</td>
                                <td style="text-align:right;font-size: 11px;" > ' . format_number(($amount_applicable_tal_grand / $extra['master']['companyLocalExchangeRate']), $extra['master']['companyLocalCurrencyDecimalPlaces']) . ' </td>
                                <td style="text-align:right;font-size: 11px;"> ' . format_number(($amount_tal_grand / $extra['master']['companyLocalExchangeRate']), $extra['master']['companyLocalCurrencyDecimalPlaces']) . ' </td>';
                                if($amount_other_tal_grand > 0) {
                                    $html .= '<div><td style="text-align:right;font-size: 11px;"> ' . format_number(($amount_other_tal_grand / $extra['master']['companyLocalExchangeRate']), $extra['master']['companyLocalCurrencyDecimalPlaces']) . ' </td></div>';
        ...
        <table class="table table-bordered table-striped" style="width: 100%">
            ...
            foreach ($extra['item_detail'] as $val) {
                ...
                if($itemBarcodeView == 1) {
                    $item_description_view = '<td style="font-size: 12px;">' . $val['itemSystemCode'] .' | '.$val['barcode']. '<br>' . $contractcd . ' ' . $val['itemDescription'] . ' -  ' . $val['remarks'] . '</td>';
                } else {
                    $item_description_view = '<td style="font-size: 12px;">' . $val['itemSystemCode'] . '<br>' . $contractcd . ' ' . $val['itemDescription'] . ' -  ' . $val['remarks'] . '</td>';
                }
                $html .= '<tr>
        ...
        if (!empty($extra['gl_detail'])) {
            ...
            foreach ($extra['gl_detail'] as $val) {
                $grop_based_col_tax_html_val = '';
                if($group_based_tax==1){
                    $grop_based_col_tax_html_val.=' <td style="text-align:right;">' . format_number($val['transactionAmount'] - $val['taxAmount'], $extra['master']['transactionCurrencyDecimalPlaces']) . '</td>
                    ...
                    $amount_tal_grand += $val['amount'];
                    $amount_applicable_tal_grand += $val['transactionAmount'] - $val['taxAmount'];
                    $amount_other_tal_grand += $val['taxAmount']-$val['amount'];
                } else {
        ...
        if($group_based_tax == 1) {
            ...
            if (!empty($extra['delivery_order'])) {
                $taxAmount_DO =  $vatAmount_DO =  $vatAmount_DO = array_sum(array_column($extra['delivery_order'], 'taxAmount'));
            ...
            if($group_based_tax == 1 && $extra['master']['transactionCurrencyID'] != $extra['master']['companyLocalCurrencyID']) {
                $html .= '<br><table style="width: 100%">
                        <tr>';
                $html .= '<td style="width: 50%;">
                            <h class="text-right" style="text-align:right;font-size: 12px;margin:5px 0px 5px 0px"> Sub Total (' . $extra['master']['companyLocalCurrency'] . ' )<!--Total--> 
                            &nbsp;&nbsp;&nbsp;&nbsp; :  ' . format_number(($gran_total - $gran_total_tax) / $extra['master']['companyLocalExchangeRate'], $extra['master']['companyLocalCurrencyDecimalPlaces']) . '</h>
                    ';
                $html .= '
