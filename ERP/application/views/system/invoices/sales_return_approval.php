...
$title = $this->lang->line('sales_markating_view_sales_return_approval');
echo head_page($title, false);



/*echo head_page('Sales Return Approval', false);*/ ?>
...
<div class="row">
    <div class="col-md-5">
        <table class="<?php echo table_class() ?>">
            <tr>
                <td><span class="label label-success">&nbsp;</span> <?php echo $this->lang->line('common_approved');?> </td><!--Approved-->
                <td><span class="label label-danger">&nbsp;</span> <?php echo $this->lang->line('common_not_approved');?></td><!-- Not Approved-->
            </tr>
        </table>
    </div>
...
<div class="table-responsive">
    <table id="salesReturn_table" class="<?php echo table_class() ?>">
        <thead>
        <tr>
            ...
            <th style="min-width: 30%"><?php echo $this->lang->line('sales_markating_narration');?></th></th><!--Narration-->
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/invoices/sales_return_approval', '', 'Sales Return Approval')
        });
        salesReturn_table();
        $('#pv_approval_form').bootstrapValidator({
            ...
            fields: {
                ...
                salesReturnAutoID: {validators: {notEmpty: {message: '<?php echo $this->lang->line('sales_markating_payment_voucher_id_is_required');?>.'}}},/*Payment Voucher ID is required*/
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data['error'] == 1) {
                        ...
                        $("#sales_return_approval_modal").modal('hide');
                        Otable.draw();
    ...
    function salesReturn_table() {
        Otable = $('#salesReturn_table').DataTable({
            "language": {
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Inventory/fetch_sales_return_approval'); ?>",
            "aaSorting": [[1, 'desc']],
            ...
            "columnDefs": [{"visible":false,"searchable": true,"targets": [10], "orderable": false}, {"targets": [0,6,7,8,9], "searchable": false}],
    ...
    function fetch_approval(salesReturnAutoID, documentApprovedID, Level) {
        if (salesReturnAutoID) {
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'html',
                data: {'salesReturnAutoID': salesReturnAutoID, 'html': true, 'approval': 1},
                url: "<?php echo site_url('Inventory/load_sales_return_conformation'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    $('#salesReturnAutoID').val(salesReturnAutoID);
                    $('#documentApprovedID').val(documentApprovedID);
                    $('#Level').val(Level);
                    $("#sales_return_approval_modal").modal({backdrop: "static"});
                    $('#conform_body').html(data);
                    $('#comments').val('');
                    customerInvoice_attachment_View_modal('SLR', salesReturnAutoID);
                    stopLoad();
