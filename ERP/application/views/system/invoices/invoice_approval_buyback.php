...
<script type="text/javascript">
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/invoices/invoice_approval_buyback', '', 'Invoice Approval');
        });
        Invoices_table();
    ...
    function Invoices_table() {
        var Otable = $('#Invoices_table').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('InvoicesPercentage/fetch_invoices_approval'); ?>",
            "aaSorting": [[1, 'desc']],
        ...
        if (invoiceAutoID) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#comments').val('');
                    customerInvoice_attachment_View_modal('HCINV', invoiceAutoID);
                    stopLoad();
                    refreshNotifications(true);
                }, error: function () {
                    stopLoad();
