    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">Journal Voucher Template</h4>
            </div>
            <div class="modal-body">
                <input type="hidden" id="JVMasterAutoId">
                <div class="row">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary btn-sm" onclick="print_jv_temp()">Print</button>
...
function journal_entry_table(){
     Otable = $('#journal_entry_table2').DataTable({
        ...
        "bDestroy": true,
        "bStateSave": true,
        "sAjaxSource": "<?php echo site_url('Journal_entry/fetch_journal_entry_buyback'); ?>",
        "aaSorting": [[0, 'desc']],
...
function delete_journal_entry(id){
    ...
    function load_printtemp(JVMasterAutoId){
        $('#printSize').val(1);
        $('#JVMasterAutoId').val(JVMasterAutoId);
        $('#print_temp_modal').modal('show');
    }

    function print_jv_temp(){
        var printSize =  $('#printSize').val();
        var jvID = $('#JVMasterAutoId').val();

        if(jvID==''){
            myAlert('e', 'Select Print Type');
        }else{
            window.open("<?php echo site_url('Journal_entry/journal_entry_conformation_buyback') ?>" +'/'+ jvID +'/'+ printSize +'/'+1);
        }
    }
</script>