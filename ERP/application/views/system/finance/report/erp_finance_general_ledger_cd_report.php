<!---- =============================================
-- File Name : erp_finance_general_ledger_report.php
-- Project Name : SME ERP
-- Module Name : Report - Finance
-- Create date : 15 - September 2016
-- Description : This file contains General Ledger.
                    ...
                    <table class="borderSpace report-table-condensed" id="tbl_report">
                        ...
                        if (!empty($category)) {
                            ...
                            foreach ($category as $key => $mainCategory) {
                                        ...
                                        if ($i == 1) {
                                            ...
                                            if ($value["fieldName"] == "documentNarration") {
                                                ...
                                                if ($value["fieldName"] == "companyLocalAmount") {
                                                    if($item[$value["fieldName"]]<0){
                                                        $subtotalloc += $item[$value["fieldName"]]*-1;
                                                        echo "<td class='" . $value["textAlign"] . "'>" . number_format(0, $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                                        echo "<td class='" . $value["textAlign"] . "'>" . number_format($item[$value["fieldName"]]*-1, $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                                    }else{
                                                        $subtotallocd += $item[$value["fieldName"]];
                                                    ...
                                                    if($item[$value["fieldName"]]<0){
                                                        $subtotalrpt += $item[$value["fieldName"]]*-1;
                                                        echo "<td class='" . $value["textAlign"] . "'>" . number_format(0, $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                                        echo "<td class='" . $value["textAlign"] . "'>" . number_format($item[$value["fieldName"]]*-1, $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                                    }else{
                                                        $subtotalrptd += $item[$value["fieldName"]];
                                                        echo "<td class='" . $value["textAlign"] . "'>" . number_format($item[$value["fieldName"]], $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                                        echo "<td class='" . $value["textAlign"] . "'>" . number_format(0, $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                                    }
                                                } else {
                                ...
                                if ($isLocCost) {
                                    echo "<td class='reporttotal text-right'>" . number_format($subtotallocd, $this->common_data['company_data']['company_default_decimal']) . "</td>";
                                    echo "<td class='reporttotal text-right'>" . number_format($subtotalloc, $this->common_data['company_data']['company_default_decimal']) . "</td>";
                                }
                                if ($isRptCost) {
                                    echo "<td class='reporttotal text-right'>" . number_format($subtotalrptd, $this->common_data['company_data']['company_reporting_decimal']) . "</td>";
                                ...
                                echo "<tr>";
                                if ($value["fieldName"] == "companyReportingAmount" || $value["fieldName"] == "companyLocalAmount"){
                                    echo "<td colspan='" . ($locKey) . "'><b>Balance</b></td>";

                                    if ($isLocCost){
                                        $totalltot=$subtotallocd-$subtotalloc;
                                        if($totalltot<0){
                                            echo "<td>&nbsp;</td>";
                                            echo "<td class='reporttotal text-right'>" . number_format($totalltot, $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                        }else{
                                            echo "<td class='reporttotal text-right'>" . number_format($totalltot, $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                            echo "<td>&nbsp;</td>";
                                        }
                                    ...
                                    if ($isRptCost){
                                        $totalrtot=$subtotalrptd-$subtotalrpt;
                                        if($totalrtot<0){
                                            echo "<td>&nbsp;</td>";
                                            echo "<td class='reporttotal text-right'>" . number_format($totalrtot, $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                        }else{
                                            echo "<td class='reporttotal text-right'>" . number_format($totalrtot, $item[$value["fieldName"] . "DecimalPlaces"]) . "</td>";
                                            echo "<td>&nbsp;</td>";
                                        }
