...
$title = $this->lang->line('transaction_bulk_transfer_approval');
...
<div class="table-responsive">
    <table id="bulk_transfer_approval_table" class="<?php echo table_class() ?>">
        <thead>
        <tr>
...
<div class="modal fade" id="bulk_transfer_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"
                    id="myModalLabel"><?php echo $this->lang->line('transaction_bulk_transfer'); ?> </h4>
                <!--Bulk Transfer-->
            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/inventory/stock_transfer_approval_bulk', 'Test', '<?php echo $this->lang->line('transaction_bulk_transfer_approval'); ?>');
        });
        bulk_transfer_table();
        $('#approval_form').bootstrapValidator({
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: data,
                url: "<?php echo site_url('Inventory/save_bulk_transfer_approval'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    ...
                    if (data[0] == 's') {
                        $("#bulk_transfer_modal").modal('hide');
                        Otable.draw();
    ...
    function bulk_transfer_table() {
        Otable = $('#bulk_transfer_approval_table').DataTable({
            "language": {
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Inventory/fetch_bulk_transfer_approval'); ?>",
            "aaSorting": [[1, 'desc']],
        ...
        if (stockTransferAutoID) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#Level').val(Level);
                    $("#bulk_transfer_modal").modal({backdrop: "static"});
                    $('#conform_body').html(data);
                    $('#comments').val('');
                    stockTransfer_attachment_View_modal('STB', stockTransferAutoID);
                    stopLoad();
                    refreshNotifications(true);
                }, error: function () {
                    stopLoad();
