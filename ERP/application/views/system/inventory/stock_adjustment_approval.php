...
$title = $this->lang->line('transaction_stock_adjustment_approval');
echo head_page($title, false);

/*echo head_page('Stock Adjustment Approval', false);*/ ?>
...
<div class="table-responsive">
    <table id="stock_adjustment_table" class="<?php echo table_class() ?>">
        ...
        <tr>
            ...
            <th style="min-width: 10%"> <?php echo $this->lang->line('transaction_stock_adjustment_code');?> </th><!--SA Code-->
            <th style="min-width: 10%"><?php echo $this->lang->line('common_date');?> </th><!--Date-->
            <th style="min-width: 30%"><?php echo $this->lang->line('transaction_common_warehouse_description');?> </th><!--Ware House Description-->
...
<div class="modal fade" id="stock_adjustment_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel"> <?php echo $this->lang->line('transaction_stock_adjustment_approval');?> </h4><!--Stock Adjustment Approval-->
            </div>
            <form class="form-horizontal" id="ST_approval_form">
                <div class="modal-body">
                    <div class="col-sm-1">
                        <!-- Nav tabs -->
                        <ul class="zx-nav zx-nav-tabs zx-tabs-left zx-vertical-text">
                            <li id="sa_attachement_approval_Tabview_v" class="active"><a href="#Tab-home-v"
                                                                                         data-toggle="tab" onclick="tabView()"><?php echo $this->lang->line('common_view');?> </a></li><!--View-->
                            <li id="sa_attachement_approval_Tabview_a"><a href="#Tab-profile-v" data-toggle="tab" onclick="tabAttachement()"><?php echo $this->lang->line('common_attachment');?> </a><!--Attachment-->
                            </li>
                        </ul>
                    </div>
                    <div class="col-sm-11">
                        <div class="zx-tab-content">
                            <div class="zx-tab-pane active" id="Tab-home-v">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        <input type="hidden" name="stockAdjustmentAutoID" id="stockAdjustmentAutoID">
                            ...
                            <div class="tab-pane hide" id="Tab-profile-v">
                                <div class="table-responsive">
                                    ...
                                    &nbsp <strong><?php echo $this->lang->line('transaction_stock_adjustment_attachments');?> </strong><!--Stock Adjustment Attachments-->
                                    <br><br>
                                    <table class="table table-striped table-condensed table-hover">
                                        ...
                                        <tbody id="sa_attachment_body" class="no-padding">
                                        <tr class="danger">
...
<div aria-hidden="true" role="dialog" id="stock_insufficient_modal" class="modal fade" style="display: none;">
    <div class="modal-dialog" style="width: 60%;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php echo export_buttons('stock_insufficient_table_SA', 'Stock Insufficient', true, false); ?>
                <form role="form" id="stock_insufficient_form" class="form-horizontal">
                    <table class="table table-bordered table-condensed no-color" id="stock_insufficient_table_SA" style="margin-top: 3px">
                        <thead>
                        <tr>
                        ...
                        <tbody id="stock_insufficient_table_body" style="background: #ffb2b2">

                        </tbody>
                        <tfoot id="stock_insufficient_table_tfoot">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/inventory/stock_adjustment_approval', 'Test', 'Stock Adjustment Approval');
        });
        stock_adjustment_table();
        $('#ST_approval_form').bootstrapValidator({
            live: 'enabled',
            ...
            fields: {
                ...
                stockAdjustmentAutoID: {validators: {notEmpty: {message: '<?php echo $this->lang->line('transaction_stock_adjustment_id_is_required');?>.'}}},/*Stock Adjustment ID is required*/
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data == true){
                        $("#stock_adjustment_modal").modal('hide');
                        Otable.draw();
                    ...
                    if(data['error']) {
                        ...
                        if (data['stock']) {
                            $('#stock_insufficient_table_body').empty();
                            if (jQuery.isEmptyObject(data)) {
                                $('#stock_insufficient_table_body').append('<tr class="danger"><td colspan="12" class="text-center"><b><?php echo $this->lang->line('common_no_records_found');?> </b></td></tr>');<!--No Records Found-->
                            } else {
                                $.each(data['stock'], function (key, value) {
                                    ...
                                    $('#stock_insufficient_table_body').append(string);
                                });
                                $("#stock_insufficient_modal").modal({backdrop: "static"});
                            }
                        }
                    }
                }, error: function () {
    ...
    function stock_adjustment_table() {
         Otable = $('#stock_adjustment_table').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Inventory/fetch_stock_adjustment_approval'); ?>",
            "aaSorting": [[1, 'desc']],
            ...
            "aoColumns": [
             ...
             "columnDefs": [{"targets": [8], "orderable": false},{"targets": [0,3,7,8], "searchable": false},{"visible":false,"searchable": true,"targets": [9,10,11] }],
    ...
    function fetch_approval(stockAdjustmentAutoID, documentApprovedID, Level) {
        if (stockAdjustmentAutoID) {
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'html',
                data: {'stockAdjustmentAutoID': stockAdjustmentAutoID, 'html': true,'approval': 1},
                url: "<?php echo site_url('Inventory/load_stock_adjustment_conformation'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    $('#stockAdjustmentAutoID').val(stockAdjustmentAutoID);
                    $('#documentApprovedID').val(documentApprovedID);
                    $('#Level').val(Level);
                    $("#stock_adjustment_modal").modal({backdrop: "static"});
                    $('#conform_body').html(data);
                    $('#comments').val('');
                    stockAdjustment_attachment_View_modal('SA', stockAdjustmentAutoID);
                    stopLoad();
                    refreshNotifications(true);
                }, error: function () {
                    stopLoad();
    ...
    function stockAdjustment_attachment_View_modal(documentID, documentSystemCode) {
        $("#Tab-profile-v").removeClass("active");
        $("#Tab-home-v").addClass("active");
        $("#sa_attachement_approval_Tabview_a").removeClass("active");
        $("#sa_attachement_approval_Tabview_v").addClass("active");
        if (documentSystemCode) {
            $.ajax({
                ...
                success: function (data) {
                    $('#sa_attachment_body').empty();
                    $('#sa_attachment_body').append('' +data+ '');
