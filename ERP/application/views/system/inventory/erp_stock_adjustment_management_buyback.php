...
echo head_page('Stock Adjustment - Buyback', true);
...
<div class="row">
    ...
    <div class="col-md-3 text-right">
        <button type="button" class="btn btn-primary pull-right"
                onclick="fetchPage('system/inventory/erp_stock_adjustment_buyback',null,'<?php echo $this->lang->line('transaction_add_new_stock_adjustment');?>','SA');"><i
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Stock Adjustment Template</h4>
            </div>
            <div class="modal-body">
                <input type="hidden" id="stockAdjustmentAutoID">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary btn-sm" onclick="print_sa_temp()">Print</button>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/inventory/erp_stock_adjustment_management_buyback', 'Test', 'Stock Adjustment');
        });
    ...
    function stock_adjustment_table(selectedID=null) {
        Otable = $('#stock_adjustment_table').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Inventory/fetch_stock_adjustment_table_buyback'); ?>",
            "aaSorting": [[0, 'desc']],
            ...
            "columnDefs": [{"targets": [7], "orderable": false},{"visible":false,"searchable": true,"targets": [8,9,10] }, {"searchable": false, "targets": [0]}],
        ...
        swal({
            ...
            function () {
                $.ajax({
                    ...
                    url: "<?php echo site_url('Inventory/delete_stock_adjustment_buyback'); ?>",
                    beforeSend: function () {
                        startLoad();
                    },
        ...
        swal({
            ...
            function () {
                $.ajax({
                    ...
                    url: "<?php echo site_url('Inventory/referback_stock_adjustment_buyback'); ?>",
                    beforeSend: function () {
                        startLoad();
                    },
                    success: function (data) {
        ...
        swal({
            ...
            function () {
                $.ajax({
                    ...
                    dataType : 'json',
                    data : {'stockAdjustmentAutoID':id},
                    url :"<?php echo site_url('Inventory/re_open_stock_adjestment_buyback'); ?>",
                    beforeSend: function () {
                        startLoad();
                    },
                    success : function(data){
    ...
    function load_printtemp(stockAdjustmentAutoID)
    {
        $('#printSize').val(1);
    ...
    function print_sa_temp(){
        var printtype =  $('#printSize').val();
        var stockAdjustmentAutoID =   $('#stockAdjustmentAutoID').val();

        if(stockAdjustmentAutoID==''){
            myAlert('e', 'Select Print Type');
        }else{
            window.open("<?php echo site_url('Inventory/load_stock_adjustment_conformation_buyback') ?>" +'/'+ stockAdjustmentAutoID +'/'+ printtype +'/'+1);
        }
    }
</script>