...
$title = $this->lang->line('transaction_material_receipt_note');
...
<div class="row">
    ...
    <div class="col-md-3 text-right">
        <button type="button" class="btn btn-primary pull-right" onclick="fetchPage('system/inventory/MaterialReceipt/erp_material_receipt',null,'<?php echo $this->lang->line('transaction_add_new_material_receipt_note')?>','MRN');"><i class="fa fa-plus"></i> <?php echo $this->lang->line('transaction_create_material_receipt_note');?>  </button><!--Create Material Receipt Note-->
    </div>
</div><hr>
<div class="table-responsive">
    <table id="material_receipt_master_table" class="<?php echo table_class(); ?>">
        <thead>
            <tr>
                <th style="min-width: 5%">#</th>
                <th style="min-width: 15%"><?php echo $this->lang->line('transaction_material_receipt_note_code');?> </th><!--MRN Code-->
...
$(document).ready(function() {
    $('.headerclose').click(function(){
        fetchPage('system/inventory/MaterialReceipt/material_receipt_note','Test','Material Receipt Note');
    });
    grvAutoID = null;
    number_validation();
    material_receipt_table();
...
function material_receipt_table(selectedID=null){
     Otable = $('#material_receipt_master_table').DataTable({
         "language": {
        ...
        "bDestroy": true,
        "bStateSave": true,
        "sAjaxSource": "<?php echo site_url('MaterialReceiptNote/fetch_material_receipt'); ?>",
        "aaSorting": [[0, 'desc']],
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            for (var i = tmp_i; (iLen + tmp_i) > i; i++) {
                ...
                if( parseInt(oSettings.aoData[x]._aData['mrnAutoID']) == selectedRowID ){
...
function delete_item(id,value){
        ...
        function () {
            $.ajax({
                ...
                dataType : 'json',
                data : {'mrnAutoID':id},
                url :"<?php echo site_url('MaterialReceiptNote/delete_material_receipt_header'); ?>",
                beforeSend: function () {
                    startLoad();
                },
        ...
        swal({
            ...
            function () {
                $.ajax({
                    ...
                    dataType : 'json',
                    data : {'mrnAutoID':id},
                    url :"<?php echo site_url('MaterialReceiptNote/referback_materialissue'); ?>",
                    beforeSend: function () {
                        startLoad();
                    },
                    success : function(data){
    ...
    swal({
        ...
        function () {
            $.ajax({
                ...
                dataType : 'json',
                data : {'mrnAutoID':id},
                url :"<?php echo site_url('MaterialReceiptNote/re_open_material_receipt'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success : function(data){
