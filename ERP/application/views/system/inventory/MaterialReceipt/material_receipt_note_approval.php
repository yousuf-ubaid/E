...
$title = $this->lang->line('transaction_material_receipt_note_approval');
echo head_page($title, false);

/*echo head_page('Material Receipt Note Approval', false); */?>
...
<div class="table-responsive">
    <table id="material_receipt_approval_table" class="<?php echo table_class() ?>">
        <thead>
        <tr>
            <th style="min-width: 5%">#</th>
            <th style="min-width: 10%"><?php echo $this->lang->line('transaction_material_receipt_note_code');?> </th><!--MRN Code-->
            <th style="min-width: 10%"><?php echo $this->lang->line('transaction_common_received_date');?> </th><!--Received Date-->
            ...
            <th style="min-width: 10%"><?php echo $this->lang->line('transaction_common_received_total');?> </th><!--Received Total-->
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"><?php echo $this->lang->line('transaction_common_material_issue_approval');?> </h4><!--Material Issue Approval-->
            </div>
            <form class="form-horizontal" id="MRN_approval_form">
                            ...
                            <div class="zx-tab-pane active" id="Tab-home-v">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        <input type="hidden" name="mrnAutoID" id="mrnAutoID">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/inventory/MaterialReceipt/material_receipt_note_approval', 'Test', 'Material Receipt Note Approval');
        });
        material_receipt_approval_table();
        $('#MRN_approval_form').bootstrapValidator({
            live: 'enabled',
            ...
            fields: {
                ...
                mrnAutoID: {validators: {notEmpty: {message: '<?php echo $this->lang->line('transaction_material_receipt_note_id_is_required');?>.'}}},/*Material Receipt Note ID is required*/
    ...
    function material_receipt_approval_table() {
         Otable = $('#material_receipt_approval_table').DataTable({
             "language": {
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('MaterialReceiptNote/fetch_material_issue_approval'); ?>",
            "aaSorting": [[1, 'desc']],
            ...
            "aoColumns": [
             ...
             "columnDefs": [{"targets": [9], "orderable": false},{"targets": [0,3,6,8,9], "searchable": false},{"visible":false,"searchable": true,"targets": [10,11,12,13] }],
    ...
    function fetch_approval(mrnAutoID, documentApprovedID, Level) {
        if (mrnAutoID) {
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'html',
                data: {'mrnAutoID': mrnAutoID, 'html': true,'approval': 1},
                url: "<?php echo site_url('MaterialReceiptNote/load_material_receipt_conformation'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    $('#mrnAutoID').val(mrnAutoID);
                    $('#documentApprovedID').val(documentApprovedID);
                    $('#Level').val(Level);
                    $("#material_issue_modal").modal({backdrop: "static"});
                    $('#conform_body').html(data);
                    $('#comments').val('');
                    materialIssue_attachment_View_modal('MRN', mrnAutoID);
                    stopLoad();
                    refreshNotifications(true);
                }, error: function () {
                    stopLoad();
