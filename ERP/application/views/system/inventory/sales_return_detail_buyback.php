        ...
        <div id="tab_1" class="tab-pane active">
            <table class="table table-bordered table-striped  table-condensed">
                ...
                <tr>
                    ...
                    <th style="min-width: 10%">No of Birds</th>
                    <th style="min-width: 10%">Gross Weight</th>
                    <th style="min-width: 10%">No of Buckets</th>
                    <th style="min-width: 10%">Bucket Size</th>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <table class="table table-bordered table-striped table-condesed ">
                            ...
                            <tr>
                                <th style="width: 5%">#</th>
                                <th style="width: 15%"><?php echo $this->lang->line('sales_markating_transaction_header_info');?> </th><!--Header Info-->
                                <th style="width: 15%"><?php echo $this->lang->line('sales_markating_transaction_secondary_item_description');?></th><!--Item Description-->
                                <th style="width: 10%"><?php echo $this->lang->line('sales_markating_transaction_sales_price');?> </th><!--Sales Price-->
                                <th style="width: 5%"><?php echo $this->lang->line('sales_markating_transaction_balance');?></th><!--Balance-->
                                <th style="width: 5%">No of Birds </th><!--Qty-->
                                <th style="width: 5%">Gross Weight	 </th><!--Qty-->
                                <th style="width: 5%">Buckets </th><!--Qty-->
                                <th style="width: 5%">B weight </th><!--Qty-->
...
<script type="text/javascript">
    ...
    function fetch_item_for_sales_return(itemAutoID) {
        $.ajax({
            ...
            url: "<?php echo site_url('Inventory/fetch_item_for_sales_return_buyback'); ?>",
            success: function (data) {
                ...
                if (!jQuery.isEmptyObject(data['data'])) {
                    ...
                    $.each(data['data'], function (val, text) {
                        txAmnt = text['totalAfterTax']/text['invoiceQty'];
                        total = (text['unittransactionAmount'] - text['discountAmount']);
                        $('#item_cinv_table_body').append('<tr><td>' + x + '</td><td>' + text['invoiceCode'] + ' - ' + text['transactionCurrency'] + ' - ' + text['invoiceDate'] + '</td><td>' + text['itemSystemCode'] + ' - ' + text['itemDescription'] + '</td><td class="text-right">' + parseFloat(total+txAmnt).formatMoney(text['transactionCurrencyDecimalPlaces'], '.', ',') + '</td><td>' + text['requestedQty'] + ' ' + text['unitOfMeasure'] + ' </td><td class="text-center"><input type="text" class="number" size="8" id="noitem_' + text['invoiceDetailsAutoID'] + '"  ></td><td class="text-center"><input type="text" class="number" size="8" value='+(text['requestedQty'])+' id="grossqty_' + text['invoiceDetailsAutoID'] + '"  onchange="calbucketweight(' + text['invoiceDetailsAutoID'] + ',this,' + text['requestedQty'] + ')" ></td><td class="text-center"><input type="text" class="number" size="8" id="buckets_' + text['invoiceDetailsAutoID'] + '" onchange="calbucketweight(' + text['invoiceDetailsAutoID'] + ',this,' + text['requestedQty'] + ')"  ></td><td class="text-center"><select class="whre_bweight" style="width: 110px;" onchange="calbucketweight(' + text['invoiceDetailsAutoID'] + ',this,' + text['requestedQty'] + ')"  id="whre_bweight_' + text['invoiceDetailsAutoID'] + '"><option value=" ">Select B weigh</option></select></td><td class="text-center"><input type="text" class="number" size="15" id="qty_' + text['invoiceDetailsAutoID'] + '" onkeyup="select_check_box(this,' + text['invoiceDetailsAutoID'] + ',' + text['requestedQty'] + ')"></td><td class="text-right" style="display: none;"><input class="checkbox" id="check_' + text['invoiceDetailsAutoID'] + '" type="checkbox" value="' + text['invoiceDetailsAutoID'] + '"></td></tr>');
                        x++;
                    });
                } else {
                    $('#item_cinv_table_body').append('<tr class="danger"><td colspan="10" class="text-center"><b><?php echo $this->lang->line('common_no_records_found');?></b></td></tr>');
                }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (jQuery.isEmptyObject(data['detail'])) {
                    ...
                    $('#item_table_body,#invoice_table_body').append('<tr class="danger"><td colspan="12" class="text-center"><b><?php echo $this->lang->line('common_no_records_found');?> </b></td></tr>');<!--No Records Found-->
                    ...
                    $.each(data['detail'], function (key, value) {
                        ...
                        var row = '<tr><td>' + x + '</td><td>' + value['itemSystemCode'] + '</td><td>' + value['itemDescription'] + '</td>' +
                            '<td>' + value['unitOfMeasure'] + '</td><td>' + value['noOfItems'] + '</td><td>' + value['grossQty'] + '</td>' +
                            '<td>' + value['noOfUnits'] + '</td><td>' + value['deduction'] + '</td>' +
                            '<td class="text-center">' + (parseFloat(value['salesPrice']) + parseFloat(value['taxAmount'])).formatMoney(value['transactionCurrencyDecimalPlaces'], '.', ',') + '</td><td class="text-center">' + value['return_Qty'] + '</td><td class="text-right">' + ((parseFloat(/*value['totalValue']*/  value['salesPrice'])  + parseFloat(value['taxAmount'])).toFixed(value['transactionCurrencyDecimalPlaces']) *  value['return_Qty']).formatMoney(value['transactionCurrencyDecimalPlaces'], '.', ',') + '</td><td class="text-right"><a onclick="edit_sales_return(' + value['salesReturnDetailsID'] + ');"><span class="glyphicon glyphicon-pencil"></span></a> &nbsp;&nbsp; | &nbsp;&nbsp;<a onclick="delete_item(' + value['salesReturnDetailsID'] + ');"><span style="color:rgb(209, 91, 71);" class="glyphicon glyphicon-trash"></span></a><input class="hidden" name="invoiceDetailsAutoID" id="invoiceDetailsAutoID" value="' + value['invoiceDetailsAutoID'] + '"></td></tr>';
                        $('#invoice_table_body').append(row);
                        x++;

                        total += (parseFloat(value['salesPrice']) + parseFloat(value['taxAmount'])).toFixed(value['transactionCurrencyDecimalPlaces'])  *  value['return_Qty'];
                        //total += parseFloat(value['totalValue']) + txAmnt;
                    ...
                    $('#invoice_table_tfoot').html('<tr> <td class="text-right" colspan="10"> Total</td><td class="text-right total">' + total.formatMoney(currency_decimal, '.', ',') + '</td><td></td></tr>')
                    $('#item_table_tfoot').html('<tr> <td class="text-right" colspan="10"> Total</td><td class="text-right total">' + total.formatMoney(currency_decimal, '.', ',') + '</td><td></td></tr>')
                }
    ...
    function save_sales_return_detail() {
        ...
        if (!jQuery.isEmptyObject(selected)) {
            $.ajax({
                ...
                type: 'post',
                dataType: 'json',
                data: {'invoiceDetailsAutoID': selected, 'salesReturnAutoID': salesReturnAutoID, 'qty': qty,'noofitems':noofitems,'grossqty':grossqty,'buckets':buckets, 'bucketweightID':bucketweightID,'bucketweight':bucketweight},
                url: "<?php echo site_url('Inventory/save_sales_return_detail_items_buyback'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
    ...
    function calbucketweight(buccketweight,id,returnval)
    {
     ...
     //   alert(returnval);
        select_check_box(qtynew,buccketweight, returnval);
