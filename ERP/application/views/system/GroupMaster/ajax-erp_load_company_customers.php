...
<?php
    foreach($companyID as $val){
       ...
       $customerval= get_group_customer_details($groupCustomerAutoID,$val);
       ?>
        <div class="row">
            ...
            <div class="form-group col-sm-5" >
                <div class="">
                    ...
                    if(!empty($customerval)){
                        echo form_dropdown('customerMasterID[]', dropdown_companyCustomers($val,$customerval['customerMasterID']), $customerval['customerMasterID'], 'class="form-control select2" id="customerMasterID_'.$val.'" required"');
                    }else{
                        echo form_dropdown('customerMasterID[]', dropdown_companyCustomers($val), '', 'class="form-control select2" id="customerMasterID_'.$val.'" required"');
                    }
                    ?>
                </div>
            </div>
