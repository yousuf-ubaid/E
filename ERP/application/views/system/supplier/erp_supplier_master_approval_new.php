...
<div class="tab-content">
    <div id="step1" class="tab-pane active">
        ...
        <hr>
        <div class="row hide" id="checklistdiv">

            <div class="form-group col-sm-4">
                <label for="supplierAddress1">Criticality</label>
                <span title="" rel="tooltip" class="fa fa-info-circle fa-1x fa-fw fc-1 fs-16" onclick="open_supplier_approval_checklist_model()" data-original-title="Approval Check List"></span>
            </div>
...
<div class="modal fade" id="assignApprovalChecklist_model" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="documentPageViewTitle">Criticality</h4>
            </div>
            <div class="modal-body">
            ...
            <div class="row" style="margin: 6px 0px;">
                        ...
                        <div class="box-tools">
                            <div class="has-feedback">
                                <input name="searchOrder" type="text" class="form-control input-sm"
                                       placeholder="Search"
                                       id="searchOrder" onkeyup="startMasterSearchApproval()">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <div id="assignApprovalChecklist_item_Content"></div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
...
<script type="text/javascript">
    ...
    var assignCheckListSyncApproval = [];
                ...
                success: function (data) {
                    if (!jQuery.isEmptyObject(data)) {
                        ...
                        if(data[2]=='2'){
                            ...
                            $('#checklistdiv').removeClass('hide');
                        }
                        //$('#supplier_btn').prop("disabled", true).val();
                        $('#confirmbtn').empty();
                        ...
                        $('#supplierCurrency').prop("disabled", true);
                        $('#suppliercountry').prop("disabled", true);
                        /*$('#liabilityAccount').prop("disabled", true).val();
                        $('#supplierCurrency').prop("disabled", true).val();
                        $('#suppliercountry').prop("disabled", true).val();*/
        ...
        if(supplierAutoID){
            $.ajax({
                ...
                success: function (data) {
                    ...
                    //     $('#approve_btn').prop("disabled", true).val();
                    //  }else{
                    //     $('#rejectbtn').prop("disabled", false).val();
                    //     $('#approve_btn').prop("disabled", false).val();
                    //  }
    ...
    function open_supplier_approval_checklist_model(search=null) {
       var search_index = '';
        ...
        $.ajax({
            ...
            type: 'post',
            dataType: 'html',
            data: {Search:search_index,documentID:'SUP',supplierAutoID:supplierAutoID},
            url: "<?php echo site_url('Supplier/assign_approval_checklist_view'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
       
                $('#assignApprovalChecklist_item_Content').html(data);
           
                $("#assignApprovalChecklist_model").modal({backdrop: "static"});
    ...
    function assign_approval_checklist_selected_check(sup) {
       
       var value = $(sup).val();
       if ($(sup).is(':checked')) {
           var inArray = $.inArray(value, assignCheckListSyncApproval);
           if (inArray == -1) {
            assignCheckListSyncApproval.push(value);
           }
       }
       else {
           var i = assignCheckListSyncApproval.indexOf(value);
           if (i != -1) {
            assignCheckListSyncApproval.splice(i, 1);
           }
       }
    }

    function startMasterSearchApproval(){ 
       // var itemAutoID =  $('#assignedSupplier_itemID').val();
        var search = $('#searchOrder').val();
        open_supplier_approval_checklist_model(search);
    }

    function assign_checklist() {
       
        if(supplierAutoID && assignCheckListSyncApproval.length>0){
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'json',
                data: {
                    'assignCheckListSyncApproval': assignCheckListSyncApproval,
                    'supplierAutoID':supplierAutoID,
                },
                url: "<?php echo site_url('Supplier/assignCheckListForSupplierApproval'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    ...
                    if (data[0] == 's') {
                        ...
                        assignCheckListSyncApproval =[];
                        open_supplier_approval_checklist_model();
                    } else {
    ...
    function delete_approval_checklist(id) {
        if (supplierAutoID && id) {
            swal({
                    title: "<?php echo $this->lang->line('common_are_you_sure');?>",/*Are you sure?*/
                    text: "<?php echo $this->lang->line('common_you_want_to_confirm_this_document');?>",/*You want to confirm this document*/
                    type: "warning",
                ...
                function () {
                    $.ajax({
                        ...
                        type: 'post',
                        dataType: 'json',
                        data: {'supplierAutoID': supplierAutoID,checklistID:id},
                        url: "<?php echo site_url('Supplier/delete_approval_checklist'); ?>",
                        beforeSend: function () {
                            startLoad();
                        },
                        success: function (data) {
    ...
    function findCurrentLevelUserAccess() {
       ...
       if(supplierAutoID){
           $.ajax({
               ...
               url: "<?php echo site_url('Supplier/findCurrentLevelUserAccess'); ?>",
               beforeSend: function () {
                   startLoad();
               },
               success: function (data) {
