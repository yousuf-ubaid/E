...
<div>
    <form role="form" id="jp_detail_add_form" class="form-horizontal">
        <input type="hidden" class="form-control" name="jpnumberadddetail" id="jpnumberadddetail" value="<?php echo $jpnumber ?>">
    ...
    <div class="row" style="margin-top: 10px;">
        <div class="form-group col-sm-12">
            <button class="btn btn-primary pull-right" type="button" id="save_btn" onclick="save_details()">Save</button>
        </div>
    </div>
    </div>
...
<script>
    ...
    function fetch_detail() {
        var jpnumber = $('#jpnumberadddetail').val();
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'json',
                ...
                success: function (data) {
                    ...
                    $.each(data['detail'], function (key, value) {
                        if (x == 0) {
                            ...
                            var arrive = '<div class="input-group datepicdetails arrivecls"><div class="input-group-addon"><i class="fa fa-calendar"></i></div> <input type="text" name="arrivedate[]"  value="'+value['dateArivedcon']+'" class="form-control"required </div>'

                            var artime = '<div class="input-group datetimepicker4 arivetimecls"><input type="text" class="form-control" value="' + value['timeArrive'] + '" name="arrivetime[]" /><span class="input-group-addon"><span class="glyphicon glyphicon-time"></span></span></div>'
                        ...
                        if (value['restTick'] == 1)
                        {
                           var selectstatus =  '<select name="restyn[]" class="form-control" tabindex="-1" aria-hidden="true"> <option value="1" selected>Yes</option> <option value="2">No</option></select>';
                        }else
                        {
                            var selectstatus =  '<select name="restyn[]" class="form-control" tabindex="-1" aria-hidden="true"> <option value="1" >Yes</option> <option value="2" selected>No</option></select>';
                        }
                        ...
                        if(value['dateDepart'] == " ")
                        {
                            var dateformatpolicy = '<div class="input-group datepicdetails"><div class="input-group-addon"><i class="fa fa-calendar"></i></div><input type="text" name="departdate[]"   value= "" class="form-control departdatecls"required></div>'
                        }else
                        {
                            var dateformatpolicy = '<div class="input-group datepicdetails"><div class="input-group-addon"><i class="fa fa-calendar"></i></div><input type="text" name="departdate[]"   value= "'+ value['dateDepartcon']+'" class="form-control departdatecls"required></div>'
                        }
                        if(value['timeDepart'] == " ")
                        {
                            var departtime = '<div class="input-group datetimepicker4"><input type="text" class="form-control departtimecls" value="" name="departtime[]" /><span class="input-group-addon"><span class="glyphicon glyphicon-time"></span></span></div>'
                        }else
                        {
                            var departtime = '<div class="input-group datetimepicker4"><input type="text" value="'+ value['timeDepart']+'" class="form-control departtimecls" name="departtime[]" /><span class="input-group-addon"><span class="glyphicon glyphicon-time"></span></span></div>'
                        }

                            $('#item_table_body').append('<tr><td class="text-center"><input type="text" class="form-control" value="'+ value['placeName'] +'" name="placenames[]"></td><td>'+arrive+'</td><td>' + artime + '</td><td>' + dateformatpolicy + '</td><td>'+ departtime +'</td><td>'+ selectstatus +'</td><td><input type="text" class="form-control" value="'+ value['sleep'] +'" name="sleepmotelname[]"></td><td class="remove-td" style="vertical-align: middle;text-align: center"> '+ deleterecordds +' </td></tr>');
                            x++;
                        });
                        ...
                        $('#item_table_body').append('<tr><td><input type="text" class="form-control" name="placenames[]"></td><td>'+arrive+'</td><td>' + artime + '</td><td>' + dateformatpolicy + '</td><td>'+ departtime +'</td><td>'+ selectstatus +'</td><td><input type="text" class="form-control"  name="sleepmotelname[]"></td><td class="remove-td" style="vertical-align: middle;text-align: center">'+ deleterecordds +'</td></tr>');
