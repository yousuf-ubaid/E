...
echo head_page('Journey Plan Approval', false);
?>
...
<div class="table-responsive">
    <table id="journey_plan_map_tour_table" class="<?php echo table_class() ?>">
        <thead>
        <tr>
            <th style="min-width: 5%">#</th>
            <th style="min-width: 10%">JP Code </th>
            <th style="min-width: 20%">Driver</th>
            <th style="min-width: 15%">Departure</th>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Journey Plan Approval</h4>
            </div>
                            ...
                            <div class="zx-tab-pane active" id="Tab-home-v">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        <input type="hidden" name="jurneyplanid" id="jurneyplanid">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/journeyplan/journey_plan_approval_map_tour', '', 'Journey Plan Approval')
        });
        journey_plan_map_tour_table();
        $('#pv_approval_form').bootstrapValidator({
            ...
            fields: {
                ...
                jurneyplanid: {validators: {notEmpty: {message: 'JP Auto Id is required.'}}},
    ...
    function journey_plan_map_tour_table() {
        Otable = $('#journey_plan_map_tour_table').DataTable({
            "language": {
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Journeyplan/jp_approval_map_tour'); ?>",
            "aaSorting": [[1, 'desc']],
            ...
            "columnDefs": [{"target": [0,7,8], "searchable": false}],
    ...
    function fetch_approval_map_tour(jurneyplanid, documentApprovedID, Level) {
        if (jurneyplanid) {
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'html',
                data: {'journeyPlanMasterID': jurneyplanid, 'html': true, 'approval': 1},
                url: "<?php echo site_url('Journeyplan/load_jp_view_tour'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    $('#jurneyplanid').val(jurneyplanid);
                    $('#documentApprovedID').val(documentApprovedID);
                    $('#Level').val(Level);
                    $("#return_approval_modal").modal({backdrop: "static"});
                    $('#conform_body').html(data);
                    $('#comments').val('');
                    customerInvoice_attachment_View_modal('JP', jurneyplanid);
                    stopLoad();
                    refreshNotifications(true);
                }, error: function () {
                    stopLoad();
