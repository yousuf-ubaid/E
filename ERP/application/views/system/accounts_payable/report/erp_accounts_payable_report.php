...
<div class="row">
    <div class="col-md-12">
        ...
        <input type="hidden" id="supplierIDpdf" name="supplierID" value="">
        <input type="hidden" id="supplierNamepdf" name="supplierName" value="">
...
<!--modal report-->
<div class="modal fade" id="ap_report_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
...
<!--modal report-->
<div class="modal fade" id="ap_report_drilldown_modal" tabindex="2" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" style="width: 95%" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel"><?php echo $this->lang->line('accounts_payable_reports_vl_drill_down');?><!--Drill Down--> - <span class="myModalLabel"></span></h4>
            </div>
            <div class="modal-body">
...
<div class="modal fade" id="ap_report_drilldown_modal_pdc" tabindex="2" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" style="width: 95%" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function(){
            fetchPage('system/accounts_payable/report/erp_accounts_payable_report','<?php echo $_POST["page_id"] ?>','<?php echo $_POST["page_name"] ?>');
        });
    ...
    function get_accounts_receivable_filter() {
        $.ajax({
            ...
            url: "<?php echo site_url('Report/get_accounts_payable_filter'); ?>",
            beforeSend: function () {
    ...
    function generateDrilldownReport(supplierID,supplierName,currency,age) {
        var fieldNameChk = [];
        $("input[name=fieldName]:checked").each(function () {
            fieldNameChk.push({name: "fieldNameChk[]", value: $(this).val()});
        });
        ...
        finalArray.push({name:"supplierID",value:supplierID});
        finalArray.push({name:"supplierName",value:supplierName});
        ...
        $.ajax({
            ...
            beforeSend: function () {
                startLoad();
                $('#supplierIDpdf').val(supplierID);
                $('#supplierNamepdf').val(supplierName);
            ...
            success: function (data) {
                ...
                $(".myModalLabel").html(supplierName);
                $('#ap_report_drilldown_modal').modal("show");
            }, error: function () {
                alert('<?php echo $this->lang->line('common_an_error_occurred_Please_try_again');?>.');/*An Error Occurred! Please Try Again*/
                stopLoad();
            }
    ...
    function generateDrilldownReportPDC(supplierID,supplierName,currency,amount,currencycode) {
        var fieldNameChk = [];
        $("input[name=fieldName]:checked").each(function () {
            fieldNameChk.push({name: "fieldNameChk[]", value: $(this).val()});
        });
        ...
        finalArray.push({name:"supplierName",value:supplierName});
        finalArray.push({name:"currency",value:currency});
        finalArray.push({name:"amount",value:amount});
        finalArray.push({name:"currencycode",value:currencycode});
        $.ajax({
            async: true,
            type: 'POST',
            dataType: 'html',
            data: finalArray,
            url: "<?php echo site_url('Report/vendoragingsummary_pdc'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                stopLoad();
                $("#reportpdcDrilldown").html(data);
                $(".myModalLabel").html(supplierName);
                $('#ap_report_drilldown_modal_pdc').modal("show");
            }, error: function () {
                alert('<?php echo $this->lang->line('common_an_error_occurred_Please_try_again');?>.');/*An Error Occurred! Please Try Again*/
                stopLoad();
            }
