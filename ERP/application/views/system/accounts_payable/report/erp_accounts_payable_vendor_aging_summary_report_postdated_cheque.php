            ...
            if (!empty($output)) { ?>
                <div class="fixHeader_Div">
                    <table class="borderSpace report-table-condensed" id="tbl_report">
                        ...
                        $grandTotal = array();
                        $grandTotal_PDCtotal = array();
                        $grandTotal_PDCtotal_toal = array();
                        if (!empty($category)) {
                                ...
                                if ($isTransCost) {
                                    foreach ($mainCategory as $key2 => $curr) {
                                        ...
                                        foreach ($curr as $key3 => $supplier) {
                                            ...
                                            $total_pdc = 0;
                                            $total_pdc_new_tot = 0;
                                            $total_pdc_new_tot_amt = 0;
                                            $total += $supplier["current"];

                                            $total_pdc += $supplier['transactionAmountPDC'];
                                            $total_pdc_new_tot_amt += $total_pdc;

                                            $total_pdc += $supplier["current"];
                                            //$total_pdc_new_tot += $total;
                                            ...
                                            /* if ($isRptCost) {
                                                 $grandTotal_PDCtotal["companyReportingAmountPDC"][] = $supplier["companyReportingAmountPDC"];
                                             }
                                             if ($isLocCost) {
                                                 $grandTotal_PDCtotal["companyLocalAmountPDC"][] = $supplier["companyLocalAmountPDC"];
                                             }*/
                                            ...
                                            $grandTotal_PDCtotal["totalPDC"][] = $total_pdc;
                                            $grandTotal_PDCtotal_toal['total'][] = $total - $total_pdc_new_tot_amt;
                                            $transTotal['totalPDC'][] = $total_pdc_new_tot_amt;


                                            echo "<td class='text-right'>" . number_format($total, $supplier["DecimalPlaces"]) . "</td>";

                                            if ($fieldName[0] == 'companyLocalAmount') {

                                                echo '<td class="text-right"><a href="#" class="drill-down-cursor" onclick="generateDrilldownReportPDC(\'' . $supplier["supplierID"] . '\',\'' . htmlspecialchars($supplierName) . '\',\'' . $fieldName[0] . '\',\'' . $this->input->post('through') . '\',\'' . $supplier["currency"] . '\')">' . number_format($supplier['companyLocalAmountPDC'], $supplier["DecimalPlaces"]) . '</a></td>';
                                                //echo "<td class='text-right'>" . number_format($supplier["companyLocalAmountPDC"], $supplier["DecimalPlaces"]) . "</td>";
                                            } else if ($fieldName[0] == 'transactionAmount') {
                                                echo '<td class="text-right"><a href="#" class="drill-down-cursor" onclick="generateDrilldownReportPDC(\'' . $supplier["supplierID"] . '\',\'' . htmlspecialchars($supplierName) . '\',\'' . $fieldName[0] . '\',\'' . $this->input->post('through') . '\',\'' . $supplier["currency"] . '\')">' . number_format($supplier['transactionAmountPDC'], $supplier["DecimalPlaces"]) . '</a></td>';
                                            } else if ($fieldName[0] == 'companyReportingAmount') {
                                                echo '<td class="text-right"><a href="#" class="drill-down-cursor" onclick="generateDrilldownReportPDC(\'' . $supplier["supplierID"] . '\',\'' . htmlspecialchars($supplierName) . '\',\'' . $fieldName[0] . '\',\'' . $this->input->post('through') . '\',\'' . $supplier["currency"] . '\')">' . number_format($supplier['companyReportingAmountPDC'], $supplier["DecimalPlaces"]) . '</a></td>';
                                            }


                                            if ($fieldName[0] == 'companyLocalAmount') {

                                                echo "<td class='text-right'>" . number_format(($total - $supplier['companyLocalAmountPDC']), $supplier["DecimalPlaces"]) . "</td>";

                                            } else if ($fieldName[0] == 'transactionAmount') {
                                                echo "<td class='text-right'>" . number_format(($total - $supplier['transactionAmountPDC']), $supplier["DecimalPlaces"]) . "</td>";

                                            } else if ($fieldName[0] == 'companyReportingAmount') {
                                                echo "<td class='text-right'>" . number_format(($total - $supplier['companyReportingAmountPDC']), $supplier["DecimalPlaces"]) . "</td>";
                                        ...
                                        echo "<td class='text-right reporttotal'>" . format_number(array_sum($transTotal["totalPDC"]), $decimalPlace) . "</td>";
                                        echo "<td class='text-right reporttotal'>" . format_number(array_sum($transTotal["total"]) - array_sum($transTotal["totalPDC"]), $decimalPlace) . "</td>";
                                        echo '</tr>';
                                    }
                                } else {
                                    foreach ($mainCategory as $key2 => $supplier) {
                                        ...
                                        $total += $supplier["current"];
                                        if($isRptCost) {
                                            $total_pdc+= $supplier["companyReportingAmountPDC"];
                                            $total_pdc_new_tot_amt += $total_pdc;
                                        }
                                        if($isLocCost) {

                                            $total_pdc+= $supplier["companyLocalAmountPDC"];
                                            $total_pdc_new_tot_amt += $total_pdc;
                                        }
                        ...
                        if ($isRptCost || $isLocCost) {
                            ...
                            if ($isRptCost) {
                                echo "<td class='text-right reporttotal'>" . format_number(array_sum($grandTotal_PDCtotal["totalPDC"]), $this->common_data['company_data']['company_reporting_decimal']) . "</td>";
                            }
                            if ($isLocCost) {
                                echo "<td class='text-right reporttotal'>" . format_number(array_sum($grandTotal_PDCtotal["totalPDC"]), $this->common_data['company_data']['company_default_decimal']) . "</td>";
                            }

                            if ($isRptCost) {
                                echo "<td class='text-right reporttotal'>" . format_number(array_sum($grandTotal_PDCtotal_toal['total']), $this->common_data['company_data']['company_reporting_decimal']) . "</td>";
                                //echo "<td class='text-right'>" . number_format($total_pdc_new_tot_amt, $supplier["DecimalPlaces"]) . "</td>";
                            }
                            if ($isLocCost) {
                                echo "<td class='text-right reporttotal'>" . format_number(array_sum( $grandTotal_PDCtotal_toal['total']), $this->common_data['company_data']['company_default_decimal']) . "</td>";
                                //echo "<td class='text-right'>" . number_format($total_pdc_new_tot_amt, $supplier["DecimalPlaces"]) . "</td>";
                            }
