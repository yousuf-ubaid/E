...
$documentAdd = $extra['master']['documentID'] . '-ADD';
?>
                        ...
                        <table>
                            <tr>
                                <td colspan="3">
                                    ...
                                    <h4><?php echo 'SRN' === $extra['master']['documentID'] ? 'Service Received Note' : $this->lang->line('transaction_common_grv_voucher');?> </h4><!--Goods Received Voucher-->
                                </td>
                            </tr>
                            <tr>
                                <td><strong><?php echo 'SRN' === $extra['master']['documentID'] ? 'SRN Number' : $this->lang->line('transaction_common_grv_number') ; ?> </strong></td><!--GRV Number-->
                                <td><strong>:</strong></td>
                                <td><?php echo $extra['master']['grvPrimaryCode']; ?></td>
                            </tr>
                            <tr>
                                <td><strong><?php echo 'SRN' === $extra['master']['documentID'] ? 'SRN Date' : $this->lang->line('transaction_common_grv_date') ; ?> </strong></td><!--GRV Date-->
                                <td><strong>:</strong></td>
...
<div class="table-responsive">
    <?php if($extra['master']['grvType'] === 'LOG') { ?>
    <table id="add_new_grv_table" class="<?php echo table_class(); ?>">
        ...
        <tbody>
            <?php
            if (!empty($logisticDetail)) {
                for ($i = 0;
                     $i < count($logisticDetail);
                     $i++) {

                    echo '<tr>';
                    echo '<td>' . ($i + 1) . '</td>';
                    echo '<td>' . $logisticDetail[$i]['purchaseOrderCode'] . '</td>';
                    echo '<td class="text-right">' . $logisticDetail[$i]['addonAmount'] . '</td>';
                    echo '<td class="text-right">' . $logisticDetail[$i]['addonBalance'] . '</td>';
                    echo '<td class="text-right">' . $logisticDetail[$i]['matchedAmount'] . '</td>';
                    echo '<td class="text-right">' . $logisticDetail[$i]['actualLogisticAmount'] . '</td>';
                    echo '</tr>';
                }
            } else {
    ...
    <table id="add_new_grv_table" class="<?php echo table_class(); ?>">
        <thead>
            ...
            <?php } ?>
            <tr>
                <th class='theadtr' style="min-width: 5%">1</th>
                ...
                <?php if ($hideCost == 0) { ?>
                        ...
                        <th class='theadtr' style="min-width: 12%">Ordered <?php echo $this->lang->line('transaction_common_qty');?> </th><!--Unit Cost-->
                    <?php if ($show_cost_grv == 1) { ?>
                ...
                <?php } else {?>
                    ...
                    <th class='theadtr' style="min-width: 5%"><?php echo $extra['master']['documentID'] ?> Qty</th><!--Qty-->
                <?php }?>
                <?php if ($show_cost_grv == 1) { ?>
                    <?php if ($hideCost == 0) { ?>
        ...
        <tbody id="grv_table_body">
            ...
            if (!empty($extra['detail'])) {
                for ($i=0; $i < count($extra['detail']); $i++) {
                    ...
                    echo '<td>'; ?>
                          <?php  if ($extra['master']['grvType'] == 'PO Base') { ?>
                        <a  onclick="requestPageView_model('PO', <?php echo $extra['detail'][$i]['purchaseOrderMastertID'] ?>)"> <?php echo $extra['detail'][$i]['itemSystemCode']; ?></a>
                          <?php  }
                            else{  ?>
                               <?php echo $extra['detail'][$i]['itemSystemCode'];
                            }
                    echo  '</td>';
                    echo '<td>'.$extra['detail'][$i]['itemDescription'];
                    if($advanceCostCapturing == 1){ 
                        echo '<td class="text-center">'.$extra['detail'][$i]['activityCodeName'];
                    }
                    ...
                    $purchaseTax = 0;
                    if($extra['detail'][$i]['purchaseQty'] != 0){
                        $purchaseTax = $extra['detail'][$i]['purchaseTax'] / $extra['detail'][$i]['purchaseQty'];
                    }

                    if($purchaseTax  > 0){
                        $extra['detail'][$i]['requestedAmount'] = $extra['detail'][$i]['requestedAmount'] - $purchaseTax;
                    }
                    ...
                    if($isGroupBasedTaxEnable == 1){ 
                        ...
                        if($extra['detail'][$i]['taxAmount'] > 0) {
                            echo ' <td style="min-width: 5%;text-align:right" class="groupByTaxEnable">
                                <a onclick="open_tax_dd(null,'.$extra['detail'][$i]['grvAutoID'].',\'' .$extra['master']['documentID'] .'\','. $extra['master']['transactionCurrencyDecimalPlaces'].' ,'. $extra['detail'][$i]['grvDetailsID'].', \'srp_erp_grvdetails\',\'grvDetailsID\',0,1) ">'. number_format($extra['detail'][$i]['taxAmount'], $extra['master']['transactionCurrencyDecimalPlaces']).'</a>          
                            </td>';
                        }else {
        ...
        <tfoot>
            <tr>
                <td class="text-right sub_total" colspan="<?php echo 7 - $show_cost_grv ?>"><?php echo $this->lang->line('transaction_ordered_item_total');?> <span class="currency"> (<?php echo $extra['master']['transactionCurrency']; ?>)</span></td><!--Ordered Item Total-->
...
<br>
<?php
    $document = array($extra['master']['documentID'],$documentAdd);
    $rcmApplicableYN = 0;
...
<div class="table-responsive">
  ...
  <a onclick="open_tax_dd(null,'.$value['grvAutoID'].',\'' . $documentAdd . '\','. $value['bookingCurrencyDecimalPlaces'].' ,'. $value['id'].', \'srp_erp_grv_addon\',\'id\',0,1) ">'. number_format($value['taxAmount'],$value['bookingCurrencyDecimalPlaces']).'</a></td>';
                                    }else {
...
<script>
    ...
    de_link="<?php echo site_url('Double_entry/fetch_double_entry_grv'); ?>/<?php echo $extra['master']['grvAutoID'] ?>/<?php echo $extra['master']['documentID'] ?>";
