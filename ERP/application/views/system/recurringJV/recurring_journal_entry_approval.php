...
$title = $this->lang->line('finance_rj_recurring_journal_voucher');
echo head_page($title, false);

/*echo head_page('Recurring Journal Voucher Approval', false); */?>
...
<div class="table-responsive">
    <table id="recurring_journal_entry_approval_table" class="<?php echo table_class(); ?>">
        <thead>
        <tr>
            <th style="min-width: 5%">#</th>
            <th style="min-width: 10%"><?php echo $this->lang->line('finance_common_rjv_code');?><!--RJV Code--></th>
            <th style="min-width: 40%"><?php echo $this->lang->line('finance_common_rjv_details');?><!--RJV Details--></th>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"><?php echo $this->lang->line('finance_rj_recurring_journal_entry_approval');?><!--Journal Entry Approval--></h4>
            </div>
                            ...
                            <div class="zx-tab-pane active" id="Tab-home-v">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        <input type="hidden" name="RJVMasterAutoId" id="RJVMasterAutoId">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('.headerclose').click(function () {
            fetchPage('system/recurringJV/recurring_journal_entry_approval', '', 'Recurring Journal Entry Approval');
        });
        ...
        $('#JE_approval_form').bootstrapValidator({
            ...
            fields: {
                ...
                RJVMasterAutoId: {validators: {notEmpty: {message: '<?php echo $this->lang->line('finance_rj_recurring_journal_entry_id_is_required');?>.'}}},/*Recurring Journal Entry ID is required*/
    ...
    function recurring_journal_entry_table() {
         Otable = $('#recurring_journal_entry_approval_table').DataTable({
             "language": {
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Recurring_je/fetch_recurring_journal_entry_approval'); ?>",
            "aaSorting": [[1, 'desc']],
            ...
            "aoColumns": [
             ...
             "columnDefs": [{"targets": [6], "orderable": false},{"visible":false,"searchable": true,"targets": [7,8,9,10,11] },{"visible":true,"searchable": false,"targets": [0,2,4,5,6] }],
    ...
    function fetch_approval(RJVMasterAutoId, documentApprovedID, Level) {
        if (RJVMasterAutoId) {
            $.ajax({
                async: true,
                type: 'post',
                dataType: 'html',
                data: {'RJVMasterAutoId': RJVMasterAutoId, 'html': true,'approval': 1},
                url: "<?php echo site_url('Recurring_je/recurring_journal_entry_conformation'); ?>",
                beforeSend: function () {
                    startLoad();
                },
                success: function (data) {
                    $('#RJVMasterAutoId').val(RJVMasterAutoId);
                    $('#documentApprovedID').val(documentApprovedID);
                    $('#Level').val(Level);
                    $("#jv_modal").modal({backdrop: "static"});
                    $('#conform_body').html(data);
                    $('#comments').val('');
                    journalEntry_attachment_View_modal('RJV', RJVMasterAutoId);
                    stopLoad();
                    refreshNotifications(true);
                }, error: function () {
                    stopLoad();
