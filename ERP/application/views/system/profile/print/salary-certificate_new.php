...
<div style="font-size: 14px; font-family: Calibri">
    ...
    <p style="padding: 50px 20px; text-align:justify; font-size: 14px">
        <?php
        echo 'This  is  to  certify  that <strong>'.$repData['empTitle'].' '.$repData['Ename2'].'</strong>, '.$repData['identity_no'].',
        is working with '.current_companyName(true).'  in the capacity of <strong>'.$repData['DesDescription'].'</strong> since 
    <strong>'.date('dS F Y', strtotime($repData['EDOJ'])).'</strong>. '.$he_she.'  is  drawing  a  total monthly salary of 
    <strong>'.(($currency == 'OMR')? 'R.O': $currency).'. '.number_format($total, $decimal).' ( '.$in_word.' and '.$fraction.'/'.$divisionBy.' ).</strong>';
        ?>
    </p>

    <p style="padding: 0px 20px 0px"> This certificate is issued at <?php echo $his_her ?> request without holding the company any responsibility. </p>
