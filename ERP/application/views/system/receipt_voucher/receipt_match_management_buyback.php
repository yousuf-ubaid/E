    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Receipt Matching Template</h4>
            </div>
            <div class="modal-body">
                <input type="hidden" id="receiptVoucherAutoId">
                <div class="row">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary btn-sm" onclick="print_rvm_temp()">Print</button>
...
function receipt_match_table(selectedID=null){
     Otable = $('#receipt_match_table').DataTable({
        ...
        "bDestroy": true,
        "bStateSave": true,
        "sAjaxSource": "<?php echo site_url('Receipt_voucher/fetch_receipt_match_buyback'); ?>",
        "aaSorting": [[0, 'desc']],
...
function load_printtemp(voucherAutoID){
        $('#printSize').val(1);
        $('#receiptVoucherAutoId').val(voucherAutoID);
        $('#print_temp_modal').modal('show');
    }

    function print_rvm_temp(){
        var printSize =  $('#printSize').val();
        var rvmID = $('#receiptVoucherAutoId').val();

        if(rvmID==''){
            myAlert('e', 'Select Print Type');
        }else{
            window.open("<?php echo site_url('Receipt_voucher/load_rv_match_conformation_buyback') ?>" +'/'+ rvmID +'/'+ printSize +'/'+1);
        }
    }   
</script>