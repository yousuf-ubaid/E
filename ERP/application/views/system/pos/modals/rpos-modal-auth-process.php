...
<script>
    ...
    function po_auth_modal_close() {
        ...
        if ($('#pos_auth_processMasterID').val() == 10) {
            ...
            var discountSource = localStorage.getItem('discountSource');
            if(discountSource == 'P'){
                var oldDiscountPercentage = localStorage.getItem('oldDiscountPercentage');
                var oldDiscountPercentageId = localStorage.getItem('discountPercentageId');
                $(oldDiscountPercentageId).val(oldDiscountPercentage);
            }else if(discountSource == 'A'){
                var oldDiscountPercentage = localStorage.getItem('oldDiscountAmount');
                var oldDiscountPercentageId = localStorage.getItem('discountAmountId');
                $(oldDiscountPercentageId).val(oldDiscountPercentage);
            }
            $('#pos_auth_modal').modal('hide');
            hideAuthModal()
        } else if ($('#pos_auth_processMasterID').val() == 22) {
            var oldValue = localStorage.getItem('oldQty');
            var qtyInputId=localStorage.getItem('qtyInputId');
    ...
    function submitUserPosAuthProcess() {
        ...
        var authProcessId = data[0].value;
        $.ajax({
            type: 'POST',
            dataType: 'json',
    ...
    function loadPosProcess(processMasterID, parameter = null, parameter2 = null) {
        switch (parseInt(processMasterID)) {
            ...
            case 22:
                ...
                break;
            case 23:
                item_wise_discount(parameter,parameter2);
                break;
            case 24:
                if(parameter=='updateDiscountPers'){
                    updateDiscountPers();
                }else if(parameter=='backCalculateDiscount'){
                    backCalculateDiscount();
                }
                break;
            case 25:
                beforeDeleteItem(parameter);
                break;
