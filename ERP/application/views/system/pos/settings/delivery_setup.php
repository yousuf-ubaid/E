...
<?php
$customerType = delivery_customer_type();
...
<script>
    ...
    function addcustomer() {
        //this is the fuction that called in discount promotion screen.
        $.ajax({
            type: "POST",
    ...
    function fetchCustomerType() {
        $('#tbl_customerType').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": false,
            "sAjaxSource": "<?php echo site_url('Pos_config/fetch_delivery_setup'); ?>",
            "aaSorting": [[0, 'asc']],
            "aoColumnDefs": [{"bSortable": false, "aTargets": [2, 4, 5, 6, 7, 8]}],
            /*"language": {
            ...
            "fnServerData": function (sSource, aoData, fnCallback) {
                aoData.push({"name": "posType", "value": 1});// 1 = RPOS
                $.ajax({
                    'dataType': 'json',
                    'type': 'POST',
                    'url': sSource,
    ...
    function edit_customerType(id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#customerTypeMasterID').val(data['customerTypeMasterID']).change();
                ...
                $('#chargeBasedOn').val(data['chargeBasedOn']).change();


                $('#ownDeliveryChargeBasedOn').val(data['ownDeliveryBasedOn']).change();
                if(data['ownDeliveryBasedOn']==1){
                    $("#commissionPercentageDiv").hide();
                    var commissionAmount = parseFloat(data['commissionAmount']);
                    commissionAmount = commissionAmount.toFixed(2);
                    $("#commissionAmount").val(commissionAmount);
                }else if(data['ownDeliveryBasedOn']==1){
                    $("#commissionAmountDiv").hide();
                    $("#commissionPercentage").val(data['commissionAmount']);
                }
    ...
    <div class="modal-dialog" style="min-width: 60%; margin-top: 6px;">
        <div class="modal-content"> <!-- <div class="color-line"></div>-->
            ...
            <form role="form" id="fromcustomer" class="form-horizontal">
                ...
                <input type="hidden" class="form-control" id="posType" name="posType" value="1">
                            ...
                            <fieldset>
                                ...
                                <div class="form-group clsDeliveryType" id="onlyForDelivery" style="display: none;">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div id='infoicon'><i class="fa fa-info-circle" aria-hidden="true"
                                                              onclick="showinfo()"></i></div>
                                ...
                                <div class="form-group RevenueGLDropdown">
                                    <label class="col-md-4 control-label" for="liabilityGLAutoID">
                                        Revenue GL
                                    </label>
                                    <div class="col-md-7">
                                        <?php echo form_dropdown('RevenueGLAutoID', $chartOfAccount, '', ' id="RevenueGLAutoID"  class="form-control input-md select2"'); ?>
                                    </div>
                                </div>
                                <div class="form-group" id="ownDeliveryChargeBasedOnDiv">
                                    <label class="col-md-4 control-label" for="liabilityGLAutoID">
                                        Charge based on
                                    </label>
                                    <div class="col-md-4">
                                        <select id="ownDeliveryChargeBasedOn" name="ownDeliveryChargeBasedOn" class="form-control input-md select2"
                                                onchange="ownDeliveryChargeOnchange.call(this)">
                                            <option value="">Please Select</option>
                                            <option value="1">Based on Amount</option>
                                            <option value="2">Based on Percentage</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group" id="commissionAmountDiv">
                                    <label class="col-md-4 control-label" for="">
                                        Amount <?php required_mark(); ?></label>
                                    <div class="col-md-4">
                                        <input type="number" step="any" class="form-control input-md"
                                               id="commissionAmount"
...
<script>
    function checkOnTimePayment(tmpThisVal) {
        ...
        $("#commissionAmountDiv").hide();
        $("#ownDeliveryChargeBasedOnDiv").hide();

        $("#commissionPercentage").attr('readonly', false);
        ...
        if (tmpValue == 1) {
            ...
            $("#commissionPercentage").attr('readonly', true);
        } else if (tmpValue == 4) {
            $(".clsDeliveryType").hide();
            ...
            $(".clsDeliveryType").hide();
            $("#isOnTimePayment").val('');
            $("#ownDeliveryChargeBasedOnDiv").show();
            $("#ownDeliveryChargeBasedOn").val('');
            $("#ownDeliveryChargeBasedOn").trigger('change');
            $("#RevenueGLAutoID").val(0);
            $("#RevenueGLAutoID").trigger('change');
    ...
    function ownDeliveryChargeOnchange(e) {
       var type = this.value;
       if(type==1){
           $("#commissionPercentageDiv").hide();
           $("#commissionAmountDiv").show();
