...
<script>
    ...
    function editMenu(id, categoryID) {
        //clearing data in add menu dialog box.
        $('.chkBox').iCheck('uncheck');
        $('.chkBox').iCheck('update');
        //-------------------------------------
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data['error'] == 0) {
                    ...
                    //$("#preparationTime").val(data['preparationTime']);
                    load_pricing(id);
                    load_warehouses(id);
                }
            },
    ...
    function load_warehouses(menuMasterID) {
        $.ajax({
            type: 'POST',
            dataType: 'JSON',
            url: "<?php echo site_url('Pos_config/get_warehouses_with_menu_assigned_status'); ?>",
            ...
            success: function (data) {
                ...
                data.map(function (item) {
                    ...
                    warehouseListElement += '   <div class="skin skin-square checkBoxPad">' +
                        '                                            <div class="skin-section extraColumns">' +
                        '                                                <input ' + checked + ' id="warehouseEdit_' + item.wareHouseAutoID + '"' +
                        '                                                       type="checkbox"' +
                        '                                                       class="columnSelected chkBox editwh"' +
                        '                                                       name="outlets[' + item.wareHouseAutoID + ']"' +
                        '                                                       value="' + item.wareHouseAutoID + '">' +
                        '                                              <label for="warehouseEdit_' + item.wareHouseAutoID + '">' +
                        item.wareHouseCode + '-' + item.wareHouseDescription + '-' + item.wareHouseLocation +
                        '                                                </label>' +
                        '                                            </div>' +
                        '                                        </div>';
                });
                $("#warehouseListForEdit").html(warehouseListElement);
                $('.editwh').iCheck({
                    checkboxClass: 'icheckbox_square_relative-orange',
                    radioClass: 'iradio_square_relative-orange',
                    increaseArea: '20%'
            ...
            <div class="modal-body" id="" style="min-height: 200px;">
                <form id="fromAddMenuCategory" name="fromAddMenuCategory" class="form-horizontal" method="post">
                    ...
                    <div class="form-group" id="inheritColorDiv">
                        ...
                        <div class="col-md-8" style="margin-top: 5px; margin-bottom: 10px;">
                            <input type="checkbox" id="inactive_subcat" name="inactive_subcat" value="1">
                            &nbsp; <span style="color: blue;">(Note : Applies to all Subcategories & Menus)</span>
                        </div>
                    </div>
                    <hr>
                    <div class="form-group">
            ...
            <div class="modal-body" id="" style="min-height: 200px;">
                <form id="fromAddMenu" name="fromAddMenu" class="form-horizontal" method="post">
                    ...
                    <!--<div class="form-group">
                        ...
                        <div class="col-md-4">
                            <input type="number" step="any" name="preparationTime" id="preparationTime" class="form-control"/>
                        </div>
                    </div>-->
                    ...
                    <div id="warehouseEdit" class="form-group">
                        <div class="col-md-12">
                            <div class="form-separator">
                                <?php echo $this->lang->line('pos_config_outlets'); ?><!--Outlets--></div>
                            <label class="col-md-3 control-label" for="checkboxes">
                                <?php echo $this->lang->line('pos_config_applicable_outlets'); ?><!--Applicable Outlets--> </label>
                            <div class="col-md-6" id="warehouseListForEdit">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="from_addMenuTax" name="from_addMenuTax" class="form-horizontal" method="post">
                        ...
                        <div class="col-md-5">
                            ...
                            <select name="taxmasterID" class="form-control" id="tax_taxmasterID"
                                ...
                                foreach ($taxes as $tax) {
                                    echo '<option data-category="' . $tax['taxCategory'] . '" data-p="' . $tax['taxPercentage'] . '" value="' . $tax['taxMasterAutoID'] . '">' . $tax['taxDescription'] . ' (' . $tax['taxShortCode'] . ') -  ' . $tax['taxPercentage'] . '%' . '</option>';
                                }
                                ?>
                    ...
                    <div class="form-group" id="vatTypeDiv" style="display: none;">
                        <label for="vatType" class="col-md-3 control-label">
                            VAT Type</label>
                        <div class="col-md-5">
...
<!-- ADD VAT -->
<div class="modal fade pddLess" data-backdrop="static" id="add_menuVatModal" tabindex="-2" role="dialog">
    <div class="modal-dialog" style="min-width: 50%; margin-top: 6px;">
        <div class="modal-content">
            <div class="modal-header modal-header-mini" style="padding: 5px 10px;">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <h4 class="modal-title">Apply VAT Percentage</h4></div>
            <div class="modal-body">
                <form id="from_addVat" name="from_addVat" class="form-horizontal" method="post">
                    <input type="hidden" value="0" id="menuCat_ID" name="menuCat_ID">
                    <div class="form-group">
                        <label for="menuVatPercentage" class="col-md-3 control-label">
                            VAT Percentage
                        </label>
                        <div class="col-md-8">
                            <input type="number" id="menuVatPercentage" name="menuVatPercentage"
                                                                             class="form-control"/>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="vatType" class="col-md-3 control-label">
                            VAT Type</label>
                        <div class="col-md-5">
                            <?php echo form_dropdown('vatType2', vat_type_dropdown(), '', 'class="form-control select2 vatType" id="vatType2" required'); ?>
                        </div>
                    </div>
            ...
            <div class="modal-footer" style="padding: 5px 10px;">
                ...
                <button class="btn btn-sm btn-primary" type="button" onclick="add_vat();">
                    <?php echo $this->lang->line('common_save'); ?><!--Save--></button>
            </div>
        </div>
    </div>
...
<script>
    ...
    function loadTexPercentage() {
        ...
        if ($("#tax_taxmasterID option:selected").val() !== '') {
            ...
            var category = $("#tax_taxmasterID option:selected").data('category');
            if(category==2){
                $("#vatTypeDiv").show();
            }else{
                $("#vatTypeDiv").hide();
    ...
    function checkBCodeDup(e) {
        var barcode_in = e.value;
        $.ajax({
            async: true,
            type: 'get',
            dataType: 'json',
            url: "<?php echo site_url('Pos_config/search_barcode?q='); ?>" + barcode_in,
            cache: false,
            beforeSend: function () {
            ...
            success: function (data) {
                if (data == null) {
                    ...
                    //$("#barcode").val("");
                    myAlert('e', 'Duplicate Barcode Entered.')
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                stopLoad();
    ...
    function open_add_taxModal(id){
        $("#from_addVat")[0].reset();
        $('#add_menuVatModal').modal('show');
        $('#menuCat_ID').val(id);
        $("#vatType2").val('').change();
    ...
    function add_vat(){
        var formData = new FormData($("#from_addVat")[0]);
        $.ajax({
            type: 'POST',
            dataType: 'JSON',
            url: "<?php echo site_url('Pos_config/add_menuVat'); ?>",
            data: formData,
            contentType: false,
            cache: false,
            processData: false,
            ...
            success: function (data) {
                ...
                if (data['error'] == 0) {
                    LoadMenuDataTable($("#menuCat_ID").val());
                    myAlert('s', data['message']);
                    $("#from_addVat")[0].reset();
                    $('#add_menuVatModal').modal('hide');
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                stopLoad();
