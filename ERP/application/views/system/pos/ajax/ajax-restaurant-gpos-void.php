...
<script>
    ...
    function loadVoidOrder() {
        $('#voidListDT').DataTable({
            ...
            "bServerSide": true,
            "bDestroy": true,
            "sAjaxSource": "<?php echo site_url('Pos/loadVoidOrders'); ?>",
            "aaSorting": [[0, 'desc']],
    ...
    function loadVoidOrderHistory() {
        $('#voidListHist').DataTable({
            ...
            "bServerSide": true,
            "bDestroy": true,
            "sAjaxSource": "<?php echo site_url('Pos/loadVaoidOrderHistory'); ?>",
            "aaSorting": [[0, 'desc']],
            "fnDrawCallback": function (oSettings) {
                if (oSettings.bSorted || oSettings.bFiltered) {
                    for (var i = 0, iLen = oSettings.aiDisplay.length; i < iLen; i++) {
    ...
    function loadPrintTemplateVoid_gpos(invID, invCode, doSysCode_refNo) {
        $.ajax({
            async: true,
            type: 'post',
            html: 'json',
            data: {'doSysCode_refNo': doSysCode_refNo,'isVoid': 1},
            url: "<?php echo site_url('Pos/invoice_print'); ?>/" + invID + "/" + invCode,
            success: function (data) {
                $('#print_template').modal({backdrop: 'static'});
                $('#print_content').html(data);
                $("#gpos_open_void_receipt").modal('hide');
                $("#gposvoidbillbtn").removeClass('hidden');
                $("#voidhnid").val(invID);
            }, error: function (jqXHR, textStatus, errorThrown) {
    ...
    function loadPrintTemplateVoid_gpos_history(invID, invCode, doSysCode_refNo) {
        $.ajax({
            async: true,
            type: 'post',
            html: 'json',
