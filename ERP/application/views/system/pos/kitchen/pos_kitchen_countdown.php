<link rel="stylesheet" type="text/css" href="<?php echo base_url('plugins/pos/pos.css') ?>">
<link rel="stylesheet" type="text/css" href="<?php echo base_url('plugins/slick/slick/slick.css') ?>">
<link rel="stylesheet" type="text/css" href="<?php echo base_url('plugins/slick/slick/slick-theme.css') ?>">
<link rel="stylesheet" href="<?php echo base_url('plugins/bootstrap-switch/bootstrap-switch.min.css'); ?>">
<style>
    .completedList li {
        font-size: 15px;
        /*list-style: square;*/
        margin: 3px;
        min-height: 10px;
    }

    .title {
        background-color: #050500;
        color: #e5de14;
        padding: 5px;
        border-radius: 2px;
        margin: 0px;

    }

    .title2 {
        background-color: #050500;
        color: #e53a45;
        padding: 5px;
        border-radius: 2px;
        margin: 0px;

    }

    table > tr {
        border: 1px dashed black !important;
    }

    .detailList {
        margin-left: 10px;
        font-size: 25px;
        font-weight: 700 !important;
        font-family: Calibri;
    }

    .detailList img {
        height: 25px !important;
        width: 25px !important;

    }

    .btnSwitchCustom {
        /*padding: 2px;
        background-color: #ffffff;
        margin: -5px -4px 0px 0px;*/
    }
</style>
<?php
$this->load->view('include/header', $title);
//$this->load->view('include/top');

$primaryLanguage = getPrimaryLanguage();
$this->lang->load('pos_restaurent', $primaryLanguage);
$this->lang->load('common', $primaryLanguage);
$this->lang->load('calendar', $primaryLanguage);
?>


<div class="row">
    <div class="col-md-12">
        <input type="hidden" class="alarm_tracker" value="<?php echo count($pendingOrders);?>" />
        <div id="orderListContainer">
            <div class="row">
                <div class="col-md-12">


                    <h4 class="text-yellow text-center"><i class="fa fa-cutlery"></i>
                        <?php echo $this->lang->line('posr_pending_order'); ?><!--Pending Order-->
                    </h4>

                    <!--Pending Orders -->
                    <table class="table table-condensed" id="pendingOrderTbl">
                        <tbody>
                        <?php
                        if (!empty($pendingOrders)) {
                            $titleID = 0;
                            $i = 0;
                            foreach ($pendingOrders as $pendingOrder) {
                                if ($titleID != $pendingOrder['menuSalesID']) {
                                    $i++;
                                    ?>
                                    <tr>
                                        <td>
                                            <!--<div class="media">
                                                <div class="media-body">
                                                    <h4 class="title">
                                                        <i class="fa fa-tag"></i>
                                                        <span style="font-weight: 800">
                                                            <?php /*echo number_pad($pendingOrder['menuSalesID'], 5) */?>
                                                        </span>

                                                    </h4>
                                                </div>
                                            </div>-->
                                            <div class="media">
                                                <div class="media-body">
                                                    <h4 class="title">
                                                        <i class="fa fa-circle-o-notch fa-spin" aria-hidden="true"></i>
                                                        <span style="font-weight: 800">
                                                            <?php echo number_pad($pendingOrder['menuSalesID'], 5) ?>
                                                        </span>
                                                    </h4>
                                                </div>
                                            </div>
                                        </td>
                                        <td><input class="mySwitch" type="checkbox" value="1"
                                                   id="isPax_<?php echo $pendingOrder['menuSalesID'] ?>" name="pending"
                                                   onchange="updateToCurrent(<?php echo $pendingOrder['menuSalesID'] ?>,this)"
                                                   data-size="small"
                                                   data-on-text="<i class='fa fa-cutlery text-red'></i> Cook"
                                                   data-on-color="default"
                                                   data-handle-width="50"
                                                   data-off-color="default"
                                                   data-off-text="<i class='fa fa-check text-green'></i> &nbsp;"
                                                   data-label-width="0"></td>
                                        <td class="title">
                                            <span class="pull-right"
                                                  style="font-weight: 800"><?php echo $this->lang->line('posr_qyt'); ?><!--QTY--></span>
                                        </td>

                                    </tr>
                                    <?php
                                }
                                $titleID = $pendingOrder['menuSalesID'];
                                ?>
                                <tr>
                                    <td colspan="3">
                                        <div class="detailList">
                                            <img src="<?php echo base_url($pendingOrder['menuImage']) ?>"
                                                 style="height: 40px; width: 40px;" alt="">
                                            <?php echo $pendingOrder['menuMasterDescription']; ?>
                                            <?php echo $pendingOrder['qty']; ?>
                                        </div>
                                        <div class="pull-right">
                                            <?php echo $pendingOrder['qty']; ?>
                                        </div>
                                    </td>
                                </tr>
                                <?php
                            }
                        }
                        ?>
                        </tbody>
                    </table>
                </div>


            </div>
        </div>

    </div>
</div>


<?php
$data['notFixed'] = true;
$this->load->view('include/footer', $data);
?>

<script type="text/javascript" src="<?php echo base_url('plugins/keyboard-short-cut/shortcut.js'); ?>"></script>
<script type="text/javascript" src="<?php echo base_url('plugins/pos/pos.js'); ?>"></script>
<script src="<?php echo base_url('plugins/slick/slick/slick.js') ?>" type="text/javascript"></script>
<script src="<?php echo base_url('plugins/printJS/jQuery.print.js') ?>" type="text/javascript"></script>
<script type="text/javascript"
        src="<?php echo base_url('plugins/bootstrap-switch/bootstrap-switch.min.js'); ?>"></script>
<script type="text/javascript">
    $(document).ready(function () {
        kitchenQue();
        $(".mySwitch").bootstrapSwitch();
    });

    function updateToCurrent(id, tmpVal, kotID) {
        var valueTmp = $(tmpVal).is(":checked");
        if (valueTmp) {
            var cook = 1;
        } else {
            var cook = 0;
        }
        var date = new Date();
        var hours = date.getHours();
        var dts = date.getFullYear()+'-'+date.getMonth()+'-'+date.getDate();
        var minutes = date.getMinutes();
        var currdtTime = dts +" "+hours + ':' + minutes +':'+ date.getSeconds();
        $.ajax({
            type: 'POST',
            dataType: 'JSON',
            url: "<?php echo site_url('Pos_kitchen/updateToCurrentCountdown'); ?>",
            data: {menuSalesID: id, value: cook, kotID: kotID, currdtTime: currdtTime},
            cache: false,
            beforeSend: function () {
                $("#loader").show();
            },
            success: function (data) {
                $("#loader").hide();
                if (data['error'] == 0) {
                    refreshOrderListContainer(0)
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                myAlert('e', '<br>Message: ' + errorThrown);
            }
        });
        return false;
    }

    function updateToCompleted(id, tmpVal, kotID) {
        var valueTmp = $(tmpVal).is(":checked");
        if (valueTmp) {
            var cook = 1;
        } else {
            var cook = 0;
        }

        $.ajax({
            type: 'POST',
            dataType: 'JSON',
            url: "<?php echo site_url('Pos_kitchen/updateToCompleted'); ?>",
            data: {menuSalesID: id, value: cook, kotID: kotID},
            cache: false,
            beforeSend: function () {
                $("#loader").show();
            },
            success: function (data) {
                $("#loader").hide();
                if (data['error'] == 0) {
                    refreshOrderListContainer(0)
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                myAlert('e', '<br>Message: ' + errorThrown);
            }
        });
        return false;
    }

    function kitchenQue(repeat=1) {
        <?php
        $locationID = $this->uri->segment('2');
        if ($locationID == 0) {
            $defaultLocation = get_kitchenLocation_default();
            if (!empty($defaultLocation)) {
                $locationID = $defaultLocation['kitchenLocationID'];
            }
        }
        ?>
        var date = new Date();
        var hours = date.getHours();
        var dts = date.getFullYear()+'-'+date.getMonth()+'-'+date.getDate();
        var minutes = date.getMinutes();
        var currdtTime = dts +" "+hours + ':' + minutes +':'+ date.getSeconds();
        var kotID = '<?php echo $locationID ?>';
        $.ajax({
            type: 'POST',
            dataType: 'html',
            url: "<?php echo site_url('Pos_kitchen/kitchen_countdown_process_ajax'); ?>",
            data: {kotID: kotID,currdtTime: currdtTime},
            cache: false,
            beforeSend: function () {
                $("#loader").show();
            },
            success: function (data) {
                $("#loader").hide();
                $("#orderListContainer").html(data);
                if (repeat == 1) {
                    setTimeout(function () {
                        kitchenQue();
                    }, 5000);
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                myAlert('e', '<br>Message: ' + errorThrown);
            }
        });
        return false;
    }

    function refreshOrderListContainer(param=1) {
        kitchenQue(param);
        return false;
        /** $.ajax({
            type: 'POST',
            dataType: 'html',
            url: "<?php echo site_url('Pos_kitchen/kitchen_manual_process_ajax'); ?>",
            data: {id: null},
            cache: false,
            beforeSend: function () {
                $("#loader").show();
            },
            success: function (data) {
                $("#loader").hide();
                $("#orderListContainer").html(data);
            },
            error: function (jqXHR, textStatus, errorThrown) {
                myAlert('e', '<br>Message: ' + errorThrown);
            }
        });
         return false;*/
    }

</script>