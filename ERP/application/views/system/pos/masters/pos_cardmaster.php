...
<style>
    ...
    .nmpd-grid {
        -webkit-box-shadow: 0px 6px 41px -4px rgba(0, 0, 0, 0.65) !important;
        -moz-box-shadow: 0px 6px 41px -4px rgba(0, 0, 0, 0.65) !important;
        box-shadow: 0px 6px 41px -4px rgba(0, 0, 0, 0.65) !important;
    }
</style>
...
<div class="modal" id="counterCreate_model" role="dialog" data-keyboard="false">
...
<script type="text/javascript">
    ...
    function open_newCounterModel() {
        ...
        counterCreate_model.modal('show');
        $('.submitBtn').prop('disabled', false);
        btnHide('saveBtn', 'updateBtn');
    }

    function editGiftCard(cardMasterID) {
        /*setTimeout(function () {
            $("#barcode").focus();
        }, 500);
        giftCard_form[0].reset();
        ...
        $.ajax({
            ...
            url: '<?php echo site_url('Pos_giftCard/editGiftCard'); ?>',
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                ...
                $('#cardMasterID').val(cardMasterID);
                $('#isConform').val(0);
                $('#outletID').val(data['outletID']).change();
                $('#cardExpiryInMonths').val(data['cardExpiryInMonths']);
