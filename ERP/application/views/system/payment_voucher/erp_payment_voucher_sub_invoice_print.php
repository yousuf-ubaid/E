            ...
            <div class="table-responsive">
                ...
                $transaction_total = 0;
                    ...
                    foreach ($val['invoices'] as $invoice) { 
                        ...
                        if($invoice['allocation_amount'] < 0.01){
                            continue;
                        }
                        ...
                        ?>
                            <tr>
                                ...
                                <td style="text-align:center;"><?php echo $invoice['supplierInvoiceNo']; ?></td>
                                <td style="text-align:center;"><?php echo date('Y-m-d',strtotime($invoice['invoiceDate'])); ?></td>
                        ...
                        $invoiced_amount = round($invoice['allocation_amount'],$extra['master']['transactionCurrencyDecimalPlaces']);

                        if($invoice['invoiceType'] == 'SupplierInvoice'){
                            $transaction_total += $invoiced_amount;
                            $grand_total += $invoiced_amount;
                        }else{
                            $transaction_total -= $invoiced_amount;
                            $grand_total -= $invoiced_amount;
                        }
                ...
                <tfoot>
                    <tr>
                        <td colspan="5" style="text-align:right;font-size:10px;"><b><?php echo $this->lang->line('common_total'); ?></b></td>
...
<div class="table-responsive">
    <h5 class="text-right"><?php echo $this->lang->line('common_grand_total'); ?><!--Grand Total-->
        ...
        : <?php if($extra['master']['rrvrID'] && $grand_total == 0){
            echo format_number($extra['master']['transactionAmount'], $this->common_data['company_data']['company_default_decimal']);
        }else{
            echo format_number($grand_total, $this->common_data['company_data']['company_default_decimal']);
        } ?></h5>
</div>
