...
if (!isset($pdf)) {
    ...
    <button type="button" id="btn_print_sales3" class="btn btn-default btn-xs"> <i
        ...
        <a href="" class="btn btn-excel btn-xs" id="btn-excel" download="Sales_Report.xls"
           onclick="var file = tableToExcel('container_sales_report4', 'Sales Report'); $(this).attr('href', file);">
            <i class="fa fa-file-excel-o" aria-hidden="true"></i> Excel
        </a>
</span>
<?php } ?>
<link rel="stylesheet" href="<?php echo base_url('plugins/pos/gpos-reports.css'); ?>"/>
<div id="container_sales_report4">
    ...
    <div class="row">
        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            <div class="outletInfo">
                <?php
                $outletInput = $this->input->post('void_outletID_f');
            ...
            if (isset($reportData) && !empty($reportData)) {
                ...
                foreach ($reportData as $data_item) {
                    ...
                    <tr>
                        ...
                        ?>
                        <td class="ac">
                            <button class="btn btn-xs btn-link" onclick="VoidInvoicePrint('<?php echo $data_item['invoiceID'] ?>', '<?php echo $data_item['documentSystemCode']?>', 'PNT')">  View bill
                            </button>
                        </td>
                    </tr>
                    <?php
...
<div aria-hidden="true" role="dialog" tabindex="-1" id="print_template_void" data-keyboard="false" class="modal"
     style="display: none;">
...
<script>
    ...
    function VoidInvoicePrint(invID, invCode, doSysCode_refNo) {
        $.ajax({
            async: true,
            type: 'post',
            html: 'json',
            data: {'doSysCode_refNo': doSysCode_refNo, 'receipt':1,'isVoid': 0},
            url: "<?php echo site_url('Pos/invoice_print'); ?>/" + invID + "/" + invCode,
            success: function (data) {
                $('#print_content_void').html(data);
                $('#print_template_void').modal({backdrop: 'static'});
            }, error: function (xhr) {
