...
$usergroup_arr = all_pos_usergroup(2);
...
<script>
    ...
    function addProcess() {
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Pos/addProcess') ?>",
    ...
    function fetchAuthProcess() {
        $('#tbl_process').DataTable({
            ...
            "bDestroy": true,
            "bStateSave": true,
            "sAjaxSource": "<?php echo site_url('Pos/fetch_auth_process'); ?>",
            "aaSorting": [[1, 'asc']],
            /*"language": {
    ...
    function fetchUserGroup(processMasterID) {
        $('#tbl_usergroupaut').DataTable({
            ...
            "fnServerData": function (sSource, aoData, fnCallback) {
                ...
                aoData.push({name: 'posType', value: 2});
                $.ajax({
                    'dataType': 'json',
                    'type': 'POST',
                    'url': sSource,
    ...
    function add_user_group() {
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Pos/add_user_group') ?>",
    ...
    function delete_Aut_process(id){
        swal({
            ...
            function () {
                $.ajax({
                    ...
                    type: 'post',
                    dataType: 'json',
                    data: {processMasterID: id,posType: 2},
                    url: "<?php echo site_url('Pos/delete_Aut_process'); ?>",
                    beforeSend: function () {
                        startLoad();
                    },
                    success: function (data) {
    ...
    function assign_capamnt_percentage(processMasterID,processAssignID) {
        $.ajax({
            async: true,
            type: 'post',
            dataType: 'json',
            data: {processAssignID: processAssignID},
            url: "<?php echo site_url('Pos/assign_capamnt_percentage'); ?>",
            beforeSend: function () {
                startLoad();
            },
            success: function (data) {
                stopLoad();
                $('#discprocessAssignID').val(processAssignID);
                $('#capAmount').val(data['capAmount']);
                $('#capPercentage').val(data['capPercentage']);
                $("#discount_config_modal").modal('show');
                if(processMasterID==19){
                    $('.capamnt').addClass('hidden');
                }else{
                    $('.capamnt').removeClass('hidden');
                }
            }, error: function () {
                alert('An Error Occurred! Please Try Again.');
                stopLoad();
    ...
    function updateDiscountConfig() {
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Pos/updateDiscountConfig') ?>",
            ...
            success: function (data) {
                ...
                myAlert(data[0],data[1]);
                if (data[0] == 's') {
                    $("#discount_config_modal").modal('hide');
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                stopLoad();
                ...
                <div class="modal-body" style="min-height: 80px; ">
                    <div class="row">
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-top: 10px">
                            <table class="<?php echo table_class_pos(1) ?>" id="tbl_usergroupaut"
                                   style="font-size:12px;">
                                <thead>
                                <tr>
                                    <th>#</th>
...
<div class="modal fade pddLess" data-backdrop="static" id="discount_config_modal" role="dialog">
    <div class="modal-dialog" style="min-width: 10%; margin-top: 6px;">
        <div class="modal-content"> <!-- <div class="color-line"></div>-->
            <div class="modal-header modal-header-mini" style="padding: 5px 10px; ">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true"
            ...
            <form role="form" id="discountConfig" class="form-horizontal">
                <input type="hidden" id="discprocessAssignID" name="processAssignID">
                <div class="modal-body" style="min-height: 80px; ">
                    <div class="row">
                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <fieldset>
                                <div class="form-group capamnt">
                                    <label class="col-md-4 control-label"
                                           for="customerName">Cap Amount</label>
                                    <div class="col-md-6">
                                        <input type="number" class="form-control " id="capAmount" name="capAmount">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-md-4 control-label"
                                           for="customerName">Percentage</label>
                                    <div class="col-md-6">
                                        <input type="number" class="form-control " id="capPercentage" name="capPercentage">
                                    </div>
                                </div>
                            </fieldset>
                        </div>
                ...
                <div class="modal-footer" style="padding: 5px 10px;">
                    <button type="button" onclick="updateDiscountConfig()" class="btn btn-primary btn-xs"><i class="fa fa-check"
                                                                                                   aria-hidden="true"></i>
                        <?php echo $this->lang->line('common_add'); ?><!--Add-->
                    </button>
                    <button type="button" class="btn btn-default btn-xs" data-dismiss="modal">
