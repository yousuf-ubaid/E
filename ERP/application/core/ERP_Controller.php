...
class ERP_Controller extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        if ($this->controllerName == 'cron' || $this->controllerName == 'supplierportal') {
            return True;
        }
        ...
        if (!$CI->session->has_userdata('status')) {
            ...
            $data['title'] = 'Login';
            $data['type'] = 'e';
            return $this->load->view(getLoginPage(), $data);
        }
        else {
            ...
            if(empty($resultDb2)){
                if($this->input->is_ajax_request() ){ /*Only for ajax request this function will work*/
                    die('<script type="text/javascript">check_session_status();</script>');
                }
            ...
            $this->db->where('company_id', $resultDb2["company_id"])->update('srp_erp_company', [
            ...
            $companylogo = $this->s3->createPresignedRequest('images/logo/'.$this->common_data['company_data']['company_logo'], '+24 hour');
            $companylogo_secondary = $this->s3->createPresignedRequest('images/logo/'.$this->common_data['company_data']['company_secondary_logo'], '+24 hour');

            $this->common_data['company_policy'] = $company_policy;
            $this->common_data['company_data']['company_logo'] = $companylogo;
            $this->common_data['company_data']['company_secondary_logo'] = $companylogo_secondary;
            ...
            $this->common_data['isGroupUser'] = trim($CI->session->userdata("isGroupUser") ?? '');
            ...
            $this->common_data['emplanglocationid'] = trim($CI->session->userdata("emplanglocationid") ?? '');
            $this->common_data['userType'] = trim($CI->session->userdata("userType") ?? '');
            $this->common_data['loginusername'] = trim($CI->session->userdata("loginusername") ?? '');
            $this->common_data['supportToken'] = trim($CI->session->userdata("supportToken") ?? '');
        }

        // Refresh CSRF token before handling any requests
    ...
    * @return void
    */
    public function sendResponse(string $status, string $message, mixed $data = []): void
    {
