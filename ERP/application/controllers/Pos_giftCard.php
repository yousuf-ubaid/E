...
class Pos_giftCard extends ERP_Controller
{
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!$cardIssueID) {
                ...
                if (!empty($cardMaster)) { /*cardMasterID*/
                    ...
                    $result_topUp = $this->topUpGiftCard(true);
                    ...
                    if ($this->db->trans_status() === FALSE) {
                        ...
                        $topUp_result = (array)json_decode($result_topUp);
                        if (!empty($topUp_result) && $topUp_result['error'] == 0) {
                            //print_r($topUp_result);
                            $output = array('error' => 0, 'message' => 'Card successfully issued.', 'receiptID' => $topUp_result['receiptID'], 'barcode' => $barCode);
                            $output = array_merge($topUp_result, $output);
                        } else {
                            $output = array('error' => 0, 'message' => 'Card successfully issued.', 'receiptID' => $SN, 'barcode' => $barCode);
                        }
    ...
    function loadHistoryGiftCard()
    {
        ...
        $this->datatables->select('topUpCard.cardTopUpID as cardTopUpID,topUpAmount, menuSalesID,outlet.wareHouseDescription as outlet,  outlet.wareHouseCode as wHouseCode, topUpCard.createdDateTime as createdDateTime,isRefund', false)
    ...
    function load_customer_name_for_telephone_no()
    {
        ...
        $customer = $this->db->select('CustomerName,posCustomerAutoID')
            ->from('srp_erp_pos_customermaster')
            ->where('customerTelephone', $telephone)
            ->get()->row_array();
        if (!empty($customer)) {
            echo json_encode(array('error' => 0, 'message' => 'done', 'CustomerName' => $customer['CustomerName'], 'posCustomerAutoID' => $customer['posCustomerAutoID']));
        } else {
    ...
    function editGiftCard(){
        echo json_encode($this->Pos_gift_card->editGiftCard());
    }

    function refundGiftCard($return = false)
    {
        $this->form_validation->set_rules('barcode', 'Bar Code', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($customerInfo)) {
                ...
                if(count($paymentTypes)==1 && $glConfigMasters[array_keys($paymentTypes)[0]] == 1){
                    $paymentAmount = array_values($paymentTypes)[0];
                    $key = array_keys($paymentTypes)[0];
                    $glConfigMasterID = $glConfigMasters[$key];
                    if ($paymentAmount > 0 && $glConfigMasterID == 1) {
                            $data_card['giftCardReceiptID'] = $SN;
                            $data_card['cardMasterID'] = $customerInfo['cardMasterID'];
                            $data_card['barCode'] = $barCode;
                            $data_card['posCustomerAutoID'] = $customerInfo['posCustomerAutoID'];
                            $data_card['topUpAmount'] = abs($paymentAmount) * (-1);
                            $data_card['points'] = 0;
                            $data_card['glConfigMasterID'] = $glConfigMasterID;
                            $data_card['glConfigDetailID'] = $key;
                            $data_card['menuSalesID'] = 0;
                            $data_card['isRefund'] = 1;
                            ...
                            $insert_id = $this->db->insert_id();
                            if ($insert_id > 0) {
                                $this->Pos_gift_card->insert_double_entries_giftCard_refund($insert_id);
                            }
                    }
                    if (!empty($data_card)) {
                        ...
                        if ($return) {
                            return json_encode(array('error' => 1, 'message' => 'Please enter the payment/s and refund again!'));
                        } else {
                            echo json_encode(array('error' => 1, 'message' => 'Please enter the payment/s and refund again!'));
                        }
                    ...
                    echo json_encode(array('error' => 1, 'message' => 'Use only cash for refund'));exit();
                }

                if ($return) {
                    return json_encode(array('error' => 0, 'message' => 'Refund completed!', 'receiptID' => $SN, 'barcode' => $barCode));
                } else {
                    echo json_encode(array('error' => 0, 'message' => 'Refund completed!', 'receiptID' => $SN, 'barcode' => $barCode));
                }
