...
class Dashboard extends ERP_Controller
{
    ...
    {
        $data['main_content'] = 'system/system_home';
    ...
    {
        $this->load->view('errors/html/coming_soon');
    }
    ...
    {
        $className = trim($this->input->post('className') ?? '');
        $tmp = mb_strpos($className, 'light-mode');
        if ($tmp != false) {
            delete_cookie("SIDE_BAR_MODE");
        } else {
            $cookie_arr['name'] = 'SIDE_BAR_MODE';
            $cookie_arr['value'] = 'dark-mode';
            $cookie_arr['expire'] = (3600 * 12);
            $cookie_arr['domain'] = '';
            $cookie_arr['path'] = '/';
            $cookie_arr['secure'] = false;
