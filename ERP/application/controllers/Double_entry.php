...
class Double_entry extends ERP_Controller
{
    ...
    function fetch_double_entry_grv()
    {
        ...
        $grvMaster = $this->Grv_modal->getGrvById($masterID);

        if('LOG' === $grvMaster['grvType'])
        {
            $doubleEntryFunction =  $this->Double_entry_model->fetch_double_entry_logistic_grv_data($masterID, $code);
        }
        else{
            $doubleEntryFunction =  $this->Double_entry_model->fetch_double_entry_grv_data($masterID, $code);
            $isGroupByPolicyExists =    $data['isGroupBasedTaxEnable'] = (existTaxPolicyDocumentWise('srp_erp_grvmaster',$masterID,'GRV','grvAutoID')!=''?existTaxPolicyDocumentWise('srp_erp_grvmaster',$masterID,'GRV','grvAutoID'):0);
            if($isGroupByPolicyExists == 1){
                $doubleEntryFunction = $this->Double_entry_model->fetch_double_entry_grv_data_groupbased($masterID, $code);
            }
        }

        $data['extra'] = $doubleEntryFunction;
    ...
    function fetch_double_entry_supplier_invoices()
    {
        ...
        if($master['documentOrigin']=='CINV'){
            $data['extra'] = $this->Double_entry_model->fetch_double_entry_supplier_invoices_insurance_data($masterID, $code);
        }else{
    ...
    function fetch_double_entry_customer_invoice()
    {
        $masterID = ($this->uri->segment(3)) ? $this->uri->segment(3) : trim($this->input->post('masterID') ?? '');
        $code = ($this->uri->segment(4)) ? $this->uri->segment(4) : trim($this->input->post('code') ?? '');
        $retantnINV = ($this->uri->segment(5)) ? $this->uri->segment(5) : 0;

        $this->db->select('tempInvoiceID,invoiceType,retensionInvoiceID');
        $this->db->where('invoiceAutoID', $masterID);
        $master = $this->db->get('srp_erp_customerinvoicemaster')->row_array();

        if(empty($master['retensionInvoiceID']) && $master['invoiceType'] == 'Project'){
            $data['extra'] = $this->Double_entry_model->fetch_double_entry_customer_invoice_data_opr($masterID, $code);
        }else{
            if(!empty($master['tempInvoiceID'])){
                ...
                }elseif($master['invoiceType'] == 'Insurance') {
                    $data['extra'] = $this->Double_entry_model->fetch_double_entry_customer_invoice_data_insurance($masterID, $code);
                }else{
    ...
    /*Sales Return Buyback - Added*/
    function fetch_double_entry_sales_return_buyback()
    {
        $masterID = ($this->uri->segment(3)) ? $this->uri->segment(3) : trim($this->input->post('masterID') ?? '');
        $code = ($this->uri->segment(4)) ? $this->uri->segment(4) : trim($this->input->post('code') ?? '');
        $data['extra'] = $this->Double_entry_model->fetch_double_entry_sales_return_buyback_data($masterID, $code);
        $data['customer']  = 'Customer Name';
        $html = $this->load->view('system/double_entry/erp_double_entry_print2', $data, true);
        if ($this->input->post('html')) {
            echo $html;
    ...
    function fetch_double_entry_receipt_voucher_suom()
    {
        $masterID = ($this->uri->segment(3)) ? $this->uri->segment(3) : trim($this->input->post('masterID') ?? '');
        $code = ($this->uri->segment(4)) ? $this->uri->segment(4) : trim($this->input->post('code') ?? '');
        $data['extra'] = $this->Double_entry_model->fetch_double_entry_receipt_voucher_suom_data($masterID, $code);
        $data['customer'] = 'Customer Name';
        $html = $this->load->view('system/double_entry/erp_double_entry_print', $data, true);
        if ($this->input->post('html')) {
            echo $html;
    ...
    function fetch_double_bulk_transfer()
    {
        $masterID = ($this->uri->segment(3)) ? $this->uri->segment(3) : trim($this->input->post('masterID') ?? '');
        $code = ($this->uri->segment(4)) ? $this->uri->segment(4) : trim($this->input->post('code') ?? '');
        $data['extra'] = $this->Double_entry_model->fetch_double_entry_bulk_transfer_data($masterID, $code);
        $html = $this->load->view('system/double_entry/erp_double_entry_print', $data, true);
        if ($this->input->post('html')) {
            echo $html;
        } else {
    ...
    function fetch_double_entry_receipt_match()
    {
        $masterID = ($this->uri->segment(3)) ? $this->uri->segment(3) : trim($this->input->post('masterID') ?? '');
        $code = ($this->uri->segment(4)) ? $this->uri->segment(4) : trim($this->input->post('code') ?? '');
        $data['extra'] = $this->Double_entry_model->fetch_double_entry_receipt_match_data($masterID, $code);
        $data['customer'] = 'Customer Name';
        $html = $this->load->view('system/double_entry/erp_double_entry_print', $data, true);
        if ($this->input->post('html')) {
            echo $html;
