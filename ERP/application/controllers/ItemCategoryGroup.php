...
class ItemCategoryGroup extends ERP_Controller{
    ...
    function save_subcategory()
    {
        ...
        $policyItem = getPolicyValues('PRF', 'All');

        if ($policyItem)
        {
            $this->form_validation->set_rules('codePrefix', 'Prefix', 'trim|required|max_length[6]');
        }
    ...
    function update_subcategory(){
        ...
        $this->form_validation->set_rules('description', 'Sub Category', 'trim|required');
        $policyItem = getPolicyValues('PRF', 'All');
    ...
    function load_all_companies_item_subcategories(){
        $company=array();
        $groupItemCategoryID=$this->input->post('groupItemCategoryID');
        $comp = customer_company_link($groupItemCategoryID);
        foreach($comp as $val){
        ...
        $data['companyID']=$company;
        $data['groupItemCategoryID']=$groupItemCategoryID;
        $html = $this->load->view('system/GroupItemCategory/ajax/erp_load_company_sub_itemcategory', $data, true);
        echo $html;
    }
    ...
    function save_sub_item_category_link()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo json_encode($this->Itemcategorygroup_model->save_sub_item_category_link());
        }
    }
    ...
    function add_buyers_to_category(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo json_encode($this->Itemcategorygroup_model->add_buyers_to_category());
        }
    }
    ...
    function load_subcategory_added_buyers()
    {
        ...
        if($type==1){
           ...
           $this->datatables->where('srp_erp_incharge_assign.groupID', $companyID);
    ...
    function delete_category_assign_buyers_group(){
        ...
        echo json_encode($this->Itemcategorygroup_model->delete_category_assign_buyers_group());
    }
