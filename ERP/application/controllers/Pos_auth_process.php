...
class Pos_auth_process extends ERP_Controller
{
    ...
    function check_has_gpos_auth_process(){
        echo json_encode($this->Pos_auth_process_model->check_has_gpos_auth_process());
    }

    function check_gpos_auth_process(){
        if($this->input->post('type') == 1){
            $this->form_validation->set_rules('username', 'Username', 'required');
            $this->form_validation->set_rules('password', 'Password', 'required');
        }else{
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            echo json_encode($this->Pos_auth_process_model->check_gpos_auth_process());
        }
    }

    function check_discount_capamnt(){
        echo json_encode($this->Pos_auth_process_model->check_discount_capamnt());
    }
