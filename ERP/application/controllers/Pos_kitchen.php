...
--REVISION HISTORY
--Date: 24 - JAN 2017 : comment started
...
class Pos_kitchen extends ERP_Controller
{
    ...
    function refreshOrderListContainer_autoPrint()
    {
       ...
       // $pendingOrders = $this->Pos_kitchen_model->get_pendingOrders_countdown_kitchen2($kotID);
        ...
        if (!empty($pendingOrders)) {
            foreach ($pendingOrders as $pendingOrder) {
                $data['masters'] = $this->Pos_kitchen_model->get_srp_erp_pos_menusalesmaster_kitchenStatus($pendingOrder['menuSalesID']);
                $data['invoiceList'] = $this->Pos_kitchen_model->get_srp_erp_pos_menusalesitems_invoiceID_kotAlarm($pendingOrder['menuSalesID'], $kotID);
                $data['waiterName'] = $this->Pos_kitchen_model->getWaiterNameByMenuSalesID($pendingOrder['menuSalesID']);
              //var_dump($data['invoiceList']);
                $data['print'] = false;
                $data['newBill'] = false;

                if ($show_price) {
                    $html .= $this->load->view('system/pos/printTemplate/restaurant-pos-dotmatric-printer-pdf-with-price', $data, true);
                } else {
    ...
    function load_KOT_print_view()
    {
        ...
        if (!empty($result)) {
            ...
            $show_price = is_show_price_in_KOT_print();
            if ($show_price) {
                $html = $this->load->view('system/pos/printTemplate/restaurant-pos-dotmatric-printer-kot-with-price', $data, true);
            } else {
    ...
    function get_hold_reference()
    {
        $menuSalesID = $this->input->post('menuSalesID');
        if ($menuSalesID) {
            $r = $this->db->select('holdRemarks')
                ->from('srp_erp_pos_menusalesmaster')
                ->where('menuSalesID', $menuSalesID)
                ->get()->row('holdRemarks');
            $r = !empty($r) ? $r : '';
            echo json_encode(array('error' => 0, 'reference' => $r));
        } else {
            echo json_encode(array('error' => 1, 'reference' => ''));
        }

    }

    function save_hold_reference()
    {
        $remarks = trim($this->input->post('reference') ?? '');
        $menuSalesID = $this->input->post('menuSalesID');
        $this->db->where('menuSalesID', $menuSalesID);
        $this->db->update('srp_erp_pos_menusalesmaster', array('holdRemarks' => $remarks));
        echo json_encode(array('error' => 0, 'reference' => ''));
    ...
    function kitchen_countdown_process()
    {
        $locationID = $this->uri->segment('2');
        if ($locationID == 0) {
            $defaultLocation = get_kitchenLocation_default();
        ...
        $this->load->view('system/pos/kitchen/pos_kitchen_countdown', $data);
    }

    function kitchen_countdown_process_ajax()
    {
        $kotID = $this->input->post('kotID');
        $data['currdtTime'] = $this->input->post('currdtTime');

        $data['pendingOrders'] = $this->Pos_kitchen_model->get_pendingOrders_countdown_kitchen2($kotID);
        $data['currentOrders'] = $this->Pos_kitchen_model->get_currentOrders_kitchen2($kotID);
        $this->load->view('system/pos/kitchen/ajax/ajax-pos-kitchen-refresh-countdown', $data);
    }

    function updateToCurrentCountdown()
    {
        ...
        if (!empty($isMenuExistForAnotherKitchen)) {
            /** There is item in the kitchen has to be processed */
            //$result = $this->updateMenuSalesItemKitchenStatus();
            $result = $this->updateToCountDownCurrent_status();
            if ($result) {
                echo json_encode(array('error' => 0, 'message' => 'updated', 'q' => $this->db->last_query()));
            } else {
                echo json_encode(array('error' => 1, 'message' => 'update fail'));
            ...
            $result = $this->Pos_kitchen_model->progressOrder_Countdown_master($menuSalesID);
    ...
    function updateToCountDownCurrent_status()
    {
        $KOT_ID = $this->input->post('kotID');
        $menuSalesID = $this->input->post('menuSalesID');
        $q = "UPDATE `srp_erp_pos_menusalesitems` SET `isOrderInProgress` = 1, isOrderCompleted =1 WHERE `menuSalesID` = '" . $menuSalesID . "' AND `kotID` = '" . $KOT_ID . "' AND isOrderPending = 1 AND isOrderInProgress=0";
        return $this->db->query($q);
    }
}