 ...
 *
 * -- =============================================
 * -- File Name : Pos_general_master.php
 * -- Project Name : POS
 * -- Module Name : POS General
 * -- Create date : 04 June 2018
 * -- Description : Pos general master controller .
 *
 * --REVISION HISTORY
 * --Date: 04-June 2018 : file created
 * -- =============================================
 **/
defined('BASEPATH') OR exit('No direct script access allowed');

class Pos_general_master extends ERP_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Pos_general_master_model');
    ...
    function loadCompanyOutlets()
    {
        $this->datatables->select('srp_erp_warehousemaster.wareHouseAutoID as wareHouseAutoID,srp_erp_warehousemaster.companyCode as companyCode,wareHouseCode,wareHouseDescription,wareHouseLocation, IF(srp_erp_warehousemaster.isActive=1,"<span class=\'label label-success\'>Active</span>","<span class=\'label label-default\'>In-Active</span>") as outletStatus, warehouseAddress as address, concat(srp_erp_segment.segmentCode, " - ",srp_erp_segment.description) as segmentDescription')
            ...
            ->join('srp_erp_segment', 'srp_erp_segment.segmentID = srp_erp_pos_segmentconfig.segmentID', 'left')
            ->where('srp_erp_pos_segmentconfig.isGeneralPOS', 1)
    ...
    function edit_warehouse()
    {
        $id = $this->input->post('id');
        if ($id != "") {
            echo json_encode($this->Pos_general_master_model->get_warehouse());
        } else {
    ...
    function save_outlet()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $result = $this->Pos_general_master_model->save_outlet();
            echo json_encode($result);
            /*if ($result) {
