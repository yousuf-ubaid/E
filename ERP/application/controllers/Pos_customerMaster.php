...
class Pos_customerMaster extends ERP_Controller
{
    ...
    {
        $this->datatables->select('posCustomerAutoID, IF(rowTbl.CustomerAutoID IS NULL,masterTbl.CustomerName, rowTbl.CustomerName)  AS  CustomerName, masterTbl.posCustomerAutoID as posCustomerAutoID, masterTbl.CustomerAutoID as CustomerAutoID, masterTbl.serialNo as  serialNo, IF(rowTbl.CustomerAutoID IS NULL,masterTbl.customerCountry, rowTbl.customerCountry) AS  customerCountry, IF(rowTbl.CustomerAutoID IS NULL, masterTbl.customerTelephone, rowTbl.customerTelephone) AS customerTelephone,masterTbl.customerCountryCode as customerCountryCode, IF(rowTbl.CustomerAutoID IS NULL,masterTbl.customerEmail, rowTbl.customerEmail) AS customerEmail, masterTbl.isFromERP as isFromERP, IF(rowTbl.CustomerAutoID IS NULL,masterTbl.CustomerAddress1, rowTbl.CustomerAddress1) AS CustomerAddress1', false)
        ...
        //$this->datatables->add_column('customerTelephone', '$1', 'removeFirstZero(customerTelephone)');
        $this->datatables->add_column('customer_telephone', '$1 $2 $3', 'customerCountryCode," ",removeFirstZero(customerTelephone)');
    ...
    function add_edit_customer()
    {
        ...
        $customerTelephone = $this->input->post('customerTelephone');
       ...
       // $this->form_validation->set_rules('CustomerAddress1', 'Address', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($posCustomerAutoID) {
                /** Update */
                $telexist = $this->db->query('SELECT customerTelephone FROM srp_erp_pos_customermaster WHERE customerTelephone="'.$customerTelephone.'" AND posCustomerAutoID!="'.$posCustomerAutoID.'"')->row_array();
                if(!empty($telexist)){
                    echo json_encode(array('error' => 1, 'message' => 'Customer Telephone Number is Already exist'));
                }else{
                ...
                /** Insert */
                $telexist = $this->db->query('SELECT customerTelephone FROM srp_erp_pos_customermaster WHERE customerTelephone="'.$customerTelephone.'"')->row_array();
                if(!empty($telexist)){
                    echo json_encode(array('error' => 1, 'message' => 'Customer Telephone Number is Already exist'));
                }else{
    ...
    {
        $this->db->select('countryID,CountryDes,countryShortCode,countryCode');
        $this->db->from('srp_erp_countrymaster');
        $this->db->order_by('CountryDes');
        $result = $this->db->get()->result_array();;

        $select = "<select class='form-control select2' name='customerCountry' id='customerCountry' required><option value=''>Select a country</option>";
                  foreach ($result as $row){
                      $select.="<option value='".$row['CountryDes']."' country-id='".$row['countryID']."' code='".$row['countryCode']."' >".$row['CountryDes']."</option>";
                  }
        $select.="</select>";
