...
class CompanyPolicy_model extends ERP_Model
{
    ...
    function policy_detail_update()
    {
        ...
        if($id==37 && $value==0){
            $this->db->delete('srp_erp_documentpolicyusergroup', array('companyID' => trim(current_companyID())));
        }
    ...
    function saveusergrouptopolicy(){
        $status=$this->db->delete('srp_erp_documentpolicyusergroup', array('companyID' => trim(current_companyID())));
        if($status){
            $this->db->trans_start();
            ...
            if(!empty($chkVal)){
                ...
                foreach ($chkVal as $val){
                    ...
                    $this->db->insert('srp_erp_documentpolicyusergroup', $data);
                }
