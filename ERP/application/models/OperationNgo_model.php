...
class OperationNgo_model extends ERP_Model
{
    ...
    function donor_commitment_confirmation()
    {
        ...
        $this->db->where('commitmentAutoId', $commitmentAutoId);
        $this->db->from('srp_erp_ngo_commitmentmasters');
        $mas_dt = $this->db->get()->row_array();
        $validate_code = validate_code_duplication($mas_dt['documentSystemCode'], 'documentSystemCode', $commitmentAutoId,'commitmentAutoId', 'srp_erp_ngo_commitmentmasters');
    ...
    function donor_collection_confirmation()
    {
        ...
        if (empty($record)) {
            ...
            if (!empty($row)) {
                ...
                $validate_code = validate_code_duplication($row['documentSystemCode'], 'documentSystemCode', trim($this->input->post('collectionAutoId') ?? ''),'collectionAutoId', 'srp_erp_ngo_donorcollectionmaster');
                if(!empty($validate_code)) {
                    return array('e', 'The document Code Already Exist.(' . $validate_code . ')');
                }
    ...
    function save_beneficiary_header()
    {
        ...
        $recommendedDate = trim($this->input->post('recommendedDate') ?? '');
        ...
        $format_recommendedDate = null;
        if (isset($recommendedDate) && !empty($recommendedDate)) {
            $format_recommendedDate = input_format_date($recommendedDate, $date_format_policy);
        }
        ...
        if ($templateType == 'DA') {
            ...
            $data['eligibleForZakathYN'] = trim($this->input->post('eligibleForZakathYN') ?? '');
            $data['recommendedBy'] = trim($this->input->post('recommendedBy') ?? '');
            $data['recommendedDate'] = $format_recommendedDate;
        }
    ...
    function load_beneficiary_header()
    {
        $companyID = $this->common_data['company_data']['company_id'];
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,DATE_FORMAT(registeredDate,\'' . $convertFormat . '\') AS registeredDate,DATE_FORMAT(dateOfBirth,\'' . $convertFormat . '\') AS dateOfBirth,DATE_FORMAT(recommendedDate,\'' . $convertFormat . '\') AS recommendedDate');
    ...
    function load_beneficiary_disaster_managment($benificiaryID)
    {
        $companyID = $this->common_data['company_data']['company_id'];
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,DATE_FORMAT(registeredDate,\'' . $convertFormat . '\') AS registeredDate,DATE_FORMAT(dateOfBirth,\'' . $convertFormat . '\') AS dateOfBirth,Em.description as Ethnicity,Jm.JobCatDescription as Occupation,ECm.shortCode as econShortCode,countryDs.CountryDes,smpro.Description as provinceNam,smdis.Description as districtName,smdiv.Description as divisionName,smsubdiv.Description as subDivisionName');
        $this->db->from('srp_erp_ngo_beneficiarymaster bm');
        $this->db->join('srp_erp_ethnicitymaster Em', 'Em.ethnicityID = bm.ethnicityID', 'left');
        $this->db->join('srp_erp_countrymaster countryDs', 'countryDs.countryID = bm.countryID', 'left');
        $this->db->join('srp_erp_statemaster smpro', 'smpro.stateID = bm.province', 'left');
        $this->db->join('srp_erp_statemaster smdis', 'smdis.stateID = bm.district', 'left');
        $this->db->join('srp_erp_statemaster smdiv', 'smdiv.stateID = bm.division', 'left');
        $this->db->join('srp_erp_statemaster smsubdiv', 'smsubdiv.stateID = bm.subDivision', 'left');
        $this->db->join('srp_erp_ngo_com_jobcategories Jm', 'Jm.JobCategoryID = bm.da_occupationID', 'left');
        $this->db->join('srp_erp_ngo_economicstatusmaster ECm', 'ECm.economicStatusID = bm.da_economicStatus', 'left');
        $this->db->where('benificiaryID', $benificiaryID);
        return $this->db->get()->row_array();
    }
    ...
    {
        $this->db->select("*,srp_erp_ngo_beneficiaryfamilydetails.name as name,r.relationship as relationshipDesc,c.Nationality as countryName,g.name as genderDesc,countryname.CountryDes as countryNameFlag");
        ...
        $this->db->join("srp_erp_countrymaster countryname", "countryname.countryID = c.countryID", "left");
    ...
    function beneficiary_image_upload()
    {
       ...
       /* $this->db->trans_start();
        ...
        $info = new SplFileInfo($_FILES["files"]["name"]);
        $fileName = 'Beneficiary_'.$this->common_data['company_data']['company_code'].'_'. trim($this->input->post('benificiaryID') ?? '') . '.' . $info->getExtension();
        $currentDatetime = format_date_mysql_datetime();
        $file = $_FILES['files'];
        if($file['error'] == 1){
            return array('e', "The file you are attempting to upload is larger than the permitted size. (maximum 5MB)");
        ...
        $path = "uploads/ngo/beneficiaryImage/$fileName";
        $s3Upload = $this->s3->upload($file['tmp_name'], $path);
    ...
    function save_beneficiary_document()
    {
        ...
        if (!empty($isExisting)) {
          ...
          $this->upload->initialize($config);*/
            ...
            $info = new SplFileInfo($_FILES["doc_file"]["name"]);
            $fileName = 'ComBeneficiary_'.$this->common_data['company_data']['company_code'].'_'. $benificiaryID . '.' . $info->getExtension();
            $currentDatetime = format_date_mysql_datetime();
...
WHERE dds.DocDesID
    ...
    function delete_project_proposal()
    {
        ...
        $queryExZq = $this->db->query("SELECT * FROM srp_erp_ngo_com_projectproposalzakatsetup WHERE proposalID='" . $proposalID . "'");

        if(!empty($queryExZq)){
            $this->db->delete('srp_erp_ngo_com_projectproposalzakatsetup', array('proposalID' => $proposalID));
    ...
    function project_proposal_confirmation()
    {
        ...
        if (!empty($row)) {
            ...
            $validate_code = validate_code_duplication($master['documentSystemCode'], 'documentSystemCode', $proposalID,'proposalID', 'srp_erp_ngo_projectproposals');
            if(!empty($validate_code)) {
                return array('e', 'The document Code Already Exist.(' . $validate_code . ')');
            }
    ...
    function project_image_upload()
    {
       ...
       /* $output_dir = "uploads/NGO/projectImage/";
        ...
        $info = new SplFileInfo($_FILES["files"]["name"]);
        $fileName = 'Project_'.$this->common_data['company_data']['company_code'].'_'. trim($this->input->post('ngoProjectID') ?? '') . '.' . $info->getExtension();
        $currentDatetime = format_date_mysql_datetime();
        $file = $_FILES['files'];
        if($file['error'] == 1){
            return array('e', "The file you are attempting to upload is larger than the permitted size. (maximum 5MB)");
        ...
        $path = "uploads/ngo/projectImage/$fileName";
        $s3Upload = $this->s3->upload($file['tmp_name'], $path);
    ...
    function save_project_proposal_image()
    {
      ...
      /*  $path = "uploads/ngo/projectProposalImage/";
        ...
        $info = new SplFileInfo($_FILES["doc_file"]["name"]);
        $fileName = 'ProjectProposal_'.$this->common_data['company_data']['company_code'].'_'. $documentID . '_' . time() . '.' . $info->getExtension();
        $currentDatetime = format_date_mysql_datetime();
        $file = $_FILES['doc_file'];
        if($file['error'] == 1){
            return array('e', "The file you are attempting to upload is larger than the permitted size. (maximum 5MB)");
        ...
        $path = "uploads/ngo/projectProposalImage/$fileName";
        $s3Upload = $this->s3->upload($file['tmp_name'], $path);
    ...
    function delete_project_proposal_image()
    {
        ...
        $this->s3->delete('uploads/ngo/projectProposalImage/'.$myFileName);
    ...
    function upload_beneficiary_multiple_image()
    {
      ...
      /*  $path = 'uploads/ngo/beneficiaryImage/'; // imagePath();
        if (!file_exists($path)) {
            mkdir("uploads/ngo/beneficiaryImage", 007);
        }
        $fileName = str_replace(' ', '', strtolower($benificiaryID)) . '_' . time();
        ...
        $this->upload->initialize($config);*/
        $info = new SplFileInfo($_FILES["doc_file"]["name"]);
        $fileName = 'BI_' . $this->common_data['company_data']['company_code'].'_'. $benificiaryID . '_' . time() . '.' . $info->getExtension();
        $currentDatetime = format_date_mysql_datetime();
        $file = $_FILES['doc_file'];
        if($file['error'] == 1){
            return array('e', "The file you are attempting to upload is larger than the permitted size. (maximum 5MB)");
    ...
    function delete_beneficiary_multiple_image()
    {
        ...
        $itemimageExist = $this->db->query("SELECT beneficiaryImage FROM srp_erp_ngo_beneficiaryimages WHERE beneficiaryImageID = $beneficiaryImageID ")->row_array();
        $this->s3->delete('uploads/ngo/beneficiaryImage/'.$itemimageExist['beneficiaryImage']);
    ...
    function save_project_proposal_attachments()
    {
       ...
       /* $path = "uploads/ngo/attachments/";
        ...
        $info = new SplFileInfo($_FILES["doc_file"]["name"]);
        $fileName = 'Project_Proposal_Attachment_'.$this->common_data['company_data']['company_code'].'_'. $ngoProposalID . '_' . time() . '.' . $info->getExtension();
        $currentDatetime = format_date_mysql_datetime();
        $file = $_FILES['doc_file'];
        if($file['error'] == 1){
            return array('e', "The file you are attempting to upload is larger than the permitted size. (maximum 5MB)");
        ...
        $path = "uploads/ngo/attachments/$fileName";
        $s3Upload = $this->s3->upload($file['tmp_name'], $path);
    ...
    function delete_project_proposal_attachment()
    {
        ...
        $companyID = $this->common_data['company_data']['company_id'];
        $itemimageExist = $this->db->query("SELECT myFileName FROM srp_erp_ngo_attachments WHERE companyID = $companyID AND attachmentID = $ngoattachmentid ")->row_array();
        $this->s3->delete('uploads/ngo/attachments/'.$itemimageExist['myFileName']);
...
//die();
    ...
    function save_beneficiary_header_itemdamage_assesment()
    {
        ...
        $data['vehicleAutoID'] = trim($this->input->post('vehicleAutoID') ?? '');
...
WHERE
    ...
    function save_project_claim_docdate_narration()
    {
        ...
        $supplierid = $this->db->query("select srp_erp_ngo_projects.contractorID, proposal.transactionCurrencyID, currencyMaster.CurrencyCode from  srp_erp_ngo_projects LEFT JOIN srp_erp_ngo_projectproposals proposal ON proposal.proposalID = srp_erp_ngo_projects.proposalID LEFT JOIN srp_erp_currencymaster currencyMaster ON currencyMaster.CurrencyID = proposal.transactionCurrencyID where ngoProjectID = $projectid AND srp_erp_ngo_projects.companyID = $comapnyid")->row_array();
        ...
        if (empty($finacialYearid)) {
            ...
            $data['transactionCurrencyID'] = $supplierid['transactionCurrencyID'];
            $data['transactionCurrency'] = $supplierid['CurrencyCode'];
            $data['transactionExchangeRate'] = 1;
            $data['transactionCurrencyDecimalPlaces'] = fetch_currency_desimal_by_id($data['transactionCurrencyID']);
            $data['companyLocalCurrencyID'] = $this->common_data['company_data']['company_default_currencyID'];
            $data['companyLocalCurrency'] = $this->common_data['company_data']['company_default_currency'];
...
WHERE
    ...
    function save_publicProperty_header()
    {
        ...
        if ($publicPropertyBeneID) {
            $this->db->where('publicPropertyBeneID !=', $publicPropertyBeneID);
        }
        $this->db->from('srp_erp_ngo_publicpropertybeneficiarymaster');
        $recordExist = $this->db->get()->row_array();
        if (!empty($recordExist)) {
            return array('w', 'Primary Phone Number is already Exist.');
            exit();
        ...
        $registeredDate = trim($this->input->post('registeredDate') ?? '');
        $commencementDate = trim($this->input->post('commencementDate') ?? '');
        $recommendedDate = trim($this->input->post('recommendedDate') ?? '');
        ...
        $format_commencementDate = null;
        if (isset($commencementDate) && !empty($commencementDate)) {
            $format_commencementDate = input_format_date($commencementDate, $date_format_policy);
        }
        $format_recommendedDate = null;
        if (isset($recommendedDate) && !empty($recommendedDate)) {
            $format_recommendedDate = input_format_date($recommendedDate, $date_format_policy);
        ...
        $this->db->trans_start();
        $data['secondaryCode'] = trim($this->input->post('secondaryCode') ?? '');
        $data['propertyType'] = trim($this->input->post('propertyType') ?? '');
        $data['subPropertyId'] = trim($this->input->post('subPropertyId') ?? '');
        $data['projectID'] = $projectID;
        $data['PropertyName'] = trim($this->input->post('PropertyName') ?? '');
        $data['PropertyShortCode'] = trim($this->input->post('PropertyShortCode') ?? '');
        $data['registeredDate'] = $format_registeredDate;
        $data['commencementDate'] = $format_commencementDate;
        $data['email'] = trim($this->input->post('email') ?? '');
        $data['phoneCountryCodePrimary'] = trim($this->input->post('countryCodePrimary') ?? '');
        $data['phoneAreaCodePrimary'] = trim($this->input->post('phoneAreaCodePrimary') ?? '');
        $data['phonePrimary'] = trim($this->input->post('phonePrimary') ?? '');
        ...
        if ($publicPropertyBeneID) {
            ...
            $data['modifiedDateTime'] = $this->common_data['current_date'];
            $this->db->where('publicPropertyBeneID', $publicPropertyBeneID);
            $update = $this->db->update('srp_erp_ngo_publicpropertybeneficiarymaster', $data);
            if ($update) {
                $projectTable = $this->db->query("select projectID,publicPropertyBeneID from srp_erp_ngo_beneficiaryprojects  WHERE projectID={$projectID} AND publicPropertyBeneID = {$publicPropertyBeneID}")->row_array();

                if (empty($projectTable)) {
                    $data_project['projectID'] = $projectID;
                    $data_project['publicPropertyBeneID'] = $publicPropertyBeneID;
                    $data_project['companyID'] = $companyID;
                    $data_project['createdUserGroup'] = $this->common_data['user_group'];
                    $data_project['createdPCID'] = $this->common_data['current_pc'];
                    $data_project['createdUserID'] = $this->common_data['current_userID'];
                ...
                if ($contactID != '') {
                    $projectWiseDocumentTable = $this->db->query("select * FROM srp_erp_ngo_documentdescriptionforms WHERE publicPropertyBeneID = {$publicPropertyBeneID}")->result_array();

                    if (!empty($projectWiseDocumentTable)) {
                        foreach ($projectWiseDocumentTable as $doc) {
                            $dataDocument['DocDesSetID'] = $doc['DocDesSetID'];
                            $dataDocument['DocDesID'] = $doc['DocDesID'];
                            $dataDocument['publicPropertyBeneID'] = $publicPropertyBeneID;
                            $dataDocument['projectID'] = $projectID;
                            $dataDocument['FileName'] = $doc['FileName'];
                            $dataDocument['companyID'] = current_companyID();
                            $dataDocument['CreatedPC'] = current_pc();
                ...
                if (!empty($me_assitanceName)) {
                    $this->db->delete('srp_erp_ngo_beneficiary_othersupportassistance', array('publicPropertyBeneID' => $publicPropertyBeneID));
                    foreach ($me_assitanceName as $key => $monthlyExpenditureTable) {
                        if ($me_year[$key] != '') {
                            $data_othersupportassistance['publicPropertyBeneID'] = $publicPropertyBeneID;
                            $data_othersupportassistance['assitanceName'] = $monthlyExpenditureTable;
                            $data_othersupportassistance['Organization'] = $me_Organization[$key];
                            $data_othersupportassistance['amount'] = $me_year[$key];
                            $data_othersupportassistance['companyID'] = current_companyID();
                ...
                if ($this->db->trans_status() === FALSE) {
                    ...
                    return array('e', 'Public Property Beneficiary Update Failed ' /*. $this->db->_error_message()*/);

                } else {
                    $this->db->trans_commit();

                    return array('s', 'Public Property Beneficiary Updated Successfully.', $publicPropertyBeneID);
                }
            }
        } else {
            //$data['systemCode'] = $beneficiarySystemCode;
            ...
            $data['createdUserName'] = $this->common_data['current_user'];
            $data['createdDateTime'] = $this->common_data['current_date'];
            $this->db->insert('srp_erp_ngo_publicpropertybeneficiarymaster', $data);
            $last_id = $this->db->insert_id();
            $this->db->trans_complete();
            if ($last_id) {
                $projectTable = $this->db->query("select projectID,publicPropertyBeneID from srp_erp_ngo_beneficiaryprojects  WHERE projectID={$projectID} AND publicPropertyBeneID = {$last_id}")->row_array();

                if (empty($projectTable)) {
                    $data_project['projectID'] = $projectID;
                    $data_project['publicPropertyBeneID'] = $last_id;
                    $data_project['companyID'] = $companyID;
                    $data_project['createdUserGroup'] = $this->common_data['user_group'];
                    $data_project['createdPCID'] = $this->common_data['current_pc'];
                    $data_project['createdUserID'] = $this->common_data['current_userID'];
                ...
                if (!empty($me_assitanceName)) {
                    foreach ($me_assitanceName as $key => $monthlyExpenditureTable) {
                        if ($me_year[$key] != '') {
                            $data_othersupportassistance['publicPropertyBeneID'] = $last_id;
                            $data_othersupportassistance['assitanceName'] = $monthlyExpenditureTable;
                            $data_othersupportassistance['Organization'] = $me_Organization[$key];
                            $data_othersupportassistance['amount'] = $me_year[$key];
                            $data_othersupportassistance['companyID'] = current_companyID();
            ...
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                return array('e', 'Public Property Beneficiary Save Failed ' . $this->db->_error_message());
            } else {
                $this->db->trans_commit();
                return array('s', 'Public Property Beneficiary Saved Successfully.', $last_id);
            }
        }
    }


    function load_publicProperty_disasterManagment($publicPropertyBeneID)
    {
        $companyID = $this->common_data['company_data']['company_id'];
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,DATE_FORMAT(registeredDate,\'' . $convertFormat . '\') AS registeredDate,DATE_FORMAT(commencementDate,\'' . $convertFormat . '\') AS commencementDate,projects.projectName,countryDs.CountryDes,smpro.Description as provinceNam,smdis.Description as districtName,smdiv.Description as divisionName,smsubdiv.Description as subDivisionName,propType.publicPropertyDescription as publicPropertyDescription');
        $this->db->from('srp_erp_ngo_publicpropertybeneficiarymaster bm');
        $this->db->join('srp_erp_ngo_projects projects', 'projects.ngoProjectID = bm.projectID', 'left');
        $this->db->join('srp_erp_countrymaster countryDs', 'countryDs.countryID = bm.countryID', 'left');
        $this->db->join('srp_erp_statemaster smpro', 'smpro.stateID = bm.province', 'left');
        $this->db->join('srp_erp_statemaster smdis', 'smdis.stateID = bm.district', 'left');
        $this->db->join('srp_erp_statemaster smdiv', 'smdiv.stateID = bm.division', 'left');
        $this->db->join('srp_erp_statemaster smsubdiv', 'smsubdiv.stateID = bm.subDivision', 'left');
        $this->db->join('srp_erp_ngo_publicproperty_types propType', 'propType.publicPropertyID = bm.propertyType', 'left');
        $this->db->where('publicPropertyBeneID', $publicPropertyBeneID);
        return $this->db->get()->row_array();
    }

    function fetch_damage_ass_details_view($publicPropertyBeneID)
    {


        $output = $this->db->query("SELECT
	bm.publicPropertyBeneID,
	bm.PropertyName,
	bm.address,
	...
	houseConditio.description as houseCondition,
	bm.da_estimatedRepairingCost,bm.da_needAssistancetoRepairYN,bm.da_paidAmount
FROM
	srp_erp_ngo_publicpropertybeneficiarymaster bm
LEFT JOIN srp_erp_ngo_itemdamagedasssesment ON srp_erp_ngo_itemdamagedasssesment.publicPropertyBeneID = bm.publicPropertyBeneID
	...
	LEFT JOIN srp_erp_ngo_houseconditionmaster houseConditio ON houseConditio.houseConditionID = bm.da_housingCondition 
	LEFT JOIN srp_erp_ngo_damagetypemaster damagetype ON damagetype.damageTypeID = bm.da_buildingDamages 
WHERE
	bm.publicPropertyBeneID = {$publicPropertyBeneID}
	GROUP BY
	bm.publicPropertyBeneID")->result_array();
    ...
    function add_publicProperty_notes()
    {
        ...
        $data['documentID'] = 7;
        $data['documentAutoID'] = $publicPropertyBeneID;
        $data['description'] = trim($this->input->post('description') ?? '');
        $data['companyID'] = $companyID;
        $data['createdUserGroup'] = $this->common_data['user_group'];
        $data['createdPCID'] = $this->common_data['current_pc'];
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            return array('e', 'Public Property Note Save Failed ' . $this->db->_error_message(), $last_id);
        } else {
            $this->db->trans_commit();

            return array('s', 'Public Property Note Added Successfully.');
    ...
    function publicProperty_image_upload()
    {
        ...
        $info = new SplFileInfo($_FILES["files"]["name"]);
        $fileName = 'publicPropertyBene_'.$this->common_data['company_data']['company_code'].'_'. trim($this->input->post('publicPropertyBeneID') ?? '') . '.' . $info->getExtension();
        $currentDatetime = format_date_mysql_datetime();
        $file = $_FILES['files'];
        if($file['error'] == 1){
            return array('e', "The file you are attempting to upload is larger than the permitted size. (maximum 5MB)");
        ...
        if (!$s3Upload) {
            ...
            $this->db->where('publicPropertyBeneID', trim($this->input->post('publicPropertyBeneID') ?? ''));
            $this->db->update('srp_erp_ngo_publicpropertybeneficiarymaster', $data);
    ...
    function load_publicProperty_header()
    {
        $companyID = $this->common_data['company_data']['company_id'];
        $convertFormat = convert_date_format_sql();
        $this->db->select('*,DATE_FORMAT(registeredDate,\'' . $convertFormat . '\') AS registeredDate,DATE_FORMAT(commencementDate,\'' . $convertFormat . '\') AS commencementDate,DATE_FORMAT(recommendedDate,\'' . $convertFormat . '\') AS recommendedDate');
        $this->db->where('publicPropertyBeneID', $this->input->post('publicPropertyBeneID'));
        $this->db->from('srp_erp_ngo_publicpropertybeneficiarymaster');
        return $this->db->get()->row_array();
    }


    function publicProperty_system_code_generator($publicPropertyBeneID)
    {
        $companyID = $this->common_data['company_data']['company_id'];

        $benificiaryDetail = $this->db->query("SELECT publicPropertyBeneID,district,countryID,province,division,subDivision FROM srp_erp_ngo_publicpropertybeneficiarymaster WHERE publicPropertyBeneID = " . $publicPropertyBeneID . "")->row_array();

        if (!empty($benificiaryDetail['countryID'] && $benificiaryDetail['province'] && $benificiaryDetail['district'] && $benificiaryDetail['division'] && $benificiaryDetail['subDivision'])) {
            $benificiarySubDivision = $this->db->query("SELECT count(subDivision) as divisionTotal FROM srp_erp_ngo_publicpropertybeneficiarymaster WHERE companyID = " . $companyID . " AND subDivision = {$benificiaryDetail['subDivision']} AND confirmedYN = 1")->row_array();
    ...
    function publicProperty_confirmation()
    {
        ...
        $this->db->select('beneficiaryProjectID');
        $this->db->where('publicPropertyBeneID', $publicPropertyBeneID);
        $this->db->where('projectID', $publicPropertyBeneID);
        $this->db->where('confirmedYN', 1);
        $this->db->from('srp_erp_ngo_beneficiaryprojects');
        $checkProjectConfirmed = $this->db->get()->row_array();
...
WHERE dds.DocDesID
	NOT IN (
		...
		FROM
			srp_erp_ngo_documentdescriptionforms  ddf
		WHERE
			ddf.projectID = {$projectID} AND ddf.publicPropertyBeneID = {$publicPropertyBeneID}
	) AND dds.projectID = {$projectID} AND ngoDocumentID = 5 AND isMandatory = 1")->result_array();

        if (!empty($checkProjectConfirmed)) {
            return array('e', 'Property Beneficiary already confirmed');
        } else if (!empty($isInUse)) {
            return array('e', 'Mandatory Documents are need to upload');
        } else {

            $checkisConfirmedProjectExist = $this->db->query("select beneficiaryProjectID from srp_erp_ngo_beneficiaryprojects WHERE publicPropertyBeneID={$publicPropertyBeneID} AND confirmedYN = 1 ")->row_array();

            if (empty($checkisConfirmedProjectExist)) {
                $benGeneratedID = $this->publicProperty_system_code_generator($publicPropertyBeneID);
                $data = array(
                    'systemCode' => $benGeneratedID,
                    'confirmedYN' => 1,
                    'confirmedDate' => $this->common_data['current_date'],
    ...
    function save_property_header_damageAssesment()
    {
        ...
        if ($publicPropertyBeneID) {
            ...
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                return array('e', 'Property Beneficiary Update Failed' . $this->db->_error_message());
            } else {
                $this->db->trans_commit();
                return array('s', 'Property Beneficiary Updated Successfully.');
            }
        }
    }
    ...
    function save_property_header_human_damageAssesment()
    {
        ...
        $humanInjuryID = trim($this->input->post('humanInjuryID') ?? '');
        $publicPropertyBeneID = trim($this->input->post('publicPropertyBeneID') ?? '');
        $effectedPersonName = trim($this->input->post('effectedPersonName') ?? '');
        $damagetypeid = trim($this->input->post('damageTypeID') ?? '');
        $totalpaidamt = trim($this->input->post('totalpaidamt') ?? '');

        $data['effectedPersonName'] = trim($this->input->post('effectedPersonName') ?? '');
        ...
        if ($humanInjuryID) {
            ...
            $q = "SELECT
                ...
                FROM
                    srp_erp_ngo_humaninjuryassesment
                WHERE 
                 humanInjuryID!='" . $humanInjuryID . "' AND publicPropertyBeneID = '" . $publicPropertyBeneID . "' AND effectedPersonName = '" . $effectedPersonName . "' AND damageTypeID ='" . $damagetypeid . "'";
            $result = $this->db->query($q)->row_array();
            if ($result) {
                return array('e', 'The selected family memeber already exist.');
            } else {
            ...
            $q = "SELECT
                ...
                FROM
                    srp_erp_ngo_humaninjuryassesment
                WHERE 
                 publicPropertyBeneID = '" . $publicPropertyBeneID . "' AND effectedPersonName = '" . $effectedPersonName . "' AND damageTypeID ='" . $damagetypeid . "'  ";
            $result = $this->db->query($q)->row_array();
            if ($result) {
                return array('e', 'The selected family memeber already exist.');
            } else {
    ...
    function load_pd_human_injury_assestment()
    {
        $humanInjuryID = $this->input->post('humanInjuryID');
        $this->db->select('humanInjuryID,FamilyDetailsID,damageTypeID,estimatedAmount,paidAmount,remarks');
        $this->db->where('humanInjuryID', $humanInjuryID);
    ...
    function save_pd_header_itemdamage_assesment()
    {
        ...
        if ($itemDamagedID) {
            ...
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                return array('e', 'Damaged Property Items Assessment Update Failed' . $this->db->_error_message());
            } else {
                $this->db->trans_commit();
                return array('s', 'Damaged Property Items Assessment Updated Successfully.');
            }
        } else {
            $data['companyID'] = $companyID;
            $data['createdUserGroup'] = $this->common_data['user_group'];
            ...
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                return array('e', 'Damaged Property Items Assessment Save Failed ' . $this->db->_error_message());
            } else {
                $this->db->trans_commit();
                return array('s', 'Damaged Property Items Assessment Saved Successfully.', $last_id);

            }
        }
    }

    function load_publicProperty_multiple_images()
    {
        $publicPropertyBeneID = trim($this->input->post('publicPropertyBeneID') ?? '');
        $this->db->select('beneficiaryImageID,isSelectedforPP,beneficiaryImage');
        $this->db->where('publicPropertyBeneID', $publicPropertyBeneID);
    ...
    function upload_publicProperty_multiple_image()
    {
       ...
       /* $path = UPLOAD_PATH_POS . 'uploads/ngo/beneficiaryImage/'; // imagePath();
        ...
        $fileName = str_replace(' ', '', strtolower($publicPropertyBeneID)) . '_' . time();
        $config['upload_path'] = $path;
        $config['allowed_types'] = 'gif|jpg|jpeg|png';
        $config['max_size'] = '5120'; // 5 MB
        $config['file_name'] = $fileName;
        ...
        $info = new SplFileInfo($_FILES["doc_file"]["name"]);
        $fileName = 'PP_'.$this->common_data['company_data']['company_code'].'_'. $publicPropertyBeneID . '_' . time() . '.' . $info->getExtension();
        $currentDatetime = format_date_mysql_datetime();
        $file = $_FILES['doc_file'];
        if($file['error'] == 1){
            return array('e', "The file you are attempting to upload is larger than the permitted size. (maximum 5MB)");
        ...
        $data = array(
            'publicPropertyBeneID' => $publicPropertyBeneID,
            'beneficiaryImage' => $fileName,
            'companyID' => current_companyID(),
            'createdPCID' => current_pc(),
            'CreatedUserName' => current_employee(),
    ...
    function update_publicProperty_multiple_image()
    {
        ...
        if ($status == 1) {
            $totlaCount = $this->db->query("select beneficiaryImageID FROM srp_erp_ngo_beneficiaryimages WHERE publicPropertyBeneID ={$publicPropertyBeneID} AND isSelectedforPP = 1")->result_array();
    ...
    function load_publicProperty_documents()
    {
        $companyID = current_companyID();
        $publicPropertyBeneID = $this->input->post('publicPropertyBeneID');
        $projectID = $this->input->post('projectID');
        return $this->db->query("SELECT MASTER.DocDesID AS DcoumentAutoID,DocDesFormID, DocDescription, isMandatory, FileName
                                 ...
                                 JOIN srp_erp_ngo_documentdescriptionsetup setup ON setup.DocDesID = master.DocDesID
                                 INNER JOIN srp_erp_ngo_documentdescriptionforms forms ON forms.DocDesID = master.DocDesID
                                 AND publicPropertyBeneID = {$publicPropertyBeneID} AND forms.projectID = {$projectID}
                                 WHERE master.companyID={$companyID} AND isDeleted=0 GROUP BY MASTER.DocDesID")->result_array();
    }
    ...
    function save_publicProperty_document()
    {
        ...
        $isExisting = $this->db->query("SELECT DocDesFormID FROM srp_erp_ngo_documentdescriptionforms WHERE publicPropertyBeneID = {$publicPropertyBeneID} AND DocDesID = {$documentID} AND companyID = $companyID")->row_array();
        ...
        $info = new SplFileInfo($_FILES["doc_file"]["name"]);
        $fileName = 'PP_'.$this->common_data['company_data']['company_code'].'_'. strtolower($publicPropertyBeneID) . '_' . time() . '.' . $info->getExtension();
        $currentDatetime = format_date_mysql_datetime();
        $file = $_FILES['doc_file'];
        if($file['error'] == 1){
            return array('e', "The file you are attempting to upload is larger than the permitted size. (maximum 5MB)");
    ...
    {
        $DocDesFormID = trim($this->input->post('DocDesFormID') ?? '');
        $itemimageExist = $this->db->query("SELECT FileName FROM srp_erp_ngo_documentdescriptionforms WHERE DocDesFormID = $DocDesFormID ")->row_array();
        $this->s3->delete('documents/ngo/'.$itemimageExist['FileName']);
    ...
    function delete_propertyDamage_master()
    {
        $publicPropertyBeneID = trim($this->input->post('publicPropertyBeneID') ?? '');
        $this->db->where('documentID', 5);
        $this->db->where('documentAutoID', $publicPropertyBeneID);
        $this->db->delete('srp_erp_ngo_notes');

        $this->db->delete('srp_erp_ngo_publicpropertybeneficiarymaster', array('publicPropertyBeneID' => trim($this->input->post('publicPropertyBeneID') ?? '')));
    ...
    function fetch_property_relate_search()
    {
        ...
        if (!empty($search_string)) {
            $data = $this->db->query('SELECT publicPropertyBeneID,secondaryCode,propertyType,subPropertyId,PropertyShortCode,PropertyName,email,countryID,province,division,subDivision,district,postalCode,address,phoneCountryCodePrimary,phoneAreaCodePrimary,phonePrimary,DATE_FORMAT(registeredDate,\'' . $convertFormat . '\') AS registeredDate,DATE_FORMAT(commencementDate,\'' . $convertFormat . '\') AS commencementDate,CONCAT(PropertyName," ", systemCode) AS "Match" FROM srp_erp_ngo_publicpropertybeneficiarymaster bm LEFT JOIN srp_erp_ngo_beneficiaryprojects bp ON bm.publicPropertyBeneID = bp.publicPropertyBeneID WHERE bm.companyID = "' . $companyID . '" AND bm.confirmedYN = 1 AND bp.projectID != ' . $projectID . ' AND (PropertyName LIKE "' . $search_string . '" OR systemCode LIKE "' . $search_string . '") ')->result_array();
            //echo $this->db->last_query();
            if (!empty($data)) {
                foreach ($data as $val) {
                    $dataArr[] = array(
                        'publicPropertyBeneID' => $val["publicPropertyBeneID"],
                        'value' => $val["Match"],
                        'secondaryCode' => $val['secondaryCode'],
                        'propertyType' => $val['propertyType'],
                        'subPropertyId' => $val['subPropertyId'],
                        'commencementDate' => $val['commencementDate'],
                        'registeredDate' => $val['registeredDate'],
                        'PropertyShortCode' => $val['PropertyShortCode'],
