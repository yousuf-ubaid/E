...
class Pos_gift_card extends ERP_Model
{
        ...
        if ($cardMasterID) {
            ...
            if ($validate) {
                ...
                if ($result) {
                    ...
                    issuedDatetime
                FROM
                    srp_erp_pos_cardissue
                WHERE
                    cardMasterID = '" . $cardMasterID . "'";
                    $IssueCard = $this->db->query($q)->row_array();

                    if(!empty($IssueCard)){
                        $month=$data['cardExpiryInMonths'];
                        $issuedDatetime=$IssueCard['issuedDatetime'];

                        $date=date_create($issuedDatetime);
                        date_add($date,date_interval_create_from_date_string("$month month"));
                        $dts= date_format($date,"Y-m-d");

                        $expiryDate['expiryDate'] =  $dts;

                        $this->db->where('cardMasterID', $cardMasterID);
                        $this->db->update('srp_erp_pos_cardissue', $expiryDate);
                    }
    ...
    function loadCardIssueData($barCode)
    {
        /**
         * NOTE:
         * We do not need to consider wareHouseAutoID
         * because srp_erp_pos_customermaster table work with the unique telephone number regardless of outlet
         */
        $q = "SELECT
                    cardIssueID,
                    cardMasterID,
                    expiryDate,
                    srp_erp_pos_customermaster.* 
                FROM
                    srp_erp_pos_cardissue
            ...
            if ($isCreditSale) {
                ...
                $q = "INSERT INTO srp_erp_customerinvoicedetails (
                        ...
                        FROM
                            ...
                            LEFT JOIN srp_erp_pos_paymentglconfigdetail config ON config.companyID='" . current_companyID() . "' AND config.paymentConfigMasterID = 7 AND srp_erp_customerinvoicemaster.wareHouseAutoID = config.warehouseID
                            LEFT JOIN srp_erp_chartofaccounts AS chartOfAccount ON config.GLCode = chartOfAccount.GLAutoID
                        WHERE
    ...
    function insert_double_entries_giftCard_refund($id)
    {
        $cardGLAutoID = $this->get_cardGlAutoID();
        if ($cardGLAutoID > 0) {
            ...
            $q = "INSERT INTO srp_erp_generalledger (
                ...
                SELECT
                    ...
                    YEAR (curdate()) AS documentYear,
                    MONTH (curdate()) AS documentMonth,
                    'POS GiftCard refund' AS documentNarration,
                    '' AS chequeNumber,
                    glConfigDetail.GLCode AS GLAutoID,
                    chartOfAccount.systemAccountCode AS systemGLCode,
                    chartOfAccount.GLSecondaryCode AS GLCode,
            ...
            $q = "INSERT INTO srp_erp_generalledger (
                ...
                SELECT
                    ...
                    company.company_default_currency AS transactionCurrency,
                    '1' AS transactionExchangeRate,
                    abs(SUM(cardTopUp.topUpAmount)) AS transactionAmount,
                    company.company_default_decimal AS transactionCurrencyDecimalPlaces,
                    company.company_default_currencyID AS companyLocalCurrencyID,
                    company.company_default_currency AS companyLocalCurrency,
                    1 AS companyLocalExchangeRate,
                    abs(SUM(cardTopUp.topUpAmount)) AS companyLocalAmount,
                    getDecimalPlaces (
                        company.company_default_currencyID
                    ) AS companyLocalCurrencyDecimalPlaces,
                    company.company_reporting_currencyID AS companyReportingCurrencyID,
    ...
    function get_giftCard_paymentInformation($receiptID)
    {
        $q = "SELECT
                ...
                srp_erp_pos_cardtopup.isRefund
                FROM
                srp_erp_pos_cardtopup
    ...
    function editGiftCard(){
        ...
        $this->db->select('barcode,outletID,cardExpiryInMonths');
        $this->db->from('srp_erp_pos_giftcardmaster');
        $this->db->where('cardMasterID', $cardMasterID);
        $cardMaster = $this->db->get()->row_array();
        return $cardMaster;
    }
