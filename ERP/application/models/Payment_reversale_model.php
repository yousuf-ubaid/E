...
WHERE
    ...
    function reverse_paymentVoucher()
    {
        ...
        $bankCurrencyAmount=$pvdetail['transactionAmount']/$pvmaster['bankCurrencyExchangeRate'];
        /*$this->db->select('sum(transactionAmount) as transactionAmount,sum(companyLocalAmount) as companyLocalAmount,sum(companyReportingAmount) as companyReportingAmount');
        $this->db->from('srp_erp_paymentvoucherdetail');
        $this->db->where('payVoucherAutoId', trim($this->input->post('payVoucherAutoId') ?? ''));
...
AND (type = "Invoice" OR type = "Advance" OR type = "debitnote" OR type = "SR")')->row_array();*/

        $pvdetailSupp = $this->db->query('SELECT
(
 ((IFNULL(Invoice.transactionAmount, 0)+IFNULL(Advance.transactionAmount, 0)+IFNULL(Item.transactionAmount, 0)+IFNULL(GL.transactionAmount, 0))-(IFNULL(debitnote.transactionAmount, 0)+IFNULL(SR.transactionAmount, 0)))+((IFNULL( addondet.taxPercentage, 0) / 100) * IFNULL( tyepdet.transactionAmount, 0))
)AS transactionAmount,
(
 ((IFNULL(Invoice.transactionAmount, 0)+IFNULL(Advance.transactionAmount, 0)+IFNULL(Item.transactionAmount, 0)+IFNULL(GL.transactionAmount, 0))-(IFNULL(debitnote.transactionAmount, 0)+IFNULL(SR.transactionAmount, 0)))+((IFNULL( addondet.taxPercentage, 0) / 100) * IFNULL( tyepdet.transactionAmount, 0))
)/companyLocalExchangeRate AS companyLocalAmount,
(
 ((IFNULL(Invoice.transactionAmount, 0)+IFNULL(Advance.transactionAmount, 0)+IFNULL(Item.transactionAmount, 0)+IFNULL(GL.transactionAmount, 0))-(IFNULL(debitnote.transactionAmount, 0)+IFNULL(SR.transactionAmount, 0)))+((IFNULL( addondet.taxPercentage, 0) / 100) * IFNULL( tyepdet.transactionAmount, 0))
)/companyReportingExchangeRate AS companyReportingAmount
FROM
	`srp_erp_paymentvouchermaster`
LEFT JOIN (
	SELECT
	...
	FROM
		srp_erp_paymentvoucherdetail
	WHERE
		srp_erp_paymentvoucherdetail.type = "Advance"
	GROUP BY
		payVoucherAutoId
) Advance ON (
	`Advance`.`payVoucherAutoId` = srp_erp_paymentvouchermaster.payVoucherAutoId
)
LEFT JOIN (
	SELECT
		SUM(transactionAmount) AS transactionAmount,
	...
	FROM
		srp_erp_paymentvoucherdetail
	WHERE
		srp_erp_paymentvoucherdetail.type = "Item"
	GROUP BY
		payVoucherAutoId
) Item ON (
	`Item`.`payVoucherAutoId` = srp_erp_paymentvouchermaster.payVoucherAutoId
)
LEFT JOIN (
	SELECT
		SUM(transactionAmount) AS transactionAmount,
	...
	`GL`.`payVoucherAutoId` = srp_erp_paymentvouchermaster.payVoucherAutoId
)
LEFT JOIN (
	SELECT
		SUM(transactionAmount) AS transactionAmount,
...
LEFT JOIN (
	...
	FROM
		srp_erp_paymentvoucherdetail
	WHERE
		srp_erp_paymentvoucherdetail.type = "SR"
	GROUP BY
		payVoucherAutoId
) SR ON (
	`SR`.`payVoucherAutoId` = srp_erp_paymentvouchermaster.payVoucherAutoId
)
LEFT JOIN (
	SELECT
		SUM(transactionAmount) AS transactionAmount,
	...
	`Invoice`.`payVoucherAutoId` = srp_erp_paymentvouchermaster.payVoucherAutoId
)
...
WHERE
   srp_erp_paymentvouchermaster.payVoucherAutoId='.$pvautoid.'
')->row_array();


        $invoiceAmount = $this->db->query('SELECT
(
 IFNULL(Invoice.transactionAmount, 0)+IFNULL(Advance.transactionAmount, 0)
)AS transactionAmount,
(
 IFNULL(Invoice.transactionAmount, 0)+IFNULL(Advance.transactionAmount, 0)
)/companyLocalExchangeRate AS companyLocalAmount,
...
')->row_array();
        ...
        $debitnoteAmount = $this->db->query('SELECT
(
 IFNULL(debitnote.transactionAmount, 0)+IFNULL(SR.transactionAmount, 0)
)AS transactionAmount,
(
 IFNULL(debitnote.transactionAmount, 0)+IFNULL(SR.transactionAmount, 0)
)/companyLocalExchangeRate AS companyLocalAmount,
...
AND (
	...
	((IFNULL(Invoice.transactionAmount, 0)+IFNULL(Advance.transactionAmount, 0))-(IFNULL(debitnote.transactionAmount, 0)+IFNULL(SR.transactionAmount, 0))) as Amount,((IFNULL(Invoice.transactionAmount, 0)+IFNULL(Advance.transactionAmount, 0))-(IFNULL(debitnote.transactionAmount, 0)+IFNULL(SR.transactionAmount, 0)))/srp_erp_paymentvouchermaster.companyLocalExchangeRate as compLocalAmount,((IFNULL(Invoice.transactionAmount, 0)+IFNULL(Advance.transactionAmount, 0))-(IFNULL(debitnote.transactionAmount, 0)+IFNULL(SR.transactionAmount, 0)))/srp_erp_paymentvouchermaster.companyReportingExchangeRate as compReportingAmount ,srp_erp_suppliermaster.liabilityAutoID,srp_erp_suppliermaster.liabilitySystemGLCode,srp_erp_suppliermaster.liabilityGLAccount,srp_erp_suppliermaster.liabilityDescription,srp_erp_suppliermaster.liabilityType,srp_erp_chartofaccounts.GLAutoID,srp_erp_chartofaccounts.GLSecondaryCode,srp_erp_chartofaccounts.systemAccountCode,srp_erp_chartofaccounts.GLDescription,srp_erp_chartofaccounts.subCategory
...
LEFT JOIN ( SELECT SUM( transactionAmount ) AS transactionAmount, payVoucherAutoId FROM srp_erp_paymentvoucherdetail WHERE srp_erp_paymentvoucherdetail.type = "Advance" GROUP BY payVoucherAutoId ) Advance ON ( `Advance`.`payVoucherAutoId` = srp_erp_paymentvouchermaster.payVoucherAutoId )
LEFT JOIN ( SELECT SUM( transactionAmount ) AS transactionAmount, payVoucherAutoId FROM srp_erp_paymentvoucherdetail WHERE srp_erp_paymentvoucherdetail.type = "Invoice" GROUP BY payVoucherAutoId ) Invoice ON ( `Invoice`.`payVoucherAutoId` = srp_erp_paymentvouchermaster.payVoucherAutoId )
WHERE srp_erp_paymentvouchermaster.payVoucherAutoId = '.$pvautoid.'
AND (type = "Invoice" OR type = "Advance" OR type = "debitnote" OR type = "SR") GROUP BY srp_erp_paymentvouchermaster.payVoucherAutoId')->row_array();
...
AND (type = "GL" OR type = "Item") GROUP BY srp_erp_paymentvouchermaster.payVoucherAutoId')->row_array();

        $bankGlBankLeager = $this->db->query('SELECT
	((( IFNULL( addondet.taxPercentage, 0 ) / 100 ) * IFNULL( tyepdet.transactionAmount, 0 ) ) + IFNULL( det.transactionAmount, 0 ) - IFNULL( debitnote.transactionAmount, 0 ) - IFNULL( SR.transactionAmount, 0 )) as Amount, ((( IFNULL( addondet.taxPercentage, 0 ) / 100 ) * IFNULL( tyepdet.transactionAmount, 0 ) ) + IFNULL( det.transactionAmount, 0 ) - IFNULL( debitnote.transactionAmount, 0 ) - IFNULL( SR.transactionAmount, 0 ))/srp_erp_paymentvouchermaster.companyLocalExchangeRate as compLocalAmount,((( IFNULL( addondet.taxPercentage, 0 ) / 100 ) * IFNULL( tyepdet.transactionAmount, 0 ) ) + IFNULL( det.transactionAmount, 0 ) - IFNULL( debitnote.transactionAmount, 0 ) - IFNULL( SR.transactionAmount, 0 ))/srp_erp_paymentvouchermaster.companyReportingExchangeRate as compReportingAmount,addondet.taxPercentage as taxPercentage,srp_erp_chartofaccounts.GLAutoID,srp_erp_chartofaccounts.GLSecondaryCode,srp_erp_chartofaccounts.systemAccountCode,srp_erp_chartofaccounts.GLDescription,srp_erp_chartofaccounts.subCategory
FROM srp_erp_paymentvouchermaster
LEFT JOIN srp_erp_paymentvoucherdetail on srp_erp_paymentvouchermaster.payVoucherAutoId = srp_erp_paymentvoucherdetail.payVoucherAutoId
LEFT JOIN (
	SELECT
...
LEFT JOIN (SELECT SUM( transactionAmount ) AS transactionAmount,payVoucherAutoId FROM srp_erp_paymentvoucherdetail WHERE
srp_erp_paymentvoucherdetail.type != "debitnote"
...
WHERE srp_erp_paymentvouchermaster.payVoucherAutoId = '.$pvautoid.' GROUP BY srp_erp_paymentvouchermaster.payVoucherAutoId')->row_array();
            ...
            if ($pvmaster) {
                ...
                if ($payReversalMaster) {
                    if ($pvmaster['pvType'] == 'Supplier' || $pvmaster['pvType'] == 'SupplierAdvance' ||  $pvmaster['pvType'] == 'SupplierInvoice') {
                        ...
                        $RVdata['bankCurrencyAmount'] = ($pvdetailSupp['transactionAmount'] / $pvmaster['bankCurrencyExchangeRate']);
                        ...
                        $RVdata['transactionAmount'] = $pvdetailSupp['transactionAmount'];
                        ...
                        $RVdata['companyLocalAmount'] = $pvdetailSupp['companyLocalAmount'];
                        ...
                        $RVdata['companyReportingAmount'] = $pvdetailSupp['companyReportingAmount'];
                        ...
                        if ($payCustomerReceiptM) {
                            ...
                            $RVDdata['type'] = 'PRVR';
                            $RVDdata['transactionAmount'] = $pvdetailSupp['transactionAmount'];
                            $RVDdata['companyLocalAmount'] = $pvdetailSupp['companyLocalAmount'];
                            $RVDdata['companyReportingAmount'] = $pvdetailSupp['companyReportingAmount'];
                            ...
                            if ($payCustomerReceiptD) {
                                if(!empty($pvdetailSupp)){
                                    ...
                                    $BSIdata['transactionAmount'] = $invoiceAmount['transactionAmount'];
                                    ...
                                    $BSIdata['companyLocalAmount'] = $invoiceAmount['companyLocalAmount'];
                                    ...
                                    $BSIdata['companyReportingAmount'] = $invoiceAmount['companyReportingAmount'];
                                    ...
                                    $BSIdata['supplierCurrencyAmount'] = $invoiceAmount['transactionAmount']/$pvmaster['partyExchangeRate'];
                                    ...
                                    if ($paySupplierM) {
                                        ...
                                        $BSIDdata['transactionAmount'] = $invoiceAmount['transactionAmount'];
                                        $BSIDdata['companyLocalAmount'] = $invoiceAmount['companyLocalAmount'];
                                        $BSIDdata['companyReportingAmount'] = $invoiceAmount['companyReportingAmount'];
                                ...
                                if(!empty($debitnoteAmount) && $debitnoteAmount['transactionAmount']>0){
                                    $companyFinanceYear = $this->db->query('SELECT  beginingDate,endingDate FROM srp_erp_companyfinanceyear WHERE companyFinanceYearID = ' . $companyid . '')->row_array();
                                    $DNdata['documentID']='DN';
                                    $DNdata['companyFinanceYearID']=$companyFinanceYearID['companyFinanceYearID'];
                                    $DNdata['companyFinanceYear']=$companyFinanceYear['beginingDate'].'-'.$companyFinanceYear['endingDate'];
                                    $DNdata['FYBegin']=$companyFinanceYear['beginingDate'];
                                    $DNdata['FYEnd']=$companyFinanceYear['endingDate'];
                                    $DNdata['FYPeriodDateFrom'] = $companyFinanceYearID['dateFrom'];
                                    $DNdata['FYPeriodDateTo'] = $companyFinanceYearID['dateTo'];
                                    $DNdata['companyFinancePeriodID'] = $companyFinanceYearID['companyFinancePeriodID'];
                                    $DNdata['debitNoteCode'] = $this->sequence->sequence_generator('DN');
                                    $DNdata['debitNoteDate'] = $reversalDate;
                                    $DNdata['comments'] = 'Payment reversal of payment voucher ' . $pvmaster['PVcode'];
                                    $DNdata['supplierID'] = $pvmaster['partyID'];
                                    $DNdata['supplierCode'] = $pvmaster['partyCode'];
                                    $DNdata['supplierName'] = $pvmaster['partyName'];
                                    $DNdata['supplierAddress'] = $pvmaster['partyAddress'];
                                    $DNdata['supplierTelephone'] = $pvmaster['partyTelephone'];
                                    $DNdata['supplierFax'] = $pvmaster['partyFax'];
                                    $DNdata['supplierliabilityAutoID'] = $invoiceGlDetails['liabilityAutoID'];
                                    $DNdata['supplierliabilitySystemGLCode'] = $invoiceGlDetails['liabilitySystemGLCode'];
                                    $DNdata['supplierliabilityGLAccount'] = $invoiceGlDetails['liabilityGLAccount'];
                                    $DNdata['supplierliabilityDescription'] = $invoiceGlDetails['liabilityDescription'];
                                    $DNdata['supplierliabilityType'] = $invoiceGlDetails['liabilityType'];
                                    $DNdata['transactionCurrencyID'] = $pvmaster['transactionCurrencyID'];
                                    $DNdata['transactionCurrency'] = $pvmaster['transactionCurrency'];
                                    $DNdata['transactionExchangeRate'] = $pvmaster['transactionExchangeRate'];
                                    $DNdata['transactionAmount'] = $debitnoteAmount['transactionAmount'];
                                    $DNdata['transactionCurrencyDecimalPlaces'] = $pvmaster['transactionCurrencyDecimalPlaces'];
                                    $DNdata['companyLocalCurrencyID'] = $pvmaster['companyLocalCurrencyID'];
                                    $DNdata['companyLocalCurrency'] = $pvmaster['companyLocalCurrency'];
                                    $DNdata['companyLocalExchangeRate'] = $pvmaster['companyLocalExchangeRate'];
                                    $DNdata['companyLocalAmount'] = $debitnoteAmount['companyLocalAmount'];
                                    $DNdata['companyLocalCurrencyDecimalPlaces'] = $pvmaster['companyLocalCurrencyDecimalPlaces'];
                                    $DNdata['companyReportingCurrencyID'] = $pvmaster['companyReportingCurrencyID'];
                                    $DNdata['companyReportingCurrency'] = $pvmaster['companyReportingCurrency'];
                                    $DNdata['companyReportingExchangeRate'] = $pvmaster['companyReportingExchangeRate'];
                                    $DNdata['companyReportingAmount'] = $invoiceAmount['companyReportingAmount'];
                                    $DNdata['companyReportingCurrencyDecimalPlaces'] = $pvmaster['companyReportingCurrencyDecimalPlaces'];
                                    $DNdata['supplierCurrencyID'] = $pvmaster['partyCurrencyID'];
                                    $DNdata['supplierCurrency'] = $pvmaster['partyCurrency'];
                                    $DNdata['supplierCurrencyExchangeRate'] = $pvmaster['partyExchangeRate'];
                                    $DNdata['supplierCurrencyAmount'] = $debitnoteAmount['transactionAmount']/$pvmaster['partyExchangeRate'];
                                    $DNdata['supplierCurrencyDecimalPlaces'] = $pvmaster['partyCurrencyDecimalPlaces'];
                                    $DNdata['confirmedYN'] = 1;
                                    $DNdata['confirmedByEmpID'] = $this->common_data['current_userID'];
                                    $DNdata['confirmedByName'] = $this->common_data['current_user'];
                                    $DNdata['approvedYN'] = 1;
                                    $DNdata['approvedDate'] = current_date();
                                    $DNdata['approvedbyEmpID'] = $this->common_data['current_userID'];
                                    $DNdata['approvedbyEmpName'] = $this->common_data['current_user'];
                                    $DNdata['currentLevelNo'] = 1;
                                    $DNdata['companyID'] = $this->common_data['company_data']['company_id'];
                                    $DNdata['companyCode'] = $this->common_data['company_data']['company_code'];
                                    $DNdata['createdUserGroup'] = $this->common_data['user_group'];
                                    $DNdata['createdPCID'] = $this->common_data['current_pc'];
                                    $DNdata['createdUserID'] = $this->common_data['current_userID'];
                                    $DNdata['createdUserName'] = $this->common_data['current_user'];
                                    $DNdata['createdDateTime'] = $this->common_data['current_date'];
                                    $DNmaster = $this->db->insert('srp_erp_debitnotemaster', $DNdata);
                                    $debitNoteMasterAutoID = $this->db->insert_id();
                                    if($DNmaster){
                                        $DNDdata['debitNoteMasterAutoID'] = $debitNoteMasterAutoID;
                                        $DNDdata['description'] = 'Payment reversal of payment voucher ' . $pvmaster['PVcode'];
                                        $DNDdata['transactionAmount'] = $debitnoteAmount['transactionAmount'];
                                        $DNDdata['companyLocalAmount'] = $debitnoteAmount['companyLocalAmount'];
                                        $DNDdata['companyLocalExchangeRate'] = $pvmaster['companyLocalExchangeRate'];
                                        $DNDdata['companyReportingAmount'] = $debitnoteAmount['companyReportingAmount'];
                                        $DNDdata['companyReportingExchangeRate'] = $pvmaster['companyReportingExchangeRate'];
                                        $DNDdata['supplierAmount'] = $debitnoteAmount['transactionAmount']/$pvmaster['partyExchangeRate'];
                                        $DNDdata['supplierCurrencyExchangeRate'] = $pvmaster['partyExchangeRate'];
                                        $DNDdata['companyCode'] = $this->common_data['company_data']['company_code'];
                                        $DNDdata['companyID'] = $this->common_data['company_data']['company_id'];
                                        $DNDdata['createdUserGroup'] = $this->common_data['user_group'];
                                        $DNDdata['createdPCID'] = $this->common_data['current_pc'];
                                        $DNDdata['createdUserID'] = $this->common_data['current_userID'];
                                        $DNDdata['createdUserName'] = $this->common_data['current_user'];
                                        $DNDdata['createdDateTime'] = $this->common_data['current_date'];
                                        $DNdetail = $this->db->insert('srp_erp_debitnotedetail', $DNDdata);
                                    }
                                }
                                ...
                                /*if ($paySupplierM) {
                                    ...
                                    if ($result) {*/
                                        ...
                                        if($bankGlBankLeager['GLAutoID']){
                                            ...
                                            $invGLbnk['transactionAmount'] = $bankGlBankLeager['Amount'];
                                            ...
                                            $invGLbnk['companyLocalAmount'] = $bankGlBankLeager['compLocalAmount'];
                                            ...
                                            $invGLbnk['companyReportingAmount'] = $bankGlBankLeager['compReportingAmount'];
                                        ...
                                        $BankLedger['bankCurrencyAmount'] = $bankCurrencyAmount;
