...
class Erp_data_sync_model extends ERP_Model
{
    ...
    function get_active_posting_id(){
    ...
    function get_company_mappings($mapping_type = null){
    ...
    function set_clent_ecommerce_settings(){
        ...
        $gl_code = $this->input->post('updated_gl_code');

        $company_id = $this->common_data['company_data']['company_id'];

        try{
            $response = $this->db->set('supplier_gl_code',$gl_code)->where('company_id',$company_id)->update('srp_erp_ecommerce_settings');
    ...
    function get_sales_client_credit_debit($mapping_id = null,$mapping_type = null){
        ...
        $mapping_columns = $this->get_company_mappings($mapping_type);
        ...
        foreach($mapping_columns as $value){
            ...
            $base_arr['store_id'] = $record['store_id'];
            $base_arr['currency'] = $record['currency'];
    ...
    function get_sales_client_credit_debit_summary($mapping_id = null,$mapping_type = null){
        
        if(empty($mapping_id)){
            $mapping_id = $this->input->post('client_sales_id');
        }

        $sales = $this->get_sales_client_credit_debit($mapping_id,$mapping_type);
    ...
    function save_supplier_invoice_header($order_auto_id,$client_sales_data,$client_mapping,$vendor_type = null){
        ...
        if($financeyearperiodYN==1) {
            ...
            $financePeriod['dateFrom'] = '2022-08-01';
            $financePeriod['dateTo'] = '2022-08-31';
    ...
    function save_data_mapping_posting(){
        ...
        $ex_posting_active = $this->db->from('srp_erp_ecommerce_posting')->where('status',1)->where('company_id',$company_id)->get()->row_array();
    ...
    function change_posting_status(){
        ...
        $ex_active_posting = $this->db->where('status',1)->from('srp_erp_ecommerce_posting')->get()->row_array();
        $ex_mapping_record = $this->db->where('id',$mapping_id)->from('srp_erp_ecommerce_posting')->get()->row_array();

        if($ex_active_posting && $ex_active_posting['id'] != $mapping_id){
    ...
    function save_customer_invoice_header($order_auto_id,$client_sales_data,$client_mapping,$doc_type = null)
    {
        ...
        if (($invoiceDate) > ($invoiceDueDate)) {
            ...
            if($financeyearperiodYN==1) {
                ...
                return json_encode($this->Invoice_model->save_invoice_header());
            }
        }
    ...
    function save_direct_invoice_detail($invoiceAutoID,$client_sales_data,$client_mapping,$selected_gl_record)
    {
        ...
        foreach($selected_gl_record as $gl_record){
            ...
            if($gl_code) {
                ...
                $data['segmentCode'] = trim($segment[1] ?? '');
                $data['discountPercentage'] = trim($discountPercentage);
                $data['discountAmount'] = trim($amount);
                ...
                if ($approvals_status == 1) {
                    ...
                    if ($validate) {
                        ...
                        $data = array(
                            ...
                            'companyLocalAmount' => (round($totalValue['total_value'] / isset($totalValue['companyLocalExchangeRate']) ? $totalValue['companyLocalExchangeRate'] : 1,$totalValue['companyLocalCurrencyDecimalPlaces'])),
                            'companyReportingAmount' => (round($totalValue['total_value'] / isset($totalValue['companyReportingExchangeRate']) ? $totalValue['companyReportingExchangeRate'] : 1 ,$totalValue['companyReportingCurrencyDecimalPlaces'])),
                            'customerCurrencyAmount' => (round($totalValue['total_value'] / isset($totalValue['customerCurrencyExchangeRate']) ? $totalValue['customerCurrencyExchangeRate'] : 1,$totalValue['customerCurrencyDecimalPlaces'])),
    ...
    function save_receiptvoucher_header($sales_id,$client_sales_data,$client_mapping)
    {
        ...
        $currency_code = 'LKR | Sri Lankan Rupee';
        //'OMR|Omani Rial';
        ...
        if ($data['RvType'] == 'Direct' || $data['RvType'] == 'DirectItem' || $data['RvType'] == 'DirectIncome') {
            ...
            $data['customerCurrencyID'] = 1;//trim($this->input->post('transactionCurrencyID') ?? '');
            $data['customerCurrencyDecimalPlaces'] = 3;//fetch_currency_desimal_by_id($data['customerCurrencyID']);
        } else {
    ...
    function save_rv_approval($autoappLevel = 1, $system_idAP = 0, $statusAP = 0, $commentsAP = 0)
    {
        ...
        if (!$items_arr) {
            ...
            if ($approvals_status == 1) {
                ...
                $bankledger_arr['partyCurrencyAmount'] = ($bankledger_arr['transactionAmount'] / isset($bankledger_arr['partyCurrencyExchangeRate']) ? $bankledger_arr['partyCurrencyExchangeRate'] : 1);
                ...
                $bankledger_arr['bankCurrencyAmount'] = ($bankledger_arr['transactionAmount'] / isset($bankledger_arr['bankCurrencyExchangeRate']) ? $bankledger_arr['bankCurrencyExchangeRate'] : 1);
    ...
    function save_journal_entry_header($sales_id,$client_sales_data,$client_mapping)
    {
        ...
        $financearray = 1423;
        $segment_det = '86|GEN';
        $financeyear_period = '01-07-2022 - 31-07-2022';
        $companyFinanceYear = '01-04-2022 - 31-03-2023';
        $currency_code = 'LKR | Sri Lankan Rupee';
        //'OMR|Omani Rial';
        $bank = 'Commercial Bank | World Trade Centre | CCEYLKLX | 8010009026 | BSA | Bank';
    ...
    function process_daily_posting_list(){
        ...
        if($posting_data && $posting_type == 1){
            ...
            foreach($supplier_ids as $supplier){
               ...
               $supplier_direct_receipt = $this->generate_group_journel_voucher($supplier['store_id'],$dateFrom,$dateTo,$posting_id);
            ...
            foreach($supplier_ids_3pl as $supplier_3pl){
                ...
                if($supplier_3pl['3pl_company_id']){
                  ...
                  $supplier_3PL = $this->generate_group_3PL_customer_invoice($supplier_3pl['3pl_company_id'],$dateFrom,$dateTo,$posting_id);
    ...
    function generate_group_supplier_invoice($supplier_id,$dateFrom,$dateTo,$posting_id){

        $orders_list = $this->get_orders_for_supplier($supplier_id,$dateFrom,$dateTo);
        ...
        $client_sales_data['date_time'] = '2022-08-12 00:00:00';
        $client_sales_data['completed_time'] = '2022-08-13 00:00:00';
    ...
    function generate_group_customer_invoice($supplier_id,$dateFrom,$dateTo,$posting_id){
    ...
    function generate_group_3PL_supplier_invoice($supplier_id,$dateFrom,$dateTo,$posting_id){
        ...
        $orders_list = $this->get_orders_for_supplier_3pl($supplier_id,$dateFrom,$dateTo);
        $total_orders = count($orders_list);
        $base_arr = array();
        $error_arr = array();
        $success_err = array();
        ...
        $client_sales_data['date_time'] = '2022-08-12 00:00:00';
        $client_sales_data['completed_time'] = '2022-08-12 00:00:00';
        $client_sales_data['order'] = $posting_id;
        $client_sales_data['store_id'] = $supplier_id;
        $client_sales_data['service_type'] = 'TMDONE';
        $client_sales_data['discount'] = '0';
    ...
    function get_orders_distinct_suppliers($from,$to){
    ...
    function get_orders_distinct_suppliers_3pl($from,$to){
    ...
    function get_orders_for_supplier($store_id,$from,$to){
    ...
    function update_client_data_group($success_arr,$err_arr,$invoice_type,$auto_id,$doc_id){
        ...
        foreach($err_arr as $err_val){
            if($invoice_type == 1){
                add_process_log_record($auto_id,'SUP',$success_val['order_id'],2,'Supplier invoice create Failed',1);
            }elseif($invoice_type == 2){
                add_process_log_record($auto_id,'CINV',$success_val['order_id'],2,'Customer invoice create Failed',2);
            }
    ...
    function get_company_posting_active(){
